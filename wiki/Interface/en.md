...
7.  The mods that you have activated (see #12), as well as the option to select them. The option marked "Free Mods" toggles whether or not players can select their own mods. If yes, they can pick any combination of mods _except for speed-altering mods like Double Time_. If no, the host decides what mods will be used. The host can pick speed-altering mods regardless of whether or not Free Mods is turned on.
...
-   `Collections` - This will show the collections you have created. _Note that this will hide beatmaps not listed in a collection!_
...
-   `By Difficulty` - Beatmaps will be sorted from easiest to hardest by star difficulty. _Note that this will split apart mapsets!_
...
_Note: You cannot have the chat console or the options sidebar open if you want to search; otherwise, anything you type will be perceived as chat text or as an options search query._
...
_Note: You can press `Shift` + the `Random` button or `F2` to go back to the beatmap you had selected before you randomized your selection._
