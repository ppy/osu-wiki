...
- \[9\] 右下角的是扩展[聊天窗口](/wiki/Chat_Console)的控制开关 (叫做"玩家列表") 和常规聊天窗口 (`F9` & `F8`, respectively).
...
2. 你的个人信息
3. 音乐控制器
4. 玩家列表 - 显示玩家的名字，他们的身份（房主或玩家），他们选择的mods（参见 #7） ,他们的osu!排名和他们的队伍（如果可用）。
5. 房间的名字和密码设定。
6. 选中的谱面。应与在单人模式中相同的样式显示。
7. 选中的mods （参见 #12） ，以及设置，叫做“Free Mods”的选项决定玩家是否能选择自己想使用的mods。如果启用，他们可以选择任意组合的mods*除了影响速度的模组像Double Time*。如果禁用，则由房主来决定使用什么模组，房主可以使用影响速度的mods不论Free Mods是否开启。
8. 分组方式和胜利条件，
9. 准备按钮
...
5. `Beatmap Ranking`部分。仅当谱面拥有在线排行榜([qualified](wiki/Beatmaps#qualified)，[ranked](wiki/Beatmaps#ranked)，或者[loved](wiki/Beatmaps#loved))时可用。
