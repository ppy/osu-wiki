...
- \[8] Le statut de la connexion à [Bancho](/wiki/fr/Bancho_(server))!. Il n'est pas présent sur l'image ci-dessus, mais le statut est représenté par un maillon de chaîne.
- \[9] En bas à droite se trouvent les boutons de contrôles du [tchat](/wiki/fr/Chat_Console)(les raccourcis clavier sont `F8` pour le bouton "Show Chat" et `F9` pour le bouton "Online Users").
...
Dans cette partie de l'écran se situent **les informations sur la difficulté de la beatmap sélectionnée.** Par défaut, la beatmap dont la musique est jouée par le jukebox est sélectionnée dans l'écran de sélection de beatmaps. En haut à gauche se situe la catégorie de la map (par exemple, deux chevrons signifient que la beatmap est _classée._). À sa droite se situe le titre de la beatmap. Par défaut, le titre romanisé est affiché, mais si vous sélectionnez `Métadonnées des maps dans leur langue d'origine dès que possible` dans les [options](/wiki/fr/Options), le titre au format Unicode sera affiché, comme on peut voir sur la première image. Le créateur de la beatmap est également affiché, ainsi que les informations de la beatmap en dessous. De gauche à droite, on peut lire :

- **Durée**: La durée totale de la beatmap, du début à la fin et en comptant les pauses. À ne pas confondre avec le [drain time](/wiki/fr/Gameplay/Drain_time).
...
- **OD**: L'[overall difficulty](/wiki/fr/Beatmapping/Overall_Difficulty) de la beatmap.
...
- `Par difficulté` - Les beatmaps seront triées en fonction de leur star rating, du plus bas au plus haut.  _Attention : ce paramètre séparera les difficultés des sets de beatmaps!_
...
_Remarque : Il est impossible d'effectuer une recherche tout en ayant la console de tchat ou le menu des options ouvert. En effet, tout ce qui sera écrit sera considéré comme un message dans le chat ou une recherche dans les options._
...
- `speed`: La vitesse de défilement sauvegardée dans le mode osu!mania. Cette valeur sera toujours égale à 0 sur les beatmaps non jouées si l'option [Se souvenir des vitesses de défilement sur chaque map osu!mania](/wiki/fr/Options#gameplay) est désactivée.
...
5. La section `Beatmap Ranking`. Elle est disponible uniquement pour les beatmaps avec un classement en ligne (elle doit être [qualifiée](/wiki/fr/Beatmap/Category#qualified), [classée](/wiki/fr/Beatmap/Category#ranked), ou [loved](/wiki/fr/Beatmap/Category#loved)). Vous devez également être en ligne pour voir cette section.
...
6. La section `Overall Ranking`. Elle est disponible uniquement pour les beatmaps avec un classement en ligne. Vous devez également être en ligne pour voir cette section.
   ...
   4. [`Score classé`](/wiki/fr/Score#score-classé): Le nombre total de points que vous avez obtenu sur toutes les beatmaps ayant un classement en ligne que vous avez jouées, chaque beatmap ne comptant qu'une seule fois.
   5. [`Score total`](/wiki/fr/Score#score-total): Le fonctionnement est identique à celui du score classé, mais il prend en compte toutes les beatmaps présentes sur le site d'osu!, ainsi que les beatmaps que vous avez quittées avant la fin et celles que vous avez échouées. Ce score compte dans votre niveau.
