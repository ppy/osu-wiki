...
- \[8\] Результат подключения к [Bancho](/wiki/BanchoBot)! На этой картинке не показано, но результат подключения выглядит как звено цепи.
...
6. Секция `Overall Ranking`. Доступно только для карт с онлайн-топом. Вам также нужно подключение к Интернету, чтобы увидеть этот раздел.
   ...
   2. `Accuracy`: средняя [точность](/wiki/Accuracy#accuracy) на всех картах, которые вы сыграли.
