...
- \[1\] [오스!](/wiki/Glossary/#cookie) 로고는 재생되고 있는 노래의 설정된 [BPM](/wiki/BPM)에 맞춰 뜁니다(둠칫둠칫!).
 노래가 재생되고 있지 않으면, 기본 설정대로 느리게 뜁니다.
- \[2\] Play(`P`)를 누르거나 오스! 로고를 클릭하면 플레이 메뉴로 넘어갑니다.
- \[3\] Edit(`E`)를 누르면 [에디터](/wiki/Beatmap_Editor)를 위한 선곡화면으로 이동합니다.
- \[4\] Option(`O`)을 누르면 [설정화면](/wiki/Options)이 왼쪽에서 튀어나옵니다.
...
- \[9\] 우측 하단의 버튼들은 [채팅 컨트롤](/wiki/Chat_Console)입니다. 채팅창을 띄우거나(오른쪽)(`F9`) 확장 채팅 인터페이스(왼쪽)\[F8\]를 띄워 접속 중인 유저 현황을 볼 수 있습니다. 둘은 항상 같이 나타나지만 채팅창을 내려야 모두 내릴 수 있습니다(유저 현황은 따로 내릴 수 있습니다.).
...
2.  `Sign Out`(로그아웃): 로그인된 계정에서 로그아웃합니다. (로그아웃 한 다음, [설정 화면](/wiki/Options)이 로그인 하기 위하여 나옵니다.)
...
- ![][Osu!] 아이콘은 [스탠다드 모드](/wiki/osu!),
- ![][Taiko] 아이콘은 [태고 모드](/wiki/osu!taiko),
- ![][Ctb] 아이콘은 [캐치 더 비트(CTB)](/wiki/osu!catch),
...
원어(유니코드)로 된 노래정보는 [일반 설정의 <비트맵 정보 원어로 보기>를](/wiki/Options#Graphics_tab) 선택 했을때만 표시됩니다.
...
- **노트 수** 는 비트맵에 쓰인 각 항목의 [노트](/wiki/Hit_Objects)들의 총 합입니다.
...
\* 모양은 [Osu!서포터](/wiki/osu!supporter)가 되어야 확인할 수 있습니다.
...
`Mods`버튼을 누르거나 `F1`키를 누르면 **[게임 내 모드](/wiki/Game_Modifiers)**창이 나타납니다.

이 화면에서는 게임 내 모드(줄여서 "모드")를 선택할 수 있습니다. 첫째 줄은 좀 더 쉽게 만드는 모드들 입니다. 대신, 점수는 떨어집니다. 반면에, 둘째 줄은 좀 더 어렵게 민듭니다. 그와 비례해 점수도 조금 더 받습니다. 마지막 줄은 스페셜 모드입니다. [릴렉스 모드와](/wiki/Game_Modes#Relax) [오토파일럿 모드](/wiki/Game_Modes#Auto_Pilot) 등이 있습니다. 간단한 설명을 보려면 해당 모드 위에 커서를 올려놓으면 되고, 클릭해서 선택합니다. Score Multiplier(점수 배율)값은 선택한 모든 모드들의 **배율 값의 곱**입니다. "모드 초기화"를 누르거나 `1`을 누르면 모든 모드의 선택이 해제됩니다. `닫기`를 누르거나 `2` 또는 `ESC`키를 누르면 선곡화면으로 돌아갑니다.
...
2.  당신의 유저 패널입니다. 당신의 [PP](/wiki/pp), PP 랭킹, 평균 [정확도](/wiki/Accuracy)와 레벨, 경험치를 알 수 있습니다. 경험치는 받은 점수에 따라 오릅니다.
3.  부가옵션입니다.
