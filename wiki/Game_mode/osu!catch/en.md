...
- The first ranked beatmap which contains at least one osu!catch difficulty is [Yousei Teikoku - Dare so Ka no Gekka (TV Size)](https://osu.ppy.sh/beatmapsets/13676), with the mapset being hosted by ::{ flag=CN }:: ::NatsumeRin::{ user=151679 } and the osu!catch guest difficulty mapped by ::{ flag=CN }:: ::Uran::{ user=133302 }.
- The first ranked osu!catch-only beatmap is [Rita - Hajimari no Toki](https://osu.ppy.sh/beatmapsets/91485) which is hosted and made by - ::{ flag=ES }:: ::Deif::{ user=318565 }.
