...
如果是红色的小圆圈，按下绑定到太鼓内侧的按键，或敲击 *TaTaCon* 又大又平的区域（中央）。如果是蓝色的小圆圈，就按下绑定到太鼓外侧的按键，或敲击 *TaTaCon* 的边缘。如果是**大**圆圈，依照颜色同时按键或敲击两侧对应区域以获得两倍得分（单侧的正确打击只会获得一倍得分）。
...
完成谱面后，会为分数分配一个[评级](/wiki/Gameplay/Grade#osu!taiko)。评级以单个字母为形式，是对准确度的简短评估。金或银 SS 表示 100% 准确度，而其他的评级（从 S 到 D）取决于 300、 100 和 miss 的数目。
