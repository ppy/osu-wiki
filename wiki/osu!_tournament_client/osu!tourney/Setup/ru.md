...
*Примечание: для использования osu!tourney нужно быть [саппортером](/wiki/osu!supporter).*

Рекомендуется использовать **отдельную инсталляцию osu!**, поскольку база данных о картах может повредиться. Иметь несколько копий osu! разрешено — достаточно держать их в разных папках. Для создания новой инсталляции скопируйте файл `osu!.exe` в пустую папку и запустите его — игра сделает всё остальное. Далее в руководстве речь пойдёт именно об этой копии osu!.

Запустите osu! и залогиньтесь, отметив пункты `Remember Username` и `Remember Password`.

Закройте osu! и создайте в её папке пустой файл `tournament.cfg`. Запустите игру — откроется клиент osu!tourney.
...
- `BufferTimeoutTime`: как долго клиент будет ожидать буферизации данных от игрока, прежде чем продолжит без него стримить.
