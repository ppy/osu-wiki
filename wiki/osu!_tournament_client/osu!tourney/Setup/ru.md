...
Рекомендуется использовать **отдельную инсталляцию osu!**, поскольку база данных карт может повредиться. Иметь несколько копий osu! разрешено — достаточно держать их в разных папках.

Для создания новой инсталляции скопируйте файл `osu!.exe` в пустую папку и запустите его — игра сделает всё остальное. Далее в руководстве речь пойдёт именно об отдельной копии osu!.
...
- `TeamSize`: число игроков в каждой команде.
