...
Agrega canciones de la map pool a la carpeta `Songs` antes de iniciar el cliente. Los beatmaps enviados que el cliente no tenga se descargarán automáticamente.
...
- `TeamSize`: El número de jugadores por equipo. El cliente abrirá de 1 a 8 ventanas por equipo según este valor. El valor máximo es 8 debido a los límites de conexión de [Bancho](/wiki/Bancho_(server)), pero esto se puede aumentar temporalmente para el usuario espectador contactando a [tournaments@ppy.sh](mailto:tournaments@ppy.sh).
...
- `BufferTimeoutTime`: La cantidad de tiempo que el cliente esperará a que un jugador se almacene en búfer antes de continuar jugando sin él.
  - No se recomienda cambiar esto.
- `BufferTotalTime`: La cantidad de tiempo que el cliente almacena en búfer para reducir la frecuencia de las pausas para esperar el búfer. Esto produce un retraso entre el cliente y los jugadores.
  - No se recomienda cambiar esto.
...
- `Height`: Altura del área transmitible del cliente.
  ...
  - El «área transmitible» del cliente no incluye el panel de control. En general, el panel de control es 200 px mayor que la altura del cliente, así que, por ejemplo, una pantalla de 1440p sería adecuada para transmitir un torneo de 1080p.
