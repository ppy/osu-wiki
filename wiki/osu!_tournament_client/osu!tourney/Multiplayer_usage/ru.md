...
Как следует из статьи про [режим зрителя](/wiki/osu!tourney/Spectator_usage), каждое окно клиента связано с отдельным слотом в мультиплеерной комнате. После того, как вы настроите параметр `TeamSize` в файле `tournament.cfg`, первые `TeamSize` слотов в комнате будут выделены под синюю команду, а следующие `TeamSize` слотов — под красную.
...
**osu!tourney не учитывает цвета слотов в комнате** — клиент смотрит только на их порядковые номера. Ниже приведены примеры расстановки окон osu!tourney при различных размерах команд. Числа в окнах обозначают номера слотов в мультиплеерной комнате. Они добавлены на скриншоты для наглядности и не отображаются при работе osu!tourney:
