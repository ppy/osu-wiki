...
O cliente suporta várias modificações, para que você possa customizá-lo para o torneio. Para fazer isso, você precisa criar a estrutura de pastas `/tournament/Skins/User` no diretório de instalação do cliente. Os elementos da *skin* podem ser colocados nessa pasta e suportam arquivos com extensão `.jpg` e `.png`.
...
O cliente exibirá ícones localizados em `/tournament/Skins/User/icons` ao lado do nome dos times. Esses podem ser utilizados para mostrar, por exemplo, bandeiras de países ou avatares.
