...
*另见：[安装/macOS](/wiki/Installation/macOS)* <!-- 和 [安装/Linux](/wiki/Installation/Linux)* -->
...
3. 双击列表内的乐曲或点击右边的`下载（Download）`按钮来下载它。
    - 如果你不想下载随谱面文件一起的视频文件，点击`下载（不包含视频）（DL NoVedio）`。
4. osu! 完成下载后，如果你处在单人（Solo）模式的[谱面选择](/wiki/Song_Selection)界面，将会自动加载谱面集。
    - 如果你因为某些原因没看到它们，在[单人（Solo）模式](/wiki/Solo_mode)内按下 `F5` 来重新处理谱面文件。
