...
만약 현재 업로드하려는 맵이 osu! 웹사이트에 등록되어있지 않은 경우, 해당 유저가 앞으로 등록할 수 있는 비트맵의 개수가 BSS 창에 표시됩니다. 만약 해당 비트맵이 이미 지명을 받았을 경우, 비트맵을 업데이트 하는 것이 지명을 초기화 시킨다는 경고문이 표시됩니다. 만약 비트맵이 무덤에 가 있을 경우, 해당 비트맵을 대기(Pending)상태로 승격시킨다는 경고문이 표시됩니다.
