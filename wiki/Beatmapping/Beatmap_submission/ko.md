...
만약 현재 업로드하려는 맵이 osu! 웹사이트에 등록되어있지 않은 경우, 해당 유저가 앞으로 등록할 수 있는 비트맵의 개수가 BSS 창에 표시됩니다. 만약 해당 비트맵이 이미 추천을 받았을 경우, 비트맵을 업데이트 하는 것이 추천을 초기화 시킨다는 경고문이 표시됩니다. 만약 비트맵이 [무덤에](/wiki/Beatmap/Category#무덤에-감) 가 있을 경우, 해당 비트맵을 대기(Pending)상태로 승격시킨다는 경고문이 표시됩니다.

## 등록 옵션

![](img/bss_submitting.png "비트맵 업로드 시스템 (Beatmap Submission System) 화면")

`새 비트맵 업로드` 또는 `비트맵 업데이트` 버튼을 누를 경우, 해당 비트맵을 `작업중 (WIP)` 또는 `대기중인 비트맵 (Pending Beatmaps)` 둘 중 하나를 선택하여 업로드 할 수 있습니다. WIP 상태의 비트맵은 추천을 받을 수 없지만, 대기(Pending) 비트맵은 추천을 받을 수 있습니다.
