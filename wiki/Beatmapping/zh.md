...
这包括了选歌、[测定歌曲时间轴 (Timing)](/wiki/Client/Beatmap_editor/Timing),、摆放[物件](/wiki/Hit_object)，并按照自己喜好[制作皮肤](/wiki/Skinning)和[故事板](/wiki/Storyboard#storyboarding)。
...
点击歌曲，就会进入[谱面编辑器](https://osu.ppy.sh/wiki/zh/Client/Beatmap_editor)并进入`歌曲设置 (Song Setup)` 界面。
...
你应该在创建其他[难度](/wiki/Beatmap/Difficulty)之前填满这里列出的信息。
...
务必确保歌曲的时间轴准确无误，再添加[物件](https://osu.ppy.sh/wiki/zh/Hit_object)。

如果你仍然不确定歌曲的时间轴是否准确，不妨另寻一位摸图员来帮忙查查谱面中的毛病。或者你也可以去[摸图队列论坛](https://osu.ppy.sh/community/forums/60) 发布求助帖。
...
在这个界面中，你可以放置物件，并调整它们的[打击音效](https://osu.ppy.sh/wiki/zh/Beatmapping/Hitsound)。

### 设计 (Design)

*主文章：[设计 (Design)](/wiki/Client/Beatmap_editor/Design) 和[制作故事板](/wiki/Storyboard#storyboarding)*

你可以在谱面的设计 (Design) 界面中设置背景图片，添加背景视频，或者创作一个基础的故事板。
如果你会[利用脚本编写故事板 (SBS)](https://osu.ppy.sh/wiki/zh/Storyboard/Scripting) 的话，这个界面可能几乎用不到。
