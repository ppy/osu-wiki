---
outdated: true
---

# Formations sur le mapping

## Créatif

### Blanket Combos

*Aussi connu sous le nom de : **Blanket Formation***

Nominateur : ::m980::{ user=3288 }

Les **blanket combos** sont constitués d'un slider ou d'un cercle comme base) avec un slider qui entoure cet objet.
L'espacement entre ces deux objets est très important car ils ne doivent pas se superposer !

### Divebomb Combos

Nominateur : ::m980::{ user=3288 }

Les **divebomb combos** sont constitués d'une ligne droite d'objets se déplaçant du haut de l'écran (ou du milieu) vers le bas.
Cela donne l'impression que les objets sont en train de plonger.

Le dernier objet doit toujours être un slider.

### Flower Combos

*Aussi connu sous le nom de : **Flower Formation***

Nominateur : ::0\1o::{ user=56708 }

Les **flower combos** consistent en 5 cercles ou plus qui sont tous placés autour d'un point central.
Le point central peut être un cercle ou un slider.

Les objets sont généralement espacés d'un demi battement.

Vous pouvez également utiliser l'extrémité d'un slider comme premier point (ou le début d'un slider comme dernier point) de la fleur.

Ils sont vraiment similaires aux [formation en étoile](#formation-en-étoile).
Cependant, la différence ici est que cela oblige le joueur à longer les bords des cercles, et non à les traverser.

### Honeycomb Beats

Nominateur : ::mm201::{ user=30655 }

Le **honeycomb beats** est un stream de cercles qui sont généralement espacés d'un demi battement.
Ceux-ci sont placés de manière à ce qu'ils s'insèrent tous dans une grille hexagonale.

L'utilisation de cette technique entraîne une confusion quant à l'ordre des cercles en regardant les cercles seuls.
Cela oblige le joueur à regarder les chiffres et/ou les approach circles pour comprendre l'ordre ou les mémoriser.

Cela pourrait être combiné avec un faible AR (approach rate) pour confondre le joueur.

### Formation en étoile

Nominateur : ::Xgor::{ user=98661 }

Une **formation en étoile** est une étoile à 5 branches, utilisant 5 ou 6 cercles (le sixième est généralement placé sous la première branche pour compléter l'étoile).
L'étoile peut apparaître visuellement en suivant les followpoints.

Les formations d'étoiles peuvent être utilisées pour effectuer des jumps.

Vous pouvez également utiliser l'extrémité d'un slider comme premier point (ou le début d'un slider comme dernier point) de l'étoile.

Ils sont très similaires aux [flower combos](#flower-combos).
Cependant, la différence ici est que cela oblige le joueur à passer à travers pour former une étoile, au lieu de la contourner.

### The Kickstream

Nominateur : ::Daru::{ user=32480 }

Le **the kickstream** utilise un kick slider (un slider très court) à l'intérieur d'un stream.
Cela oblige le joueur à faire une pause au milieu du stream pour terminer le kick, avant de continuer le stream.

### Tornado Combos

Nominateur : ::Dragvon::{ user=126927 }

La **tornado formation** sont des ensembles de notes qui obligent le joueur à frapper des cercles dans un mouvement circulaire à un rythme élevé.
Il s'agit essentiellement d'une combinaison de petits jumps (en termes de distance) et de streams (en termes de temps de réaction).

Il peut également s'agir d'une [formation en carrée](#formation-en-carrée) et/ou d'une [formation en diamant](#formation-en-diamant) répétée plusieurs fois, où l'une des notes sert de point de pivot.

### ztrot Spacing

*Aussi connu sous le nom de : **ztrot Formation***

Nominateur : ::Jarby::{ user=25615 }

Le **ztrot spacing** utilise des battements cohérents placés dans une forme de cercle.
La distance entre chaque note augmente, puis diminue.
Les cercles suivants seront toujours situés sur des côtés opposés du "cercle".

Il est recommandé de le faire en espaçant les battements de moitié (ou plus).

Une fois le "cercle" terminé, quelques autres cercles sont placés pour le traverser de part en part.

## Symmetrical Shapes

*Voir [Techniques de mapping/Basique](/wiki/Beatmapping/Mapping_techniques/Basics#symétrie) pour une description de la symétrie.*

### Formation en cercles

¯\\\_(ツ)\_/¯

### Formation en diamant

Nominateur : QiiXshinez

La **formation en diamant** est identique à la [formation en carrée](#formation-en-carrée) ou à la [formation en carrée v2](#formation-en-carrée-v2) mais utilise des losanges au lieu de carrés.

### Double Stacks

Nominateur : ::-Xero-::{ user=179314 } et [F1r3tar](https://osu.ppy.sh/users/5590317)

Le **double stacks** utilise deux notes (ou une note et une extrémité du slider) qui sont empilées l'une sur l'autre.

### QQ Diamonds

Nominateur : ::Lybydose::{ user=64501 }

Le **qq diamonds** sont quatre cercles placés à un temps d'intervalle les uns des autres en forme de losange le long des quatre bords (et non des coins) de l'écran.
C'est similaire à la [formation en diamant](#formation-en-diamant), mais ils sont placés aux extrémités de l'axe des x et de l'axe des y.

Dans les jeux, "QQ" vient de Warcraft II, où le joueur peut appuyer sur `Alt` + `Q` + `Q` pour quitter immédiatement la bataille.
Lorsqu'un joueur vous dit "QQ" dans le tchat, il vous dit de partir parce que vous n'avez pas les compétences nécessaires pour jouer.

### Slider en zig-zag inversé

Nominateur : *none*

Le **slider en zig-zag inversé** est une variante des [sliders en zig-zag](#slider-en-zig-zag) mais les curseurs sont orientés vers l'extérieur et s'éloignent du slider suivant.
Cela crée un jump rapide pour le joueur.

### Formation en carrée

Nominateur : Beuchi-chan

La **formation en carrée** sont 4 cercles formant une forme carrée.
Ces derniers ont généralement un certain espace entre eux.
Le carré peut être incliné pour former un diamant.
L'ordre des combos oblige le joueur à longer le carré plutôt que de le traverser.

### Formation en carrée v2

Nominateur : Beuchi-chan et ::mm201::{ user=30655 }

La **formation en carré v2** est à peu près la même que la [formation en carré](#formation-en-carrée), mais l'ordre des cercles vous oblige à traverser le carré en diagonale.

### Triangle Beats

*Aussi connu sous le nom de : **Triangle Formation***

Nominateur : ::kingcobra52::{ user=9934 }

Le **triangle beats** est une technique de mapping très courante où 3 cercles sont disposés en triangle.
L'ordre des 3 cercles et la taille du triangle n'ont pas d'importance.

### Slider en zig-zag

*Aussi connu sous le nom de : **Zig-Zag Formation***

Nominateur : ::0\1o::{ user=56708 }

Le **slider en zig-zag** sont constitués de 1/2 slider disposés en zig-zag vers l'intérieur.

Cette technique nécessite au moins deux sliders d'une longueur d'un demi battement.
