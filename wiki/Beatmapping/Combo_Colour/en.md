...
In osu!standard, all combosets have a colour. A new combo is started when the colour of hit objects change. Colours will keep repeating themselves depending on the amount defined by a beatmapper in the [Song Setup](/wiki/Beatmap_Editor/Song_Setup) panel (or how many colours your skin's `skin.ini` file defines). In osu!catch, each fruit is coloured in the order given. Droplets and end-slider fruit will always have the same colour as the start-fruit. However, bananas will always have a tint of yellow. osu!mania and osu!taiko do not use combo colours.
