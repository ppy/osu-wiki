...
Le **Grid snapping** désigne l'utilisation du bouton `Grid Snap` dans l'[éditeur de beatmap](/wiki/Beatmap_Editor). Si vous l'activez, chaque [hit objects](/wiki/Hit_object) avec lequel le créateur interagit se déplace à la position la plus proche sur la grille, ou, en d'autres termes, "snapping". Le bouton `Grid Snap` se trouve sur le côté droit de l'éditeur.
