...
In [Taiko](Taiko "wikilink") mode, the song accuracy is calculated by sum of all note accuracy divided by number of notes. A GREAT (?) counts as 100%, a GOOD (?) as 50% (half) and a MISS/BAD (??) as 0% (which breaks the combo). Drumrolls and spinners do not influence the accuracy.
