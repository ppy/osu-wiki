...
Снизу экрана находится [основное окно чата](/wiki/Chat_Console). При входе в комнату в чате автоматически открывается новая вкладка `#multiplayer`, в которую [BanchoBot](/wiki/BanchoBot) отправит ссылку на историю матча с подписью `(Match history available here)`. История матча открывается по ссылке вида `https://osu.ppy.sh/community/matches/x` (где "x" — это идентификатор матча в виде цифр).
