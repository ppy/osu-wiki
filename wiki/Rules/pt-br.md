...
9.  **Em canais de idioma específico, você deve falar o idioma dominante (inglês para #english, tailandês para #thai, etc) se você está participando da conversa.** Quando o idioma de um canal não é explicitamente claro (ou especificado no nome do canal, assim como português para `#portuguese` e inglês para `#english`), converse sempre em inglês. O `#lobby` e `#help` são utilizados para qualquer tipo de idioma, diferente dos outros canais. *Membros da [equipe de moderação](/wiki/The_Team/Global_Moderation_Team) não precisam seguir essa regra caso estejam fazendo seu trabalho.*
