...
9. **특정 언어를 사용하는 채널에서는 반드시 그 언어만 사용해 주세요. (예를 들어보자면 #english 채널에서는 영어, #thai 채널에서는 태국어)** 채널에 특정 언어가 명시되지 않은 경우에는 영어만 사용이 가능하지만, 예외적으로 `#lobby`나 `#help` 채널에서는 모든 언어가 허용됩니다. *[모더레이터](/wiki/People/The_Team/Global_Moderation_Team)에 한해서, 중재를 할 때만큼은 이 규칙이 적용되지 않습니다.*
...
5. **시그니처와 아바타를 문제가 되지 않도록 설정해 주세요.** 움직이는 이미지를 아바타 혹은 시그니처로 사용하고 있다면 눈을 아프게 할 이미지는 피해주세요. 또한 느린 인터넷을 사용하는 유저들을 위해 가능하다면 500KB 미만의 이미지 사용을 권장합니다.
