...
9.  **특정 언어를 사용하는 채널에서는 반드시 그 언어만 사용해주세요. (예를 들어보자면 #english 채널에서는 영어, #korean 채널에서는 한국어)** 채널에 특정 언어가 명시되지 않은 경우에는 영어만 사용이 가능하지만, 예외적으로 `#lobby`나 `#help` 채널에서는 모든 언어가 허용됩니다. *[모더레이터](/wiki/The_Team/Global_Moderation_Team?locale=ko)에 한해서, 중재를 할때만큼은 이 룰이 적용되지 않습니다.*
