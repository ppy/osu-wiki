...
Все изображения должны оцениваться ровно в том виде, в каком они представлены, и без учёта похожих случаев. Принимайте решение на основании того, что видите.
...
В спорных случаях старайтесь выбрать решение, в целом подходящее наибольшей части сообщества.
...
- **Откровенное позирование или соблазнение**;
...
Процедура оценки, упомянутая ниже, **не применима** к изображениям, не входящим в состав карт (аватарки, обложки для профилей, юзерпейджи и т.д.) — они могут быть удалены в любое время по решению команды глобальной модерации.

Если вы сомневаетесь в допустимости изображения, которое содержится в карте в любом виде, **сообщите о нём через [веб-сайт для номинаторов](https://bn.mappersguild.com/reports).** После этого по поводу изображения будет проведено голосование, где оно будет либо одобрено значительным большинством голосов, либо автоматически отклонено.
