...
2. Сообщение проверяется NAT и GMT[^beatmap-nominators]. В любых спорных случаях один из членов NAT открывает голосование, в котором могут участвовать все номинаторы, модераторы и члены NAT.
3. Пока голосование не завершено, любые карты с данным медиа нельзя номинировать или ранкать. Если карта уже находится в Qualified, то в её обсуждении оставляют замечание, которое будет блокировать процесс ранкинга до завершения голосования.
...
Некий игрок считает, что фон карты от StableStreamer нарушает правила, и сообщает о ней по описанному выше сценарию.

Сообщение рассматривают члены GMT и NAT. Они не приходят к единому мнению, поэтому кто-нибудь из них открывает голосование и оставляет в обсуждении карты замечание, блокирующее процесс ранкинга.

Спустя 7 дней голосование завершается с результатом: 71% GMT и NAT за то, чтобы разрешить карту, против — 29%.

Поскольку порог в 70% голосов преодолён, то голоса номинаторов не учитываются.

Кто-нибудь из участников снимает с карты замечание и пишет StableStreamer, что карту можно ранкнуть без изменений.

## Примечания

[^beatmap-nominators]: [Номинаторы](/wiki/People/Beatmap_Nominators) вместо заполнения формы могут сразу открывать голосование.
