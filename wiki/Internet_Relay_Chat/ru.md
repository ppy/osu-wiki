...
Перед подключанием клиент нужно настроить, добавив туда адрес сервера (как именно это сделать, описано в документации выбранного вами клиента):
...
| `/me <action>` | Паписать в чат от третьего лица |
...
## Отключение JOIN/PART/QUIT-сообщений

По умолчанию, большинство IRC-клиентов сообщают, когда кто-то заходит на канал или покидает его. В масштабах osu!Bancho такие уведомления становятся шумом и с лёгкостью захламляют чат. Для удобства ниже описано, как их выключить в некоторых популярных клиентах:

| IRC-клиент | Где отключать |
| :-- | :-- |
...
1. Убедитесь, что вы вводите пароль со страницы [IRC Authentication page](https://osu.ppy.sh/p/irc), и что в него не попал лишний символ.
...
- Префикс `@` (статус «chat operator») даётся модератору чата ([GMT](/wiki/GMT) или [NAT](/wiki/NAT)).
