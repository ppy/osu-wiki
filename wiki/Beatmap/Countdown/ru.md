...
При включении анимацией обратного отсчёта можно управлять с помощью кнопок `Countdown Speed` (`Half`, `Normal`, `Double`) и `Countdown Offset`. Кнопки `Countdown Speed` меняют скорость анимации, замедляя или ускоряя её вдвое, а также возвращая ей обычную скорость. Кнопка `Countdown Offset` отвечает за сдвиг отсчёта относительно первой [ноты](/wiki/Hit_object), который измеряется в [ударах в минуту](/wiki/Beatmapping/Beats_per_minute). Например, анимация обратного отсчёта с оффсетом 2 начинается на 2 удара раньше, чем обычно. Если в карте есть интро, то при нажатии на кнопку «Skip» произойдёт переход сразу к обратному отсчёту.

Оформление анимации обратного отсчёта можно настроить через [скиннинг](/wiki/Skinning/Interface#countdown).
