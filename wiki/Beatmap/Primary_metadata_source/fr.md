...
Une **source de métadonnées primaire** est une source pour les champs `Artiste`, `Titre`, et `Source` dans les [métadonnées](/wiki/Beatmap_Editor/Song_Setup#song-and-map-metadata) d'une [beatmap](/wiki/Beatmap), officiellement promu par l'artiste de la chanson ou son label.
...
Les beatmaps visant le statut de [classé](/wiki/Beatmap/Category#ranked) doivent utiliser des métadonnées provenant d'une source primaire. Voir les [critères de classement](/wiki/Ranking_Criteria#metadata) pour plus de règles et de directives concernant les métadonnées des beatmaps.
