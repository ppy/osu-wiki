---
tags:
  - approach time
  - AR
  - reading
  - читалка
  - чтение
  - АР
---

# Approach rate

*Для рекомендуемых значений AR см. [Критерии ранкинга](/wiki/Ranking_Criteria)*

**Approach rate** (жарг. AR, рус. Скорость подхода) — один из параметров сложности [карты](/wiki/Beatmap), который определяет, когда [ноты](/wiki/Hit_object) начинают появляться относительно того, когда они должны быть нажаты или пойманы. Она существует только в [osu!](/wiki/Game_mode/osu!) и [osu!catch](/wiki/Game_mode/osu!catch).

Значения AR варьируются от 0 до 10. С одной стороны, более высокая AR означает, что объекты будут видны в течение более короткого периода времени, что даёт игроку меньше времени на реакцию. С другой стороны, более низкая AR даёт больше времени на реакцию, но может привести к переизбытку объектов на экране одновременно.

## Время анимации

Длительность пребывания объекта на экране (без каких-либо игровых модификаторов) варьируется от 1800 мс при AR0 до 450 мс при AR10. Уровни AR масштабируются на 120 мс для уровней ниже 5, и на 150 мс для уровней выше 5.

Обзор см. в инфографике и формулах ниже:

```
                                       X = нажатие/ловля
             p r e e m p t             ↓
├───────────────────────┬──────────────┤
0%       fade_in         100% видимости
```

Объект начинает появляться в `X - preempt`, где:

- AR < 5: `preempt = 1200ms + 600ms * (5 - AR) / 5`
- AR = 5: `preempt = 1200ms`
- AR > 5: `preempt = 1200ms - 750ms * (AR - 5) / 5`

Количество времени, необходимое для полного появления объекта, также зависит от AR:

- AR < 5: `fade_in = 800ms + 400ms * (5 - AR) / 5`
- AR = 5: `fade_in = 800ms`
- AR > 5: `fade_in = 800ms - 500ms * (AR - 5) / 5`

## Игровые модификаторы

Есть четыре игровых модификатора (мода), которые при включении изменяют AR:

- [Easy](/wiki/Game_modifier/Easy): Уменьшает значение AR вдвое.
- [Hard Rock](/wiki/Game_modifier/Hard_Rock): Умножает значение AR на 1,4, максимум до 10.
- [Double Time](/wiki/Game_modifier/Double_Time): Значение AR не меняется, хотя из-за ускорения карты на 50%, объекты остаются на экране на 33% меньше времени.
- [Half Time](/wiki/Game_modifier/Half_Time): Значение AR не меняется, хотя из-за замедления карты на 25%, объекты остаются на экране на 33% больше времени.

Хотя Half Time и Double Time не меняют значения AR, разница в скорости карты приводит к явному изменению скорости подхода. Об этих скоростях часто говорят с точки зрения их воспринимаемого значения. Например, «AR8+DT» также может быть написано как «AR9,6».

## osu!taiko и osu!mania

Скорость подхода видна при просмотре информации карты в [osu!taiko](/wiki/Game_mode/osu!taiko) и [osu!mania](/wiki/Game_mode/osu!mania), но она никак не влияет на игровой процесс. Скорость прокрутки этих режимов определяется другими факторами.

<!-- TODO: Insert lots of links -->

<!-- TODO: a chart would be nice for comparing speeds/values with mods -->
