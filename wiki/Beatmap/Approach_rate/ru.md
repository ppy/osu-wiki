...
::: alert-note
**Примечание:** Подробнее о рекомендуемых значениях AR см. [Критерии ранкинга](/wiki/Ranking_criteria)
:::
...
```
               p r e e m p t             X = нажатие/поимка
0%                        66%            ↓
├──────────────────────────┬─────────────┤
...
- *preempt* — общее время существования объекта;
- *fade_in* — период, в течение которого объект появляется;
- *full opacity* — период, в течение которого объект полностью виден.
...
Таким образом, объект становится полностью виден в последнюю треть времени его «жизни».
