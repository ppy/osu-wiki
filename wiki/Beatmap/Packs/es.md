...
Un **paquete de beatmap** es un archivo comprimido que contiene [beatmaps](/wiki/Beatmap). Se pueden encontrar en el [listado de paquetes de mapas](https://osu.ppy.sh/beatmaps/packs).

Vale la pena señalar que los paquetes tienen un tamaño promedio de aproximadamente 100 MB y contienen todos los recursos de beatmap disponibles, incluidos los vídeos.

## Nombres

El nombre de archivo de un paquete se basa en su contenido, categoría y orden de subida. Por ejemplo, en un archivo de paquete de beatmap llamado:

```
F2 - Rohi Pack.zip
```

`F2` se refiere al segundo paquete de beatmap de la categoría Artista destacado, mientras que `Rohi Pack` se refiere al nombre del paquete que se muestra en la [lista de paquetes de mapas](https://osu.ppy.sh/beatmaps/packs/F2). Este paquete solo incluye las canciones de [Rohi](https://osu.ppy.sh/beatmaps/artists/82).
...
3. El juego debería importar automáticamente los beatmaps y eliminar los archivos `.osz` originales. En caso de problemas, intente presionar `F5` en la [pantalla de selección de canciones](/wiki/Client/Interface#song-select) para reprocesar los beatmaps.
...
Se recomienda descargar paquetes de beatmaps directamente desde la lista oficial. Sin embargo, si tiene dificultades para descargar paquetes desde allí, existen dos alternativas.
