...
La tolerancia de apilamiento, junto con la [velocidad de aproximación](/wiki/Beatmap/Approach_rate), controla qué círculos y partes del slider se consideran para apilar.[^stacking-algorithm] La pila se compone de objetos que no están separados por más de `preempt * StackLeniency` milisegundos, donde `preempt` es el [ancho de la ventana de aproximación](/wiki/Beatmap/Approach_rate#animation-timing), y `StackLeniency` es el valor tomado del archivo `.osu` del beatmap.
