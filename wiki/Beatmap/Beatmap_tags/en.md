...
| gimmick/memory | A map designed around a memorization concept. | *all* |
...
| gimmick/storyboard | Includes a storyboard that changes how the map is played, usually by changing a map's visuals by using storyboard elements in place of showing the map's hit objects. | *all* |
| gimmick/video | Employs patterning that closely references the included background video. | *all* |
...
| Tag name | Description | Game mode |
| :-- | :-- | :-- |
| meta/collab | A map with two or more associated mappers. | *all* |
| meta/custom skin | Utilizes custom skin elements and graphics. | *all* |
| meta/custom song | Maps a song made specifically for the map. This includes songs created for a mapping contest which the map participated in. | *all* |
| meta/hitsounds | A map that makes use of non-default hitsound samplesets. | osu!taiko |
| meta/keysounds | Contains hitsounds that use various pitched samples to create a melody, typically following one within the song. | *all* |
| meta/marathon | A map with a drain time of over 5 minutes. | *all* |
| meta/mega collab | A map with 8 or more associated mappers. | *all* |
| meta/mega marathon | A map with a drain time of over 10 minutes. | *all* |
| meta/multi-song | Contains multiple songs within the audio. | *all* |
| meta/time signatures | Audio contains many changes in time signature or uses an uncommon time signature. | *all* |
| meta/variable timing | Contains multiple timing points, usually required for songs not recorded to a metronome. | *all* |
...
| scene/mapping contest | An entry for a mapping contest. | *all* |
| scene/tag | Includes gameplay designed for the multiplayer tag mode, where multiple players complete a map co-operatively. | *all* |
| scene/tournament custom | A custom map for a playing tournament. | *all* |
...
| skillsets/reading | Tests a player's reading skill, i.e. patterns that obfuscate note order and/or timing. | *all* |
...
| song representation/difficulty spike | Contains a sudden and significant increase in challenge within the map's gameplay. | *all* |
| song representation/high contrast | Uses flashy ideas to follow changes in the music, creating stark differences between different sections of the song. | *all* |
| song representation/high sv | Features prominent high slider velocity usage as a key part of map design. | *all* |
...
| song representation/low sv | Features prominent low slider velocity usage as a key part of map design. | *all* |
| song representation/playfield usage | Includes deliberate use of the playfield within the map design. | osu! |
| song representation/progression | Contains a gradual advancement in difficulty or concept across the map. | *all* |
| song representation/repetition | Features the use of recognizable identical patterns or gameplay elements. | *all* |
| song representation/simple | Accessible and straightforward map design. | *all* |
| song representation/swing | Features a prominent use of swing rhythms. | *all* |
...
| Tag name | Description | Game mode |
| :-- | :-- | :-- |
| style/avant-garde | Employs boundary-pushing and experimental philosophies to map design, often foregoing gameplay and aesthetic conventions to extreme measures. | *all* |
...
| Tag name | Description | Game mode |
| :-- | :-- | :-- |
| tap/bursts | Patterns requiring continuous movement and alternating, typically 9 notes or less. | *all* |
...
| tap/streams | Patterns requiring continuous note hits, typically more than 9 notes. | *all* |

### Tech

| Tag name | Description | Game mode |
| :-- | :-- | :-- |
| tech/accelerating bpm | Features progressively increasing tempo. | *all* |
...
| tech/complex snap | Maps that feature prominent usage of mixed or unusual snap divisors. | *all* |
...
| tech/speed | A map that requires constant tapping at high BPMs. | *all* |
