...
**Критерии ранкинга osu!taiko** — это свод [правил и рекомендаций](/wiki/Ranking_criteria) для [карт](/wiki/Beatmap) режима [osu!taiko](/wiki/Game_mode/osu!taiko), которые участвуют в [процедуре ранкинга](/wiki/Beatmap_ranking_procedure).
...
- **[Шаблонный тайко-фон](/wiki/Beatmap/Background/Taiko_template_background):** фоновое изображение, имитирующее аутентичное игровое поле Taiko no Tatsujin.
...
- **Не используйте [шаблонные тайко-фоны](/wiki/Beatmap/Background/Taiko_template_background),** поскольку они подходят не под все разрешения экрана.
...
- **Если [игровое время](/wiki/Beatmap/Drain_time) каждой сложности…**
  - **…короче 2:30**, то самая низкая сложность не должна быть сложнее Futsuu.
  - **…между 2:30 и 3:15**, то самая низкая сложность не должна быть сложнее Muzukashii.
  - **…между 3:15 и 4:00**, то самая низкая сложность не должна быть сложнее Oni.
...
- **Между сложностями ниже Inner Oni не должно быть значительных скачков в сложности.** Гостевым мапперам делаются небольшие поблажки ради упрощения совместной работы.
...
### Рекомендации для перерывов

Перерыв в тайко (англ. *rest moment*) — это небольшая пауза между паттернами, чтобы игрок мог передохнуть. Перерывы нужны, чтобы игроки медленнее выдыхались, особенно на более загруженных ритмах.

Перерывы можно ставить пореже, если они не подходят музыке, либо если замапанный ритм и так достаточно щадящий.

**Для каждой сложности указаны отдельные рекомендации по перерывам:**

| Сложность | Длина перерыва | Пример | Число нот |
| :-: | :-: | :-: | :-- |
| ![](/wiki/shared/diff/easy-t.png?20211215) **Kantan** | 3/1 или дольше | ![3/1](img/kantan.png "3/1") | Перерыв каждые 32–36 музыкальные доли |
| ![](/wiki/shared/diff/normal-t.png?20211215) **Futsuu** | 2/1 или дольше | ![2/1](img/futsuu.png "2/1") | Перерыв каждые 32–36 музыкальные доли |
| ![](/wiki/shared/diff/hard-t.png?20211215) **Muzukashii** (вариант 1) | 3/2 или дольше | ![3/2](img/muzu1.png "3/2") | Перерыв каждые 32–36 музыкальные доли |
| ![](/wiki/shared/diff/hard-t.png?20211215) **Muzukashii** (вариант 2) | 3 паузы по 1/1 или дольше, идущие подряд | ![3 паузы по 1/1 подряд](img/muzu2.png "3 паузы по 1/1 подряд") | Перерыв каждые 32–36 музыкальные доли |
| ![](/wiki/shared/diff/insane-t.png?20211215) **Oni** | 1/1 или дольше | ![1/1](img/oni.png "1/1") | Перерыв каждые 16-20 музыкальных долей |

### ![](/wiki/shared/diff/easy-t.png?20211215) Kantan

#### Правила

- **Используйте 1/2 только в примитивных паттернах, после которых идёт [перерыв](#рекомендации-для-перерывов).** В них также нельзя переключаться между цветом финишеров. Если песня построена на свинг-ритме, это ограничение заменяется на 1/3.
...
- **Используйте не более семи 1/1 нот подряд.** Более длинные паттерны, скорее всего, перенапрягут новичков. После подобных паттернов должен быть [перерыв](#рекомендации-для-перерывов).
- **При маппинге используйте преимущественно 2/1, 4/1 или более медленные ритмы.** Допускается иногда использовать 1/1.
- **Между нотой и началом спиннера должна быть пауза длиной минимум 1/2,** чтобы не было сильного пересечения, и оба объекта легко читались.
...
- HP Drain Rate: 8 или выше.[^hp-note]
...
- **В паттернах используйте не более двух 1/3 нот подряд.** Более длинные паттерны достаточно внезапны и, скорее всего, будут слишком сложными для новичков. После паттернов с 1/3 оставляйте [перерыв](#рекомендации-для-перерывов) длиной как минимум 2/1, где не будет паттернов с 1/2 (или более быстрых).
...
- HP Drain Rate: 7 или выше.[^hp-note]
...
- **Длина паттернов, состоящих из 1/6, не должна превышать четырёх нот при медленном BPM (~140).** Более длинные группы нот достаточно внезапны и зачастую слишком сложны. После подобных паттернов стоит вставлять [перерыв](#рекомендации-для-перерывов) (а при более быстром BPM — вообще так не мапать).
...
- **Если в паттерне больше трёх нот 1/4, то их цвет должен меняться максимум один раз, желательно — в начале или конце паттерна.** После подобых паттернов должен быть [перерыв](#рекомендации-для-перерывов). Более изощрённые комбинации нот будут слишком сложными для игроков.

#### Рекомендации по настройке сложности

- Overall Difficulty: 5 или ниже.
- HP Drain Rate: 6 или выше.[^hp-note]
...
- **Используйте не более двух 1/8 нот подряд.** Более длинные паттерны внезапны и, скорее всего, переутомят игроков. После таких паттернов должен быть [перерыв](#рекомендации-для-перерывов).
...
- **Между нотой и началом спиннера должна быть пауза длиной минимум 1/4,** чтобы не было сильного пересечения, и оба объекта легко читались.
...
- HP Drain Rate: 5 или выше.[^hp-note]
...
[^hp-note]: HP Drain Rate можно сделать немного ниже при достаточном количестве нот или длине песни, и наоборот: выше — при нехватке нот или на более короткой песне.
