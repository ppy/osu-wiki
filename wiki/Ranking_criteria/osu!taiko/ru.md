...
Перерыв в тайко (англ. *rest moment*) — это небольшая пауза между паттернами, чтобы игрок мог передохнуть. Перерывы нужны, чтобы игроки медленнее выдыхались, особенно на плотных ритмах.
...
- **Используйте не более двух 1/8 нот подряд.** Более длинные паттерны внезапны и, скорее всего, перенапрягут игроков. После таких паттернов должен быть [перерыв](#рекомендации-для-перерывов).
