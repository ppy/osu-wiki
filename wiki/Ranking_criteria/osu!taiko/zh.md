...
具体难度的规定和准则只适用于它们所对应的难度，因此*不是**每个** osu!taiko 难度都适用*。与节奏相关的规定和准则适用于 BPM 约为 180 的谱面。如果歌曲明显更快或更慢，某些规则内限制的指标会不一样。更详细的说明请参考：[上架标准中的 BPM 缩放规则](/wiki/Ranking_Criteria/Scaling_BPM)。
