...
[^proper-spread]: Подборка (англ. *proper spread*) *для сложностей уровня Rain и выше* — это набор сложностей, где между ними сложность повышается примерно так же, как между более низкими [сложностями](/wiki/Beatmap/Difficulty#difficulty-levels) из [соответствующего раздела данной статьи](#критерии-для-отдельных-сложностей).
