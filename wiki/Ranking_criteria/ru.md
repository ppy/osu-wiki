...
Некоторые разделы критериев ранкинга вынесены в отдельные статьи и действуют на все карты вне зависимости от режима игры, длины или любых других ограничений.

### Краткое содержание

- **[Упрощённые критерии ранкинга](Simplified_ranking_criteria):** пересказ самых важных моментов в доступной форме, если не нужны сложные детали из полноценной версии критериев. Обратите внимание, что эта статья не является исчерпывающей, и в случае сомнений следует использовать соответствующие статьи с критериями ранкинга.
...
- **[Содержание песен](/wiki/Rules/Song_content_rules)**: рекомендации, затрагивающие весь аудиоконтент в картах.
- **[Кодекс поведения при моддинге и маппинге](/wiki/Rules/Code_of_conduct_for_modding_and_mapping)**: правила и рекомендации, связанные с моддингом карт.
- **[Процедура ранкинга](/wiki/Beatmap_ranking_procedure)**: процесс, через который проходят все карты в ходе ранкинга.

### Общие критерии

- **[Метаданные](Metadata)**: правила и рекомендации по тому, как идентифицировать карту.

### Критерии для конкретных режимов игры

У каждого [режима игры](/wiki/Game_mode) есть дополнительные критерии ранкинга, действующие вместе с основными.
...
Изменения в критерии ранкинга можно предложить в [соответствующем разделе форума](https://osu.ppy.sh/community/forums/87).

1. **Создайте новую тему на [форуме критериев ранкинга](https://osu.ppy.sh/community/forums/87).** Постарайтесь максимально детально описать свою идею и её влияние на процедуру ранкинга.
2. **Обсудите идею с другими пользователями и придите к финальному варианту,** после чего спросите кого-нибудь из [NAT](/wiki/People/Nomination_Assessment_Team), можно ли внедрить ваше предложение. В спорных случаях NAT может действовать в качестве арбитра, но идеи, по поводу которых нет чёткого согласия, скорее всего, и так не будут приняты.
3. **Создайте пулл-реквест в [репозитории osu! wiki на GitHub](https://github.com/ppy/osu-wiki).** Обычно это делает член NAT, принявший решение по вашему предложению.
4. **Подождите одобрения [peppy](https://osu.ppy.sh/users/2).**

Правки, которые не затрагивают смысловое содержание критериев (переформулировка предложений, исправление опечаток, изменение структуры и т.д.), можно вносить без предварительного обсуждения на форуме. Их всё равно рекомендуется передавать на проверку кому-нибудь из опытных пользователей, чтобы понять, реально ли они необходимы.

## Инструменты

Для проверки карты настоятельно рекомендуется использовать [Mapset Verifier (MV)](https://github.com/Naxesss/MapsetVerifier), поскольку [AiMod](/wiki/Client/Beatmap_editor/AiMod) из стабильной версии osu! устарел и давно не обновляется, а в [osu!(lazer)](/wiki/Client/Release_stream/Lazer) инструмент для проверки [находится в разработке](https://github.com/ppy/osu/issues/12091#issuecomment-878760791).

При этом важно помнить следующее:

- Запуск автоматических проверок не избавляет вас от необходимости прочитать критерии ранкинга и самостоятельно проверить карту.
- Замечания от подобных программ нужно воспринимать критически и не следовать им бездумно.
- Программы призваны помогать мапперам и моддерам, а не заменять их.
...
- **Всё содержимое карты должно соответствовать правилам [использования медиа](/wiki/Rules/Content_usage_permissions#разрешения-артистов).** Если артист или трек отмечены на той странице как запрещённые, ранкнуть песню можно только при наличии явного разрешения на конкретный трек, которое выдано владельцу карты. Ссылку или скриншот такого разрешения нужно разместить в описании карты или на странице обсуждения.
- **Ноты, хитсаунды и тайминг карты должны быть созданы напрямую человеком, без использования генеративных инструментов.** Создание карт — это преимущественно творческий процесс, поэтому использование таких инструментов, как генеративный ИИ, неприемлемо для ранкинга.
...
- **Значения [настроек сложности](/wiki/Client/Beatmap_editor/Song_setup#difficulty) должны быть такими, чтобы их можно было выставить через [внутриигровой редактор](/wiki/Client/Beatmap_editor).**
- **Параметр `Letterbox during breaks` (чёрные полосы во время перерывов) должен быть одинаковым на всех сложностях одного и того же игрового режима, если они содержат перерывы или один и тот же сториборд.**
- **В описании карт, содержащих спойлеры к сериалам, фильмам и т.д., должно быть соответствующее предупреждение.**

### Рекомендации

- **Авторы материалов, которые используются в карте, должны быть упомянуты в её описании.** Согласно [руководству по использованию медиа](/wiki/Rules/Content_usage_guidelines), творческие работы можно использовать в osu! только с разрешения их создателей — скорее всего, в тексте лицензии будет подробно написано, как нужно указывать их создателей. Авторство можно не указывать по разным уважительным причинам, среди которых — сложность определения авторства, желание создателей остаться незамеченными, а также их упоминание в названии, источнике или исполнителе песни.
- Содержимое, созданное ИИ, должно быть помечено как таковое явным образом. Не надо выдавать его за человеческое творчество, чтобы не искажать представление о процессе создания.
...
::: alert-notice
**Внимание**
в рамках этого раздела каждый из режимов [osu!mania](/wiki/Game_mode/osu!mania) (4K, 5K и т.д.) считается отдельным режимом игры.
:::
...
- **Каждая сложность должна следовать критериям ранкинга своего режима игры** — см. критерии ранкинга для [osu!](/wiki/Ranking_criteria/osu!), [osu!taiko](/wiki/Ranking_criteria/osu!taiko), [osu!catch](/wiki/Ranking_criteria/osu!catch) и [osu!mania](/wiki/Ranking_criteria/osu!mania).
...
- **Наивысшая сложность карты должна соответствовать общей атмосфере и ритму песни.** Карта может состоять из одной сложности Easy или Normal, если её ритм не был упрощён. В противном случае необходимо добавить более высокие сложности (например, Hard).
- **Названия сложностей не должны вводить в заблуждение.**
  - **Описательные названия сложностей должны относиться к песне.** Если в название сложности добавляется какой-либо термин, например, `Tragic Love`, он должен относиться к песне.
  - **Избегайте непонятных комбинаций ников в названии [гостевых сложностей](/wiki/Beatmap/Guest_difficulty).** Например, название `Mamayucha` у [совместной сложности](/wiki/Beatmap/Beatmap_collaborations) от `Mani`, `Mari`, `Yuzu` и `Mocha` теряет почти всю информацию о соавторах по сравнению с перечислением их ников. Изменение коллаборативного ника на `Collab` или на описание вроде `osu! Mascots' Expert` может оказаться понятнее.
...
- **[«Красные» тайминг-секции](/wiki/Client/Beatmap_editor/Timing#красные-тайминг-секции) должны использоваться только для настройки тайминга,** поскольку они могут сбить пульсацию логотипа osu! в главном меню, добавить лишние звуки во время игры с модом [Nightcore](/wiki/Gameplay/Game_modifier/Nightcore), или сбить тайминг. Исключения:
  ...
  - Расстановка объектов в участках песни, где требуется делитель доли, который не поддерживается редактором (например, 1/11).
...
- **Все объекты на карте должны отклоняться от тиков на временной шкале не более чем на 2 мс.** Проверка производится через [AiMod](/wiki/Client/Beatmap_editor/AiMod); он изредка выдаёт ложные ошибки — в основном на концах слайдеров и на их реверсах — которые нужно перепроверять вручную и с помощью других инструментов. Если объект нужно поставить на момент, где в редакторе нет тика на временной шкале (например, в случае 1/11), допускается один из двух вариантов:
  - Оставить объект анснапнутым, но чтобы он был расположен согласно воображаемому делителю доли.
...
- **Аудиофайл с песней:**
  - **...должен быть в формате MP3 (`.mp3`) или [Ogg Vorbis](https://en.wikipedia.org/wiki/Vorbis) (`.ogg`).**
  - **...должен иметь средний битрейт не выше 192 кбит/с в случае `.mp3`, и не выше 208 кбит/с — в случае `.ogg`.**
  - **...должен иметь частоту дискретизации не больше 48 кГц.**
  - **...должен иметь средний битрейт не ниже 128 кбит/с**, если такой источник аудио существует. Если нет, используйте наивысшее качество аудио из того, что доступно.
...
- **Все самостоятельные объекты и их части, по которым можно кликнуть, должны иметь минимум один хитсаунд, для которого выполняются оба пункта:**
...
- **Длина аудиофайла каждого хитсаунда должна быть как минимум 25 мс.** Если файл короче, он может вообще не воспроизвестись аудиокартой.
