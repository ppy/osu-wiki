...
*若想查阅简短版本的谱面上架标准，请参阅：[简版上架标准](Simplified_ranking_criteria)。*
...
- **如果使用了[精选艺术家列表](https://osu.ppy.sh/beatmaps/artists)内授权的歌曲来制作谱面，则必须将 `featured artist` 添加到标签字段内。其他情况禁止添加。** 这个标签仅用于标注 osu! 是否拥有此歌曲的授权。
...
- **如果歌曲是非官方剪切的版本，则必须在标题结尾加上 `(Cut Ver.)` 标记。** 如果标题中已经出现了用于标记长度的字段，那么必须使用 `(Cut Ver.)` 代替它。这是为了把歌曲的剪切版和完整版区分开，以避免混淆。
  - **如果歌曲被故意剪切，并用于复现官方发布的歌曲版本时，此歌曲可不被判作裁切版本。** 这仅适用于剪切过的音频，与官方版本几乎无法区分时。
  - **如果官方版本的歌曲可拆分为相同的歌曲段落，而剪切过的歌曲仅包含官方版本中的一个完整段落时，此歌曲可不被判作裁切版本。**
- **如果歌曲是非官方延长的版本，则必须在标题结尾加上 `(Extended Edit)` 标记。** 如果标题中已经出现了用于标记长度的字段，那么应使用 `(Extended Edit)` 代替它。这是为了把歌曲的非官方延长版和官方延长版区分开，以避免混淆。
  - **如果歌曲被故意延长，并用于复现官方发布的歌曲版本时，此歌曲可不被判作延长版本。** 这仅适用于延长过的音频，与官方版本几乎无法区分时。
