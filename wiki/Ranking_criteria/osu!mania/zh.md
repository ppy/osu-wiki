...
这套 **osu!mania 上架标准**列出了 [osu!mania](/wiki/Game_mode/osu!mania) 专属的[谱面](/wiki/Beatmap)在推进[谱面上架流程](/wiki/Beatmap_ranking_procedure)时必须遵循的[规定和准则](/wiki/Ranking_Criteria)。
...
- **衩 (Bracket)**：物件在多个相邻的轨道内交替出现的键型。
- **多押 (Chord)**：需要同时按下两个或以上按键的键型。
- **胶囊（双押切，三押切，四押切） (Chordstream (Jumpstream, Handstream, Quadstream))**：混杂了不同种类多押的切换键型。
...
- **放手 (Release)**：长键含有尾判。若要获得长键物件的全部分数，则需要在长键尾靠近判定线时，精确松开按键。
...
全局规定和准则适用于所有种类的 osu!mania 难度。与节奏相关的规定和准则适用于歌曲为 4/4 拍，约 180 BPM 的谱面。如果歌曲明显快于或慢于 180 BPM，某些规则内限制的指标会不一样。更详细的说明请参考：[上架标准中的 BPM 缩放规则](/wiki/Ranking_Criteria/Scaling_BPM)
...
- **避免在 hitnormal 音效较弱时使用按键音。** 如果按键音与音乐过于相似，玩家就不能获得足够的反馈。不过，当仅使用音效来复现原歌曲时，也可以忽略此条。
...
- **如果谱面的[掉血时间](/wiki/Beatmap/Drain_time)符合以下条件：**
  - **...小于 2:00**，则每个[键位模式](#常用术语)和[游玩方式](#常用术语)的最低难度不得高于 Normal，或者每个键位模式提供包含至少 4 个难度的合理难度分布[^proper-spread]。
  - **...在 2:00 到 2:45 之间**，则每个[键位模式](#常用术语)和[游玩方式](#常用术语)的最低难度不得高于 Hard，或者每个键位模式提供包含至少 3 个难度的合理难度分布[^proper-spread]。
  - **...在 2:45 到 3:30 之间**，则每个[键位模式](#常用术语)和[游玩方式](#常用术语)的最低难度不得高于 Insane，或者每个键位模式提供包含至少 2 个难度的合理难度分布[^proper-spread]。

## 具体难度

具体难度的规定和准则只适用于它们所对应的难度，因此*不是**每个** osu!mania 难度都适用*。与节奏相关的规定和准则适用于 BPM 约为 180 的谱面。如果歌曲明显更快或更慢，某些规则内限制的指标会有所不同。更详细的说明请参考：[上架标准中的 BPM 缩放规则](/wiki/Ranking_Criteria/Scaling_BPM)。
...
- **对于胶囊键型，多押之间的间隔需大于等于 1/1 拍。** 更密集的多押会影响读图。
- **对于胶囊键型，可以偶尔使用三押，三押之间的间隔需大于等于 4 拍（1 小节）。**
...
- **对于胶囊键型，避免使用 3 个物件以上的 1/4 拍叠键。** 对于这个分段的玩家来说，击打这种键型所需的手控能力太难掌握。
- **对于较长的胶囊键型，若多押出现的频次为...**
  - ...每 1/1 拍一个，则多押包含的物件数量不能多于 3 个。允许在胶囊头尾的时间点放置四押。
...
[^proper-spread]: *对于大于等于 Insane 的难度*，“合理”的难度分布是指这些难度之间实际游玩难度的跨度，与[具体难度标准](#具体难度)中规定的较低[难度等级](/wiki/Beatmap/Difficulty#难度等级)之间的跨度相似。
