...
- **Las versiones cortadas no oficiales de las canciones deben añadir un marcador `(Cut Ver.)` al final del título actual.** Si ya hay un marcador de duración en el título de la canción, `(Cut Ver.)` debe sustituirlo. Esto se hace para distinguir los cortes no oficiales de una canción de las versiones completas.
  - **Las canciones que se cortan intencionalmente para recrear una versión oficial no se consideran versiones cortadas.** Esto solo se aplica si el audio es casi indistinguible de la versión oficial.
  - **Las canciones que son un bucle completo de una pista en bucle no se consideran versiones cortadas.**
- **Las versiones extendidas no oficiales de las canciones deben añadir un marcador `(Extended Edit)` al final del título actual.** Si ya hay un marcador de duración en el título de la pista, `(Extended Edit)` debe sustituirlo. Esto se hace para distinguir las versiones extendidas no oficiales de una canción de las versiones extendidas oficiales.
  - **Las canciones que se editan intencionalmente para recrear una versión oficial no se consideran ediciones extendidas.** Esto solo se aplica si el audio es casi indistinguible de la versión oficial.
