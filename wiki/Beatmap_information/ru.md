...
Иконки в правом верхнем углу показывают имеющиеся [режимы игры](/wiki/Game_mode), а также выбранный сейчас режим. Числа около них — это количество [сложностей](/wiki/Beatmap/Difficulty), доступных для каждого режима. Серым цветом обозначены режимы, для которых нет сложностей. Сложности, замапанные в режиме [osu!](/wiki/Game_mode/osu!), автоматически доступны в других режимах в виде [конвертов](/wiki/Beatmap/Converts).

## Отдельная сложность

![Меню с выбором сложностей](img/difficulty-menu.png "Меню с выбором сложностей")

В области, посвящённом каждой отдельной сложности, размещена самая важная информация о карте. В качестве фона используется обрезанный фон сложности.
...
Здесь указано название песни и её исполнитель, ник и аватар автора карты, а также дата публикации и последнего изменения карты.
...
Под названием сложности расположены разные счётчики: сколько раз её сыграли и добавили в избранное, а также — [в некоторых случаях](/wiki/Beatmap/Category#wip-and-pending) — число номинаций. Если навести курсор на счётчик избранного, можно увидеть аватары пользователей, добавивших карту в закладки (до 50 аватаров).

Под счётчиками указано название песни и её исполнитель. Кликнув по ним, можно перейти к поиску других карт с таким же названием или исполнителем.
...
Справа от меню, посвящённому маппингу, расположен блок статистики. Наверху в нём указана категория карты, а также — при наличии сториборда или фонового видео — соответствующие иконки. Под названием категории есть кнопка для прослушивания аудиопревью (через неё же превью можно поставить на паузу).
...
Если карта находится в категориях [Qualified](/wiki/Beatmap/Category#qualified), [Ranked](/wiki/Beatmap/Category#ranked), [Approved](/wiki/Beatmap/Category#approved) или [Loved](/wiki/Beatmap/Category#loved), то под настройками также будет виден её рейтинг. Проголосовать за карту можно один раз, если пройти любую её сложность в клиенте osu!(stable) — после этого карте можно поставить оценку по шкале от 1 до 10.

В случае положительного голоса (6 и выше) полоска будет покрашена в зелёный, в случае отрицательного — в жёлтый. Числа слева и справа показывают общее число положительных и отрицательных голосов, а число в скобках — среднюю оценку.

Диаграмма ниже показывает распределение голосов по оценкам.
...
- [Жанр](/wiki/Beatmap/Genre_and_language#list-of-genres): основной музыкальный жанр песни.
- [Язык](/wiki/Beatmap/Genre_and_language#list-of-languages): основной язык песни, если в ней есть текст, либо пометка instrumental, если его нет.
- [Теги от маппера](/wiki/Beatmap/Beatmap_tags#mapper-tags): ключевые слова, связанные с картой и песней.
...
После того, как карту ранкнули, игроки могут голосовать в osu!(lazer) за различные [пользовательские теги](/wiki/Beatmap/Beatmap_tags#user-tags). Теги, набравшие 5 и более голосов, будут показаны в строке `Теги от игроков`.
...
Когда карта попадает в категории [Qualified](/wiki/Beatmap/Category#qualified), [Ranked](/wiki/Beatmap/Category#ranked), [Approved](/wiki/Beatmap/Category#approved) или [Loved](/wiki/Beatmap/Category#loved), на ней открываются таблицы рекордов, где игроки могут соревноваться друг против друга.
...
В таблице показаны первые 50 рекордов, под наивысший из которых выделена отдельная строчка.

Если навести курсор на отдельный рекорд, в правом конце строки появятся три точки, за которыми скрыто следующее меню:
...
С помощью кнопки `Подписаться`, расположенной справа, можно начать седить за картой и получать уведомления о новых комментариях.
...
- Если маппер переименовал аудиофайл у карты, то его невозможно обновить через osu!(stable). В таких случаях автор карты обычно указывает в описании, когда её нужно удалить и перекачать.
