...
When using [Autopilot (AP)](/wiki/Gameplay/Game_modifier/Autopilot) in osu!(stable), the player cannot fail during a beatmap. This is not the case in osu!(lazer). Another difference to osu!(stable) is that scores achieved with this mod will be submitted to the global leaderboard (without awarding [performance points](/wiki/Performance_points)), show up under *Recent Plays (24h)* in player profiles and count towards play count.
