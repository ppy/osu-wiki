...
Grow has a score multiplier of `1.00x`. However, when using [Hidden (HD)](/wiki/Gameplay/Game_modifier/Hidden_(lazer)) along with Grow, Hidden's multiplier is reduced by `0.02x`, see [Hidden (HD) § Scoring](/wiki/Gameplay/Game_modifier/Hidden_(lazer)#scoring).
