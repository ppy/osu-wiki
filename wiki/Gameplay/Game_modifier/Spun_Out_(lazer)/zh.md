---
stub: true
tags:
  - SO
  - spunout
---

# Spun Out（lazer 模组）

::: Infobox

<!-- lint ignore heading-increment -->

#### Spun Out

![Spun Out 模组图标](/wiki/Gameplay/Game_modifier_(lazer)/img/mods/SO.png?1)

*转盘会自动完成。*

|  |  |
| :-- | :-- |
| 缩写 | SO |
| 类别 | 自动化 |
| 默认快捷键 | `B` |
| 游戏模式 | ![][osu!] |
| 得分系数 | `0.95x` |
| 状态 | 计入排名 |
| 不兼容的模组 | [Autoplay (AT)](/wiki/Gameplay/Game_modifier/Autoplay_(lazer)), [Cinema (CN)](/wiki/Gameplay/Game_modifier/Cinema_(lazer)), [Autopilot (AP)](/wiki/Gameplay/Game_modifier/Autopilot_(lazer)), [Target Practice (TP)](/wiki/Gameplay/Game_modifier/Target_Practice_(lazer)) |

:::

::: alert-note
**注:** 对于该文章的 osu!(stable) 版本，请见 [Spun Out (模组)](/wiki/Gameplay/Game_modifier/Spun_Out)
:::

::: alert-note
**注:** 对于 [lazer](/wiki/Client/Release_stream/Lazer) 模组的完整列表，请见：[游戏模组 (lazer)](/wiki/Gameplay/Game_modifier_(lazer))
:::

**Spun Out** 模组协助玩家自动完成所有[转盘](/wiki/Gameplay/Hit_object/Spinner)。该模组每分钟旋转转盘的速度 (SPM) 因[谱面](/wiki/Beatmap)的[判定严度](/wiki/Beatmap/Overall_difficulty)而异。与 [Autoplay (AT)](/wiki/Gameplay/Game_modifier/Autoplay_(lazer)) 及 [Autopilot (AP)](/wiki/Gameplay/Game_modifier/Autopilot_(lazer)) 不同，Spun Out 模组顺时针旋转转盘。

**该模组不支持自定义。**

[osu!]: /wiki/shared/mode/osu.png "osu!"
