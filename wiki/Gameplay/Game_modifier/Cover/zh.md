---
stub: true
tags:
  - CO
---

# Cover（模组）

::: Infobox

<!-- lint ignore heading-increment -->

#### Cover

![Cover 模组图标](/wiki/Gameplay/Game_modifier_(lazer)/img/mods/CO.png)

*缩小游玩区域的可见范围。*

|  |  |
| :-- | :-- |
| 缩写 | CO |
| 类别 | 增加难度 |
| 默认快捷键 | `K` |
| 游戏模式 | ![][osu!mania] |
| 得分系数 | 1.00x |
| 状态 | 不计入排名 |
| 不兼容的模组 | [Fade In (FI)](/wiki/Gameplay/Game_modifier/Fade_In_(lazer)), [Hidden (HD)](/wiki/Gameplay/Game_modifier/Hidden_(lazer)), [Flashlight (FL)](/wiki/Gameplay/Game_modifier/Flashlight_(lazer)) |

:::

*对于 [lazer](/wiki/Client/Release_stream/Lazer) 模组的完整列表，请见：[游戏模组 (lazer)](/wiki/Gameplay/Game_modifier_(lazer))*

与 [Fade In](/wiki/Gameplay/Game_modifier/Fade_In_(lazer)) 和 [Hidden](/wiki/Gameplay/Game_modifier/Hidden_(lazer)) 模组相似，**Cover** 模组会隐藏部分游玩区域，但支持自定义隐藏区域的大小。

## 自定义

![游戏客户端中 Cover 模组的自定义设置](/wiki/Gameplay/Game_modifier_(lazer)/img/customise/CO.png)

- **覆盖范围**（0.2-0.8，默认为 0.5）：游玩区域中音符隐藏区域的高度占比。
- **方向**（默认：与滚动方向相同）：覆盖区域延展的方向。
  - **与滚动方向相同**；从顶部盖住游玩区域。
  - **与滚动方向相反**：从底部盖住游玩区域。

[osu!mania]: /wiki/shared/mode/mania.png "osu!mania"
