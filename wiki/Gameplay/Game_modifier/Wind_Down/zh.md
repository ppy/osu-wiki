...
| 不兼容的模组 ![][osu!catch] | [Half Time (HT)](/wiki/Gameplay/Game_modifier/Half_Time_(lazer)), [Daycore (DC)](/wiki/Gameplay/Game_modifier/Daycore), [Double Time (DT)](/wiki/Gameplay/Game_modifier/Double_Time_(lazer)), [Nightcore (NC)](/wiki/Gameplay/Game_modifier/Nightcore_(lazer)), [Wind Up (WU)](/wiki/Gameplay/Game_modifier/Wind_Up) |
...
**Wind Down** 模组使用户以逐渐减慢的速度游玩。开始游玩[谱面](/wiki/Beatmap)时，歌曲播放速度为`初始速率 (Initial rate)`（见[自定义](#自定义)）并线性减退，直到谱面结束时达到`最终速率 (Final rate)`。

## 自定义

![游戏客户端内的 Wind Down 自定义设置](/wiki/Gameplay/Game_modifier_(lazer)/img/customise/WD.png)

- `初始速率 (Initial rate)` (0.51x–2.00x，默认 1.00x): 谱面开始时的速度。
- `最终速率 (Final rate)` (0.50x–1.99x，默认 0.75x): 谱面结束时达到的速度。
...
在 osu! 模式中，Wind Down 模组的得分倍率由`初始速率`与`最终速率`设置决定，计算方法如下：[^multiplier]
...
3. 该模组的得分倍率为 `0.2 * intial_multiplier + 0.8 * final_multiplier`。

#### 示例

若`初始速率`为 1.20x，`最终速率` 为 0.91x，则初始与最终倍率分别是 `1.082x` 与 `0.76x`。由此得出模组的得分倍率为 `0.2 * 1.082 + 0.8 * 0.76 = 0.8244x`。

### ![][osu!taiko] ![][osu!catch] ![][osu!mania] 其他游戏模式

在 osu!taiko、osu!catch 与 osu!mania 中，Wind Down 模组的得分倍率均为 `0.50x`。
