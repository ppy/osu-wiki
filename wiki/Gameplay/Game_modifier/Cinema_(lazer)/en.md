---
stub: true
tags:
  - CN
---

# Cinema (lazer mod)

::: Infobox

<!-- lint ignore heading-increment -->

#### Cinema

![Cinema mod icon](/wiki/Gameplay/Game_modifier_(lazer)/img/mods/CN.png)

*Watch the video without visual distractions.*

|  |  |
| :-- | :-- |
| Acronym | CN |
| Type | Automation |
| Default shortcut key | `X` |
| Game modes | ![][osu!] ![][osu!taiko] ![][osu!catch] ![][osu!mania] |
| Score multiplier | 1.00x |
| Status | Unranked |
| Incompatible mods | No Fail (NF), Sudden Death (SD), Perfect (PF), Accuracy Challenge (AC), Autoplay (AT), Relax (RX), Autopilot (AP), Spun Out (SO), Alternate (AL), Single Tap (SG), Magnetised (MG), Repel (RP), Adaptive Speed (AS), Touch Device (TD) |

:::

*For the osu!stable version of this article, see: [Cinema (mod)](/wiki/Gameplay/Game_modifier/Cinema)*\
*For the full list of all [lazer](/wiki/Client/Release_stream/Lazer) mods, see: [Game modifier (lazer)](/wiki/Gameplay/Game_modifier_(lazer))*

<!-- TODO description and settings -->

[osu!]: /wiki/shared/mode/osu.png "osu!"
[osu!taiko]: /wiki/shared/mode/taiko.png "osu!taiko"
[osu!catch]: /wiki/shared/mode/catch.png "osu!catch"
[osu!mania]: /wiki/shared/mode/mania.png "osu!mania"
