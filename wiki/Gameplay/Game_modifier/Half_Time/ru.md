...
- При активации Half Time новые значения параметров `Length`, `BPM` и `Objects` подсвечиваются голубым (см. ниже).
  - Параметр `Objects` подсвечен, хотя число объектов не изменяется.
- Около значений настроек сложности `CS`, `AR` и `HP` появляется треугольник, указывающий вниз и означающий, что значения немного понизились (см. ниже).
