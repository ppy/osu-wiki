...
Deflate has a score multiplier of ``1.00x`` by default. For each ``0.1`` step the **Starting Size** is increased above its default value, this multiplier is reduced by ``0.002x``. Decreasing the **Starting Size** below the default value does not affect the multiplier.
