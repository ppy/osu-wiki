...
Thus, the score multiplier is given by `1 - max(0, 0.02 * (starting_size - 2))`. Since `Starting Size` can be chosen from 1.0 to 25.0, the score multiplier ranges from `0.54x` to `1.00x`.
