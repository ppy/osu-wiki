---
stub: true
tags:
  - SW
---

# Swap（模组）

::: Infobox

<!-- lint ignore heading-increment -->

#### Swap

![Swap 模组图标](/wiki/Gameplay/Game_modifier_(lazer)/img/mods/SW.png)

*咚变成咔，咔变成咚！*

|  |  |
| :-- | :-- |
| 缩写 | SW |
| 类别 | 转换 |
| 游戏模式 | ![][osu!taiko] |
| 得分系数 | `1.00x` |
| 状态 | 计入排名 |
| 不兼容的模组 | [Random (RD)](/wiki/Gameplay/Game_modifier/Random_(lazer)) |

:::

::: alert-note
**注:** 对于 [lazer](/wiki/Client/Release_stream/Lazer) 模组的完整列表，请见：[游戏模组 (lazer)](/wiki/Gameplay/Game_modifier_(lazer))
:::

**Swap** 模组将[谱面](/wiki/Beatmap)中的所有[咚](/wiki/Gameplay/Hit_object/Hit)和[咔](/wiki/Gameplay/Hit_object/Hit)进行交换。[长条](/wiki/Gameplay/Hit_object/Drumroll)与[转盘](/wiki/Gameplay/Hit_object/Swell)不受该模组影响。

**该模组不支持自定义。**

[osu!taiko]: /wiki/shared/mode/taiko.png "osu!taiko"
