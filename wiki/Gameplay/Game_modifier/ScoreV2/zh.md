...
*注意：启用 ScoreV2 模组会导致得分不计表现分。*\
*对于与 ScoreV2 计分系统相关的信息，参见：[ScoreV2](/wiki/Gameplay/Score#scorev2)*

**ScoreV2** 模组会使用 ScoreV2 蓝本计分系统替代每个[游戏模式](/wiki/Game_mode)当前的计分系统。简而言之，在得分倍率为 1.00 时，ScoreV2 计分系统会尽力将[打击物件](/wiki/Gameplay/Hit_object)的总分控制在一百万分的硬性限制以下（[osu!](/wiki/Game_mode/osu!) 中的转盘加分、[osu!taiko](/wiki/Game_mode/osu!taiko) 中的转盘、[osu!catch](/wiki/Game_mode/osu!catch) 中的香蕉除外）。
