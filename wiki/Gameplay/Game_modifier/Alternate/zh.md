...
**Alternate** 模组强制玩家交替使用两个输入按键，即不能连续两次使用同一个按键。当玩家尝试使用一个键点击两个相邻的[打击物件](/wiki/Gameplay/Hit_object)时，会自动将第二个物件判定为 Miss。在每个[休息时段](/wiki/Beatmap/Break)后，玩家可以重新选择要使用的按键。[转盘](/wiki/Gameplay/Hit_object/Spinner)总可以使用任意按键完成。
