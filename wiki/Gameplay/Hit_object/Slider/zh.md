...
*滑条 (slider)* 是 [osu!](/wiki/Game_mode/osu!) 中的[打击物件](/wiki/Hit_object)，含有滑条头、滑条体和滑条尾。一旦缩圈像[打击圈](/wiki/Hit_object/Hit_circle)一样到达滑条头边缘，玩家应当点击或触碰滑条起点，按住不放同时跟着移动的球（也被称作滑条球）沿着轨迹移动，直到到达滑条尾。如果有折返箭头，玩家应当跟随滑条球沿相同路径返回。带返回箭头的滑条被称作[回转滑条](/wiki/Hit_object/Reverse_slider)。滑条的速度由[滑条速度](/wiki/Hit_object/Slider_velocity)与当前时间点下`时间轴`部分的滑条速度系数决定。
...
谱师也可以在 `测定时间轴 (Timing)` 部分调节谱面上滑条点的数目。更多的点使连击数目更快上升（每个点对应一个连击），因此也提供了得到更高分数的机会。但由于[判定](/wiki/Gameplay/Judgement)更严，滑条点也为玩家带来了更高的失败机率。
