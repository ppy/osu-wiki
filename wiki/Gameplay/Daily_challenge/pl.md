---
no_native_review: true
---

# Wyzwania dnia

**Wyzwania dnia** to wieloosobowy tryb gry w [osu!(lazer)](/wiki/Client/Release_stream/Lazer), w którym gracze mogą budować serię poprzez przechodzenie kolejnych beatmap dnia ze zwiekszającą się trudnością, która resetuje się co 7 dni.

Każda beatmapa jest starannie wybierana przez grupę współtwórców. Czasami beatmapy mogą być wybierane według wspólnego gatunku, artysty lub stylu mapy. Pod koniec każdego miesiąca jeden tydzień przeznaczony jest na prezentację wysokiej jakości beatmap z poprzedniego miesiąca. Zajmuje się tym kilkoro członków [Zespołu Zapewniana Jakości (NAT)](/wiki/People/Nomination_Assessment_Team).

Beatmapy mają niekiedy narzuconego moda, z którym użytkownik musi przejść mapę.

Obecne i wszystkie poprzednie wyzwania dnia można znaleźć na [stronie rankingów wyzwań dnia](https://osu.ppy.sh/rankings/daily-challenge).

## Menu gry

Z menu głównego do menu wyzwań dnia można przejść w następujący sposób:

1. Kliknij przycisk `graj` lub naciśnij `P`.
2. Kliknij przycisk `wyzwanie dnia` lub naciśnij `D`.

![](img/daily-challenge-menu.png?20250824 "Zrzut ekranu menu wyzwań dnia z dnia 24 sierpnia 2025.")

Po wejściu w menu użytkownik może zobaczyć prezentację mapy, którą musi przejść, oraz wymagane mody.

Po zakończeniu prezentacji, użytkownik może wyświetlić dane dotyczące dzisiejszego wyzwania. Lewa strona przedstawia rożne informacje dotyczące wyników, takich jak łączna ilość przejść i sumę ich wyników. Znajdująca się pośrodku tabela wyników pokazuje najlepsze wyniki uzyskane przez graczy. Użytkownicy mogą dyskutować o wyzwaniu dnia na czacie po prawej.

## Zakres trudności beatmap

![](img/Beatmap-selection-criteria.png "Dokładne zakresy trudności, których beatmapy muszą przestrzegać przez cały tydzień")

## Kamienie milowe serii

Serie są wyświetlane na profilu użytkownika w różnych kolorach w zależności od osiągniętego kamienia milowego:

<!-- zdjęcia poziomów: https://www.figma.com/design/tc79qAgJ35KQvdTO0Oj3dN/Daily-Challenge-Counter?node-id=0-1&t=xjRm9Ke0tUMtAQlh-1 -->

|  | Poziom | Łączny udział | Długość serii w dniach | Długość serii w tygodniach |
| --: | :-: | :-: | :-: | :-: |
| ![Olśniewający poziom](img/tiers/lustrous.png) | Olśniewający | 1080 dni | 360 dni | 53 tygodni |
| ![Promienisty poziom](img/tiers/radiant.png) | Promienisty | 720 dni | 240 dni | 36 tygodni |
| ![Rodowy poziom](img/tiers/rhodium.png) | Rod | 360 dni | 120 dni | 19 tygodni |
| ![Platynowy poziom](img/tiers/platinum.png) | Platyna | 180 dni | 60 dni | 10 tygodni |
| ![Złoty poziom](img/tiers/gold.png) | Złoto | 90 dni | 30 dni | 6 tygodni |
| ![Srebrny poziom](img/tiers/silver.png) | Srebro | 30 dni | 10 dni | 3 tygodnie |
| ![Brązowy poziom](img/tiers/bronze.png) | Brąz | 15 dni | 5 dni | 2 tygodnie |
| ![Żelazny poziom](img/tiers/iron.png) | Żelazo | mniej niż 15 dni | mniej niż 5 dni | mniej niż 2 tygodnie |

## Współtwórcy

Projekt jest organizowany przez ::{ flag=TN }:: [Hivie](https://osu.ppy.sh/users/14102976). Za wybieranie beatmap odpowiedzialni są poniżsi użytkownicy:

- ::{ flag=IT }:: [-kevincela-](https://osu.ppy.sh/users/266596)
- ::{ flag=SE }:: [byd](https://osu.ppy.sh/users/6398464)
- ::{ flag=FI }:: [fllecc](https://osu.ppy.sh/users/14060327)
- ::{ flag=IT }:: [gansijiye](https://osu.ppy.sh/users/9704802)
- ::{ flag=KR }:: [momoyo](https://osu.ppy.sh/users/12469536)
- ::{ flag=MX }:: [Riot](https://osu.ppy.sh/users/4256461)
- ::{ flag=US }:: [TheMagicAnimals](https://osu.ppy.sh/users/17274052)
- ::{ flag=SE }:: [Walavouchey](https://osu.ppy.sh/users/5773079)
- ::{ flag=US }:: [Willy](https://osu.ppy.sh/users/3521482)
- ::{ flag=US }:: [Wispy](https://osu.ppy.sh/users/11106929)

## Ciekawostki

::: Infobox
![](img/yt-comment.jpg "komentarz użytkownika waxxx14 na YouTube sugerujący format \"cup of the day\"")
:::

- Pomysł na wyzwania dnia został wzięty z komentarza użytkownika waxxx14 proszącego o format "cup of the day" z [TrackManii](https://en.wikipedia.org/wiki/TrackMania) w osu!, zamieszczonego pod filmem o rozwoju osu!(lazer) ["deciding what to do with lazer"](https://www.youtube.com/watch?v=xUSxEjQQ1UI).
- Wyzwanie dnia zostało wypuszczone 25 czerwca 2024 w publicznej wersji [2024.725.0](https://osu.ppy.sh/home/changelog/lazer/2024.725.0) osu!(lazer) wyłącznie dla [trybu gry osu!](/wiki/Game_mode/osu!).
- Na początku nie można było wybierać otwartych modów, a wyświetlanie łącznej ilości przejść i sumy ich wyników zostało dodane dopiero w późniejszej aktualizacji.
