...
L'écran devrait s'assombrir et un son sera joué lorsque le quick restart sera relancé.
Relâcher trop tôt ne permettra pas au quick retry de s'effectuer.
...
- Jouer uniquement le pass storyboard,
- Mettre l'assombrissement de l'arrière-plan à 0%, et
- jouer les hitsounds sur les points de synchronisation où les notes seraient complétées.
...
Cliquez sur `Show Comments` pour activer les commentaires qui défileront concernant la beatmap(set) au milieu de l'écran.
...
Le replay sera mis sur pause, assombri, avec le logo d'osu! en avant-plan au centre jusqu'à ce que le commentaire soit terminé (appuyez sur `Entrée`) ou annuler (appuyez sur `Echap`).
...
Ajouter cette beatmap(set) aux *Collections*.
Ce bouton ouvrira le menu des collections pour ajouter la beatmap(set) en cours dans une des collections ou renommer les collections.
...
La vitesse de replay change entre `1x`, `2x`, `0.5x`, ou revenir à la vitesse de lecture `1x` par clic.
...
Il est fortement conseillé de **ne pas renommer le nom du fichier crypté**, et d'utiliser la touche `F2` d'exportation à la place.
...
Il est impossible d'essayer de redémarrer le beatmap pour la rejouer directement dans cet état.
...
Les replays ne peuvent **pas être sauvegardé en interne et ne pas être pris en compte dans le classement local**.
...
L'ouverture d'un fichier exporté du mode Auto placera le jeu dans le classement local avec un modificateur de jeu Auto unique utilisé.

Notez que la notice **UNRANKED** ne peut pas être retirée.

#### Partie fictive

Lorsque vous cliquez sur le bouton `Voir le skin dans une partie fictive` dans le `Options`, Auto va **aléatoirement choisir une difficulté/beatmap** basé sur le **mode de jeu actuellement sélectionné (choisi sur l'écran de sélection des beatmaps en Solo)** à un **moment aléatoire de la beatmap**.

Dans ce type de replay, Auto manquera de temps en temps les objets pour montrer à quoi ressemble les éléments du skin.
Si le bouton `Voir le skin dans une partie fictive` n'a pas été appuyé de nouveau pour changer de difficulté/beatmap et que celle-ci se termine, l'écran de résultats de cette partie apparaîtra et les `Options` seront fermée.

Les `Options` ne peut pas être rouverte pendant la partie ou sur l'écran de résultats.
...
Si un nouveau record global du top 50 a été ajouté, le précédent détenteur de la position \#50 sera supprimé du replay côté serveur.
...
En fonction de l'intervalle de temps aléatoire sélectionné, il peut commencer à la *fin de la musique* sans aucun objet, ce qui conduit directement à l'écran des résultats avec 0 score, 0 combo, 0x hit bursts, note D, aucun graphique de performance et une précision de 100,00%.
