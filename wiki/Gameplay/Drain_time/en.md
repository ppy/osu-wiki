...
The beatmap info page on the [website](https://osu.ppy.sh) calculates drain time (labelled `Drain length`) based on only hit objects' start times, so if the last object of the map is a [slider](/wiki/Hit_object/Slider) or [spinner](/wiki/Hit_object/Spinner), the drain time will be displayed incorrectly.
