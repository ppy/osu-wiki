...
С выходом osu!(lazer) вышла и изменённая версия нотлока, которая не была перенесена в stable. Изменение заключается в принципе работы: первый объект перестает блокировать второй объект после того, как временное окно первого объекта достигло смещения в 0 мс.

![](img/notelock-v2.gif)

Благодаря изменениям принципа работы нотлок больше не убьет вас на картах со [стримами](/wiki/Beatmap/Pattern/osu!/Stream) или [джампами](/wiki/Beatmap/Pattern/osu!/Jump) с низким [OD](/wiki/Beatmap/Overall_difficulty) из-за одного промаха, потому что он позволит вам нажать следующую ноту в такт и не заблокирует её.
