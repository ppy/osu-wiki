...
Их обычно называют количеством очков за них (за исключением промахов), т.е GREAT обычно называют "300" и так далее.
...
[Ноты](/wiki/Gameplay/Hit_object/Hit_circle) могут быть оценены как GREAT, OK, MEH, и как MISS, исходя из точности вашего нажатия. Нажатие на ноту до того момента, как начнёт действовать окно попадания, не дает эффекта (вы получите [нотлок](/wiki/Gameplay/Judgement/Notelock)), и непопадание в ноту приведет к промаху после того, как окно попадания выйдет за предел оценки "MEH".
...
Чтобы попасть слайдер, надо нажать на него в пределах окна попадания "MEH". Однако, если активен [ScoreV2](/wiki/Gameplay/Game_modifier/ScoreV2), начало слайдера будет требовать точности попадания, а итоговая оценка слайдера будет ограничена оценкой за попадание в начало.

Также есть некоторые особенности того, как слайдер влияет на [комбо](/wiki/Gameplay/Combo_(score_multiplier)):

- Слишком раннее нажатие на слайдер (перед началом действия временного окна "MEH"), промах по слайдер-тику и промах в реверс не приводят к полноценному промаху, но приводят к [сбросу комбо](/wiki/Gameplay/Judgement/Combobreak). Остальные части слайдера можно пройти, удерживая на нем кнопку. Это прямо относится к [слайдербрейкам](/wiki/Gameplay/Judgement/Slider_break).
- Непопадание в конец слайдера не приведет к промаху, но не даст дополнительное комбо.

### Спиннеры

Каждый [спиннер](/wiki/Gameplay/Hit_object/Spinner) имеет свое количество вращений для его прохождения. Это количество зависит от [OD](/wiki/Beatmap/Overall_difficulty#sliders-and-spinners) карты. Скорость вращения спиннера подсчитывается, исходя из скорости курсора, и не всегда совпадает с реальным количеством вращений.
...
Если спиннер очень короткий, то необходимое количество вращений может быть принято за 0 и спиннер пройдется автоматически.

## История

Алгоритм оценки спиннеров значительно поменялся в [версии Stable 20190513.2](https://osu.ppy.sh/home/changelog/stable40/20190513.2). Вот что изменилось:

- Разница между OK и MEH, также как между MEH и GREAT, была равна половине оборота, делая все оценки, кроме промаха, куда сложнее.
