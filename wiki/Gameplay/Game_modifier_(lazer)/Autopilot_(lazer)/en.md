---
stub: true
tags:
  - AP
  - auto pilot
---

# Autopilot (lazer mod)

::: Infobox

<!-- lint ignore heading-increment -->

#### Autopilot

![Autopilot mod icon](/wiki/Gameplay/Game_modifier_(lazer)/img/mods/AP.png)

*Automatic cursor movement - just follow the rhythm.*

|  |  |
| :-- | :-- |
| Acronym | AP |
| Type | Automation |
| Default shortcut key | `V` |
| Game modes | ![][osu!] |
| Score multiplier | `0.10x` |
| Status | Unranked |
| Incompatible mods | [Autoplay (AT)](/wiki/Gameplay/Game_modifier_(lazer)/Autoplay_(lazer)), [Cinema (CN)](/wiki/Gameplay/Game_modifier_(lazer)/Cinema_(lazer)), [Relax (RX)](/wiki/Gameplay/Game_modifier_(lazer)/Relax_(lazer)), [Spun Out (SO)](/wiki/Gameplay/Game_modifier_(lazer)/Spun_Out_(lazer)), [Magnetised (MG)](/wiki/Gameplay/Game_modifier_(lazer)/Magnetised), [Repel (RP)](/wiki/Gameplay/Game_modifier_(lazer)/Repel), [Touch Device (TD)](/wiki/Gameplay/Game_modifier_(lazer)/Touch_Device_(lazer)) |

:::

::: alert-note
**Note:** For the osu!(stable) version of this article, see [Autopilot (mod)](/wiki/Gameplay/Game_modifier/Autopilot)
:::

::: alert-note
**Note:** For the full list of all [lazer](/wiki/Client/Release_stream/Lazer) mods, see [Game modifier (lazer)](/wiki/Gameplay/Game_modifier_(lazer))
:::

The **Autopilot** mod automatically moves the cursor through the [beatmap](/wiki/Beatmap) perfectly, allowing the user to play by only clicking on the [hit objects](/wiki/Gameplay/Hit_object) on time. The mods also moves the cursor in a spinning motion whenever a [spinner](/wiki/Gameplay/Hit_object/Spinner) appears, but the player still has to press a key or mouse button for the spinner to be completed. Like [Autoplay (AT)](/wiki/Gameplay/Game_modifier_(lazer)/Autoplay_(lazer)), Autopilot spins counterclockwise at a speed of 477 spins per minute (SPM).

In osu!(lazer), completing a beatmap with Autopilot triggers a score submission without awarding [performance points](/wiki/Performance_points). It is also possible, unlike in osu!(stable), to fail the beatmap during the play.

**This mod cannot be customised.**

[osu!]: /wiki/shared/mode/osu.png "osu!"
