...
*Полный список см. тут: [Список сезонов Beatmap Spotlights](Seasons)*
...
Руководство также менялось. В мае 2012 г. проект перешёл под руководство ::{ flag=US }:: ::SapphireGhost::{ user=388602 },[^charts-manager-sg] затем его сменили ::{ flag=US }:: ::DeathXShinigami::{ user=49516 } [^charts-manager-dxs] и — позднее — ::{ flag=US }:: ::Makar::{ user=686389 }.[^charts-manager-makar] В декабре 2013 г. им на смену пришли ::{ flag=DE }:: ::Loctav::{ user=71366 } и ::{ flag=DE }:: ::OnosakiHito::{ user=290128 }.[^charts-manager-loctav]
...
В марте 2017 г. чарты получили современное название — Beatmap Spotlights[^charts-renamed-into-spotlights] — но сохранили суть, разнообразив её дополнительными наградами вроде медалей, а также более существенным процессом презентации карт. Во время внутренних перестановок в Quality Assurance Team проект перешёл под руководство ::{ flag=HU }:: ::Kurokami::{ user=260933 } и вновь вернулся к идее отдельной команды, ответственной за выбор карт. В ноябре 2018 г. частота выпуска подборок замедлилась до ежесезонной.[^spotlights-seasonal] В марте 2020 г. проектом занялся ::{ flag=DE }:: ::Loctav::{ user=71366 }, и вместе с Kurokami они придумали ему новый формат, а заодно сформировали отдельную команду кураторов.[^spotlights-reworked-20-june]

::{ flag=HU }:: ::Kurokami::{ user=260933 } ушёл из Beatmap Spotlights в августе 2020 г. В ноябре того же года ушёл и ::{ flag=DE }:: ::Loctav::{ user=71366 }, после чего проект переняли ::{ flag=PL }:: ::Venix::{ user=5999631 } и ::{ flag=US }:: ::pishifat::{ user=3178418 }.

После завершения весеннего сезона 2021 г. проект был заморожен до сентября, когда на смену ::{ flag=US }:: ::pishifat::{ user=3178418 } пришёл ::{ flag=TN }:: ::Hivie::{ user=14102976 }. В феврале 2022 г. Beatmap Spotlights продолжил работу после очередных структурных и внутренних изменений.

В октябре 2023 г. менеджером проекта стал ::{ flag=AU }:: ::Crumpey::{ user=3518705 }, занявшийся разными организационными вопросами. ::{ flag=TN }:: ::Hivie::{ user=14102976 } ушёл с поста менеджера в ноябре того же года.
