...
-   `HP Drain (HP)`: 4 or less
-   `Circle Size (CS)`: 2 or 3
-   `Approach Rate (AR)`: 4 or less
-   `Overall Difficulty (OD)`: 4 or less
-   `Slider velocity (SV)` (found in the [Timing](/wiki/Beatmap_Editor/Timing) tab): According to the BPM, can be from 0.8x on faster songs to 1.6x on slower songs.

You will want to use [distance snapping](/wiki/Beatmap_Editor/Distance_Snap) to allow equal distances over time. So, **adjust the distance snap multiplier so that hit circles only one beat apart do not overlap each other.** For a Circle Size (CS) of 4, a distance snap of 0.5 should work. New players often find hit circles that overlap each other like that hard to read. Most of the time, just having them next to each other without overlapping is good enough.
...
Now, your main rhythm here should be every other beat. Start with the [downbeat](/wiki/Beatmap_Editor/Timing) and start off by placing a hit circle every other beat. However, you will find that this is boring and puts you to sleep. This is why we go back and edit things to spruce things up a bit so that players will not totally die of boredom. You are going to want to do three things:

Being on the beat
-----------------

Assume that a `.` is a beat without a [hit circle](/wiki/Hit_Objects), and an `O` is a beat with a hit circle (Easy difficulties should be mapped to the beat for the most part, with a few exceptions). Here is a simple pattern you can try:
...
That pattern works well too. If you want to be lazy, you can use the first pattern during the verse and the second pattern during the chorus. But, you do not want it to be too obvious that you are being lazy, so you can try to use even patterns with red ticks like: (on 1/2) `O..O..O`
...
2-beat-long sliders and measure-long sliders are intuitive here. 1-beat-long sliders often do not work well here simply because your slider velocity is so low that 1-beat-long sliders look like stubs, not sliders. A general rule of thumb is if your slider start and end circles are overlapping each other, your slider is too short and/or your slider velocity is too low.
...
Avoid using sliders that only use 1/2 of a beat in the timeline. The rhythm these are conveying are often easily covered by extending a previous slider. 1/2 repeat sliders are just plain confusing to new players since they will not know when the slider will stop repeating.
