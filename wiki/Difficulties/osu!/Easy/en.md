...
You will want to use [distance snapping](/wiki/Beatmap_Editor/Distance_Snap) to allow equal distances over time. So, **adjust the distance snap multiplier so that hit circles only one beat apart don't overlap each other.** For a Circle Size (CS) of 4, a distance snap of 0.5 should work. New players often find hit circles that overlap each other like that hard to read. Most of the time, just having them next to each other without overlapping is good enough.
...
Assume that a `.` is a beat without a [hit circle](/wiki/Hit_Objects), and an `O` is a beat with a hit circle (Easy difficulties should be mapped to the beat for the most part, with a few exceptions). Here's a simple pattern you can try:
