...
_For a list of in-game client commands, see: [Chat Console](/wiki/Chat_Console#commands-list)_
...
_Notice: The `REQUEST` command is not included because it is no longer supported._

_Note: The user argument is case-insensitive, as usernames are case-insensitive._
...
_Notice: The `user` parameter for the `REPORT` command is missing._
...
_Note: BanchoBot will not tell you which game mode the stats came from._

_Notice: BanchoBot will respond with `User not found` if the user has never played a game, even if the user does exist._
...
_Note: By default, this only shows the user's country. If the user has `Share your city location with others` enabled, it may also display their city._
...
_Notice: If you want to report a moderator, send an email to [support@ppy.sh](mailto:support@ppy.sh) for moderator complaints._

_Note: You can also use the user card to report a user._
