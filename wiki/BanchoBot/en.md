...
`!report` notifies the [Global Moderation Team](/wiki/Global_Moderation_Team) about a user's inappropriate behaviour. If a user has spaces in their username, replace them with underscores (e.g. `really cool username` becomes `really_cool_username`). To report a moderator, refer to the [Global Moderation Team § Staff complaints](/wiki/People/The_Team/Global_Moderation_Team#staff-complaints) section. An example of reporting a normal user through BanchoBot is shown below:
