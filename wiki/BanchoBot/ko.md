...
_인-게임 클라이언트 명령어의 목록은 [Chat Console](/wiki/Chat_Console#확장-채팅-콘솔) 을 참조하세요._
...
_주의: `REQUEST` 명령어는 더 이상 지원되지 않으므로 이 문서에 포함되어 있지 않습니다._

_부록: 유저 관련 명령어는 대소문자 구별이 없습니다. 이는 유저 닉네임에 대소문자 구별이 없음을 따른 것입니다._
...
_부록: BanchoBot은 출력된 스탯이 어떤 모드를 기준으로 출력된 것인지에 대한 정보를 제공하지 않습니다._

_주의: `User not found` 의 답장이 오는 것은 해당 유저가 게임을 한 번도 플레이하지 않은 경우일 수 있으며, 이는 계정이 존재해도 발생할 수 있습니다._
...
_부록: 이 명령어는 기본적으로 해당 유저가 플레이 중인 국가를 출력합니다. 만약 해당 유저가 `위치 정보(도시) 공유하기` 옵션을 활성화한 경우, 도시 정보도 같이 보여줄 수 있습니다._
...
_주의: 관리자를 신고할 일이 생길 경우 [support@ppy.sh](mailto:support@ppy.sh) 로 신고 내용을 보내주세요._

_부록: 유저 카드에 있는 신고 항목을 이용할 수도 있습니다._
