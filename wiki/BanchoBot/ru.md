...
**BanchoBot** *(БанчоБот)*, иногда называемый просто *Банчо*, — чат-бот в osu!, созданный для помощи игрокам в чате и показывающий игровые оповещения (например, о количестве повторных попыток или о перезапуске сервера). Помимо этого, он обеспечивает работу [Bancho IRC](/wiki/Community/Internet_Relay_Chat) (Internet Relay Chat). Автором бота является ::{ flag=NZ }:: ::Echo::{ user-id=431 }.

Кроме того, у бота есть собственные профили в ::osu!::{ user-id=3 } и [Twitter](https://twitter.com/banchoboat).
