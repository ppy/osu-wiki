...
`!stats` показывает краткую статистику пользователя и его текущий статус. Работает она по тому игровому режиму, в который указанный пользователь играл в последний раз, но при этом BanchoBot не сообщает, про какой игровой режим идёт речь. Если пользователь еще ни разу не сыграл в osu!, то BanchoBot выведет `User not found` (`Пользователь не найден`), даже если игрок с таким ником существует. Пример:
...
Команда `!stats` может отобразить один из семи статусов: Editing (Редактирует), Idle (Не занят), Lobby (Ищет лобби), Modding (Делает мод), Multiplayer(В лобби), Multiplaying(Играет в лобби) и Playing(Играет). Пример использования команды на пользователе, который сейчас играет:
