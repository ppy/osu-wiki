...
1. Gehe zum Ordner der Beatmap.
   - Du kannst es leicht erreichen, indem du `Datei` und dann `Songs-Ordner öffnen` im [Beatmap-Editor](/wiki/Client/Beatmap_editor) anklickst.
...
4. Gebe deinen Benutzernamen bei `Creator` an. (Wenn dein Name Leerzeichen enthält, dann nutze Leerzeichen.)
5. Setze `BeatmapID` auf `0`.
6. Setze `BeatmapSetID` auf `-1`.
7. Überprüfe, dass der Name des Beatmap-Ordners nicht mit einer Zahlenfolge beginnt.
...
7. Gebe irgendetwas anderes als Nutzername bei `Creator` ein, aber nutze nicht deinen Benutzernamen.
8. Setze `BeatmapID` auf `0`.
9. Setze `BeatmapSetID` auf `-1`.
10. Sobald du fertig bist, packe die Dateien wieder in die `.zip`-Datei.
11. Nenne die `.zip`-Datei um zu `.osz`.
