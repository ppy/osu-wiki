...
## Methods of timing songs with #/8 time signatures
...
This is the go-to-method for timing songs with complex time signatures, as it significantly reduces the timing effort and ensures easy handling of the timeline. As a mapper, you are always allowed to use the more accurate first method, but are only forced to use it when the song's beat value is consistently of three eighth notes (e.g. in "Erghen Diado", both timing methods are acceptable as the song has an inconsistent beat-value of `[2+2+3]`). For mixed meters, the start of every measure is required to land on a big white tick. Compound and complex `#/2` or `#/16` time signatures are to be dealt with similarly.
