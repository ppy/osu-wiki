...
在`视频设置`界面，将`基础（画布）分辨率`设置为显示器的原生分辨率，并将 `FPS` 设置为 `60`。你也可以选择 `60 或 30 均可，优先选择 60`，但如果电脑难以将帧率保持在 60 FPS，则也不易录制出高质量的 osu! 视频。
...
默认情况下，OBS Studio 会输出 `.mkv` 文件。这种视频格式能够应对 OBS 意外崩溃与使用单独的音频频道的情况，但许多视频编辑器无法处理，因此推荐将输出格式切换为 `.mp4`。
...
- 如果你的录制配置超出了电脑的承受范围，OBS Studio 的左下角会显示警告。在这种情况下，视频回放时很有可能会出现延迟。

![视频设置](img/recording_osu_3.png "此处列出 osu! YouTube 频道上视频使用的配置以供参考")

## 录制

在 OBS Studio 的主界面有`场景`与`源`两个框。一个场景由多个源组成，但出于本教程考虑，只会添加一个源来显示你的 osu! 窗口。
...
如果你只要录制 osu! 游戏本身，大概会想去在`混音器`中，点击`麦克风/Aux` 一栏的喇叭图标将其静音。
