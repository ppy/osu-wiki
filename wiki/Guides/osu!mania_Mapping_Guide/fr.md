...
Très bien ! Faisons glisser le `.mp3` de la chanson que nous voulons mapper dans osu! et une nouvelle map sera créée automatiquement. Chaque fois que vous créez une beatmap pour la première fois dans l'éditeur, la configuration de la chanson s'ouvre automatiquement.
...
| Nom | Description |
| :-- | :-- |
| Artist : | Ici, vous devrez ajouter le nom original de l'artiste de vos chansons. Peu importe qu'il soit japonais, allemand ou russe. Si le nom de l'artiste comporte au moins un caractère spécial, vous devez l'ajouter ici ! Par exemple : Si le nom de votre artiste est "Die Ärzte", vous devez l'ajouter ici à cause du "Ä". |
...
| Title & Romanised Title : | Exactement la même chose que pour l'artiste, mais avec le titre de la chanson. |
...
| Difficulty : | [Règles de dénomination des difficultés des critères de classement**](/wiki/Ranking_Criteria#beatmap) Vous indiquez ici le nom de la difficulté. À titre indicatif, les noms de difficulté standard pour osu!mania sont "Easy", "Normal", "Hard", "Insane" et "Expert". Si votre chanson est issue d'un autre jeu, vous pouvez utiliser les noms de difficulté de ce jeu ! Par exemple Sound Voltex, qui utilise les noms suivants pour leurs difficultés respectives : "BASIC", "NOVICE", "ADVANCED", "EXHAUST", "INFINITE", "GRAVITY".  Pour les noms personnalisés, reportez-vous aux critères de classement liés ci-dessus. |
| Source : | Ici, vous devez indiquer d'où vient votre chanson. Est-ce qu'elle vient d'un autre jeu ? Ou d'un anime ? Peut-être d'une émission de télévision ? Ajoute-le ici ! Par exemple : Si votre chanson vient de Sound Voltex, vous devez l'ajouter ici. N'oubliez pas qu'il doit s'agir du nom exact du jeu ! |
| Tags : | Vous pouvez ajouter des informations supplémentaires sur votre chanson ici. Par exemple, des noms d'albums ou de producteurs. Tout ce qui peut aider à trouver votre chanson dans la liste des beatmaps. Vous devez également ajouter le nom de chaque mappeur d'osu! qui a fait une difficulté dans votre mapset. Les balises sont séparées par des espaces. |
...
| Widescreen Support : | Cette case ne doit être activée que si votre map a un storyboard en écran large. |
...
Beaucoup de chansons n'ont pas un BPM constant. Pour ceux-là, vous devez ajouter un autre point de BPM au point de changement.
...
**Les difficultés "Insane" et "Extreme"** : Maintenant, nous nous concentrons sur les difficultés Insane et Extreme. Encore une fois, nous augmentons la densité du pattern et commençons à utiliser des motifs encore plus compliqués ! Vous pouvez maintenant utiliser toutes sortes de pattern et essayer de les combiner. De plus, vous pouvez maintenant utiliser des LN 1/8 ainsi que des chord trills ! Une difficulté Insane et Extreme est surtout destinée aux joueurs plus expérimentés, donc vous devriez essayer de créer une difficulté vraiment agréable à jouer et toujours excitante. Essayez de combiner les jumpstreams avec les trills et le heavy chord mashing. Vous pouvez maintenant utiliser les chords à 3 notes pour faire plus que simplement souligner la fin d'une certaine partie de votre chanson. Mais n'oubliez pas : Si vous envisagez d'ajouter une autre difficulté, par exemple une difficulté supplémentaire, vous ne devez pas aller jusqu'aux limites ! Vous devez laisser un peu d'espace pour créer une difficulté encore plus difficile. Je ne peux pas vous dire grand-chose car vous devez simplement créer une difficulté plus difficile que la difficile dans une bonne répartition. Si vous prévoyez d'ajouter une difficulté supplémentaire, vous pouvez aller jusqu'aux limites de la chanson. Vous pouvez utiliser des jumpstreams lourds avec des chordmashs lourds. Vous pouvez utiliser des chords de 3 notes dans des jumpstreams entre des chords de 2 notes. Vous devez essayer de repousser les limites de la musique. Mais prenez note de ce qui suit : Une ligne directrice dit que vous devez être capable de passer vos propres difficultés et nous sommes d'accord avec ce point. Si vous voulez vraiment créer une bonne difficulté insane/extreme jouable, vous devez savoir comment elles se jouent et pour cela vous devez être capable de les passer.
...
Les Hitsounds sont importants dans tous les modes de jeu. Ils donnent un retour d'information lorsque vous appuyez sur une note, ce qui permet au joueur de savoir ce qu'il vient de frapper. De plus, le fait d'avoir différents hitsounds dans votre beatmap peut lui donner un sentiment spécial car la musique sonne différemment tout d'un coup, peut-être même mieux que sans hitsounds ! osu!mania propose deux façons différentes d'utiliser les hitsounds.
...
Les hitsounds suivants peuvent être modifiés dans le dossier des chansons :
...
**Note également :** Le "xxx-hitnormal" est le hitsound qui est joué lorsque vous ne donnez pas à la note un hitsound spécifique. Par exemple, lorsque vous changez le "soft-hitnormal", toutes les notes de votre map qui ont le sample set soft actif auront ce hitsound ! Soyez prudent lorsque vous les modifiez.
...
Tous vos hitsounds du sample set 1 sont maintenant activés ! Si vous utilisez un autre sample set, choisissez le troisième point et choisissez le sample set que vous avez également choisie pour vos hitsounds.

Une fois que vous avez enfin vos hitsounds, n'oubliez pas d'activer "Lock Notes". De cette façon, aucune note ne pourra être déplacée lorsque vous ferez un mouvement sur votre beatmap. Le Hitsounding ne prend pas beaucoup de temps. Vous pouvez choisir une ou plusieurs notes en même temps, changer le jeu d'échantillons en haut à gauche de l'écran et ajouter l'effet sur le côté droit. Maintenant, toutes ces notes ont un hitsound !
...
Le Slider Velocity agit un peu différemment dans osu!mania ; elle modifie la vitesse à laquelle les notes défilent vers le bas. Plus le Slider Velocity est élevée, plus les notes descendent vite, et plus il est faible, plus les notes descendent lentement.

Ceci est principalement utilisé pour mettre l'accent sur des parties spécifiques des beatmaps ! Par exemple, lorsque la chanson devient calme et plus lente, vous pouvez utiliser un slider velocity faible, pour que la beatmap corresponde à l'humeur de la musique. Mais quand la musique a quelques breaks courts et rapides, vous pouvez utiliser une vitesse plus élevée pour rendre la carte plus excitante, de cette façon, elle s'adapte encore mieux à la chanson !

Vous devez également ajouter les changements de SV à votre beatmap lorsque votre chanson comporte plusieurs changements de BPM. Par exemple : Si votre chanson passe de 150 à 200 BPM, la vitesse de défilement de vos notes va également augmenter ! Pour éviter cela, nous devons ajouter un point SV au changement de BPM pour ajuster la vitesse de défilement au BPM moyen.
...
Le style DCP/LCP met l'accent sur la pertinence musicale grâce à des techniques de mapping telles que la pertinence de la hauteur et la théorie du jack. (La théorie du jack consiste essentiellement à créer des jacks pour mettre en valeur un certain instrument, comme un crash de cymbale). Les chartes DCP/LCP sont plus appréciées par les autres chartes que par les joueurs car elles se concentrent principalement sur la pertinence musicale et pas tellement sur la jouabilité.

Le style yolomania s'appuie plus sur le paternalisme qu'autre chose. Il s'attache davantage à créer de bons tableaux jouables pour le joueur et la communauté. Bien qu'il utilise toujours le concept de pertinence de la hauteur, il n'est pas de la plus haute importance.
