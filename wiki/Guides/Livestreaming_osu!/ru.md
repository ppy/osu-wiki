...
Следующим разделом, который необходимо настроить, являются параметры видео. Выберите разрешение экрана для поля 	`Базовое (холстовое) разрешение` (должно определяться автоматически) и `Kибо 60, либо 30, но предпочитайте 60, когда это возможно` в поля `FPS`.
