---
no_native_review: true
no_native_review_since: 84e7e8be479d7188143e72f8db15ce19981e4a10 LmanTW
---

# 指南

以下是一個由 osu! 社群中的成員所製作的簡短指南清單，大部分的文章最初都來自 osu! 論壇，後來才被收入進 osu! wiki。

## 圖譜製作

*主頁面: [圖譜製作](/wiki/Beatmapping)*

- [音頻編輯指南](Audio_editing)
- [更改標題文字](/wiki/Beatmap/Title_text#changing-title-text)
- [壓縮檔案](Compressing_files)
- [自訂義打擊音效庫](Custom_hitsound_library)
- [自定義音效名稱](Custom_sample_overrides)
- [如何更改圖譜的藝術家或標題](Changing_the_artist_or_title)
- [如何解決圖譜提交系統的問題](BSS_issues)
- [如何開始一個圖譜專案](Starting_a_beatmap_project)
- [如何為歌曲定軸](How_to_time_songs)
- [樂理](/wiki/Music_theory)
- [osu!mania 圖譜製作指南](osu!mania_mapping_guide)
- [調整偏移值來對其節拍](Setting_the_offset_on_the_correct_beat)
- [使用自定義打擊音效](Using_custom_hitsounds)
- [定軸 #/8 拍的歌曲](Timing_songs_with_N_per_8_time_signatures)

## 摸圖

*主頁面: [摸圖](/wiki/Modding)*

- [常見的摸圖錯誤](Common_modding_mistakes)
- [如何讓你的圖譜被審核](Getting_your_map_modded)
- [osu!mania 摸圖指南](osu!mania_modding_guide)

## 遊玩

- [如果使用偏移嚮導](How_to_use_the_Offset_Wizard)
- [新手教學](Beginner's_tutorial)

## Skin 製作

*主頁面: [Skin 製作指南與重要的論壇帖](/wiki/Skinning/Guides_and_important_threads)*

- [如何從複雜的背景中裁切圖片](Cropping_with_complex_backgrounds)
- [如何從簡單的背景中裁切圖片](Cropping_with_simple_backgrounds)
- [如何讓 Skin 中的大輪盤正確的居中](Making_properly_centred_spinners)

## 其他

- [Discord 遊戲狀態](Discord_Rich_Presence)
- [值播 osu!](Livestreaming_osu!)
- [OpenGL 支援問題](OpenGL_support_issues)
- [錄製 osu!](Recording_osu!)
