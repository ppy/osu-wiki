...
For a simpler example, take a look at the provided set of Queen's "We Are The Champions", which is a song that's purely in the `6/8` time signature. Examining the song's timing reveals that the small white ticks in-between the measures land precisely on the song's actual beats, and all the eighth notes are mapped by using the 1/3 snap divisor.
