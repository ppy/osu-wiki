...
**AiMod** 是一种内置在beatmap editor](/wiki/Beatmap_Editor)的一个工具 用于检测[beatmap](/wiki/Beatmap)中的问题。 可以通过菜单选项打开AiMod `File` > `Open AiMod` 或者是快捷键](/wiki/Shortcut_key_reference) `Ctrl` + `Shift` + `A`。

虽然AiMod可用于查找谱子中的问题，但是它不能完全替代真人完成Modding。AiMod不能评估地图质量或查找复杂的问题，比如[技术类](/wiki/Beatmap/Pattern)或者[timing](/wiki/Guides/How_to_Time_Songs)。 在[创建地图](/wiki/Beatmapping)的时候, 建议先解决方案AiMod的警告和错误，然后再去寻求其他[modders'](/wiki/Modding/Modder)的帮助。

##  标签

- **All:** 显示全部问题
- **Compose:** 只显示关于Compose的问题。这些问题一般都是物件放置的问题，可能需要选中`Check distance snap` 选项框查看物件距离问题（因为检查物件距离可能会导致一些延迟，取决于铺面的长度/大小）。
- **Design:** 只显示关于Design的问题。这些问题大部分是关于背景、故事版等。
- **Timing:** 只显示关于Timing的问题。当出现这些问题时，请检查时间线，可能是放置错误。
- **Meta:** 只显示关于Meta的问题。当出现这些问题时请检查[song setup](/wiki/Beatmap_Editor/Song_Setup)。
- **Mapset:** 只显示关于Mapset的问题，这些问题会影响到整个铺面。
...
| There are no hitsounds on any objects. Hitsounds are required for rankable beatmaps. | *如提供信息所示*  | 在铺面上添加whistles, claps和finishes这些hitsound |
...
| 禁止同时存在超过6个物件。 | 仅osu!mania谱面，大多数键盘仅允许同时按下6个键。 | 检查您的铺面有没有出现6键多按的情况 |
...
| 缺少文件:{0} | 您的铺面缺少了文件。 | 尝试恢复丢失的文件或者确保铺面没有使用不存在的文件 |
...
| Easy/Normal难度中变速太多 | 仅osu!mania 谱面 |  |
...
另外 建议使用其他程序，比如 ![][flag_SE] [Naxess](https://osu.ppy.sh/users/8129817) 的 [Mapset Verifier](https://github.com/Naxesss/MapsetVerifier/releases)。除了AiMod，如果你想让你的铺面被[Ranked](/wiki/Beatmap/Category#ranked)。其他程序可以比AiMod更了解最新的[Ranking criteria](/wiki/Ranking_Criteria)，并且能检测到的问题比AiMod多。但是，请不要完全忽略了AiMod，因为[Ranking criteria](/wiki/Ranking_Criteria)中的某些规则需要依赖于它。
