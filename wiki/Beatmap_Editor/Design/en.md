...
For the readings, **x/y** are your _own_ mouse coordinates on the beatmap and will change frantically as you move your mouse. **{number}ms** is the timestamp in milliseconds. **SB Load** is the amount of processing power required to play the storyboard _alone_ only. Generally, keep the SB load as low as possible (1.00~2.00) during playtime and let it roll during breaktime/intro/outro.
...
-   `.osu` (osu!beatmap difficulty): Difficulty-specific file. Holds _a lot_ of information.
...
-   Sprite coordinate is _always_ 320,240. You will need to use Move command once to set the location (endpoint not required).
    -   If you are _also_ doing [Storyboard Scripting](/wiki/Storyboard_Scripting), you will need to _read an extra line_ per object done in Design tab.
