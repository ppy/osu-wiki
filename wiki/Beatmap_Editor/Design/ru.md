**Редактор сториборды** — одна из вкладок [редактора карт](RU:Beatmap_Editor "wikilink"), позволяющая легко (относительно [ручного скриптинга](RU:Storyboard_Scripting "wikilink")) создать собственный [сториборд](RU:Storyboarding "wikilink").

Возможности
-----------

-   Показывает текущую позицию курсора и время в миллесекундах.
-   Показывает текущую [загрузку сториборды](RU:SB_Load "wikilink").
...
Для действий выше встроенный редактор карт не годится, для соответствующих изменений нужно использовать [ручной скриптинг](RU:Storyboard_Scripting "wikilink").
