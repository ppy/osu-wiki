...
| ULEB128 | 可变   | 一个可变长度的Integer。参阅 <wikipedia:ULEB128>.                                                                                                                                                                 |
| String  | 可变   | 拥有3部分： 第一个byte如果是0x00，表示下面两部分不存在。如果是0x0b（十进制为11），表示下面两部分存在。如果是0x0b，下面会有一个ULEB128，代表字符串的长度。然后是字符串本身，使用UTF-8编码，参阅 <wikipedia:UTF-8> |
...
[Category: File Formats/ZH](Category:_File_Formats/ZH "wikilink")
