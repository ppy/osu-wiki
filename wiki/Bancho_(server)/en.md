...
**Bancho** (previously *osu!bancho*) is a code name of the server component which handles real-time interactions between players running stable versions of osu!:
...
During maintenance periods, Bancho is sometimes restarted. This is relayed to all online users via an in-game announcement system, as well as periodic warning messages in the chat with countdown. Bancho restarts only affect chat and multiplayer, as all other functionality is handled via different APIs. [^score-upload-ref]
