...
| Name | Effect | Notes |
| --- | --- | --- |
| `Background dim` | Darken the playfield (including storyboards). | During breaks, the dim is decreased by 30% (max 0%). This effect can be disabled in the options. |
...
| `Ignore beatmap hitsounds` | Use the the player's selected skin's hit sounds instead of the beatmap's custom hit sounds, if any. | Requires a retry to take effect. |
| `Disable video` | Do not play the background video. This does not remove the storyboard. | Requires a retry if activated after the gameplay begins. This option is disabled if there is no background video to play. |
