...
**시각적 설정**은 플레이 화면 밑에 가려져 있는 숨겨진 설정입니다.
게임플레이 도중 일시정지를 했을때나, 비트맵을 불러오고 있을때 변경 가능합니다.
커서가 설정창 안에서 벗어나기 전까지 비트맵은 일시정지된 상태로 유지됩니다.
...
| `스토리보드 끄기` | 스토리보드를 끕니다(kiai time 배경비디오 등등 포함). | 이설정은 간질 위험이 있는 비트맵을 플레이할때 추천됩니다. 만약 비트맵에 스토리보드가 없다면 활성화가 되지 않습니다. |
