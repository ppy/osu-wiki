...
- \[11\] Число скачанных [сложностей карт](/wiki/Beatmap/Difficulty) (несмотря на подпись), длительность игровой сессии и текущее системное время.
...
| Сортировка | Описание |
| :-: | :-- |
| `By Artist` | Карты сортируются в алфавитном порядке по исполнителям. |
...
Строка поиска позволяет отфильтровать сложности по заданным критериям. По умолчанию osu! использует полнотекстовый поиск и показывает только те карты, которые содержат все указанные слова. Кроме того, по картам можно искать по [Approach Rate](/wiki/Beatmap/Approach_rate), [сложности в звёздах](/wiki/Beatmap/Star_rating) и многим другим параметрам с помощью фильтров вроде `ar=8` или `stars>=5`.
