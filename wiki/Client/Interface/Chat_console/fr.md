...
Vous pouvez afficher la console de chat d'osu! sur la plupart des écrans en appuyant sur `F8` ou sur `Show Chat` en bas à droite de l'écran.

![La console de chat](img/Chatconsole1.png "La console de chat")
...
- Cliquez sur la case `Show Ticker` pour afficher le dernier message lorsque la console de chat n'est pas ouverte.
- Cliquez sur la case `Auto-Hide` pour cacher automatiquement la console de chat pendant le gameplay (Sauf pendant l'intro, l'outro et les pauses).
- Cliquez sur la case `Hide Chat` ou de nouveau sur `F8` pour cacher la console de chat.
...
Sur la plupart des écrans vous pouvez cliquer sur `F9` ou sur `Online Users` pour afficher la console de chat étendue. En plus de la console de chat, le chat étendu superpose une liste de panneaux dans les deux tiers restants de l'écran, affichant des informations sur les utilisateurs actuellement connectés à osu!.

![La console de chat étendue](img/Chat_Console-Extended.png "La console de chat étendue")
...
- Les utilisateurs qui n'ont pas de statistiques dans leur panneau sont connectés au chat en utilisant leur client IRC.
...
3. `Discuter` : Ouvre un onglet de chat privé avec l'utilisateur.
...
7. `Ignorer l'utilisateur` : Toute entrée de chat par l'utilisateur n'apparaîtra pas dans votre console de chat.
...
| `/chat [user]` | Ouvre un nouvel onglet de chat avec l'utilisateur spécifié. | `/chat Amigo` | (L'onglet Amigo est ouvert) |
| `/clear` | Efface le contenu du chat actuel. | `/clear` | (Efface pratiquement tout ce qui se trouve dans l'onglet actuel) |
| `/ignore [user][@chp]` | Ignore tous les messages de l'utilisateur spécifié pour cette session. En ajoutant un @ suivi des lettres, c, h, et/ou p, vous pouvez les ignorer respectivement dans le chat, les highlights, ou les PMs. | `/ignore Amigo@chp` | BanchoBot: You will no longer hear Amigo {chat} {highlights} {PM} (Votre console de chat est réglée sur : ignore any text written by Amigo \[c\], any possible text highlighting you by Amigo \[h\] any Private Message sent to you by Amigo \[p\]) |
...
| `/unignore [user]` | Arrête d'ignorer cet utilisateur pour cette session. | `/unignore Amigo` | You may now hear Amigo. (Votre console de chat permettra d'accéder à tout commentaire fait par Amigo.) |
...
| `/np` | Imprime dans le chat la musique en cours d'écoute ou en cours de jeu. | `/np` | (Si vous jouez) * John is playing [Peter Lambert - osu! tutorial \[Gameplay Basics\]](https://osu.ppy.sh/beatmapsets/3756#osu/22538) |
| `/reply` ou `/r` | Répondre au dernier message privé reçu. | `/r Connais tu un bon médecin ?` | (Dans l'onglet d'Amigo) \[Commentaires précédents\] John: Je suis malade à la maison. Amigo: Really? John: Connais tu un bon médecin ? |
| `/savelog` | Enregistre l'onglet de chat actuel dans un fichier texte. | `/savelog` | (Un dossier appelé "Chat" sera créé dans le répertoire d'osu! qui contiendra toutes les sauvegardes des futurs onglets de chat.) |
...
| `F8` | Agrandit/Réduit la console de chat. |
| `F9` | Agrandit/Réduit la console de chat étendue. |
