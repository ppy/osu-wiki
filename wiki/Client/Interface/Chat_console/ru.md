...
*В [osu!academy](/wiki/Community/Video_series/osu!academy) рассказали об этом в [6 эпизоде (6:52)](https://www.youtube.com/watch?v=cyYRl-a5xII), как и про [мультиплеер](/wiki/Gameplay/Multiplayer).*
...
| `/ignore [пользователь][@chp]` | Скрывать все сообщения пользователя до перезапуска игры. Дописав после ника символ `@` и буквы `c`, `h` или `p`, вы можете скрыть сообщения в чате, упоминаниях или личной переписке соответственно. | `/ignore Amigo@chp` | BanchoBot: You will no longer hear Amigo {chat} {highlights} {PM} (Теперь все сообщения \[c\], все упоминания \[h\] и личные сообщения \[p\] от Amigo будут скрыты. |
