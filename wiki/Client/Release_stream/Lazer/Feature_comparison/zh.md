...
玩家在看到滑条后能理解怎么去击打。但这也增加了计算游玩的复杂度（也潜在地增加了回放中的错误，尤其是判定严度 (OD) 与滑条速度 (SV) 较高时）。

这个改变背后的理由很简单：如果不这样做，当玩家较晚点击滑条头时，启用`渐出滑条`设置时，从视觉上来考虑，会很麻烦。比如，较晚点击时，在滑条头原处留个圆圈，推迟滑条头缩回的时机？如果按照这样做，那么滑条头该怎么样跟上滑条球的实际位置？除此之外，难道还能暂不缩回滑条，让滑条头与滑条主体分离吗？不要考虑太多。
