...
Если osu!(lazer) установлен на компьютере, место хранения файлов можно перенести, открыв настройки игры и нажав на кнопку `Change folder location...`.
...
Такой подход позволяет экономить место на диске, не давая расплодиться разным копиям одного и того же файла, а также ограждает файлы от вмешательства со стороны пользователей и других программ. Неудобства, вызванные тем, что файлы одного объекта «разбросаны», можно обойти, либо экспортировав карту или скин, либо изменив их внутри игры. Если инструментов для редактирования нет, они со временем будут добавлены.
...
В большинстве версий Windows osu!(lazer) сможет самостоятельно импортировать данные из стабильной версии клиента, не создавая лишних копий. Это возможно благодаря тому, что операционная система поддерживает *жёсткие ссылки*.
...
Поскольку жёсткие ссылки работают на уровне файловой системы, для этого типа миграции должны выполняться два условия. Во-первых, оба клиента должны быть установлены на один и тот же диск, а во-вторых, у этого диска должна быть файловая система NTFS.
