...
*提示：本文列出的是默认键盘快捷键你可以在[设置](/wiki/Client/Options)中修改它们。*
...
*提示：按下快捷键来打开或关闭 Mod。您必须进入 Mod 菜单才可以使用这些快捷键。*\
*提示：Target practice (osu!)，1K 和 2K (osu!mania)，Co-op (osu!mania)，还有 Mirror (osu!mania) 没有快捷键。*
...
| `Ctrl` + `C` | 复制。 |
| `Ctrl` + `V` | 粘贴。 |
| `Ctrl` + `D` | 仿制选择的区域。这将自动粘贴在所选物件的一小节之后。 |
| `Ctrl` + `M` | 调整节拍细分。 |
| `Delete` | 删除。 |
...
| `Ctrl` + `X` | 剪切所选素材。 |
| `Ctrl` + `C` | 复制所选素材。 |
| `Ctrl` + `V` | 在当前位置粘贴所选素材。 *提示：除非保存并重新加载谱面，否则粘贴的素材不会显示在图层列表中。* |
| `Delete` | 从图层中删除选定的素材。 |
...
| 在调整 BPM 和偏移量时 `Shift` | 按住以5倍正常的量调整。 |
| 在调整BPM时 `Ctrl` | 按住以0.25调整。 |
| 在调整偏移量时 `Ctrl` | 按住以1调整。 |
| 在调整滑条速度时 `Ctrl` | 按住以0.1调整。 |
