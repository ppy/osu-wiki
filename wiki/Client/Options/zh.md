...
如果你还没登录到游戏客户端，启动时设置菜单会自动打开，由此提醒你登录。你可以按 `Esc` 键、点击返回按钮或转到歌曲选择页面忽略它。如果没有登录，客户端会将你标为“游客 (Guest)”，你也无法获取在线分数、上传分数与玩[多人模式](/wiki/Client/Interface/Multiplayer)。
...
| `登录` | 用给定信息登录 osu!。在用户名或密码输入框中按`回车`键也可以登录。 | 按钮 |  |
...
| 名称 | 描述 | 类型 | 默认值 |
| :-- | :-- | :-- | :-- |
| `更新版本` | 显示能够使用的版本列表。 | 下拉菜单 | `Stable` |
| `osu! 已经是最新版！` | 点击这里来让客户端强制检查更新。如果有则下载更新。 | 按钮 |  |
| `打开 osu! 所在文件夹` | 打开本地 osu! 文件夹，其中存有你的皮肤、谱面等文件。 | 按钮 |  |

---

打开`更新版本`的下拉菜单后，会出现如下选项：

| 名称 | 描述 |
| :-- | :-- |
| `正式版（每月更新）` | 稳定的公开发行版。 |
| `测试版（不定期更新）` | 开发版——更早获得更新，但 bug 可能更多。 |
...
| 名称 | 描述 | 类型 | 默认值 |
| :-- | :-- | :-- | :-- |
| `帧数限制` | 设置帧数限制，详情见下。 | 下拉菜单 | `Optimal` |
| `显示帧数 (FPS)` | 打开 FPS 计数器，显示在屏幕右下角。 | 复选框 | `禁用` |
| `兼容模式` | 使用 osu! 过去用于旧电脑的渲染器。会重启客户端。 | 复选框 | `禁用` |
| `减少掉帧` | 调整图形设置，以减少掉帧与卡顿。 | 复选框 | `禁用` |
| `检测到性能问题` | 若电脑上的其他进程可能会影响客户端性能，则会发出警告。 | 复选框 | `启用` |
...
| `全屏切屏时最小化 (Minimise on alt-tab during fullscreen)` | 按下 `Alt` + `Tab` 时，最小化主窗口。 | 复选框 | `启用` |
| `按本地分辨率渲染 (Render at native resolution)` | 完全使用本地分辨率，但会在屏幕中央较小的地方显示 osu!。 | 复选框 | `禁用` |
...
- 如果禁用`全屏模式`，下方会出现`注意：非全屏模式下可能会有输入延迟！`提示。

### 详细设置

| 名称 | 描述 | 类型 | 默认值 |
| :-- | :-- | :-- | :-- |
| `渐出滑条` | 滑条会从起点缓缓“滑出”。 | 复选框 | `启用` |
| `背景视频` | 在游玩期间播放谱面背景视频（可按谱面单独禁用）。 | 复选框 | `启用` |
| `故事板 (Storyboards)` | 在游玩期间显示谱面故事板（可按谱面单独禁用）。 | 复选框 | `启用` |
| `连击提示图` | 在达成连击里程碑时显示提示图。 | 复选框 | `禁用` |
| `击打闪光` | 在每次点击后显示细微的发光效果。不会禁用 Kiai 时间的闪光。 | 复选框 | `启用` |
| `着色器效果` | 呈现影院级（图形）效果。如果你的电脑不支持，可能会自动禁用。 | 复选框 | `禁用` |
...
| `显示提示` | 每次访问主菜单时都显示一条提示。（测试版除外） | 复选框 | `启用` |
| `问候语` | 在打开游戏时播放 “welcome” 音效，关闭游戏时播放 “see ya” 音效。 | 复选框 | `启用` |
| `osu! 主题音乐` | 当启用时，客户端启动后会播放主题音乐。在切换歌曲后，只能重启客户端才能再次播放主题音乐。 | 复选框 | `启用` |
| `节日背景图片 (Seasonal backgrounds)` | 将获奖的同人图设置为主菜单（以及没有背景图片的谱面）的背景。切换歌曲时会循环显示。 | 下拉菜单 | `Sometimes` |

---

打开`节日背景图片 (Seasonal backgrounds)` 的下拉菜单后，会出现如下选项：

| 名称 | 描述 |
| :-- | :-- |
| `偶尔显示 (Sometimes)` | 在每个节日开始的几周内会使用节日背景图片。osu! 开发团队将择期移除节日背景，换用你习惯的朴素背景。 |
| `从不显示 (Never)` | 不使用节日背景图片，总是使用默认背景。 |
| `总是显示 (Always)` | 总是使用当前节日的背景图片。 |
...
| `休息时段不改变背景暗度 (Don't change dim level during breaks)` | 休息时段期间不要调亮背景。 | 复选框 | `禁用` |
| `游戏中显示排行榜 (Show leaderboard during gameplay)` | 单人游戏与多人游戏期间，在左侧显示排行榜。 | 复选框 | `启用` |
| `进度条位置` | 配置歌曲进度条的显示位置与方式。详情见下。 | 下拉菜单 | `右上角（饼状）` |
...
打开`进度条位置`的下拉菜单后，会出现如下选项：

| 名称 | 描述 |
| :-- | :-- |
| `右上角（饼状）` | 使用饼图显示歌曲开始前与结束前的剩余时间，显示于准确度的左侧。 |
| `右上角（条状）` | 使用短横条显示结束前的剩余时间，显示于分数下方、准确度上方。 |
| `右下角` | 使用短横条显示结束前的剩余时间，显示于右下角。 |
| `底部（长条）` | 使用长横条显示结束前的剩余时间，显示于底部。 |

---

打开`分数计模式`的下拉菜单后，会出现如下选项：

| 名称 | 描述 |
| :-- | :-- |
| `无` | 不使用指示器显示玩家的打击计时情况。 |
| `色块模式` | 使用色块显示打击计时情况。（osu!catch 总会使用此设置） |
| `打击偏差` | 使用指示条显示打击计时情况。会显示玩家是否打击太早或太晚。 |
...
| 名称 | 描述 | 类型 | 默认值 |
| :-- | :-- | :-- | :-- |
| `全局偏移量 (Offset)` | 所有谱面使用的偏移量（以毫秒为单位，除本地偏移量外） | 滑块 | `0ms` |
...
| 名称 | 描述 | 类型 | 默认值 |
| :-- | :-- | :-- | :-- |
| `皮肤示例图` | 显示所选皮肤中的多种游玩元素。点击可以循环显示不同元素集。 | 按钮 |  |
| `当前皮肤` | 更改皮肤。下拉菜单中的项目根据 `osu!/Skins` 文件夹中的内容给出（名称从文件夹名获取）。 | 下拉菜单 | `Default` |
| `预览皮肤` | 使用 Auto 模组随机游玩谱面来预览皮肤。游戏模式取决于选歌页面中的选择。 | 按钮 |  |
...
| 名称 | 描述 | 类型 | 默认值 |
| :-- | :-- | :-- | :-- |
| `鼠标灵敏度` | 调节鼠标光标的灵敏度，更改时会自动启用`原始输入`。 | 滑块 | `1x` |
| `原始输入 (Raw input)` | 不经过后期处理，直接从硬件读取鼠标或数位板的位置信息（以忽略鼠标加速）。 | 复选框 | `禁用` |
| `绝对坐标只映射在 osu! 窗口中` | 将带有绝对坐标功能的输入设备（比如数位板）限制在客户端窗口中。 | 复选框 | `启用` |
...
| 名称 | 描述 | 类型 |
| :-- | :-- | :-- |
| `按键设置` | 显示一个对话框，用于查看或更改按键绑定设置。详情参见[按键绑定](/wiki/Client/Options/Keyboard_bindings)。 | 按钮 |
| `osu!mania 键位排列` | 显示一个对话框，用于查看或更改 osu!mania 按键绑定设置。详情参见 [osu!mania 布局](/wiki/Game_mode/osu!mania)。 | 按钮 |
...
| `游戏中自动隐藏聊天窗口` | 如果在休息时段或游戏内菜单中打开了聊天窗口，游玩继续时其将自动隐藏。 | 复选框 | `启用` |
| `当其他玩家提到你的名字时弹出通知` | 当有人在聊天中[提到你的用户名](/wiki/Client/Interface/Chat_console/Highlight)时显示通知。 | 复选框 | `启用` |
| `显示新消息提醒` | 当收到新消息时显示通知。 | 复选框 | `启用` |
| `当其他人提到你的名字时播放音效` | 当有人在聊天中[提到你的用户名](/wiki/Client/Interface/Chat_console/Highlight)时播放音效。 | 复选框 | `启用` |
| `显示你的所在城市或地区` | 在用户卡片中显示你的城市位置（注意，国家或地区是总会显示的）。 | 复选框 | `禁用` |
| `显示旁观者` | 游玩期间在屏幕左侧显示当前旁观者列表。 | 复选框 | `启用` |
| `自动向观众发送图的下载地址` | 当你被旁观时，将正在游玩的谱面发送到 `#spectator` 频道。 | 复选框 | `启用` |
| `游戏中实时显示通知` | 在游玩期间显示通知。如果禁用，客户端会在游玩结束后再显示通知。 | 复选框 | `启用` |
| `显示好友上线/离线通知` | 当好友上线或下线时显示通知。 | 复选框 | `启用` |
| `允许来自所有玩家的多人游戏邀请` | 允许任何人向你发送多人游戏邀请。如果禁用，就只允许好友发出邀请。 | 复选框 | `启用` |

### 关联

| 名称 | 描述 | 类型 | 默认值 |
| :-- | :-- | :-- | :-- |
| `Discord Rich Presence` | 向 Discord 提供[详细状态数据](/wiki/Guides/Discord_Rich_Presence)。 | 复选框 | `启用` |
...
查看游戏客户端的当前版本，以及接收更新的类型。点击这里后，会用默认浏览器打开更新日志页面。
...
- `{修订版本号}` 是修订的版本号
  - 如果没有修订版本号，会自动移除前面的点。
- `{类型}` 是版本类型
  - 如果版本类型值为空，则默认为`稳定版 (Stable)`。

## 冷知识

- 如果你填入了用户名，但将密码框留空，则 osu! 在本地保存分数时会使用此名字。
- 打开侧边栏会自动触发 `osu! 已经是最新版！`的功能（开始检查更新）。
- 在 osu! 开发团队收到积极反馈后，加入了`节日背景图片 (Seasonal backgrounds)` 设置。
  - 详见：[main menu background changes](https://osu.ppy.sh/community/forums/topics/606931)
