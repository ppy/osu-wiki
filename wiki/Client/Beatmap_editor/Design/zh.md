...
![设计 (Design) 标签页](img/SE_base-ZH.jpg "设计 (Design) 标签页")
...
![带有上/下移动与淡入变换的物件示例。](img/SE_Move-ZH.jpg "带有上/下移动与淡入变换的物件示例。")
...
| 淡入淡出 | 淡入/淡出。（变暗对变亮） |
| 旋转 | 旋转一定的“弧度”（不是角度）。 |
| 色块模式 | 强制为精灵上色。可以通过在终点使用不同的颜色来逐步改变元素的颜色。“永久效果”，这是基本解释。 |
...
**分别显示不同图层下故事板对象的列表。**点击 “Sprite Library” 来添加故事板元素。o allocate the new sprite (SB object), just drag-and-drop to the respective layer.新精灵默认位于“前景”图层。点击精灵名会显示并跳转到精灵的位置及时间帧。复制的精灵使用的名称与原精灵相同。使用 `Delete` 键或顶部菜单的编辑 -> 删除来删除精灵。

**注意** “>” 表示图层遮挡顺序：物件 > 前景 > 通过时/失败时 > 背景。

#### 精灵库 (Sprite Library)

![所使用精灵的视觉表现。](img/SE_SL.jpg "所使用精灵的视觉表现。")

点击 "Browse..." 按钮来搜索要使用的精灵。你可以在精灵库窗口中点击缩略图来复制精灵。
