...
| Сохранить как новую сложность... | Сохранить текущие изменения как новую сложность. После сохранения старая сложность остаётся без изменений. |
| Восстановить сохранённое (`Ctrl` + `L`) | Вернуть сложность и сториборд к последнему сохранённому состоянию. |
| Восстановить и переоткрыть ресурсы (`Ctrl` + `Shift` + `L`) | Вернуть сложность и сториборд к последнему сохранённому состоянию и переоткрыть все файлы (спрайты, сэмплы и т.д.). |
...
| Выгрузить на сервер... (`Ctrl` + `Shift` + `U`) | [Загрузить](/wiki/Beatmapping/Beatmap_submission) карту на форум [Works In Progress](https://osu.ppy.sh/community/forums/10). |
| Экспортировать в [`.osz`](/wiki/Client/File_formats/osz_(file_format))... | Экспортировать карту в формате [`.osz`](/wiki/Client/File_formats/osz_(file_format)) (для самостоятельного распространения), затем открыть папку Exports. |
| Распаковать | Извлечь карту в формате `.osz2`<!-- TODO(TicClick: add a link when the article is up)--> во временную папку, чтобы переопределить файлы во время моддинга.[^osz2-note] |
| Импортировать из... | **bms/bme:** открыть файл `.bms`/`.bme` для импорта сложностей в osu!mania. |
| Открыть папку с песней | Открыть папку карты, в которой содержуются соответствующие игровые файлы. |
| Открыть `.osu` в блокноте | Открыть файл выбранной в данный момент сложности в MS Notepad. |
| Открыть `.osb` в блокноте | Открыть файл [сториборда](/wiki/Storyboard) карты в MS Notepad. |
| Выйти (`Esc`) | Выйти из редакторе карты с предварительной возможности сохранения.[^exit-note] |
