...
| Импортировать из... | **bms/bme:** импортировать файл с расширением `.bms`/`.bme` как сложность в osu!mania. |
...
| Клонировать (`Ctrl` + `D`) | Скопировать и вставить выделенные объекты через одну [долю](/wiki/Music_theory/Beat) после конца выделения. |
...
| Растянуть... (`Ctrl` + `Shift` + `S`) | Растянуть выделенные объекты, перемещая их ближе или дальше от центра игрового поля по одной или двум осям. |
...
| Создать многоугольник (`Ctrl` + `Shift` + `D`) | Добавить многоугольник, используя для размещения нот текущий шаг временно́й шкалы. |
...
| Режим лайв-маппинга (`Ctrl` + `Tab`) | Включить режим расстановки объектов под музыку с помощью игровых клавиш ([нот](/wiki/Gameplay/Hit_object/Hit_circle) в osu!taiko или osu!mania, либо [фруктов](/wiki/Gameplay/Hit_object/Fruit) в osu!catch).[^live-mapping-note] |
