...
![](img/beat-snap-divisor.jpg "Делитель доли")

| Название | Описание |
| :-- | :-- |
| [Делитель доли](/wiki/Client/Beatmap_editor/Beat_snap_divisor) | Разделяет музыкальные доли на шкале на равные части, куда при маппинге ставятся игровые объекты. Движение вправо увеличивает число видимых частей, движение влево — уменьшает. |
| `Insert Break Time` | Вставляет [перерыв](/wiki/Beatmap/Break) в текущий момент карты. |
| x/y | Показывает координаты выделенного объекта на игровом поле. Если ничего не выбрано, показывает координаты курсора. |

При зажатой клавише `Alt` делитель доли заменяется на [настройку спейсинга](/wiki/Client/Beatmap_editor/Distance_snap). Множитель спейсинга можно изменять в диапазоне от 0.1x до 6.0x.
...
| :-- | :-- |
| `Sampleset` | Настройка [сэмплсета](/wiki/Beatmapping/Sampleset) для выбранных объектов (в т.ч. их звука удара). При выборе `Auto` будет использоваться сэмплсет из активной [тайминг-секции](/wiki/Client/Beatmap_editor/Timing#тайминг-секции). |
...
| Кнопка (горячая клавиша) | Применение | Описание |
| :-- | :-- | :-- |
| `Grid Snap` (`T`) | Чтобы временно включить, зажмите `Shift`. | Привязывает объекты к [сетке игрового поля](/wiki/Beatmapping/Grid_snapping) при их расстановке и перемещении. |
| `Distance Snap` (`Y`) | Чтобы временно включить, зажмите `Alt` (инструмент появится на месте делителя доли). `Alt` + `Колёсико мыши`: настройка множителя расстояния. | Включает режим, при котором расстояние между объектами подбирается в [зависимости от времени](/wiki/Client/Beatmap_editor/Distance_snap) между ними. Рекомендуется использовать, когда песня поставлена на паузу. |
| `Lock Notes` (`L`) | `Левая кнопка мыши`: включить/выключить. | Запрещает перемещать игровые объекты во времени и пространстве. |
...
По центру расположена шкала с метками и кнопками для управления музыкой. При наведении курсора на шкалу появятся кнопки для работы с закладками. Кнопка `Test` справа от шкалы запускает [процесс тестирования](/wiki/Client/Beatmap_editor/Test_mode) карты, начиная с текущей позиции.
...
| Жёлтый | Аудиопревью |
| Зелёный | [Секция без смены тайминга](/wiki/Client/Beatmap_editor/Timing#зелёные-тайминг-секции) |
| Красный | [Секция со сменой тайминга](/wiki/Client/Beatmap_editor/Timing#красные-тайминг-секции) |
| Голубой | Закладка |
...
::: alert-note
**Основная статья:** [Делитель доли](/wiki/Client/Beatmap_editor/Beat_snap_divisor)
:::

![](img/beat-snap-divisor-mania.jpg "Делитель доли в osu!mania")
...
| Центр | Игровое поле, поделенное в соответствии с [делителем доли](/wiki/Client/Beatmap_editor/Beat_snap_divisor) |

#### Цвета

::: alert-note
**См. также:** [Делитель доли](/wiki/Client/Beatmap_editor/Beat_snap_divisor)
:::
...
Процесс разметки сэмплов (звуков) на отдельные ноты называется **сэмплингом**. Для этого удерживайте `Alt` и нажмите на нужную ноту, чтобы открыть окно для выбора сэмплов.
...
| `Use basic sample` | Менять громкость и сэмплсет только у выбранных нот. |
