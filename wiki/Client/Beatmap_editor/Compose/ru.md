...
| Название | Описание |
| :-- | :-- |
| [Beat snap divisor](/wiki/Client/Beatmap_editor/Beat_snap_divisor) | Разделяет музыкальные доли на шкале на равные части, куда при маппинге ставятся игровые объекты. Движение вправо увеличивает число видимых частей, движение влево — уменьшает. |
...
| `Distance Snap` (`Y`) | Чтобы временно включить, зажмите `Alt` (инструмент появится на месте делителя временной шкалы). `Alt` + `Колёсико мыши`: настройка множителя расстояния. | Включает режим, при котором расстояние между объектами подбирается в [зависимости от времени](/wiki/Client/Beatmap_editor/Distance_snap) между ними. Рекомендуется использовать, когда песня поставлена на паузу. |
