...
**SB 负载**（故事板负载的缩写）is a number used in [Storyboarding](/wiki/Storyboard) to indicate how much more load the Storyboard is causing on the graphics program. It is a measure of how many times the full 640x480 area needs to be redrawn in a frame.

不使用故事板时，负荷为 1x （640x480 的空间只需要绘制一次）。Including a single image that takes up exactly half of the screen would result in 1.5x; two images that overlap entirely and take up half of the screen would result in 2x.

谱面的 SB 负载最好不要超过 5x。这样可以确保大多数玩家能在不为显卡施加过大压力的情况下浏览谱面。
