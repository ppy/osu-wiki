...
- **Compose:** проблемы, связанные с расстановкой объектов на игровом поле. Чтобы увидеть больше предупреждений, отметьте флажок `Check distance snap` (по умолчанию он снят, чтобы не вызывать лаги на картах с большим числом объектов).
- **Design:** проблемы, связанные с визуальным оформлением карты: фон, сториборд и т.д.
- **Timing:** проблемы, связанные с расположением нот на временной шкале.
- **Meta:** проблемы, связанные с настройками карты в окне `Song Setup`.
...
::: alert-notice
**Примечание**
Вместо фигурных скобок, например, `{0}`, в сообщениях от AiMod будут подставлены нужные слова или числа.
:::
...
| Your beatmap is shorter than 45 seconds. Consider making it longer. |  | Так как ваша карта идёт меньше 45 секунд, замапайте побольше. | Это предупреждение можно проигнорировать, если карта сделана такой намеренно. |
...
Если вы хотите [ранкнуть](/wiki/Beatmap/Category#ranked) свою карты, то, помимо AiMod, рекомендуется пользоваться и другими программами — например, [Mapset Verifier](https://github.com/Naxesss/MapsetVerifier/releases) от ::{ flag=SE }:: ::Naxess::{ user=8129817 }. В них проверка идёт по более актуальным критериям ранкинга, и они находят гораздо больше проблем, чем AiMod. Тем не менее, не игнорируйте AiMod целиком и полностью, поскольку иногда он всё-таки предупреждает о вещах, запрещённых критериями ранкинга.
