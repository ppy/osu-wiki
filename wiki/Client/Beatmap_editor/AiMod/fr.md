...
- **Mapset :** Ces problèmes affectent la beatmap dans son ensemble.
...
| Message | Explication | Solution |
| :-- | :-- | :-- |
| Le taux de drain de santé des difficultés Easy et Normal devrait être au minimum 4. | beatmaps osu!mania uniquement ! | Allez dans le Song Settings et réglez le taux de drain de santé sur 4 ou plus. |
| Le taux de drain de santé des difficultés Hard et supérieures devrait être au minimum 7. | beatmaps osu!mania uniquement ! | Allez dans le Song Settings et réglez le taux drain de santé sur 7 ou plus. |
| L'overall difficulty des maps contenant peu de sliders devrait être au minimum 7. | beatmaps osu!mania uniquement ! | Allez dans Song Settings et réglez le taux d'OD sur 7 ou plus. |
| Le slider velocity devrait être égal à 1,40 ou 1,60. | beatmaps osu!taiko uniquement ! | Allez dans l'onglet Timing et modifiez le slider velocity à 1,40 ou 1,60. |
...
| Ces deux objets apparaissent exactement au même moment. | Deux objets se trouvent l'un sur l'autre. Cela rendra la beatmap impossible à classer ! | Trouvez les deux objets et déplacez-les ou supprimez-les. |
| Il n'y a pas de hitsounds sur aucun objet. Les hitsounds sont nécessaires pour les beatmaps classables. | *Auto-explicatif* | Ajouter des hitsounds sur la beatmap en utilisant des whistles, des claps et des finishes. |
...
| Message | Explication | Solution |
| :-- | :-- | :-- |
| Le temps de drain doit être supérieur à 30 secondes. | La longueur de votre beatmap du début à la fin, sans compter les temps de pause, est inférieure à 30 secondes. AiMod ne détecte pas les spinners qui peuvent s'étendre au-delà de la marque des 30 secondes de temps de drain. | Essayez d'utiliser un spinner pour terminer la beatmap si votre musique le permet. Sinon, votre musique est peut-être trop courte. Il est possible de prolonger votre mp3 afin d'atteindre la barre des 30 secondes. |
...
| Ce combo est très long. Pensez à le diviser. | La longueur des combos peut affecter la difficulté de la beatmap, ainsi que la quantité de fruits qui seront empilés sur l'attrapeur. | Allez voir les combos fautifs et décidez où commencer le nouveau combo (Il est préférable que la longueur des combos soit, au maximum, de 15-18). |
...
| Ce spinner est trop court. le mod Auto doit obtenir au moins 1000 points de bonus sur les spinners. | Le spinner spécifiée est trop courte, ce qui rendra impossible un SS. | Trouvez le spinner et supprimez-le ou allongez-le d'environ 3 temps entiers. La longueur peut varier en fonction du BPM de la musique. |
...
| Les pauses ne sont pas suggérées pour les map du mode osu!mania. | beatmaps osu!mania uniquement ! | Insérez des notes à l'endroit où se trouve la pause. |
...
| Le Letterboxing n'est pas autorisé en mode osu!mania. | beatmaps osu!mania uniquement ! | Décochez la case `Letterbox during breaks` dans l'onglet "Design" de l'onglet Song Setup. |
| Il est suggéré que le taux d'OD soit d'au moins 5. | beatmaps osu!mania uniquement ! | Réglez le paramètre `Difficulté générale` dans l'onglet "Difficulty" de l'onglet Song Setup sur 5 ou plus. |
| Il est suggéré que le taux d'OD pour les maps comportant très peu de sliders soit d'au moins 7. | beatmaps osu!mania uniquement ! |  |
| <!-- Both seem to occur, not sure what changes the value -->Il est suggéré que le taux d'OD pour les maps comportant très peu de sliders soit d'au moins 8. | beatmaps osu!mania uniquement ! |  |
| <!-- potential removal? tried using countdown on taiko maps, and aimod does not complain -->Le compte à rebours n'est pas autorisé en mode osu!taiko. | beatmaps osu!taiko uniquement ! |  |
| <!-- same as above -->L'avertissement d'épilepsie n'est pas autorisé en mode osu!taiko. | beatmaps osu!taiko uniquement |  |
| Le Letterboxing n'est pas autorisé en mode osu!taiko. | beatmaps osu!taiko uniquement |  |
...
| Ce mapset a besoin d'au moins 2 difficulté en mode osu!catch. | beatmaps osu!catch uniquement ! |  |
| Ce mapset a besoin d'au moins 1 difficulté Easy/Normal. |  |  |
| Ce mapset a besoin d'au moins 2 difficultés en mode osu!mania. | beatmaps osu!mania uniquement |  |
| Ce mapset a besoin d'au moins 2 difficultés en mode osu!. | beatmaps osu! uniquement |  |
| Ce mapset a besoin d'au moins 2 difficultés osu!taiko. | beatmaps osu!taiko uniquement |  |
...
- **AiMod peut vous avertir de l'absence de problèmes**, comme le fait que les beatmaps nécessite deux difficultés du mode osu! sur une beatmap [marathon](/wiki/Beatmap/Marathon) et que l'option [kiai time](/wiki/Gameplay/Kiai_time) soit activée pendant moins de 15 secondes.
