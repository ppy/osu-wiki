...
You can strings of text, up to a whole line, as **variables** for use elsewhere in an .osb file (note that the value can't change during the gameplay, so for those with software experience, think of it more like a constant). _.osu files do not support this._
...
Note that variables are carried over when saving in the Beatmap Editor, but _all_ instances of the variable's value will be replaced with the variable. Therefore, you shouldn't make variable too short or general, e.g.:
