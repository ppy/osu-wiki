...
글쓰기 에디터 화면이 글을 포맷하기 위한 간단한 도구들을 제공해주긴 하지만, BBCode는 직접 사용자가 수동으로 작성할 수 있습니다.

BBCode에 사용되는 tag들은 대소문자 구분이 없습니다.

### BBCode 버튼 사용 이해

아무 텍스트도 선택하지 않은 채 BBCode 버튼을 누르면 글쓰기 화면에 커서가 위치한 곳 양쪽으로 tag가 생성될 것입니다.

글쓰기 화면에 텍스트를 선택 한 후 BBCode 버튼을 누르면 tag가 선택된 텍스트 양면에 자동 생성됩니다.

### 여러 tag들 합쳐 사용하기

더더욱 화려한 연출을 위해 여러 가지 tag들을 섞어 사용할 수 있습니다.

하지만 이렇게 여러 tag들을 사용할 때 중요한 것은, 꼭 tag들이 열린 순서에 맞게 닫히도록 나열해주세요. 
순서에 맞지 않게 tag들을 나열하거나 닫은 경우 해당 글의 포맷팅 또는 화면에 버그가 생기는 불상사가 발생할 수 있습니다.

예를 들면 (tag의 순서에 주목해주세ㅇ):
...
단어 또는 문장을 **강조** 하거나 [기운 글꼴을 사용하는 것보다](#italic) ** 더 강력하게** 어필하는 방법입니다.

- 사용하는 버튼: ![강조체 버튼](img/bold.jpg "강조체 버튼")
- 알아둘 점:
  - 너무 과다하게 사용하지 마세요.
...
[강조체](#bold) 보다 더 _약한_ 방법으로 단어 또는 문장을 **강조**하는 방법입니다.

- 사용하는 버튼: ![기울임체 버튼](img/italic.jpg "기울임체 버튼")
- 알아둘 점:
  - 너무 과다하게 사용하지 마세요.
...
- 사용하는 버튼: ![밑줄 버튼](img/underline.jpg "밑줄 버튼")
- 알아둘 점:
  - 너무 과다하게 사용하지 마세요.
    자주 사용하다 보면 글을 읽기에 불편하거나 글을 읽기 어려워질 수도 있습니다.
- 사용법:
...
- 사용하는 버튼: ![취소선 버튼](img/strike.jpg "취소선 버튼")
- 알아둘 점:
  - 너무 과다하게 사용하지 마세요.
    자주 사용하다 보면 글을 읽기에 불편하거나 글을 읽기 어려워질 수도 있습니다.
- 사용법:
...
- 사용하는 버튼:
![색상 파레트](img/colour.jpg "색상 파레트")
- 알아둘 점:
  - 색상 파레트는 텍스트 에디터의 오른쪽에 위치해 있습니ㄷ.
  - 너무 과다하게 사용하지 마세요.
...
- 사용하는 버튼:
![폰트 사이즈 옵션](img/font-size.jpg "폰트 사이즈 옵션")
- 알아둘 점:
  - 너무 과다하게 사용하지 마세요.
...
_[스포일러 상자](#spoilerbox)와 헷갈리지 않도록 주의해주세요._
...
- 사용하는 버튼: ![스포일러 버튼](img/spoiler.jpg "스포일러 버튼")
- 알아둘 점:
...
_[박스](#box) 또는 [스포일러](#spoiler)와 헷갈리지 않도록 주의해주세요._

글을 읽고 있는 사용자가 바로 읽지 못하도록 미리 이름이 정해져 있는 상자 속에 **문장을 숨깁니다**.
해당 글을 읽고 있는 사용자는 이 스포일러 박스를 클릭해 숨겨진 문단을 연 후 속에 있는 문장들을 읽을 수 있습니다. 

- 사용하는 버튼: ![스포일러 박스 버튼](img/spoilerbox.jpg "스포일러 박스 버튼")
- 알아둘 점:
  - 아래의 방법으로 만든 스포일러 박스는 `collapsed text`라는 이름을 가진 [박스](#box)를 생성할 것입니다.
- 사용법:
```prolog
[spoilerbox]
...
...
- 사용하는 버튼: ![박스 버튼](img/box.jpg "박스 버튼")
- 알아둘 점:
...
- 사용하는 버튼: ![인용 버튼](img/quote.jpg "인용 버튼")
- 알아둘 점:
...
- 사용하는 버튼: ![코드 버튼](img/code.jpg "코드 버튼")
- 알아둘 점:
...
- 사용하는 버튼: ![중심 버튼](img/centre.jpg "중심 버튼")
- 알아둘 점:
...
- 사용하는 버튼: ![URL 버튼](img/url.jpg "URL 버튼")
- 알아둘 점:
...
- 사용하는 버튼: ![구글 버튼](img/google.jpg "구글 버튼")
- 알아둘 점:
...
- 사용하는 버튼: ![Lucky 버튼](img/lucky.jpg "Lucky 버튼")
- 알아둘 점:
...
- 사용하는 버튼:
  - List (목록) : ![목록 버튼](img/list.jpg "목록 버튼")
  - List type (목록 종류) : ![목록 종류 버튼](img/list-type.jpg "목록 종류 버튼")
  - List bullet (목록 요소): ![목록 요소 버튼](img/list-bullet.jpg "목록 요소 버튼)
- 알아둘 점:
...
그렇기 때문에 위처럼 사진 공유용 사이트를 사용하는것을 추천드립니다. 사진 공유용 사이트는 사진을 공유하기 위해 만들어진 사이트이니까요. 
...
- 사용하는 버튼: ![유튜브 버튼](img/youtube.jpg "유튜브 버튼")
- 알아둘 점:
