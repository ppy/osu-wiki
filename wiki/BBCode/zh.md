...
- 注：
  - 取色器位于文本框的右侧。
  - 尽量少用这个标签。过度使用可能会给读者造成不适。这个标签可能会使你的文本难以辨认。
  - 如果你选择了一种颜色，请确保它能与背景颜色形成对比，这样能让任何人都可以识别出文本。
  - `HEXCODE` 参数不应该使用引号包围。
- 语法：
  - `HEXCODE` 表示颜色，值为十六进制数或颜色名。
    - 如果参数为一个十六进制数，它必须以`#`开头，然后为 6 个十六进制（0-9，A-F）字符。
    - 如果参数为颜色名称，它必须为一个有效的 HTML 颜色名。
```
[color=HEXCODE] ... [/color]
```
...
使用 `Monospaced` 字体格式化代码，并将代码放在一个灰色的盒子里。当你为一个项目发布故事版代码或源代码时，这个标签是非常有用的。

- 专用按钮： N/A
- 注：
  - 这将保留空白的间隔，这意味着除非你手动打乱代码的格式，否则没有一行代码的格式会被打乱。
  - 多出的文本行将会启用一个水平滚动条。
- 语法：
```
[code]
...
**非常不推荐**使用这个标签！使用这个标签的问题是，在玩家获得一个 [osu!supporter 标签](/wiki/osu!supporter) 后，用户名可以被更改。一旦他们更改了用户名，这个链接就会失效。
