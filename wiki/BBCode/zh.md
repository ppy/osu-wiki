...
**BBCode** 是在 osu!论坛 中使用的一种标记语法。是网络上绝大多数论坛所支持的文本格式。它是由包围在文本前后的标签所组成的，可以包含特定的属性。在 osu! 网站上，你可以在帖子、签名和用户页面中使用 BBCode 。

## 注意

虽然帖子编辑器提供了一些基本的格式化工具，但手动编写 BBCode 也是可行的。BBCode 标签对大小写不敏感。
...
使用比 [斜体](#斜体) 更**醒目的方式强调**单词或段落。

- 专用按钮： ![粗体](img/bold.png)
- 注：
  - 尽量少用这个标签。过度使用只会适得其反，使文章更难阅读。
- 语法：
```
[b] ... [/b]
```
...
- 注：
  - 取色器位于文本框的右侧。
  - 尽量少用这个标签。过度使用只会适得其反。这个标签可能会使你的文本难以辨认。
...
- 专用按钮： N/A
- 注：
  - 这将保留空白的间隔，这意味着除非你自己打乱代码格式，否则每行代码会保持原有的格式。
  - 多出的文本行将会启用一个水平滚动条。
- 语法：
```
[code]
...
请将图片上传至 [imgur](https://imgur.com/ "imgur") 或 [puush](https://puush.me/ "puush") 。一旦图片成功上传，复制粘贴图片标签间提供的直链。另外，一些网站不愿意用户直接访问到他们的图片（直接访问图片也被称为热链）。像上面列出的一些网站，都是直链，因为它们是图片分享网站。
