...
**BBCode** 是一种在 osu! 论坛中使用的[标记语言](https://zh.wikipedia.org/wiki/%E7%BD%AE%E6%A0%87%E8%AF%AD%E8%A8%80)。往更大了说，它也用于互联网上的其他绝大多数论坛。它由环绕文本的标记组成，用于表示用于启用富文本的格式、属性、嵌入等。它被用于 osu! 网站的各个地方，比如论坛贴、玩家签名、玩家主页和谱面描述。

![论坛中的编辑器与按钮](img/editor.jpg "论坛中的编辑框")

## 表现

不框选文本，直接单击标记按钮时，文本编辑器会在光标前后会插入一组标记。但如果框选了文本，编辑器会在框选文本前后添加标记。

用户如果希望在文本中使用多种格式的组合，需要将 BBCode 标记嵌套使用。组合时，必须按照标签的**顺序和嵌套**逻辑组合，否则将达不到预期效果。
...
像其他许多标记语言一样，BBCode 内格式化文本的标记由一对方括号 (`[]`) 表示。标签分为开始和结束两种，位于需要格式化文本的前后。结束标签中会包含正斜杠 (`/`)。
...
工具栏按钮：![字体大小设置](img/font-size.png "字体大小")
...
为了获得图像地址，必须导航到其来源的网站，将鼠标悬停在图像上，右键单击图像，然后选择“复制图像地址”。然后，地址应该被复制并粘贴在标签之间。
