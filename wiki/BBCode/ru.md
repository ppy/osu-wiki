...
Тег `[color]` изменяет цвет текста. Сам цвет задаётся либо [шестнадцатеричным кодом](https://ru.wikipedia.org/wiki/Цвета_HTML#Hex_triplet "Wikipedia"), либо ключевым словом вроде `red` или `green`, которые нужно подставить в примере вместо `HEXCODE`. Кавычки в параметре использовать не нужно.
...
На форумах osu! в виде фрагментов кода чаще всего оформляются [сториборды](/wiki/Storyboards), а также параграфы, где необходимо подчеркнуть обязательное использование тегов.
...
Тег `[url]` создаёт кликабельную ссылку с указанным названием.
...
Списки другого типа можно создать, подставив вместо `TYPE` разные типы маркеров: `1` для нумерованного списка, `a` для списка со строчными буквами, `A` — для списка с заглавными буквами, `i` или `I` — для списка с римскими числами, написанными строчными или заглавными буквами.
...
Источник может быть любым, но стоит учесть, что некоторые сайты могут запрещать прямые ссылки на свои изображения или со временем их удалять. Если нужно, чтобы изображение было всегда доступно, загрузите его на любой известный хостинг картинок вроде [Imgur](https://imgur.com "Imgur").
...
Тег `[youtube]` встраивает в пост видео с [YouTube](https://youtube.com "YouTube"). Для этого нужно заменить `VIDEO_ID` на код видео — 11 символов, которые идут сразу после `?v=` или `&v=` в ссылке на него (ссылку целиком вставлять не нужно).
...
Тег `[audio]` добавляет [HTML5](https://ru.wikipedia.org/wiki/HTML5 "Wikipedia")-плеер, воспроизводящий аудио. Вместо `URL` нужно указать **прямую** ссылку на аудио из интернета (ссылки на файлы на вашем компьютере, вроде `C:\Users\Name\Music\audio.mp3`, работать не будут).
