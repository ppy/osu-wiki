...
*อย่าสับสนกับ [Spoilerbox](#spoilerbox)*

```
[spoiler]ข้อความ[/spoiler]
```

แท็ก `[spoiler]` ใช้สำหรับปกปิดข้อมูลสำคัญด้วยแถบสีดำทึบ ซึ่งจะแสดงข้อความข้างใต้เมื่อมีการคลุมแถบสี (Highlight) หากใช้ร่วมกับแท็ก [`[color]`](#colour) แถบสีดำที่ปิดทับจะไม่เปลี่ยนไป แต่ข้อความข้างใต้จะยังมีสีตามที่กำหนดไว้ไม่ว่าจะอ่านออกหรือไม่ก็ตาม
...
*Spoilerbox* เป็นกล่อง BBCode ประเภทพิเศษที่ไม่สามารถระบุชื่อ `NAME` ได้ โดยชื่อของกล่องจะถูกแสดงเป็นคำว่า `SPOILER` เสมอ กล่องสปอยเลอร์มีแท็กเฉพาะของตัวเอง (`[spoilerbox]`) แต่การทำงานเหมือนกับ [กล่องข้อความ (Box)](#box) ทุกประการ
...
## # โค้ดในบรรทัด (Inline code) {#inline-code}

*อย่าสับสนกับ [บล็อกโค้ด (Code block)](#code-block)*

```
[c]ข้อความ[/c]
```

แท็ก `[c]` ใช้สำหรับเน้นข้อความในบรรทัดด้วยฟอนต์แบบความกว้างคงที่ (Monospace) บนเว็บไซต์ osu! ข้อความจะถูกจัดรูปแบบให้อยู่ในกล่องสีเทา แท็กนี้ต่างจาก [บล็อกโค้ด](#code-block) ตรงที่สามารถใช้ได้เฉพาะในบรรทัดเดียวเท่านั้น

ในฟอรัม osu! แท็กนี้มีประโยชน์มากสำหรับการเน้นสิ่งต่างๆ เช่น ปุ่มลัดบนคีย์บอร์ด หรือคำอธิบายปุ่มกด

## # บล็อกโค้ด (Code block) {#code-block}

*อย่าสับสนกับ [โค้ดในบรรทัด (Inline code)](#inline-code)*
...
- เคยมีบัคที่ทำให้ผู้ใช้สามารถทำข้อความโปร่งใสได้โดยการใช้ [แท็กสี](#colour) และพิมพ์คำว่า "transparent" หลังเครื่องหมายเท่ากับ (`=`)
