...
**Темп** (англ. *tempo*) — мера скорости музыки. Темп обычно измеряется в **ударах в минуту** (англ. **beats per minute**, **BPM**) — это число обозначает, сколько [музыкальных доле́й](/wiki/Music_theory/Beat) присутствует в одной минуте песни. Например, темп в 60 BPM означает одну долю, или *удар*, в минуту, а темп в 120 BPM — два удара в минуту (в два раза быстрее). От темпа песни напрямую зависит ряд факторов геймплея, например, [скорость слайдеров](/wiki/Gameplay/Hit_object/Slider/Slider_velocity) или частота пульсации [киаи](/wiki/Gameplay/Kiai_time). На странице [информации о карте](/wiki/Beatmap_information) темп песни показывается около иконки [метронома](https://ru.wikipedia.org/wiki/Метроном) — устройства, помогающего музыкантам держать постоянный ритм.
