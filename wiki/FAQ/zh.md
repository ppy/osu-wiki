...
需要留意的是，您在approved的谱面里取得的分数不会算进您的上榜总分数(但是会计算准确度，也会让你获得pp)。approved的map会用“箭头”标志来标明，和以“爱心”标明的上榜谱面有所区别。 另外，Pending的谱面里取得的所有分数均不会算进您的上榜总分数，也不会计算准确度。Pending的map会用“问号”标志来标明。最常见的Pending例子是内置的教程(osu! tutorial)。
