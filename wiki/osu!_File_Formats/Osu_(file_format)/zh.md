...
**.osu** 是一个人类可读的，描述单个难度的铺面信息的文件。

大体结构
----------------

第一行确定了.osu文件格式的版本号。示例:

`osu file format v12`

接下来的行分成了下面的几个部分，通过单独的一行（使用中括号将该部分名称括起来）来表示一个部分的开始。

-   General - 一些关于这个铺面的基础设置。
-   Editor - 作图的时候保存的一些设置。
-   Metadata - 歌曲和铺面的信息。
-   Difficulty - 和铺面难度相关的一些属性。
-   Events - 设置背景、背景视频、休息点和Storyboard。
-   Timing Points - 设置铺面的Timing。
-   Colours - 设置连击颜色和其他的颜色属性
-   Hit Objects - 对铺面里具体打击物件的描述
