...
在 `20130319` 版本或之后版本创建的回放内，用于分数的32位随机整数 seed 会以 `-12345|0|0|seed` 的格式编码进LZMA流末尾的附加回放帧中。（原文：the 32-bit integer RNG seed used for the score will be encoded into an additional replay frame at the end of the LZMA stream, under the format `-12345|0|0|seed`.）
