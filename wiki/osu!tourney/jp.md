...
Users encountering **problems** with the client may read the **[ Troubleshooting](JP:Osu!tourney#Troubleshooting "wikilink")** section or **[post in this thread](https://osu.ppy.sh/forum/t/327120)**
...
-   **TeamSize:** Define the size of your teams here. Depending on this. osu!tourney either opens 1-4 windows per team. Maximum value is 4.
-   **privateserver:** Leave this untouched. It does not matter for you.
-   **acronym:** Put an abbreviation for your tournament here, e.g. 'OWC 2013'. More details at **[Multiplayer session creation and handling](JP:Osu!tourney#Multiplayer_session_creation_and_handling "wikilink")**.
...
To make the client work properly, please assure that the players occupy the appropriate slots in the multiplayer room. More details about how to do that correctly can be found at **[Multiplayer session creation and handling](JP:Osu!tourney#Multiplayer_session_creation_and_handling "wikilink")**. At the bottom of the client, you will see the control panel.
...
It shows the template you need to follow to name the multiplayer room you wish to spectate. It consists of the acronym you defined in the `tournament.cfg` file and of 2 team names. Read **[Multiplayer session creation and handling](JP:Osu!tourney#Multiplayer_session_creation_and_handling "wikilink")** for more information.
...
-   **Exit:** This either cancels the current spectator session (i.e. leaving the room as spectator) or shuts down the entire client, if you are not spectating anyone.
...
`Your_acronym_in_tournament.cfg` gets entirely replaced with the value you defined in the tournaments.cfg during the **[Setup](JP:Osu!tourney#Setup "wikilink")**. In our example, the value was defined as “Test Tourney”. So here our example uses the template:
...
The osu!tourney client will successfully recognize the room. Make sure to assign the correct slots to the players joining the room using the !mp move and !mp team commands. As explained in the **[Using osu!tourney](JP:Osu!tourney#Using_osu!tourney "wikilink")**, each window in the client is assigned to a slot in the multiplayer lobby. Depending on your value in the `TeamSize` variable in the `tournament.cfg`, the left team will occupy the first slots and the right team will occupy the slots right below that.
...
Make sure your background file is properly located at `.../osu!/Skins/User/tournament/background.png`. See **[Skinning](JP:Osu!tourney#Skinning "wikilink")**.
...
Your multiplayer room is named incorrectly then or you use the wrong acronym in the `tournament.cfg`. Make sure you use the correct room name (see **[Match Creation](JP:Osu!tourney#Match_creation "wikilink")**. Renaming an existing multiplayer room doesn’t work. Recreate the entire room, if you did something wrong.
