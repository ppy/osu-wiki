...
O cliente suporta várias modificações, para que você possa customizá-lo para o torneio. Para fazer isso, você precisa criar a estrutura de pastas `/Skins/User/tournament` no diretório de instalação do cliente. Os elementos de skin podem ser colocados nessa pasta e suportam arquivos com extensão `.jpg` e `.png`.
...
Nome dos ícones devem corresponder ao nome do time. Por exemplo, se a sala é chamada `Test Tourney: (Team 1) vs (Team 2)`, os ícones devem ser nomeados `Team 1.jpg` e `Team 2.jpg`. Ícones podem estar nos formatos `.jpg` ou `.png` e ter uma resolução de `50x50px`.
