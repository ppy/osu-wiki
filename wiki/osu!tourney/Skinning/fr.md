...
- `background-win1` (optionnel) - l'image qui sera affichée en fondue après que l'équipe bleue n'ait gagnée pendant que les résultats du match sont affichés.
- `background-win2` (optionnel) - l'image qui sera affichée en fondue après que l'équipe rouge n'ait gagnée pendant que les résultats du match sont affichés.
- `tourney-title` (optionnel) - l'image affichée en bas du client, par dessus l'image de fond
...
Lec lient affichera les icônes se trouvant sous le chemin `/Skins/User/tournament/icons` pour les mettre à côté du nom des équipes. Ce pêut être utilisé pour montrer le drapeau du pays ou des avatars.
