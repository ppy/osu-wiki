...
Pour le faire, copiez le `osu!.exe` dans un dossier vide et lancez-le. Une nouvelle installation dosu! sera créée. C'est la seule installation que sera modifiée.

Ouvrez osu! et authentifiez-vous, cochez les cases `Retenir le nom d'utilisateur` et `Retenir le mot de passe`. Changez le canal de mise à jour à `CuttingEdge` et attendez pour redémarre osu!
...
Ne modifiez pas le fichier `tournament.cfg` quand osu! est ouvert. Fermez osu! puis éditez et ajoutez les lignes ci-dessus avec les options suivantes :
...
- `BufferTimeoutTime`: Le temps que le client attendra un joueur avant de continuer de jouer sans lui.
  - Il n'est pas recommandé de changer ça.
...
- `Height`: La hauteur streamable du client.
  - Le cient calculera la largeur automatiquement avec le ration 16:9.
