...
- `!mp make <nom>` - Crée une salle de tournoi avec le nom spécifié. Un maximum de 4 salles de ce type peuvent être créées.
  ...
  - Quand vous avez fini d'utiliser une salle, utilisez `!mp close` pour fermer la salle.
...
- `!mp lock` - Bloque la salle, le joueurs ne pourront pplus changer leur équipe, ni leur slot.
- `!mp unlock` - Débloque la salle.
- `!mp size <size>` - Définit le nombre de slots dans cette salle (1-16).
...
- `!mp move <nom d'utilisateur> <slot>` - Déplace un joueur de la salle vers le slot spécifié.
...
- `!mp aborttimer` - Stop le compteur actuel (à la fois le normal et celui du match)
...
- `!mp addref <nom d'utilisateur> [<nom d'utilisateur>] …` - Ajoute un arbitre à la salle. Un maximum de 8 arbitres peuvent être ajoutés. Seul le créateur de la salle peut ajouter un arbitre.
  ...
  - Les arbitres peuvent gérer la salle comme le créateur, cependantt, ils ne peuvent pas ajouter ou supprimer d'autres arbitres eux-mêmes.
...
Les éléments entre crochets angulairs ( `<>` ) définissent les « paramètres » de la commande. Les paramètres enfermés entre crochets ( `[]` ) sont optionnels.  
...
le host original d'une salle mutlijoueur peut aussi utiliser ces commande. Si le host originel quitte, le host suivant n'héritera pas des commandes. Le host originel sera capable de réutiliser les commandes s'il rejoint la salle à nouveau.
...
- `!mp move Loctav 4` - Déplace Loctav au slot 4 de la salle.
...
Nous espérons que vous agirez en tant que personnes professionnelles et responsables pour la gestion de votre tournoi. N'importe quel abus de ces commande pour empêcher les autres de jouer sera considéré comme une violation de nos [règles de communauté](/wiki/Rules "Règles de communauté").
