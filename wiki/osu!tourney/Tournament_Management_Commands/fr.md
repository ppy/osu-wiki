...
Les commandes de tchat suivantes sont faitesLes commandes de tchat suivantes sont fournies pour la gestion à distance des salles de tournoi multi-joueurs :
...
- `!mp lock` - Verrouille la salle pour que les joueurs ne puissent pas changer d'équipe et de créneau.
- `!mp unlock` - Inverse la commande précédente.
- `!mp size <size>` - Définit le nombre de créneaux disponibles (1 à 16) dans la pièce.
...
- `!mp move <username> <slot>` - Déplace un joueur dans la salle vers l'emplacement indexé 1 spécifié.
...
- `!mp team <username> <colour>` - Déplace un joueur vers l'équipe spécifiée.
  - `colour` - red, blue
- `!mp map <mapid> [<playmode>]` - Change la beatmap et le playmode de la salle.
  - `playmode` - 0 : osu!, 1 : osu!taiko, 2 : osu!catch, 3 : osu!mania
- `!mp mods <mod> [<mod>] [<mod>] …` - Supprime tous les mods actuellement appliqués et applique ces mods à la pièce.
...
Le host original d'une salle multijoueur peut aussi utiliser ces commandes. Si le host originel quitte, le host suivant n'héritera pas des commandes. Le host originel sera capable de réutiliser les commandes s'il rejoint la salle à nouveau.

## Exemples d'utilisation

Voici un exemple d'utilisation des commande :
