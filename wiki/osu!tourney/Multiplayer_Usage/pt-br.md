...
A sala multipayer deve ser nomeada baseado no modelo listado no painel de controle do cliente osu!tourney.

O modelo consiste em:
`Acronym_in_tournament.cfg: (Team Name 1) vs (Team Name 2)`

`Acronym_in_tournament.cfg` é inteiramente substituído pelo valor de `acronym` no arquivo `tournament.cfg` do [guia de setup](/wiki/osu!tourney/Setup).
Se o valor for definido como `Test Tourney`, o modelo aparecerá como `Test Tourney: (Team Name 1) vs (Team Name 2)`.

Os nomes dos times (`Team Name 1` e `Team Name 2`) podem ser substituídos por qualquer nome, **mas mantenha os parênteses `()` envolta deles**.
