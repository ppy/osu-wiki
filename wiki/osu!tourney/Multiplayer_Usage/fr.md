...
### Commandes de gestion du tournoi

_Voir aussi: [Commandes de gestion du tournoi](/wiki/osu!tourney/Tournament_Management_Commands "Commands de gestion du tournoi")_
...
Comme expliqué dans le [guide d'utilisation du spectateur](/wiki/osu!tourney/Spectator_Usage), chaque fenêtre dans le client est assignée à une position dans le lobby multijoueur. Après avoir défini la valeur `TeamSize` (nombre de joueurs dans chaque équipe) dans le fichier `tournament.cfg`, l'équipe bleue occupera la premier nombre de positions de `TeamSize` et l'équipe rouge occupera l'autre nombre de positions `TeamSize` en dessous d'eux.
