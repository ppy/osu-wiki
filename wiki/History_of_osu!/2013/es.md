...
- [This week (month) in osu! - blog de ppy](https://blog.ppy.sh/post/46924535831/this-week-month-in-osu-13)
...
El nombre de los modificadores del juego en los resultados se ha acortado (por ejemplo, «Hard Rock» a «HR») para eliminar el uso limitado del espacio. *La mayoría* de las repeticiones rotas en el pasado se han arreglado. Un nuevo sistema de espectador en el que puedes seguir al objetivo del espectador si estuvieras viendo a otra persona. El sistema de actualización se ha rediseñado, en particular, ya no forzará reinicios a menos que sea necesario.
...
Para el modo multijugador, los jugadores ahora pueden usar el mod especial (Relax/Autopilot/SpunOut/ManiaKeys) cuando el anfitrión habilite el Mod libre. En el lado de skinning, se han agregado sprites predeterminados de pantalla ancha de taiko (taiko `@2x`). Los favoritos A/B se eliminaron por completo y fueron reemplazados por las colecciones.
