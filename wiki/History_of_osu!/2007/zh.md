...
在七月底，菜单系统为了给游戏添加一些风格进行了一些改变。最早的滑条功能实现已经完毕，其本质上就是一个没有任何动画的橙色球以及一些比较丑的曲线。音符拆分和时间线功能一同加入了谱面编辑器，而一并加入的是一个更加具有“鲁棒性”的界面（跟现在的[作曲](/wiki/Client/Beatmap_editor/Compose)界面有点像）。
...
[连击特效](/wiki/Gameplay/Combo_fire)、[血量](/wiki/Gameplay/Health)条、鼠标尾迹和更好看的滑条使游戏体验得到了加强。此时，osu! 已经有了一个由5人左右的测试团队所做的大概10张谱面所组成的一个谱面库。
...
在公众版本发布之后，最初的[游戏模组](/wiki/Gameplay/Game_modifier) [Eazy](/wiki/Gameplay/Game_modifier/Easy) 和 [No Fail](/wiki/Gameplay/Game_modifier/No_Fail) 迅速加入。与其一同加入的还有休息时间（对于这一功能的编辑至今仍被忽略、并且缺失）不少改进立马使得 osu! 成为了一个独立音游。

![](img/easy.png)![](img/no_fail.png)

## 十月

2007年10月6日是在线榜单向公众公开的第一天。同样在这一天，新的 `.osz` 谱面格式和一个早期的基于在线提交系统的谱面库发布。在线排行榜被加入游戏内界面，同时也被显示在了网站上。
...
新的SS等级被加入了评分系统，用于替换在这之前达到AP的S级评分。 [No Video](/wiki/Gameplay/Game_modifier/No_Video) 和 [Hidden](/wiki/Gameplay/Game_modifier/Hidden) 两个模组和配套的SH(银S)和SSH（银SS）两个等级加入游戏。也是第一次，玩家们可以基于每一段计时部分来进行换肤。现在可以通过双击或将其拖动到osu!的图标或窗口中来加载 `.osz` 谱面。
...
现在，将音乐拖入 osu! 的窗口可以直接为它制作谱面；主界面和制谱器中的每个按钮加入了对应的提示；一些新的优质皮肤由一些早期的[皮肤制作者](/wiki/Skinning/Skinner)([Holiday by LgigiHann](https://osu.ppy.sh/community/forums/topics/1139), [WindWaker by awp](https://osu.ppy.sh/community/forums/topics/761))；选曲界面有了新的排序方式；peppy可以给所有osu!的玩家发公告；游玩模式做出了一些小更改（比如对不活动的打几圈进行暗化）.
