...
本地偏移量针对的是单一谱面。它会改变所有游戏组件（包括[打击物件](/wiki/Gameplay/Hit_object)，后台[故事板](/wiki/Storyboard)，以及视频和故事板声音）相对于音轨的位置（以毫秒计算）：

- **负**值会使所有游戏组件**更早**出现。
- **正**值会使所有游戏组件**更晚**出现。

注意：[全局偏移量](/wiki/Offset/Universal_offset)与此不同，它向相反的方向移动组件。

通常来说，本地偏移量最好保持在 0（如果谱面时间轴测定准确），除非存在一个特定的硬件问题。为防止几个玩家设定相同的偏移量[^hit-difference]，最好联系[审核评估团队](/wiki/People/Nomination_Assessment_Team)的成员，以确认谱面问题，并设定一个[在线偏移量](/wiki/Offset/Online_offset)。
