...
根据偏移量的类型，它们可以由谱面创作者设定，由玩家自定义，或者由一位网站管理员调整。大部分偏移量都针对单一谱面，除了[全局偏移量](#类型)。
...
- [非继承时间点](/wiki/Client/Beatmap_editor/Timing#非继承时间点（红线）)（红线），新的[时间轴区域](/wiki/Client/Beatmap_editor/Timing)的起点。这一时间点本身有时候也被称作“偏移量”（即使它只是一个相对于音频文件起点有偏移量的点）。
