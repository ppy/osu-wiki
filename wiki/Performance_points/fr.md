...
Il vise à déplacer le centre d'intérêt de la progression des compétences de la quantité de temps joué à une représentation réelle de la compétence du joueur. Pour ce faire, il calcule un score unique basé sur la difficulté d'une beatmap et la performance du joueur sur cette [beatmap](/wiki/Beatmap).

## Histoire

La première mise en œuvre initiale d'un tel score a été révélée au public au cours du mois d'avril 2012 et n'était connue que sous le nom du mystérieux projet *'???'*, le système énigmatique a finalement reçu son nom complet plus tard dans le mois.
...
Plusieurs mois après sa révélation, la version 20120722-24 d'osu! a officiellement mis en œuvre le système pour remplacer entièrement l'ancien système de score [Classée](/wiki/Beatmap/Category#Classée), les nouveaux scores étant calculés toutes les 30 minutes. Plus tard, en août de la même année, le système a été amélioré pour être mis à jour en temps réel.

*Remarque : ppv1, la version originale du système de points de performance, comportait également un journal des modifications, qui peut être consulté dans son [sujet du forum](https://osu.ppy.sh/community/forums/topics/92185).*

Il a continué à exister dans cette capacité pendant plus d'un an de service jusqu'à ce que [Tom94](https://osu.ppy.sh/users/1857058), le créateur de la métrique de notation *osu!tp*, rejoigne l'[équipe d'osu!](/wiki/People/The_Team) et implémente sa conception dans le système. Le système résultant a été intitulé *ppv2*, et est devenu opérationnel le 27 janvier 2014, renommant ainsi l'ancien système *[ppv1](/wiki/Performance_points/ppv1)*.
...
- Introduction d'un facteur d'échelle pour le gain supplémentaire en pp des beatmaps avec un approach rate de 11 en fonction de la longueur d'une beatmap.
- Introduction d'un facteur d'échelle à la perte de pp affectée par le mod [No Fail](/wiki/Game_modifier/No_Fail) basé sur le nombre de ratés dans une partie.
...
*Aim* est une valeur fondamentale qui prend en compte la difficulté de frapper de manière cohérente des notes consécutives dans une beatmap.
...
En conséquence, des mods comme [Double Time](/wiki/Game_modifier/Double_Time) et [Half Time](/wiki/Game_modifier/Half_Time) affectent de manière significative la vitesse d'une beatmap prise en compte par l'algorithme des points de performance. De même, ces mods affectent de manière significative les gains de pp lorsqu'ils sont utilisés.
...
*Strain* est une valeur fondamentale qui prend en compte le nombre de fois et la durée pendant laquelle un joueur est soumis à des sections de haute intensité dans une beatmap en particulier.
...
### Où puis-je consulter le classement des performances ?
...
Vous pouvez également accéder aux classements en utilisant le panneau déroulant `Classement` en haut de l'ancienne conception Web, et en choisissant l'option `Performances`.

### Comment puis-je augmenter mon rang et mon pp global ?

**Votre performance est classée principalement en fonction de vos scores sur les beatmaps individuelles.**

La meilleure façon de s'améliorer est de s'efforcer d'obtenir de bons scores sur des beatmaps difficiles ou de jouer à une grande variété de beatmaps.
...
- Visez des combos plus élevés. Les full combos (FC) ou les scores [SS](/wiki/Grade) donnent un score énorme.
...
### Quel est le bonus de pp accordé pour avoir beaucoup de points sur les beatmaps classées ?

**Jusqu'à 416,6667 pp de bonus sont accordés pour l'établissement d'un grand nombre de scores. Ce nombre est atteint à environ 25397 points.**

Vous pouvez calculer le montant exact de ce bonus beatmaps classées avec un ensemble de points :

`416.6667 * (1 - 0.9994 ^ N)`

Le nombre médian de points requis pour atteindre la moitié de ce bonus est d'environ 1 155 points. Comme vous pouvez le constater, le nombre de points requis augmente fortement vers l'extrémité supérieure du spectre.
...
À ce stade, cependant, vous aurez obtenu des scores comparativement plus impressionnants, ce qui signifie que votre pp sera plus élevé dans l'ensemble, car les scores plus élevés que vous avez obtenus l'emportent sur les plus anciens.

### Pourquoi ai-je perdu des pp pour avoir établi un nouveau score ?

**Vous pouvez occasionnellement perdre des pp pour avoir obtenu un meilleur score de combo avec une moins bonne précision, ou pour avoir joué avec des mods avec une moins bonne précision globale.**
...
### Certains mods se sentent en "surpoids" ou en "sous-poids". Pourquoi cela ?
