...
Первая версия новой системы ранкинга была реализована и выпущена в апреле 2012 г. под названием *'???'*. Полное название было обнародовано чуть позже, в том же месяце.
...
Система ppv2 действует в настоящий момент, с её развитием можно ознакомиться в [истории изменений](https://osu.ppy.sh/p/changelog?category=pp).
...
В формулу расчёта сложности входят четырёх основных компонента: **[аим](#аим)**, **[скорость](#скорость)**, **[точность](#точность)** и **[нагрузка](#нагрузка)**. Над праметрами, которые им соответствуют, выполняются разные действия, в результате чего получается некоторое общее число, соответствующее отдельной [сложности](/wiki/Beatmap/Difficulty) карты и тому, как игрок справился с её прохождением.
...
*Нагрузка* (англ. *strain*) — ключевой компонент, показывающий, как много в карте секций с высокой интенсивностью (а также насколько они длинные).

Нагрузку увеличивают участки карты, имеющие крайне высокую скорость или сложные [паттерны](/wiki/Beatmap/Pattern). В качестве примера можно привести карты с обилием [стримов](/wiki/Beatmap/Pattern/Stream) или секций с быстрыми прыжками — они принесут много PP.
...
**Ваша производительность оценивается по рекордам на отдельных картах.**
...
- Несколько крутых рекордов принесут больше PP, чем сотня обыкновенных.<!-- "farming" will need to be linked once a stub or section is created -->
