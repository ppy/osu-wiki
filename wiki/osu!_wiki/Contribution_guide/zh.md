...
1. 进入 [`ppy/osu-wiki`][osu_wiki] 仓库，点击右上角的 `Fork` 按钮，来创建并复制一份你可操控的 wiki 分支。要进入你的副本，前往 [`ppy/osu-wiki`][osu_wiki] 并再次点击 `Fork` 按钮。
   - 如果你之前已经创建好了分支，则按照[最好方法 § 同步复刻 (fork)](/wiki/osu!_wiki/Contribution_guide/Best_practices#同步复刻-(fork))来同步你的分支。
...
所有在 osu! wiki 上做出的更改都需要审阅。在这个阶段，其他合作者会指出可能出现的错误以及修复它们的方法，无论任何形式，还是建议直接在拉取请求 (PR) 中编辑。和同行评审一样，你需要[应用他们的建议](/wiki/osu!_wiki/Contribution_guide/Best_practices#应用审核面板中的修改建议)，或者解释你不想修改的原因。
