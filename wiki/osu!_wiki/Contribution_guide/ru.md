...
- Загляните на канал `#osu-wiki` в [дискорд-сервере osu!](/wiki/Community/Discord_servers#официальный-сервер) и попросите там о ревью, либо оставьте комментарий к своему пулл-реквесту.
...
Рано или поздно ваш пулл-реквест проверит один из [администраторов вики](/wiki/People/osu!_wiki_maintainers) – обычно это происходит после ревью от других людей. Если ничего не происходит в течение нескольких дней, позовите администраторов в комментариях к пулл-реквесту, либо на канале `#osu-wiki` в [дискорд-сервере osu!](/wiki/Community/Discord_servers#официальный-сервер). После того, как вашу ветку вольют в `master`, соответствующие статьи обновятся (чаще всего это происходит за пару секунд, в худшем случае – в течение пяти часов).
