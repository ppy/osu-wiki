...
![编写 .osb 脚本的例子。](img/SBS_Base.jpg "编写 .osb 脚本的例子。")
...
对象接受 PNG 与 JPEG 格式。JPEG 格式是有损的，意味着文件大小较小，但并不会精确存储每个像素，也不支持透明，因此适用于背景图片、方形图像与真实感的图像。PNG 格式是无损的，意味着它会保留每个像素的信息，但文件大小会比 JPEG 大。PNG 图像支持透明，因此通常是前景对象或文本的最佳选择。

动画的处理在引擎中完成，因此不应使用 PNG 图层体系或动画的功能。你应该将每一帧图像保存为单独的文件，每个文件的扩展名前加一个数字（如对于两帧的动画“sample.png”，可使用“sample0.png”与“sample1.png”）。
...
- 在**同一**层重叠的对象会按指定的顺序绘制（例如，如果对象 1 先在 .osb 或 .osu 文件中被指定，然后是对象 2，二者处于同一层，则对象 2 会显示在对象 1 上层）。
...
- 当处于第一个颜色连击，或前一个连击末尾获得“激”（或“精英节拍！”，连击中全部获得 300）时，处于通过状态。
- 否则为失败状态。注意与 DS 游戏（有三个状态）不同，“喝”（或“节拍！”）判定没有对应的状态。
...
- 时间从谱面主要音频文件 (`.mp3`/`.ogg`) 起始处开始，以毫秒为单位（1000 毫秒 = 1 秒），也可以用负值表示前奏。
- 故事板的时间与谱面本身的计时设置（如小节数、每分钟拍数）无关。因此，推荐你在制作故事板前就将谱面的时间轴测好，因为这些设置日后会更难调整。
- 时间不受歌曲长度限制。歌曲开始前（前奏）事件的时间可以使用负值，事件也可以使用游玩部分与音频文件结束后的时间（尾奏）。
