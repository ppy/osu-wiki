...
As storyboards são armazenadas em ficheiros de beatmap como [arquivos `.osb`](/wiki/Client/File_formats/osb_(file_format)) autônomos ou extensões para a secção `[Events]` de um [arquivo `.osu`](/wiki/Client/File_formats/osu_(file_format)). Por causa disso, é possível criar diferentes storyboards para diferentes mapas dentro de um [beatmap](/wiki/Beatmap).
