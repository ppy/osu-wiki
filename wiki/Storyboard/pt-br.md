...
As storyboards são armazenadas em pastas de beatmap como [arquivos `.osb`](/wiki/Client/File_formats/Osb_(file_format)) autônomos ou extensões para a seção `[Events]` de um [arquivo `.osu`](/wiki/Client/File_formats/Osu_(file_format)). Por causa disso, é possível criar diferentes storyboards para diferentes mapas dentro de um [beatmap](/wiki/Beatmap).
