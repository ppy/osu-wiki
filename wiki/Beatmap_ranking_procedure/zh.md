...
[摸图 (Modding)](/wiki/Modding) 指接受与谱面相关的建设性批评的过程，这有助于提高谱面质量。玩家通常在谱面讨论页[发表意见和建议](/wiki/Beatmap_discussion)，或者直接与谱师私聊讨论谱面中可能存在的问题。

经验不足的新谱师想要让自己的谱面质量达到上架标准，往往需要大量的修改，有时候甚至需要推倒重来 (Remap)。

在通过审核并上架之前，谱面通常都需要至少获取 5 个来自不同玩家的[推荐 (hype)](/wiki/Beatmap/Hype)。

## 提名 {id=nominations}

::: Infobox
![](img/nomination-progress-beatmapset2464556-ZH.png "显示在谱面讨论页上的谱面审核进度，推荐进度条已完全填满。")
:::
...
有时，BN 或 NAT 成员可能会对某个谱面行使[否决权 (veto)](/wiki/People/Beatmap_Nominators/Beatmap_Veto)，阻止其进入[上架 (Ranked)](/wiki/Beatmap/Category#ranked) 状态。这通常发生在 BN 或 NAT 认为谱面质量存在严重问题的情况下。谱面否决可以保证在继续推进审核流程之前，社区就谱面的任何需要被解决的关键问题进行更深入的讨论和明确。
