...
Si la beatmap qu'un utilisateur télécharge n'est pas déjà sur le site d'osu!, la fenêtre BSS indique combien de nouveaux téléchargements un utilisateur peut publier. Si la beatmap est nominé, la fenêtre prévient que la nomination sera réinitialisée par la mise à jour de la beatmap. Si le beatmap est dans le cimetière, la fenêtre prévient que la beatmap sera élevé à la catégorie en attente.

## Options de soumission

![](img/bss_submitting.png "Écran de téléchargement du Beatmap Submission System")

En cliquant sur le bouton `Télécharger un nouveau beatmap` ou `Mettre à jour une beatmap`, l'utilisateur aura le choix de télécharger son beatmap dans les catégories `Work in Progress/Help` ou `Pending Beatmaps`. Les beatmaps WIP ne peuvent pas être nominés pour le classement, alors que les beatmaps en attente le peuvent.
...
Les beatmaps ne seront pas soumis s'ils dépassent la taille du fichier en ligne ou la limite de difficulté. La taille limite des fichiers est de 5 Mo, plus 10 Mo supplémentaires pour chaque minute, et elle est plafonnée à 100 Mo. La limite de difficulté est actuellement de 128.
