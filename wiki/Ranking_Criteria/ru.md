...
- **Рекомендации:** Рекомендации могут быть нарушены в **исключительных** обстоятельствах. Эти исключительные обстоятельства должны быть оправданы исчерпывающим объяснением того, почему они были нарушены, и почему при их **ненарушении** качество игры ухудшится.
...
- **Если песня является `TV size`, используйте маркер `(TV Size)` в конце текущей строки заголовка.** Если в заголовке уже есть пометка `TV Size`, то она заменяется на `(TV Size)`. Если в названии песни есть маркеры `Short Ver` или `Game Ver`, то о должны использовать `(Short Ver.)` или `(Game Ver.)`.
    - Примечание: Если песня в мапсете содержит совпадающие секции в песне `TV-size`/`Short Ver`/`Game Ver`, в том же порядке и длине, как в официальном версии, версия мапсета также будет считаться `TV-size`/`Short Ver`/`Game Ver`. Каверы или ремиксы не считаются.
