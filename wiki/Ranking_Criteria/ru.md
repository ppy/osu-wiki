...
Для карт короче требуется полный набор сложностей, а попасть они могут только в категорию [Ranked](/wiki/Beatmaps).
...
* **Не изменяйте название песни.** Сюда относится ИЗМЕНЕНИЕ капитализации, приписывание Short Ver, TV Size и так далее.
...
[Битрейт](http://ru.wikipedia.org/wiki/Битрейт) аудиофайла должен быть не ниже 128 кбит/с и не выше 192 кбит/с; последнее ограничение не относится к переменному битрейту (VBR).
...
Многие [аудиоредакторы](http://osu.ppy.sh/forum/t/34303) позволяют легко обрезать песню с постепенным затуханием.
...
Если требуется полностью убрать звук, вы **должны** использовать [этот пустой файл](http://up.ppy.sh/files/blank.wav).
...
Пример особого случая — [шум моря в конце карты](https://osu.ppy.sh/s/104260).
  * Если хитсаунд с задержкой используется во **встроенном стандартном скине** (например, `normal-hitfinish`), это правило можно опустить.
...
* **Киаи долен начинаться с сильной доли (белого тика).**
...
Уменьшение размера картинок может это исправить.
