...
- **Big don/Big red note/Finisher note :** Un rythme fort qui peut être frappé avec les deux touches du Drum Centre en même temps pour un score supplémentaire.
- **Big kat/Big blue note/Finisher note :** Un rythme fort qui peut être frappé avec les deux touches du Drum Rim en même temps pour un score supplémentaire.
...
- **Overlap :** Une note qui recouvre partiellement ou totalement une ou plusieurs autres notes dans le terrain de jeu.
...
- **Slider velocity :** Vitesse à laquelle les notes/drumrolls/spinners/bar lines se déplacent horizontalement de droite à gauche dans le terrain de jeu. Le slider velocity de base peut être contrôlée dans le panneau de chronométrage et des modifications supplémentaires peuvent être apportées grâce aux points de chronométrage hérités (verts).
...
- **Taiko template background :** Images d'arrière-plan qui simulent le jeu authentique de Taiko no Tatsujin. Elles comprennent généralement une barre noire indiquant l'artiste et le titre de la musique en blanc sous le terrain de jeu.
...
- **Évitez d'utiliser des changements de slider velocity sur des sections qui comprennent un variable snapping.** Cela a un impact sur la lisibilité de ces snappings, il faut donc que la variation soit suffisamment faible pour éviter l'overlapping.
- **Évitez les changements brusques de slider velocity à l'intérieur de pattern qui se chevauchent déjà (par exemple, les stream 1/4).** Dans ces cas, il faut utiliser des changements de slider velocity en douceur pour que les patterns restent lisibles.
...
- **Évitez les rythmes qui ne sont en aucun cas prévisibles.** Le rythme peut être rendu intuitif par l'utilisation d'intervalles de temps cohérents entre les différentes prises de vue ou par des moments de repos.
- **Le kiai time ne doit être utilisé que pour le refrain ou les parties accentuées d'une musique.** Les Kiai flashes/short kiais sont déconseillés pour plusieurs raisons : ils perturbent l'expérience de jeu, notamment sur les utilisateurs de PC bas de gamme, et peuvent causer des problèmes aux utilisateurs épileptiques.
...
- **Évitez d'obstruer visuellement les notes sur le terrain de jeu avec des spinners actifs.** Les spinners couvrent la majorité de l'écran, donc les placer trop près des notes suivantes peut entraîner des pics de lecture. En général, une distance de 1/2 entre un spinner et la note suivante résout ce problème.
...
- **Si un pattern 1/2 est utilisé, les patterns doivent rester simplistes et être suivis d'un moment de repos.** Les notes de finition ne doivent pas être utilisées dans les patterns en tant que tels. Pour les musiques qui suivent un rythme swing, cette limite est de 1/3.
- **Les notes doivent être espacées d'au moins 1/2 d'un temps.** Tout ce qui est plus rapide est trop complexe pour les joueurs débutants. Pour les musique qui suivent un rythme swing, cette limite est plutôt de 1/3.
...
- **Si un pattern 1/3 est utilisé, les patterns doivent rester simplistes.** Les notes finisher ne doivent pas être utilisées dans les patterns en tant que tels.
...
- **Il est possible d'utiliser des simple slider velocity.**
...
- **Les changements de slider velocity doivent être utilisés avec précaution.** Les changements ne doivent se produire que pour des sections de pacings différents et le slider velocity ne doit pas varier de façon drastique.
...
- **Les notes finisher ne doivent pas être utilisées dans des patterns 1/4 ou plus rapides dans cette difficulté.** L'utilisation du finisher sur ces patterns est trop compliquée pour le public de ce niveau.
...
- **La manipulation du slider velocity est autorisée**, mais le slider velocity ne doit être modifiée que pour les sections de la musique ayant des rythmes différents et ne doit pas être radicalement variable.
...
- **Les notes finisher ne doivent pas être utilisées dans les patterns 1/6 ou plus rapides de cette difficulté.** L'utilisation du finisher sur ces patterns est trop compliquée pour le public de ce niveau.
- **Les finishers pour les patterns de 1/4 dans cette difficulté ne doivent être utilisées qu'à la fin de ce pattern.** D'autres placements peuvent perturber la lisibilité pour le public de ce niveau.
...
- **Au moins un moment de repos de 1/1 ou plus doit être inséré après 16/1 à 20/1 de mapping continue.** Les joueurs peuvent être trop sollicités s'il n'y a pas assez de repos ou si les repos sont trop courts.
