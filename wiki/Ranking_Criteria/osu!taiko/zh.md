...
- **Don / Red note（咚 / 红音符）**：在太鼓中心击打的节拍（默认按键是 `X`，`C`）。
...
- **Slider / Drumroll（滑条 / 黄条）**：一个黄条中带有打点，玩家需要按照节拍击打黄条，颜色任意。这些打点通常是间隔 1/4 拍（根据歌曲的 BPM 不同而不同）。如果 BPM 低于 125，标记的间隔会变成 1/8 拍。如果 BPM 高于 250，间隔会变成 1/2 拍。如果滑条打点率（slider tick rate）被设为 3，滑条标记间隔会变成 1/3 拍。
- **Spinner（转盘）**：一个转动的物件，要求玩家交替击打红圈和蓝圈固定的次数，次数显示在转盘的中心。敲击次数由谱面的整体难度（OD）和转盘的长度决定。
- **Bar line（小节线）**：游戏区域中显示的线，用于判断小节的开始。
- **Overlap（重合音符）**：在游戏区域内音符相覆盖。
- **Rest moment（留白）**：一段没有音符的时间，通常用于让玩家放松双手，迎接即将到来的音符。
- **Stream（串/鱼蛋）**：一连串没有留白的音符。一般间隔是 1/4 拍。
- **Snapping（时间线）**：放置物件的时间线记号。
- **Variable snapping（变节拍）**：根据歌曲本身的特点，在一小段时间内使用多种不同的组合（如四分音符和六分音符混合）方式来排列音符。
- **Slider velocity（滑条速度）**：决定了谱面（包括音符、黄条、小节线等等）的水平移动速度。基础滑条速度可以从 timing panel （时间窗口）设置，每一段落的滑条速度则通过继承（绿色）时间节点控制。
- **Smooth slider velocity changes（滑条速度渐变）**：一种从高到低或者从低到高逐渐改变滑条速度（SV）的技巧。为了达到这一效果，这些渐变的音符会有变化的滑条速度。
- **Improvisation**：拥有比歌曲实际更多的音符。
- **Taiko template background（太鼓模板背景）**：模仿原版太鼓达人游戏界面的背景图片。通常包含在休息时间显示的艺术家和歌曲名称。
...
- **如果使用了 1/3 音型，配色一定要足够容易，并且在该段后适当留白。** 该音型中禁止使用大音。
- **不允许比 1/3 更密的音型。** 这些节奏对于新手来说太复杂。

如果此 Futsuu 难度是谱面集的*最低难度*，除上述准则外，其还应遵循这些准则：

- **不允许比 1/2 更密的音型。** 这些节奏对于新手来说太复杂。对于三分拍的曲子，此限制为 1/3 拍。
 
#### 指南

- **1/3 的音符不宜超过 2 连。** 稍长的连续音符，尤其是 1/3 音符对新手太复杂。请在这样的音型后适当留白。
