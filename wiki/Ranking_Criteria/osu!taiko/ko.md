...
- **스트림:** 서클이 연속적으로 모인 부분. 일반적으로 ¼ 비트에 스냅 합니다.
...
모든 **규칙**은 가이드라인이 **아니므로**, **어떠한 상황**에서도 **어기면 안됩니다**.

- **각 노트는 이전 및 이후에 올 노트와 명확히 구분이 가능한 수 있는 색상을 가지고 있어야 합니다.**
- **모든 음은 그것이 표현하려고 하는 음악 레이어나 노래가 제공하는 레이어를 강화하기 위한 레이어 유닛, 아니면 매퍼에 의해 즉흥적으로 만들어진 추가적인 레이어 유닛인지 분명하게 지정할 수 있어야 합니다.** 없는 속도 변화를 만들거나, 일반적인 음악의 움직임과 모순되거나, 노래의 강도를 잘못 해석하는 방식으로 즉흥 매핑을 하지 마세요. 즉흥 매핑은 노래의 레이어를 강화하거나, 새로운 레이어를 추가해야 합니다. 그렇지 않으면, 노래와의 관계를 포기하는 것이 되므로 리듬 게임의 주된 목적과는 모순되게 됩니다.
- **상속된 포인트의 기능 중 하나인 `Omit first bar line`은 BPM 혹은 메트로놈의 재설정이 불필요한 바 라인을 추가하여 미적인 부분에서 방해가 되는 경우에는 반드시 사용해야 합니다.** 
...
- **쓸데없이 시각적인 방해 없이 색 구분을 하기 쉽게 심한 오버랩은 피해주세요.** 오버랩은 그 지점의 노래의 속도나 박자를 맞추기 위해 노트를 찍는 것이 정당화될 수 있는 경우에만 이루어져야 합니다.
