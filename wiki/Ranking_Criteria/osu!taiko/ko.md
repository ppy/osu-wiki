...
- **모든 노트는 곡에서 자신이 나타내려고 하는 부분을 명확하게 지정할 수 있어야 합니다.** 없는 속도 변화를 만들거나, 일반적인 음악의 움직임과 모순되거나, 노래의 강도를 잘못 해석하는 방식으로 매핑을 하지 마세요. 즉흥적으로 매핑을 할 때는 노래의 레이어를 강화하거나, 새로운 레이어를 추가하는 방향으로 해야 합니다. 그렇지 않으면, 노래와의 관계를 포기하는 것이 되므로 리듬 게임의 주된 목적과는 모순되게 됩니다.
