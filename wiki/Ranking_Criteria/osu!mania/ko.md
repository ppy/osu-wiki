...
-   **히트 노말 (hitnormal)없이 keysound(키 사운드)를 사용하지 마십시오.** hitsound가 곡과 완벽하게 조화를 이룬다면, hitsound에 대한 피드백은 별로 없을 것입니다. 그러나 곡에서 직접 추출한 keysound가 곡의 오디오를 대체하는 데 사용될 때는 hitnormal의 사용이 필요하지 않습니다.
...
_Key mode별 Guideline은 가장 흔하게 랭크되는 key mode인 (4키 및 7키)를 기반으로 작성됩니다. 기타 모든 key mode는 해당되는 경우 이 guideline을 따라야 합니다._
...
Normal이 _비트맵 세트의 가장 낮은 난이도인 경우_, 다음과 같은 추가 **rule** 을 준수해야 합니다. :
...
_4 key Normal_ 난이도의 추가적인 guidelines :
...
_7 key Normal_ 난이도의 추가적인 guidelines :
...
If Hard is the _비트맵 세트에서 가장 낮은 난이도인 경우_, 다음과 같은 추가 **guideline** 을 준수해야 합니다. :
...
_4 key Hard_ 난이도의 추가 guidelines :
...
_7 key Hard_ 난이도의 추가 guidelines :
...
_4 key Insane_ 난이도의 추가 guidelines :
...
_7 key Insane_ 난이도의 추가 guidelines :
