...
Difficulty-specific의 rule과 guideline은 아래에 나열된 난이도 수준에만 적용되므로 모든 osu!mania 난이도에는 적용되지 않습니다. 리듬과 관련된 guideline은 약 180의 BPM을 가진 맵을 기준으로 삼습니다. 만약 당신의 노래가 엄청나게 빠르거나 느리면, 어떤 guideline은 어겨야 할 수도 있습니다. 이 경우에는 합리적인 판단하에 적용하십시오.
...
- **2개 이상의 박자가 손으로 이어지는 split-jumptrill을 사용하지 마십시오.** 비트맵이 Jack이나 다른 복잡한 패턴을 자주 사용하지 않는 한, 이 패턴은 맞추기가 어렵습니다. Hand패턴이 필요한 경우 Mini jack 을 만들지 않도록 노트를 삭제하십시오.
