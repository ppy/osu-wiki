...
osu!mania 특유의 난이도를 랭크시키기 위해서는, 나열된 정해진 기준에 따라야 합니다. 어떤 상황에서든지 **모든 Rule에는 따라야 하지만,** 예외적인 상황에서 Guideline은 무시될 수 있습니다. 이러한 예외적인 상황에서, 맵 제작자는 왜 Guideline 무시하는것이 맵에 더 좋은 영향을 주는지 자세히 설명하여야 합니다.
...
- **Key sounding(키 사운딩)**: 샘플을 사용하는 히트 사운드가 해당 구간의 노래와 매우 흡사하거나 같은 경우. 이 히트 사운드 방법은 플레이어로부터 피드백을 거의 받지 않습니다.
...
- **Release(릴리즈)**: 전체 노트에 제대로 된 점수를 매기기 위해 확실한 타이밍 시간대에 끊어야 하는 롱 노트의 끝.
...
- **Split-jumptrill(스플릿 점프트릴)**:  양손을 동시에 사용해야 올바르게 수행되는 점프트릴(즉,양손이 동시에 치는 한손 트릴이다.예시: |1||4|->|2|3||).
...
- **`Auto` mod(`오토` 모드) 는 모든 난이도에서 만점 (1,000,000점)을 달성할 수 있어야 합니다.**
- **두 개의 노트를 동일한 타임스탬프에서 하나의 column에 배치할 수 없습니다.**
- **모든 column엔 노트가 배치되어있어야 합니다.** key를 적게 사용하시고 싶으시다면 노래 설정에서 key의 수를 변경하십시오.
- **Insane 난이도나 그보다 더 낮은 난이도에서 한번에 6개 이상의 노트를 누르게 하지 말아야 합니다.** 또한 6개 이상의 노트를 동시에 사용하는 것은 하위 난이도와의 난이도 격차에 영향을 끼쳐서는 안됩니다. 이 규칙은 롱 노트를 끊는것에는 적용되지 않으며. 롱 노트를 누르는것에는 적용됩니다.
- **비트맵은 4~9 key까지 사용될 수 있습니다. 나머지 항목은 랭킹 섹션에 지원되지 않습니다.
- **비트맵 세트의 각 key mode에는 자체적인 스프레드가 필요합니다. 예를 들어, 4 key 비트맵은 비트맵 세트에 존재하는 다른 key mode와는 독립적으로 적절한 스프레드를 생성하기 위해 4 key mode의 노멀/하드/인세인 (Normal/Hard/Insane)난이도가 필요할 것입니다.
- **단일 비트맵 세트에 여러 개의 key mode가 있는 경우, key mode를 난이도 이름에 표시해야 합니다. 예시: Easy(4k)/Normal(4k)/Hard(4k)/Insane(4k)**
...
이 때,음악을 완전히 정확하게 나타내는것의 예외로 음악을 단순화시켜서 노트를 두는것이 허용되는 경우는...
  - 완전히 정확하게 표현할 경우 플레이 하기에 적절하지 않은 경우나,
  - 일관되고 예측 가능한 스냅으로 효과적으로 표현될 수 있는 경우이거나,
  - 부적절한 난관을 야기할 수 있는 경우입니다.
- **노트에 적용된 hitsound(히트 사운드)와 유사한 볼륨의 storyboard sampled hitsounds(스토리보드 샘플 히트 사운드)를 사용하지 마십시오.
스토리보드로 된 hitsound가 비트맵의 객체로부터 오디오 피드백을 압도하고 왜곡할 수 있으므로, 노트에 hitsound를 배치하는 것이 스토리보드 샘플을 배치하는 것보다 우선되어야 합니다. 단, 주요 사운드를 곡에서 직접 추출할 때, 그리고 낮은 난이도에 필요한 hitsound를 지원하기에는 해당 난이도의 노트가 충분하지 않을 때 스토리보드 샘플이 사용될 수 있습니다.
- **히트 노말 (hitnormal)없이 key sound(키 사운드)를 사용하지 마십시오.** hitsound가 곡과 완벽하게 조화를 이룬다면, hitsound에 대한 피드백은 별로 없을 것입니다. 그러나 곡에서 직접 추출한 key sound가 곡의 오디오를 대체하는 데 사용될 때는 hitnormal의 사용이 필요하지 않습니다.
- **chord를 표현하는 여러 노트에 동일한 히트 사운드를 사용하지 마십시오** 이것은 피드백을 필요 이상으로 많이 발생하게 만듭니다. 이 경우는 같은 타임스탬프에서 사용되는 hitsound와 storyboard hitsound에도 적용됩니다.
...
- **슬라이더 속도 변화는 곡의 박자 구조에 따라 끊어져야 합니다.** 곡이 올곧은 박자를 사용하고 있다면 1/4, 1/8, 1/16박자를 사용해야 하고, 변환된 박자를 사용하고 있다면 1/6, 1/12박자를 사용해야 합니다. 추천하는 박자와 다른 위치에 박자가 있다면 실제 박자에 스냅하는 것이 항상 우선입니다.(곡이 1/7박자나 1/11박자를 사용하는 경우 노트가 그 박자를 따르기 위해서 기존 타이밍 라인에 스냅되지 않아도 허용이 됩니다.)
...
Difficulty-specific의 rule과 guideline은 아래에 나열된 난이도 수준에만 적용되므로 모든 osu!mania 난이도에는 적용되지 않습니다. 리듬과 관련된 guideline은 약 180의 BPM을 가진 맵을 기준으로 삼습니다. 만약 당신의 노래가 엄청나게 빠르거나 느리면, 어떤 guideline은 어길수도 있습니다. 이 경우에는 합리적인 판단하에 적용하십시오.

*Key mode별 Guideline은 가장 흔하게 랭크되는 key mode인 (4키 및 7키)를 기반으로 작성됩니다. 기타 모든 key mode는 해당되는 경우 이 guideline을 따라야 합니다.*
...
- **다른 롱 노트가 눌려지고 있을 때, 또다른 롱 노트가 먼저 끝나서는 안됩니다.** 이것은 이 난이도 수준의 플레이어들이 치기엔 너무 어렵습니다.

#### Guidelines

- **HP와 OD 값은 7 이하여야 합니다.**
...
- **5개 이상의 1/2노트를 연속적으로 사용하지 마십시오.**
- **연속적인 1/2박자의 패턴이 나오는동안 chord를 사용하지 마십시오.** 하지만, 패턴을 끝낼 때 chord를 사용하는 것은 괜찮습니다.
- **음이 2개 이상인 chord는 피해야 합니다.** 여기에는 두 개 이상의 column에 있는 노트를 포함하는 롱 노트 패턴이 포함됩니다.
...
- **1/4 박자를 사용하는 노트를 5개 이상 연속적으로 배치하지 마십시오.**
...
- **연속적인 1/4 노트 스냅은 3개 이상은 허용되지 않습니다.** 비트맵 세트의 가장 낮은 난이도에서 밀도가 높은 리듬은 적합하지 않습니다.
- **슬라이더 속도 변화는 허용되지 않습니다.** BPM이 변하는 비트맵에서 속도를 맞추기 위해 사용되는 속도 변화만이 예외가 됩니다.
- **BPM이 변하는 비트맵의 스크롤 속도는 정규화되어야 합니다.**

*4 key Normal* 난이도의 추가적인 guidelines :
- **2개의 노트보다 많은 노트를 사용하는 chord는 피하십시오.** 여기에는 두 개 이상의 column 자리를 차지하는 롱 노트 패턴이 포함됩니다.
- **3개 또는 그 이상의 노트를 사용하는 anchor 패턴의 사용을 피하십시오.** 그것들은 플레이어가 읽기 쉬운 반면에, 더 긴 anchor는 힘들 수 있습니다.

*7 key Normal* 난이도의 추가적인 guidelines :
- **3개의 노트보다 많은 노트를 사용하는 chord는 피하십시오.** 여기에는 세 개 이상의 column 자리를 차지하는 롱 노트 패턴이 포함됩니다.
...
- **1/4 박자를 사용하는 stream 패턴 에서 chord는 적게 사용해야 합니다.** 이러한 패턴의 밀도는 종종 이 수준의 플레이어들에게 너무 어렵습니다.
...
- **9개 이상의 노트를 trill 패턴에 연속적으로 사용하지 마십시오.** 이 패턴은 이 기술 수준의 플레이어들에게는 너무 어렵습니다.
...
If Hard is the *비트맵 세트에서 가장 낮은 난이도인 경우*, 다음과 같은 추가 **guideline** 을 준수해야 합니다. :
...
- **노트가 3개 이상인 chord는 피하십시오.** 여기에는 세 개 이상의 column 자리를 차지하는 롱 노트 패턴이 포함됩니다.
- **5개 또는 그 이상의 노트로 구성된 anchor는 피하십시오.** 그것들은 플레이어가 읽기 쉬운 반면에, 더 긴 anchor는 힘들 수 있습니다.
- **Jump stream 패턴에서 1/1박자 이상의 간격으로 jump간의 거리를 유지하십시오.** 그것보다 더 밀도가 높은 것은 가독성을 해칠 수 있습니다.

*7 key Hard* 난이도의 추가 guidelines :
- **노트가 4개 이상인 chord는 피하십시오.** 여기에는 네 개 이상의 column 자리를 차지하는 롱 노트 패턴이 포함됩니다.
- **5개 또는 그 이상의 노트로 구성된 anchor는 피하십시오.** 그것들은 플레이어가 읽기 쉬운 반면에, 더 긴 anchor는 힘들 수 있습니다.
- **chord stream 패턴에서 1/1박자 이상의 간격으로 chord간의 거리를 유지하십시오.** 그것보다 더 밀도가 높은 것은 가독성을 해칠 수 있습니다.
...
- **4박자 이상 지속되는 split-roll 패턴이나 복잡한 1/8 stream을 사용하지 마십시오.** 이런 스냅 유형의 stream은 플레이어가 치기 어려우며, 곡이 그것을 보증할 경우에만 사용해야 합니다.
