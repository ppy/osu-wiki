...
- **Column（列）**： 游戏区域中用于放置物件的垂直线性区域。游戏编辑器使用 `0|1|2|3|4|5|6|7|8` 作为时间戳中的标签进行标记。 
- **Key mode（键位模式）**： 难度中的列数量。
- **Keysounding**： 在音乐对应的时间点，使用与音乐很相近的音效，或者直接从音乐中截取一小段作为谱面音效。这种音效的做法通常给玩家的打击反馈感很少。
- **Slider velocity （滑条速度）**： 物件与节拍线在游戏区域中移动的速度。谱面基础的滑条速度可以在 Timing 界面设置，额外的滑条速度改变可以通过继承时间点（绿线）来完成。
...
- **Trill（交互）**： 两个或两个以上的物件在两列间不间断交替出现。
...
如果此 Normal 难度是谱面集的*最低难度*，除上述规定外，其还应遵循这些规定：
...
如果此 Normal 难度是谱面集的*最低难度*，除上述指南外，其还应遵循这些指南：
...
如果此 Hard 难度是谱面集的*最低难度*，除上述指南外，其还应遵循这些指南：
