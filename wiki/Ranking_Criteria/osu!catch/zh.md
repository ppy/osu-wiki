...
- **Drop(水滴)：** 一个代表滑条节奏点的中等大小物件。
- **Droplet（小水滴）：** 一个代表滑条体的小物件。没接住它们会降低你的准确度，但是与前面两种物件不同的是，它不会造成连击中断。
- **Banana（香蕉）：** 一个在转盘时出现的物件。它们有额外得分，但是不会影响准确度和连击。
- **Active object（有效物件）：** 为达到全连所必须接住的物件，也就是水果和水滴。
- **Walk（走）：** 不使用跳就能接住的物件间距。
- **Dash（跳）：** 需要按住加速键来接住两个物件的间距。
- **Hyperdash（红果跳）：** 两个有效物件间间距太大，只按住加速键是接不到的。在游戏中，会由前一个物件发光来表示。也用来指这两个物件之间的距离。
- **Trigger distance（触发距离）：** 两个有效物件之间产生红果跳所需的最短距离。
- **Edge dash（边缘跳）：** 当两个有效物件之间距离较大但又未达到触发距离时，第一个物件必须用盘子的边缘去接才能接住第二个物件。
- **Basic dash/hyperdash（基础跳/红果跳）：** 两个有效物件间时间间隔在允许跳的最小时间两倍或以上时的跳或红果跳。例如，Platter 难度中时间间隔在 250 毫秒的跳是一个基础跳。
