...
- **Спрайт ловца должен быть совместим с форматом скинов v2,** чтобы корректно отрисовываться во всех скинах. Используйте файлы `fruit-catcher-idle.png`, `fruit-catcher-kiai.png` и `fruit-catcher-fail.png`.
- **Спрайты объектов должны добавляться для всего набора элементов сразу, а также быть покрашены только в оттенки серого.** Это необходимо для полноты скина, а также для поддержания его качества. Список требуемых элементов можно найти в статье про [скиннинг osu!catch](/wiki/Skinning/osu!catch). Кроме того, в качестве оверлеев рекомендуется использовать только прозрачные элементы.
...
- **Постарайтесь добавить `lighting.png` в набор спрайтов с ловцом для его полноты.** Этот элемент необязательный и по умолчанию имеет размер 184x184 пикселя, но, в зависимости от ваших планов, можно использовать и другой размер.
...
- **Запрещены любые [дэши](/wiki/Gameplay/Dash) и [гипердэши](/wiki/Gameplay/Hyperdash),** чтобы новичкам было комфортно. Таким образом, сложность должна быть проходимой на SS без использования клавиши, активирующей дэш.
...
- **Запрещены любые [гипердэши](/wiki/Gameplay/Hyperdash),** чтобы новички могли нормально освоить новый уровень сложности.
- **Между объектами в [дэшах](/wiki/Gameplay/Dash) должно быть минимум 125 мс.**
- **[Простой дэш](/wiki/Gameplay/Dash_snapping#простой-дэш) не должен использоваться в идущих подряд [фруктах](/wiki/Hit_object/Fruit) больше двух раз.**
- **После [простых дэшей](/wiki/Gameplay/Dash_snapping#простой-дэш) всегда должен идти [переход](/wiki/Gameplay/Walk).**
- **[Пиксельные дэши](/wiki/Gameplay/Edge_dash) запрещены:** они требуют точно выверенного тайминга, чего нельзя ожидать от менее опытных игроков.
- **Перед [спиннерами](/wiki/Hit_object/Spinner) и после них должна быть пауза длиной как минимум 250 мс,** чтобы их можно быть прочитать.

#### Рекомендации

- **По внешнему виду паттернов должно быть понятно, как их поймать: с помощью [перехода](/wiki/Gameplay/Walk) или [дэша](wiki/Gameplay/Dash).** Так игроки лучше учатся узнавать паттерны, где требуется дэш.
- **Не ставьте несколько [простых дэшей](/wiki/Gameplay/Dash_snapping#простой-дэш) подряд между объектами с разным снаппингом** (например, когда после дэша между 1/1 сразу же идёт дэш между 1/2).
- **После [быстрых дэшей](/wiki/Gameplay/Dash_snapping#быстрый-дэш) не должно быть паттернов, использующих [анти-флоу](/wiki/Beatmapping/Antiflow).**
...
- **[Простой гипердэш](/wiki/Gameplay/Dash_snapping#простой-дэш) не должен использоваться в идущих подряд [фруктах](/wiki/Hit_object/Fruit) больше двух раз.**
- **[Быстрый гипердэш](/wiki/Gameplay/Dash_snapping#быстрый-дэш) нельзя сочетать с любым другим [дэшем](/wiki/Gameplay/Dash) или гипердэшем.**
- **Между объектами в [дэшах](/wiki/Gameplay/Dash) должно быть минимум 62 мс.**
- **[Простой дэш](/wiki/Gameplay/Dash_snapping#простой-дэш) не должен использоваться в идущих подряд [фруктах](/wiki/Hit_object/Fruit) больше четырёх раз.**
- **[Быстрые дэши](/wiki/Gameplay/Dash_snapping#быстрый-дэш) можно использовать в идущих подряд [фруктах](/wiki/Hit_object/Fruit) до двух раз, если при этом не меняется направление движения.**
- **[Пиксельные дэши](/wiki/Gameplay/Edge_dash) запрещены:** они требуют точно выверенного тайминга, чего нельзя ожидать от менее опытных игроков.
...
- **Не используйте сильные [гипердэши](/wiki/Gameplay/Hyperdash).** Рекомендуется использовать [простые гипердэши](/wiki/Gameplay/Dash_snapping#простой-дэш) со спейсингом в 1.5 раза больше [порогового расстояния](/wiki/Glossary/Trigger_distance). Для [быстрых гипердэшей](/wiki/Gameplay/Dash_snapping#быстрый-дэш) рекомендуется спейсинг в 1.3 раза больше порогового расстояния.
- **[Простые гипердэши](/wiki/Gameplay/Dash_snapping#простой-дэш) можно использовать вместе с паттернами с [анти-флоу](/wiki/Beatmapping/Antiflow).** Если в этом случае после гипердэша идёт [переход](/wiki/Gameplay/Walk), используйте спейсинг максимум в 1.2 раза больше [порогового расстояния](/wiki/Glossary/Trigger_distance), а если [простой дэш](/wiki/Gameplay/Dash#простой-дэш), то используйте спейсинг максимум в 1.1 раза больше порогового расстояния.
- **После [быстрого гипердэша](/wiki/Gameplay/Dash_snapping#быстрый-дэш) не должно быть паттерна с [анти-флоу](/wiki/Beatmapping/Antiflow).** В противном случае, используйте спейсинг максимум в 1.1 раза больше [порогового расстояния](/wiki/Glossary/Trigger_distance).
...
- **[Простой гипердэш](/wiki/Gameplay/Dash_snapping#простой-дэш) не должен использоваться в идущих подряд [фруктах](/wiki/Hit_object/Fruit) больше четырёх раз.**
- **[Простой гипердэш](/wiki/Gameplay/Dash_snapping#простой-дэш) не должен использоваться в одном слайдере более двух раз.** Слайдер при этом должен легко проходиться.
- **[Быстрый гипердэш](/wiki/Gameplay/Dash_snapping#быстрый-дэш) нельзя сочетать с [быстрым дэшем](/wiki/Gameplay/Dash#быстрый-дэш) или любым другим гипердэшем.**
- **[Быстрый гипердэш](/wiki/Gameplay/Dash_snapping#быстрый-дэш) нельзя использовать в слайдере.**
...
- **Не ставьте несколько [простых гипердэшей](/wiki/Gameplay/Dash_snapping#простой-дэш) подряд между [фруктами](/wiki/Hit_object/Fruit) с разным снаппингом** (например, когда после гипердэша между 1/2 сразу же идёт гипердэш между 1/4).
- **Если после [быстрого гипердэша](/wiki/Gameplay/Dash_snapping#быстрый-дэш) идёт паттерн с [анти-флоу](/wiki/Beatmapping/Antiflow), для прохождения которого нужен [дэш](/wiki/Gameplay/Dash), между ними должно пройти не менее 250 мс.**
- **Используйте только одиночные [пиксельные дэши](/wiki/Gameplay/Edge_dash), не сочетая их с другими [дэшами](/wiki/Gameplay/Dash) или [гипердэшами](/wiki/Gameplay/Hyperdash)).**
...
- **Используйте [гипердэши](/wiki/Gameplay/Hyperdash) вместе с [дропами](/wiki/Hit_object/Juice_stream#drop) или повторами слайдеров, только если слайдер легко проходится.** Это необходимо, чтобы избежать хаотичного или излишне сложного движения по слайдеру: ловить дропы на слайдерах сложной формы обычно достаточно неприятно, точность при этом ощутимо падает, а взамен игрок не получает почти ничего.
- **Не ставьте [гипердэши](/wiki/Gameplay/Hyperdash) между идущими подряд парами объектов в 1/8 или с более мелким снапом.**
- **Используйте [пиксельный дэш](/wiki/Gameplay/Edge_dash) не более чем на трёх объектах подряд, а также не ставьте его после [гипердэша](/wiki/Gameplay/Hyperdash).**
- **Длина [комбо](/wiki/Beatmapping/Combo) не должна превышать 16 объектов вместе с реверсами и концами слайдеров.** [Спиннеры](/wiki/Hit_object/Spinner) в комбо не учитываются.
- **При маппинге используйте преимущественно 1/2 с добавлением 1/4 и/или 1/3 с добавлением 1/6.** Пореже применяйте 1/8 и более мелкий снап.
