...
- **合理的分布（Reasonable spread）：** 一个谱面集没有跳过难度相关的规定与准则中的任意难度级别，并在两个相邻难度间没有巨大难度差异。也包含只有一个难度的谱面集。
- **游戏模式（Game-mode）：** osu!，osu!taiko，osu!catch 和每一种 osu!mania 的键数都被看成单独的游戏模式。
- **谱面集创建者（Mapset host）：** 上载和管理谱面集的用户。
- **客串谱面（Guest difficulty）：** 由不是谱面集创建者的其他人添加到谱面集中谱面难度。
- **合作难度（Collaborative difficulty）：** 由多个作图者所做的单张谱面，也被称为 "collab"。

### 规定

- **谱面集内的每个游戏模式都必须有合理的难度分布，从该歌曲长度所需的最低难度开始。** 每个难度分布都必须符合该游戏模式的难度相关准则。
...
- **一个谱面集中的难度名称必须有明显的渐进来指示其相对难度，除了：**
  - 最高难度。
...
- **此外，谱面集创建者的名字不能出现在难度名称中。**（例如：Mapset Host's Insane）。 做了多张拥有相同元信息和合作难度而造成冲突时除外。
...
- **元数据必须在谱面集的所有谱面中保持一致。**
- **客串作图者、故事板作者和音效作者必须添加到谱面集的标签中。** 这是表彰他们的贡献，让其他人了解该谱面集的主要贡献者。
...
- **如果歌曲是 TV size，请在当前标题字符串的末尾使用标准的 `(TV Size)` 标记。** 如果标题中存在 TV Size 标记，使用标准标记替换它。此外，标题中已有 `Short Ver` 或者 `Game Ver` 标记的歌曲必须使用标准的 `(Short Ver.)` 或者 `(Game Ver.)` 标记。
  - 注：如果谱面集的歌曲包含与电视/短版/游戏尺寸歌曲匹配的部分，顺序一致，长度与官方电视/短版/游戏尺寸歌曲大致相同，谱面集也将计为电视/短版/游戏尺寸。封面曲和重编曲不计入。
...
- **谱面集中只应包含一个音频文件用于所有难度。** osu! 不支持单谱面集中的多个音乐文件，这会导致具有预览时间、元数据等出现错误。
...
- **预览时间点必须在同一个谱面集的各个难度间保持一致。** 这用于歌曲选择菜单和在线缩略图预览。
