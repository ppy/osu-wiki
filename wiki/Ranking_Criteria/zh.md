...
- **一个谱面集的最高难度应符合玩家对该歌曲的普遍感觉。** 如果 Easy / Normal 难度的节奏没有被过度简化，则它们可以当做谱面的唯一难度。最高难度高于 Hard 时应当遵守此准则。
...
- **谱面内所有难度的歌曲元数据必须一致。**
- **必须在谱面集的标签中添加[客串难度](/wiki/Beatmap/Guest_difficulty)作者名称、故事板作者名称、音效作者名称。** 这是为了表彰他们的贡献，能让其他人了解该谱面集的主要贡献者。用户名中包含由空格分隔的单个字符时，必须将空格替换为下划线`_`。
- **歌曲元数据必须参考[主要元数据来源](/wiki/Beatmap/Primary_metadata_source)**，若非格式化标准化规则需要，请不要修改主要元数据来源的信息。没有可用的来源时，可以使用最常见或可辨识的。
- **歌曲的艺术家必须可以追溯到真实存在的人。** 如果作者已不可考，则应使用 `Unknown Artist`。若非作为作者的代号，则歌曲的唯一作者不能设为某个虚拟的角色或软件。
- **如果歌曲来自电子游戏、电影、系列、公开活动等等，则必须填写 Source （来源）字段。** 这也适用于歌曲在发布后，某些媒体使用或者冠名，但最初的来源为人所知的情况。如果歌曲有多个可用来源，任意来源都可使用。
  - 对于混音、重编曲、或是基于原曲的翻唱，应使用原曲的歌曲元信息。
  - 网站名只能在歌曲与平台的特定文化现象相关、或是歌曲本身作为网站的主题曲 / 背景音乐时使用。
  - 为避免[歌曲汇编](/wiki/Beatmapping/Song_compilation)、混音、歌曲串烧等等歌曲包含的原曲有多个来源，这些原曲的来源都应放在标签 `Tags` 字段内，而并非 `Source` 字段。（除非此歌曲与某个来源高度相关，则应把这个高度相关来源填写至 `Source` 字段。）
  - `osu!` 不应作为歌曲的来源，但除非歌曲是为 osu! 特别创作的（又称为 osu! original），则可使用。例如某些精选艺术家为 osu! 锦标赛创作的决胜图专属歌曲。
- **必须缩短超过了字段最大长度限制（81个字符）的艺术家或标题字段。** 从删除附加标记开始，如果这还不够，合理缩写并使用 `...` 来表示这首歌的标题已经缩短。
- **如果艺术家或标题字段因字符长度限制而被缩短，略去的信息应补入 Tags 字段中。**
- **如果该歌曲已经过精选艺术家授权，则必须在 Tags 中加入 `featured artist` 。**
- **Tags 必须与谱面相关，且不得对搜索结果产生误导。** 与谱面相关的字段一般是：描述谱面的风格、歌曲、故事板、背景视频、背景图片等。
...
- **如果一个作为来源字段系列含有子系列，则应使用最细化的子系列作为来源。** 如果歌曲有多个子来源可供使用，则应返回使用主来源字段。
...
- **带有俄语或西里尔字符的元数据必须使用 BGN/PCGN 体系来罗马音化。** 同样适用于 Source 字段，如果作者偏好罗马音化表达。如果 `Е` 和 `е` 单独出现，或在 `a`，`e`，`ё`，`и`，`о`，`у`，`ы`，`э`，`ю`，`я`，`й`，`ъ`，`ь` 之后出现，应该罗马音化为 `ye`。在其他情况中，应该罗马音化为 `e`。`ё` 应该罗马音化为 `ye`，但是，使用 `yo` 或 `o` 以避免使用特殊字符。若不使用提供的文件，而使用其他规则都是无关紧要，或是对游戏没有帮助的。对于大部分其他字符的转换规则，请参阅[这个文件的第一页](https://assets.publishing.service.gov.uk/government/uploads/system/uploads/attachment_data/file/807920/ROMANIZATION_OF_RUSSIAN.pdf)。
- **带有日文元数据的歌曲必须使用[平文式罗马字](https://en.wikipedia.org/wiki/Hepburn_romanization#Features)罗马音化。** 如果作图者偏好罗马音的 Source，则也应用于该字段。对于非 Unicode 域，长元音如 `おう` 和 `うう` 应该写作罗马音 `ou` 和 `uu`，以避免长元音符号。
- **带有中文元数据的歌曲，除艺人名称外，都要按照汉字的口音和方言，采用逐字罗马音化，字与字之间加空格的方法处理。** 在任何情况下，必须省略所有拼音声调标记。
...
**当一首歌被另一个艺术家翻唱或改编，且元数据与原始歌曲不同时，需使用常识来确定这是艺术家有意为之还是艺术家无意犯错。**
**当多个元数据都可用，应优先选取容易辨识，且容易追溯到原曲或来源的元数据。** 使用官方发布的罗马音化元数据或翻译能较易识别和使用。

#### 技术

- **如果是作图者改编或翻唱了歌曲，则可以随意给它取适当的名字以表明这首歌是一个特殊版本。** 在这种情况下，为了让玩家能够轻易搜索到原歌曲，原歌曲的名字仍必须清楚地指示出来。
- **对于歌曲串烧或重编曲，应在 Tags 标签字段中填写所使用原歌曲的曲名和艺术家。** 这是为了保证玩家可以通过搜索同一关键字来找到与同一首歌相关的所有谱面，而不会得到很多不同的结果。
- **若元数据中包含难以打出或难以辨认的 Unicode 字符，应在 Tags 标签字段中填写易于搜索的变形字段或者罗马音化字段。**
- **如果歌曲来源同时有 Unicode 与罗马音化形式，应在 Tags 标签字段中填写之前未在 Source 字段中使用的形式。**
- **歌曲风格与语言也应加入 Tags 字段中。** 这是为了让玩家能在游戏内也能像在网站上一样搜索到它。对于纯音乐，使用 `instrumental` 作为语言标记。这不包含语言或曲风不清晰的场合。如果适用于多个语言或曲风，则都应填写。
- **Tags 还应加入相关艺术家、歌曲的另一个标题、或另一个艺术家名称的拼写、元数据字段中简写词汇的完整字段，以及任何能够帮助玩家找到该谱面的内容。**
...
- **对于重编曲或翻唱，原始艺术家可以被用作艺术家字段，同时修改标题字段来指明这是一个重编曲。** 此标记应总是在括号内，并在描述词汇之后跟上重编曲艺术家。
...
- **谱面的第一个[非继承时间点](/wiki/Client/Beatmap_editor/Timing#uninherited-timing-point)不能用于开启 Kiai。** 这样做将导致 Kiai 在出现之前闪烁。此时必须使用与第一个[非继承时间点](/wiki/Client/Beatmap_editor/Timing#uninherited-timing-point)重叠的[继承时间点](/wiki/Client/Beatmap_editor/Timing#inherited-timing-point)来使用 Kiai。
...
- **谱面只应包含一个用于所有难度的音频文件。** osu! 不支持单谱面集中的多个音乐文件，这会导致预览时间点、元数据等出现错误。
- **音频文件和音效必须有合理的质量。** 请尽可能尝试查找可用的最高质量音乐文件，而不是从流视频网站中提取。歌曲音量应被标准化为原始发布版本的音量，并且不应编码成高于原始文件的码率。
- **谱面必须添加音效。** 打击音效能给予玩家反馈，并且多样的音效（口哨、鼓掌和结束音）可以强调歌曲中最重要的部分。
  - **osu!mania 谱面可不添加打击音效。** 这可以使 osu!mania 谱师更轻松地制作谱面，当然，我们也推荐添加打击音效来润色谱面。如果不添加音效，则应注意非继承时间点的使用（除了调整段落默认打击音量）。
- **所有可击打的物件都必须包含能听见的[主动打击音效](/wiki/Beatmapping/Hitsound#active-hitsound)。** osu!mania 谱面除外，因为这个模式的节奏构成与其他模式不同。
- **[主动打击音效](/wiki/Beatmapping/Hitsound#active-hitsound)必须使用 `.wav` 或者 `.ogg` 格式。** `.mp3` 文件有轻微的延迟，因此只能在更长的[被动音效](/wiki/Beatmapping/Hitsound#passive-hitsound)中使用，比如环境噪音。
- **[主动打击音效](/wiki/Beatmapping/Hitsound#active-hitsound)的开头延迟时间不能超过 5 毫秒。** 「延迟」是从音效的峰值时间点向前计算，而不是音效的开始时间点计算。这保证敲击音效可以有效地给歌曲提供几乎同时的反馈。默认皮肤的 `normal-hitfinish.wav` 带有轻微的延迟，但仍可被用作自定义音效。
...
- **[歌曲混编](/wiki/Beatmapping/Song_compilation)应良好地编曲，不含突然的终止或者曲间过长的留白间隔。** 用于混编的歌曲应具有相似的音质，音量与长度。这是为了保证混编歌曲谱面与其他普通谱面类似，有着良好、整体的游戏体验。
- **剪辑应保留原曲的印象与强度。** 对原曲结构造成重大改变的剪辑（如去掉或重新排序歌曲的前奏、段落、副歌、尾声）可能带有误导性，并经常降低游戏体验。官方剪辑及其再创作除外。
- **[游戏音效](/wiki/Skinning/Sounds#gameplay) 中除了[主动音效](/wiki/Beatmapping/Hitsound#active-hitsound) 都应该使用 `.mp3` 或 `.ogg` 格式。** 这些文件通常在相同体积下具有更长时间，而 `*.wav` 文件会不必要地过大。但是当文件较小时， `.wav` 也可使用。
...
- **激活时间（Active）：** 在对象的命令中，从第一个开始到最后一个结束的时间。
...
- **在歌曲结束后不应有仍被激活的精灵图和命令。** 为了契合故事板的特定效果，精灵图可以持续到歌曲结束后的额外数秒，但不应超过一定长度。
- **考虑在被旋转的精灵图的故事板图片周围设置 1px 的透明边框。** osu! 没有图像的抗锯齿算法，因此，如果边缘可见的精灵图被旋转，锯齿或边框将会非常明显。
- **尽量避免任何明显的性能问题。即使被优化过，具有一致的帧速率对于打图体验也是至关重要的。** 在摸图时测试谱面来确认这一点。
- **避免使用故事板音效，在打图过程中容易与敲击音效混淆。** 这违背了听觉反馈这一概念，因为故事板音效将独立于玩家的操作自动播放。
...
- **请尽可能优化故事板，** 无论如何都要做到这一点。
  - **避免精灵图或谱面背景在渲染时完全遮挡不可见。** 出于性能考虑，当元素不可见时，最好将它们隐藏。如要隐藏谱面的背景，请将相同的背景图像作为精灵图，`Background` 或 `0` 作为第二个参数，然后隐藏掉它。
  - **避免精灵图在整个渲染过程中离开屏幕或者阻碍视线。** 在这种情况下，应该裁剪图像的相应部分，除非它们对故事板中的效果来说是必要的。
  - **避免故事板图像周围不必要的透明。** 为了提高性能，应在保留所需效果下尽可能多地裁剪图像。
  - **对于多次使用的命令，使用循环指令，除非这样做会影响视觉效果。** 使用 Loop 指令通常会大量减少代码行数，从而减少文件大小。
  ...
  - **避免同时激活多个不可见的精灵图。** 无论皮肤元素是否可见，激活的元素仍然会处理命令。如果这种元素不可见情况的持续时间较长，请创建一个新的元素在其重新出现时启用。
  - **在一个精灵图上使用相同类型的多个指令时，请使它们的开始时间之间至少保留 16 毫秒。** 每秒 60 个命令足以使精灵图平滑过渡，且可以减小文件大小，减少加载时间。
  - **由触发器激活的精灵图要在使用后隐藏。** 触发器将从其第一个可能的激活命令开始，保持活动状态直到谱面结束，所以最好在完成后将其隐藏。
