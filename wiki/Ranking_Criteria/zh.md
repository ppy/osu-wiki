...
- **谱面内所有难度的歌曲元数据必须一致。**
- **必须在谱面的标签中添加[客串难度](/wiki/Beatmap/Guest_difficulty)作者名称、故事板作者名称、音效作者名称。** 这是为了表彰他们的贡献，能让其他人了解该谱面集的主要贡献者。用户名中包含由空格分隔的单个字符时，必须将空格替换为下划线`_`。
...
- **如果歌曲来自电子游戏、电影、系列、公开活动等等，则必须填写 Source （来源）字段。** 这也适用于歌曲在发布后，某些媒体使用或者冠名，但最初的来源为人所知的情况。如果歌曲有多个可用来源，任意来源都可使用。
  ...
  - 为避免[歌曲汇编](/wiki/Beatmapping/Song_compilation)、混音、歌曲串烧等等歌曲包含的原曲有多个来源，这些原曲的来源都应放在标签( `Tags`) 字段内，而并非来源( `Source`) 字段。（除非此歌曲与某个来源高度相关，则应把这个高度相关来源填写至 `Source` 字段。）
...
- **如果艺术家或标题字段因字符长度限制而被缩短，略去的信息应补入标签字段中。**
- **如果该歌曲已经过[精选艺术家](/wiki/Featured_Artists)授权，则必须在 Tags 中加入 `featured artist` 。**
...
- **[歌曲风格与语言](/wiki/Beatmap/Genre_and_language)也应加入 Tags 字段中。** 这是为了让玩家能在游戏内也能像在网站上一样搜索到它。使用 `instrumental` 作为语言标记纯音乐类歌曲。且不应在语言或曲风不清晰时使用这个标记。如果歌曲适用于多个语言或曲风，则都应填写。
...
- **谱面必须添加[音效](/wiki/Beatmapping/Hitsound)。** 打击音效能给予玩家反馈，并且多样的音效（口哨、鼓掌和结束音）可以强调歌曲中最重要的部分。
...
- **[游戏音效](/wiki/Skinning/Sounds#gameplay) 中除了[主动音效](/wiki/Beatmapping/Hitsound#active-hitsound) 都应该使用 `.mp3` 或 `.ogg` 格式。** 这些文件通常在相同体积下具有更长时间，而 `.wav` 文件则会太大。但是当音效持续时间短，文件较小时，也可使用 `.wav`。
