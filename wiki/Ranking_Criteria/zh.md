...
- **谱面集内的每个游戏模式都必须有合理的难度分布，这些难度应从适合歌曲长度的最小难度开始。** 每个游戏模式中的难度分布都必须遵守该游戏模式中相应难度所适用的准则。
...
- **此外，谱面集创建者的名字不能出现在难度名称中。**（例如：Mapset Host's Insane）。 有多张相同元数据的谱面或客串谱面所造成的一些命名冲突的情况除外。
