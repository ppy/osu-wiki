...
Ranking Criteria 内列出的规定和准则适用于歌曲为 4/4 拍，约 180 BPM 的谱面。而这个页面将阐述如何根据歌曲的 BPM 来确定某些规则内的限制指标。
...
在大多数情况下，120 BPM 和 240 BPM 都类似于一个转折点，来确定规则内的限制指标是否应翻倍或减半。当歌曲 BPM 处于这两个值之间时，应按照上述内容调整这两个转折点需使用的节奏类型在谱面内占据的密度。对于以上这条作为例子的 Normal 难度准则来说，尽量避免在低于 120 BPM 时使用 1/4 节奏，也不在高于 240 BPM 时使用 1/2 节奏就可以了。
