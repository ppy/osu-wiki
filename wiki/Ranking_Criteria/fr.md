...
- **Évitez les caractères Unicode non alphanumériques dans le nom d'une difficulté.** Ceux-ci peuvent provoquer des erreurs dans le [système de soumission des beatmaps](/wiki/Submission) et des problèmes pour certains utilisateurs lorsqu'ils apparaissent dans le tchat. 
...
- **Il ne doit pas y avoir de [points de timing non hérités](/wiki/Client/Beatmap_editor/Timing#points-de-timing-non-hérités) supplémentaires dans aucune difficulté.** Ils peuvent accidentellement affecter les pulsations du menu principal, ajouter des sons indésirables au mod Nightcore ou provoquer un décalage du timing. Les utilisations acceptables sont les suivantes :
  - Alignement des battements du [mod Nightcore](/wiki/Game_modifier/Nightcore) avec le début des sections musicales.
...
- **Les objets doivent être snapped à moins de 2 ms de tout tick de la timeline.**
[AiMod](/wiki/Client/Beatmap_editor/AiMod) AiMod signalera ces problèmes, ainsi que de rares faux positifs. Les faux positifs se produisent principalement sur les extrémités des slider ends et les reverse slider, et doivent être vérifiés manuellement ou avec d'autres outils. Les objets d'une section musicale nécessitant des diviseurs de temps non supportés (par exemple 1/11) peuvent soit :
