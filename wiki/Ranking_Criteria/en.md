...
- **If the song is `TV size`, use a `(TV Size)` marker at the end of the current title string.** If there is an existing `TV size` marker in the title, the `(TV Size)` marker would replace it. Additionally, songs with existing `Short Ver` or `Game Ver` markers in their titles must use `(Short Ver.)` or `(Game Ver.)` markers. Songs edited to have higher tempos must add or replace existing relevant markers with `(Sped Up Ver.)`.
...
- **If a series applicable to be used as a source is clustered into multiple sub-series, the most precise label must be prioritised.** If a song has multiple applicable sources however, the main series/franchise may be used as a source.
...
- **A beatmapset's audio file must use the `.mp3` file format and have a constant bit rate no greater than 192kbps.**
...
- **Background images must not exceed a width of 2560 pixels, a height of 1440 pixels, and a filesize of 2.5MB.** Images with lower vertical or horizontal resolutions than that of the player's will be upscaled to fit the entire screen.
