...
- **A video's audio track must be removed from the video file.** The audio track in video is not used in osu!, so removing it reduces that file size of the beatmap. This includes videos with muted audio tracks. A guide to removing the audio can be found [here](/wiki/guides/Compressing_files/en.md).
