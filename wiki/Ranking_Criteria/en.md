...
- **A beatmapset's audio file must be no higher than 192kbps.** Variable bit rate songs must average between that range.
...
- **The audio file of a song should not be re-encoded to a bit rate higher than its source file.** This can be tested in [audio spectogram generators](http://spek.cc/), which typically show 128kbps and 192kbps songs with average peak frequencies of 16kHz and 18kHz respectively. Avoid audio files with peak frequencies below 16kHz unless no other file can be found.
