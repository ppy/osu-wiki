...
- **Stack（堆叠）：** 两个或者更多物件放在同一格点，其位置被加上了自动的偏移。
- **Stack leniency（堆叠容忍度）：** 一种用来决定物件完全重叠或者稍微错开的设定。增大数值会使物件完全重叠所需的时间轴跨度同时增加。
- **Overlap（接触）：** 物件之间互相接触，但不完全重合。
- **Perfect overlap（完全重叠）：** 多个物件位于统一格点，且没有被堆叠。
- **Snapping（时间线）：** 时间轴上用于物件摆放的线。
- **Slider head/tail（滑条头/尾）：** 滑条开始和结束的位置。
- **Slider border（滑条边缘）：** 用于描绘滑条路径的可见边框，如果滑条因相互交叉重叠而过于扭曲，会更难看清滑条路径。
- **Slider anchors（滑条锚点）：** 用于固定滑条形状的工具，灰色锚点用于让滑条产生一个弧度，而红色锚点用于结束这里的弧度。也被称作「Nodes」。
- **Slider velocity（滑条速度）：** 滑条的速度决定了在给定的节拍内滑条会经过多长的距离。谱面基础的滑条速度可以在 Timing 界面设置，另外的滑条速度变更可以通过继承时间点（绿线）来完成。
- **Short reversing slider（短折返滑条）：** 1 次或者多折返的短滑条（通常是 1/4 或者 1/8），使玩家可以将光标停留在一个地方来完成折返。
- **Extended slider（延长滑条）：** 尾巴不在音乐特定位置上的滑条，但是它表达延长音，通常的用法是 3/4 拍滑条。
- **Slider tick rate（滑条打点率）：** 音乐中每一拍出现的滑条点的数量。
- **Spinner recovery time（转盘恢复时间）：** 转盘结束到下一个物件开始之间的时间。
- **Time-distance equality（时间 - 间距等比例）：** 物件之间的间距与物件相隔的时间成固定比例。也叫固定间距。
- **Hit object/Note density（物件密度）：** 物件之间的平均时间跨度，用节拍数表示。（比如 1/1 表示一拍，1/2 表示半拍）。
- **Keysounding：** 使用与音乐很相近的音，或者直接用音乐中的音来作为谱面音效。 这种音效的做法通常给玩家的打击反馈感很少。
- **Spaced stream（大间距连打）：** 圆圈间相互不接触的连打。
- **Difficulty spike（难度陡峰）：** 难度突然增加，与谱面的其余部分相比对歌曲的强度有误导性。
...
如果此 Normal 难度是谱面集的*最低难度*，除上述准则外，其还应遵循这些准则：
