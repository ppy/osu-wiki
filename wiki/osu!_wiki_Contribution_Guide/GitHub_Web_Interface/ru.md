...
*Эта статья является продолжением основного руководства.* Если вы собираетесь редактировать несколько статей или файлов, воспользуйтесь [GitHub Desktop](/wiki/owcg/GitHub_Desktop).
...
5. При внесении правок следуйте [критериям оформления статей](/wiki/ASC), насколько это возможно.

6. [Сделайте коммит](#коммит), чтобы сохранить изменения.

### Перемещение файлов

*Внимание: через веб-интерфейс GitHub **не рекомендуется** перемещать несколько файлов, потому что на каждое перемещение будет создан отдельный коммит. Это, в свою очередь, делает историю изменений [менее понятной и наглядной](/wiki/owcg/Common_Issues#ничего-не-значащие-описания-коммитов).*
...
4. Введите местоположение и/или имя файла статьи, которую вы хотите создать. Путь к файлу должен иметь следующий вид: `wiki/`, затем название статьи, затем [код языка](/wiki/ASC#locales) (`ru` для русского), затем `.md`. Пример: `wiki/Legal/Copyright/ru.md`.
...
*Внимание: через веб-интерфейс GitHub **не рекомендуется** удалять несколько файлов, потому что на каждое удаление будет создан отдельный коммит. Это, в свою очередь, делает историю изменений [менее понятной и наглядной](/wiki/owcg/Common_Issues#ничего-не-значащие-описания-коммитов).*
...
7. После этого ваши изменения будут сохранены, и перед вами откроется окно для создания пулл-реквеста. **Ничего в нём не делайте — оно оносится к вашей копии репозитория.**

   ![](img/pull-request-pippi-osu--osu-wiki.jpg "Это окно можно закрыть")

8. Переходите к [публикации изменений](/wiki/owcg#публикация-изменений) в оригинальный репозиторий.
