...
**只需要把用户的名字添加到忽略列表中，即可阻止他们在游戏内给你发消息。**

1. 点击游戏主菜单的 `Options` 按钮。
2. 直接输入 `ignore` 就可以搜索到`屏蔽以下玩家的发言`。
3. 把你希望屏蔽的用户的用户名添加到列表，用空格分隔每个用户名。如果该用户的用户名中有空格，请用下划线 `_` 替代(例如，*The Gatekeeper* 这个名字需要写作 *The\_Gatekeeper*)。

你将不再看到列表中任何用户的公聊发言，也不会收到来自他们的私信。
