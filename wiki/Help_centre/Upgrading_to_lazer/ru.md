...
osu!(lazer) можно скачать [отсюда](https://github.com/ppy/osu#running-osu). В ближайшем будущем на него можно будет переключиться прямо в игре (через настройку `Ветка обновлений`), а также скачать его через основной сайт.
...
Если lazer и stable установлены на один и тот же диск, то при импорте будут использованы [жёсткие ссылки](/wiki/Client/Release_stream/Lazer/File_storage#via-hard-links), которые сэкономят место.
...
#### Мне нравится классический режим, где можно набрать очень много очков
...
Мы постараемся это реализовать и вместе с тем не замедлить разработку других возможностей. Скины заработают, но потом.

#### Как сделать, чтобы в меню тоже использовался курсор из скина?
...
В этом нет смысла: выше какого-то числа FPS разницы уже не видно. Lazer использует новые подходы, которые позволяют достичь быстрого отклика и не требуют высокой частоты отрисовки экрана. Дальше будет только лучше, так как мы планируем добавить ещё пару улучшений.

Lazer опрашивает устройства ввода с частотой 1000 раз в секунду вне зависимости от FPS, поэтому и число FPS ограничено 1000.

Если вы хотите проверить, как это влияет на задержку ввода, а также испытать своё восприятие, запустите тест задержки (latency certifier), открыв в настройках раздел `Отладка`.
...
#### Если устройства ввода опрашиваются с частотой 1000 Hz, как это скажется на моей мыши с частотой 8000 Hz?

Операционная система будет опрашивать устройство намного чаще, но это даст незначительные преимущества и может привести к непредвиденным нагрузкам. Мы рекомендуем ограничить частоту устройств ввода 1000 Hz для поддержки стабильности системы.
...
#### В lazer нет нужной мне функции! / Мне не нравится какое-то изменение / Я нашёл баг, куда пожаловаться?

Скорее всего, мы уже знаем и планируем это исправить — поищите в [списке известных проблем](https://github.com/ppy/osu/issues) и [на странице обсуждений](https://github.com/ppy/osu/discussions). Если там нет ничего, похожего на вашу проблему, смело создавайте [новое обсуждение](https://github.com/ppy/osu/discussions/new).
...
#### Почему lazer так долго в стадии разработки?
...
[^offset-calibration-lazer]: Перед повторным прохождением можно настроить оффсет на основе предыдущей игры.
[^can-disable]: Можно отключить.
[^note-lock]: Он всё ещё есть, но действует в "щадящем" режиме.
[^score-reset-balance]: Рекорды потом могут быть удалены ради сохранения баланса в игре.
...
[^all-files]: Все файлы, имеющиеся в карте.
