...
osu!(lazer)（也称为 osu!lazer ）是游戏的下一个大更新。这凝结了几年以来精心幕后工作的结晶，并重构了游戏。

我们最终的目标，是将这个版本作为一个更新发布，并将完全取代现在的游戏版本 (稳定版)，但在实现之前还有些工作要做。最后，我们将继续支持旧版本，直到玩家们都迁移到新版本。至于何时完全取代，由**玩家**群体决定。

"lazer" 是这个版本的代号。当它成为游戏的主要版本时，最终将删除掉这个代号。为方便起见，本文以下部分将 osu!(lazer) 简称为 "lazer"，osu!(stable) 简称为 "稳定版"。
...
你能在[这里](https://github.com/ppy/osu#running-osu)下载到游戏。在不久的将来，你能从稳定版切换到 lazer（在`更新通道 (更新版本)` 设置里切换），并在 osu! 官网上找到下载链接！
...
目前，谱面、皮肤、分数、游戏回放和收藏夹都可以导入 lazer 。注意，**设置还不能导入**，所以你需要从头开始配置它。
...
也就是说，现在只能从 lazer 导出个人的分数和谱面，并手动导入进稳定版中。
...
不管游戏帧率如何， lazer 的输入都是 1000 HZ，所以游戏的最大帧率也限制为 1000 FPS。
