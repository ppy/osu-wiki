...
*注意：我们仍在调整游戏平衡，开发游戏新机制。目前，**在激光版 (lazer) 上取得的分数不会永久保存**。*
...
下面的表格是激光版**当前版本**与稳定版功能的对比表。注意这是一个不断变化的目标——最终目标是随着时间推移，实现所有玩家关心的功能。
...
目前，谱面、皮肤、分数、游戏回放和收藏夹都可以导入激光版。注意，**设置还不能导入**，所以你需要从头开始配置它。
...
我们认为，游戏帧率没理由在特别高的帧率之上运行。激光版采用了各种新技术，确保在不需要高帧率的情况下实现最低延迟。这将继续改进，并且我们还有一些改进亟待实施。
