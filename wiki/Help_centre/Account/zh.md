...
任何用户都可以通过向[accounts@ppy.sh](mailto:accounts@ppy.sh)发送电子邮件来请求**锁定**他们的账号。该账号的密码将被打乱，其电子邮件地址将被清空，从而将用户锁定在账号之外。用这种方式锁定账号后，有一周的冷却时间，然后用户才能请求将其解锁，以防止滥用。

锁定的账号将保持公开可见，而不会更改其个人资料中的可用信息。

要解锁账号，请使用请求锁定账号中相同的的电子邮件地址向[accounts@ppy.sh](mailto:accounts@ppy.sh)发送电子邮件，并表达您解锁账号的意图。

此功能不可滥用。对于那些不想离开osu的玩家来说，锁定账号应该是最后的手段之一！永远，但**需要**专门的时间远离 osu！出于正当理由。

您可以通过将您的电子邮件和密码更改为您永远不会记住的随机单词或字母并退出账号来将自己锁定在您的帐户之外。这与锁定基本上是相同的过程。准备好重返社区后，请发送电子邮件至[accounts@ppy.sh](mailto:accounts@ppy.sh)请求重置密码。确保您使用您的帐户之前使用的电子邮件地址。
