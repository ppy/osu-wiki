...
Hai problemi con qualcosa? Siamo qui per aiutarti! Guarda le soluzioni ai problemi comuni nella barra al lato sinistro del tuo schermo. Se il tuo problema non è nella lista, o se hai ancora problemi, manda una mail a [support@ppy.sh](mailto:support@ppy.sh). Oppure, puoi chiedere aiuto al [Forum di Supporto](https://osu.ppy.sh/community/forums/5), dove qualcuno saprà aiutarti.
...
| [Beatmap e Editor](/wiki/Help_centre/Beatmapping) | Gestione delle beatmap, possesso delle beatmap, slot delle beatmap |
...
| [Aggiornare a Lazer](/wiki/Help_centre/Upgrading_to_lazer) | Passaggio alla [nuova versione principale](/wiki/Client/Release_stream/Lazer) di osu! |
...
8. Seleziona il file di log richiesto (il membro del team di supporto ti dirà quale), e aggiungilo al ticket di supporto o sul post del forum.

### Visualizzatore Eventi {id=event-viewer}

**Visualizzatore Eventi è un componente di Windows che può essere usato per trovare dei crash log quando osu! non ne mostra alcuno. Questi crash log possono essere usati per determinare le cause del problema.**
...
1. Dopo il crash di osu!, premi `Win` + `R` per aprire la finestra Esegui.
2. Nella finestra Esegui digita `eventvwr` e premi `Invio`. Questo aprirà il Visualizzatore Eventi.
...
8. Copia il testo da lì e incollalo nel ticket di supporto o sul post del forum.

### Frame Time Graph {id=frame-time-graph}

**Il Frame Time Graph è una funzione di osu! che può aiutare a ottenere più informazioni sui problemi di performance che potresti avere.**
...
6. Chiudi il Frame Time Graph nello stesso modo in cui l'hai aperto, premendo `Ctrl` + `F11`.
