...
### Tôi đã thay đổi cài đặt của mình, và bây giờ tôi không thể bắt đầu osu! hoặc bị treo {id=incorrect-settings}
...
#### Bạn có thiết lập trình duyệt web mặc định chưa? {id=default-browser}

Trong đại đa số các trường hợp, hầu hết các bản cài đặt Windows đều đã thiết lập trình duyệt web mặc định.

Để kiểm tra, hãy thử các bước sau (dưới Windows 8 trở lên):

1. Mở menu Start.
2. Nhập `default app settings` vào thanh tìm kiếm trong menu Start và nhấp vào cài đặt hiển thị.
3. Cuộn xuống thẻ tiêu đề `Web browser` và kiểm tra xem trình duyệt mà bạn đã cài đặt được thiết lập đúng không.
...
Bạn cũng có thể thử ép buộc trò chơi tự cập nhật.
...
1. Mở osu!
2. Nhấn vào nút `options` trên menu chính, hoặc ấn `Ctrl` + `O`
...
3. Nhập `update`  vào ô tìm kiếm nhanh để đưa thẳng đến tùy chọn.
4. Nhấn vào nút `chạy cập nhật osu!`

Bạn cũng có thể ép buộc trò chơi cập nhật bằng cách tạo một tệp `help.txt` trong thư mục cài đặt của osu!. Tệp này không cần chứa nội dung gì, nó chỉ cần tồn tại, và trò chơi sẽ tự động ép buộc cập nhật trong lần khởi động tiếp theo.
...
Nếu bạn thấy lỗi này khi cố gắng mở osu! hoặc bản cập nhật của nó, điều đó có nghĩa là bạn đã xóa hoặc mất các phông chữ hệ thống quan trọng đi kèm với Windows. Những phông chữ này được sử dụng trên toàn bộ hệ điều hành Windows OS, vì vậy bạn có thể gặp phải các vấn đề khác, như phông chữ sai, thiếu văn bản và các ứng dụng khác bị sự cố trên toàn bộ Windows.

Dưới đây là một bộ sưu tập các phông chữ phổ biến được sử dụng trong các phiên bản Windows khác nhau và trên toàn web, có thể giúp bạn khôi phục các phông chữ bị thiếu:
...
**Hãy chắc chắn rằng bạn không định dạng nhóm các beatmap của mình theo bất kỳ tiêu chí phân loại nào ở góc trên bên phải của màn hình (tức là phần "group" phải được đặt là "không sắp xếp").**

Nếu bạn vừa tải một beatmap, bạn có thể thử làm mới danh sách beatmap của mình bằng cách nhấn `F5` tại màn hình chọn bài.

Hãy chắc chắn rằng bạn không đang giới hạn những beatmap được hiển thị trong tùy chọn trò chơi.

Để kiểm tra hoặc thay đổi những beatmap hiển thị trong menu chọn bài, hãy làm theo các bước sau:

1. Mở osu!.
2. Nhấp vào nút `Options` trên menu chính, hoặc nhấn `Ctrl + O`.
3. Nhập `song select` vào ô tìm kiếm nhanh để đưa thẳng đến tùy chọn.
...
6. Nhấp chuột phải vào tệp đó, sau đó chọn `Rename`.
7. Đổi tên tệp theo ý bạn muốn, tên gì cũng được, miễn là nó không trùng với "osu!", rồi nhấn `Enter`.
8. Khởi động lại osu!.
...
**Thông thường điều này xảy ra do một thiết bị nhập liệu bị lỗi đang kết nối với máy tính. Hãy thử rút bất kỳ tay cầm hay joystick nào ra.**

Những vấn đề này cũng có thể do các ứng dụng giả lập hoặc thay đổi phím tắt, chẳng hạn như Xpadder hoặc X-Mouse Button Control. Nếu bạn đang dùng những ứng dụng này cho game khác, hãy tắt chúng.

Hiện tượng cũng có thể xuất phát từ bàn phím số, vì các phím này có thể được dùng để cuộn danh sách chọn bài. Nhấn phím `NumLock` để tắt bàn phím số và sau đó thử nhấn các phím `9`, `8`, `3` hoặc `2` trên bàn phím số để khắc phục vấn đề.

Bạn cũng nên kiểm tra xem có phím nào trên thiết bị ngoại vi bị kẹt hoặc bị hỏng không
...
Hầu hết các gói beatmap đều có định dạng tệp nén `.rar`, đây là một dạng tệp lưu trữ nén chứa các beatmap. Bạn sẽ cần phải giải nén chúng bằng phần mềm giải nén tệp (chúng tôi khuyên dùng [7-Zip](https://7-zip.org) vào thư mục 'songs' của bạn trước tiên.

Khi tất cả các tệp `.osz` từ gói lưu trữ đã nằm trong thư mục `Songs`, nhấn `F5` ở màn hình chọn bài sẽ làm mới bộ nhớ đệm và tải các beatmap mới vào game.
...
**Việc cài lại client trò chơi sẽ tự động xóa tất cả điểm số các nhân của bạn.**

Rất tiếc rằng những điểm số này đã mất

Những điểm số bạn đã nộp trực tuyến vẫn còn ở đó, tuy nhiên — chỉ đơn giản tải lại bất kỳ bài hát nào bạn đã chơi trước đó, và điểm số của bạn sẽ xuất hiện trở lại.
...
**Chơi ở chế độ game khác (osu!taiko, osu!catch hoặc osu!mania) sẽ làm thay đổi chế độ hiển thị điểm số trong game thành chỉ hiển thị điểm số của chế độ đó. Điều này sẽ ẩn điểm số của bạn ở các chế độ khác.**

Một nguyên nhân phổ biến phát sinh vấn đề này là từ việc chơi beatmap của chế độ game khác, khiến trò chơi tự động đặt thành bài hát đã chọn để sử dụng điểm số của chế độ đó cho đến khi bạn chuyển lại sang chế độ cũ.

Bạn có thể thay đổi lại chế độ game bằng cách nhấp vào nút `Mode` ở góc dưới bên trái màn hình trong màn hình chọn bài, rồi chọn chế độ phù hợp mà bạn muốn xem điểm số của mình.
...
Chơi game hoặc làm việc khác sẽ giúp client xử lý dần trong nền.
...
**Một số chương trình chạy trong nền có thể chiếm tài nguyên hệ thống với osu! và làm cho trải nghiệm chơi không ổn định.**

Hãy thử đóng bớt những chương trình bạn cảm thấy không cần thiết khi đang chơi game/hoặc tắt hết chúng đi.

#### Máy tính của bạn có được bảo mật không? {id=lag-infected-pc}

**Virus, phần mềm gián điệp, chương trình độc hại hoặc các phần mềm không mong muốn khác có thể gây ra các vấn đề hiệu năng nghiêm trọng ngay cả với những máy cấu hình mạnh.**

Giữ cho máy tính của bạn sạch sẽ và an toàn, sau đó bạn có thể nhận thấy hiệu năng được cải thiện. Hãy thử chạy phần mềm quét virus trên máy tính của bạn để phát hiện và loại bỏ những mối nguy hại.

#### Máy tính của bạn bao nhiêu tuổi rồi? {id=lag-old-pc}

Những máy tính rất cũ sử dụng chipset đồ họa tích hợp Intel đời cũ có thể không đủ mạnh để chạy osu!, ngay cả ở mức thiết lập thấp nhất.

Cách duy nhất để giải quyết điều này là nâng cấp máy tính của bạn!

#### Driver của bạn đã được cập nhật không? {id=lag-old-driver}

**Driver cũ có thể gây ra các vấn đề hiệu năng nghiêm trọng. Hãy đảm bảo rằng bạn luôn cập nhật tất cả driver cho các thiết bị của mình bất cứ khi nào có thể.**
...
Một cách đơn giản để kiểm tra là thử rút bộ chia USB ra trong một lúc. Nếu tình trạng lag biến mất, bạn đã tìm ra nguyên nhân rồi đấy!

#### Bạn có đang chạy osu! ở chế độ cửa sổ hoặc chế độ cửa sổ không viền không?  {id=lag-windowed}

**Cả chế độ cửa sổ lẫn chế độ cửa sổ không viền đều gây ra một lượng *độ trễ đầu vào đáng kể*. Toàn màn hình (fullscreen) là cài đặt tối ưu nhất để có trải nghiệm chơi game mượt mà.**

#### Trò chơi của bạn có bị đứng hình trong chốc lát không? {id=lag-background-process}

**Đôi khi các chương trình chạy dưới nền hoặc ứng dụng khác trên máy tính của bạn có thể sử dụng lượng lớn tài nguyên cùng lúc, khiến máy tính của bạn khó xử lý kịp.**

Thông thường vấn đề này sẽ biến mất sau khi bạn đóng mọi thứ không cần thiết trước khi chơi osu!. Phần mềm torrent là thủ phạm phổ biến, đặc biệt nếu nó dùng chung ổ đĩa cài osu!.
