...
토너먼트 밴은 플레이어가 어떤 자격으로든 공식적으로 지원되는 토너먼트에 참여하는 것을 금지합니다. 여기에는 대회 플레이 및 스태프 활동과 대회에 관련한 지원이 포함됩니다.
...
일반 항소를 통해 제한이 해제되어 게임에 복귀하는 모든 유저는 [계정 지원팀](/wiki/People/The_Team/Account_support_team)의 재량에 따라 연장이 가능한 [최소 1년의 임시 토너먼트 밴](/wiki/Help_centre/Account_restrictions#reasons)이 적용됩니다.
...
- 위와 같이 우위를 점하기 위해 위해 치트 프로그램 또는 유틸리티(프로그램) 사용
- 여러 계정을 이용한 토너먼트 참가
- 다른 계정을 이용하여 사일런스나 위반 행위를 회피하려고 시도하는 경우
- 공식적으로 지원되는 토너먼트에서의 계정 공유
- 우위를 점하기 위해 비공개 정보에 접근하거나 이에 대해 음모를 꾸미는 행위(다른 팀보다 먼저 맵풀 정보를 입수하는 경우)
- 우위를 점하기 위해 신뢰관계를 남용하는 행위(스케쥴 조작, 부당한 팀 변경, 이유가 없는 특정 플레이어의 참가자 자격 박탈)
...
| 일반적인 항소를 통해 게임에 복귀하는 사용자 | 1-2 년 | 계정 지원팀의 재량에 따라 더 연장될 수 있습니다 |
