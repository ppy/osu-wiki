...
Il existe trois différents types d'accuracy qu'un joueur peut avoir. Le premier étant l'accuracy de la beatmap qui dépend des points (300,100,50...) reçu lors d'un clic sur un objet. L'autre étant l'accuracy totale du joueur qui est pondéré pour permettre aux meilleurs scores d'être mis en avant. Et pour finir, la [pp](/wiki/pp) accuracy qui est dépendant du score envoyé aprés avoir joué une beatmap.
...
Le graphique de performance est un tableau qui affiche les performances du joueur (basé sur sa barre de vie) au cours de la partie. D'autres informations peuvent y être indiquées lorsque le joueur passe son curseur sur le graphique. 
