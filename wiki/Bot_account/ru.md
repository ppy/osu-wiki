...
**Бот-аккаунт** — это учётная запись osu! особого типа, предназначенная для автоматизированного использования. Отличить бота от обычных игроков просто: в его профиле есть белый значок группы `BOT`, а все информативные поля, кроме юзерпейджа (`me!`), скрыты. Аккаунты подобного типа обычно создаются для чат-ботов, которыми можно пользоваться при игре в [мультиплеере](/wiki/Multi) или через систему личных сообщений.

Чтобы завести бот-аккаунт, [прочтите требования](#создание-аккаунта) и отправьте заявку. Не пытайтесь зарегистрировать новый аккаунт самостоятельно: [правила osu!](/wiki/Rules) запрещают иметь несколько обычных учётных записей.

## Преимущества

Главное отличие бот-аккаунтов — более лояльные ограничения на [использование чата](/wiki/Chat_Console), что снижает риск получения [сайленса](/wiki/Glossary#сайленс) при слишком частой отправке сообщений:
...
Ограничения действуют только на личные сообщения и каналы `#multiplayer` и `#spectator`, в остальные каналы писать запрещено.

## Создание аккаунта

Первое время запускайте чат-бота из-под своего аккаунта, не забывая при этом про более строгие ограничения (см. выше). С течением времени, когда ваш сервис наберёт популярность, у службы поддержки можно будет попросить отдельный профиль.

### Минимальные требования

- Работа из-под вашего аккаунта в течение минимум 6 месяцев;
...
Если все требования выше соблюдены, свяжитесь с техподдержкой по адресу [accounts@ppy.sh](mailto:accounts@ppy.sh). Это нужно сделать с почтового адреса, привязанного к вашему аккаунту osu!, со фразой `Bot Account Request` в теме письма.
...
Техподдержка рассмотрит заявку и сообщит о своём решении: это будет либо мотивированный отказ, либо — в случае одобрения — инструкции по использованию бот-аккаунта.
