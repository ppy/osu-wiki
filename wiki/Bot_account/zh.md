...
机器人账号仅能通过[申请流程](#创建机器人帐号))创建，任何试图直接为机器人通过正常流程创建账号的行为都被视为违反[行为准则](/wiki/Rules#社区规定)中关于多账号的规定。
...
该速率限制仅限于私聊、`#multiplayer`、`#spectator`，机器人账号不允许在其他的频道中发言。
...
- 一些个人账号属于该用户组，是由于号主不想使用机器人账号，或并未请求独立的机器人账号，例如 ![][flag_DE] [Tillerino](https://osu.ppy.sh/users/2070907) 和 ![][flag_FR] [ThePooN](https://osu.ppy.sh/users/718454)。该模式现已不再支持，机器人账号必须使用给定的专用账号。
