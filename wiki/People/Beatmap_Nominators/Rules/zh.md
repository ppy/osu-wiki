...
BN 能够改造 osu! 世界。他们能提名谱面，把这些新内容推送到已上架谱面列表内，从而为众人所知；因此，他们始终需要遵循以下这些规则。违反以下任意一条规则都可能获得从 BN 团队中除名的惩罚。若你发现了有 BN 违反规则，可以使用 [BN 管理网站](https://bn.mappersguild.com/reports)直接向 NAT 报告。

## 期望

所有 BN 都应该尽最大努力发扬[《对 BN 的期望》](/wiki/People/Beatmap_Nominators/Expectations)这篇文章中所阐述的核心精神。

若 BN 的所作所为与这些核心精神背道而驰，NAT 能因此除名 BN。

## 行为

- **不要遵守任何未确定的规则或它的修订方案。**
- **时刻遵循 [osu! 社区规则](/wiki/Rules)和[摸图和作图的行为规范](/wiki/Rules/Code_of_conduct_for_modding_and_mapping)。** 作为一名称职的 BN，你应作为社区其他玩家的正面榜样。若违反这些行为规范，则可能收到行为警告，更严重者，免除职务。
- **请勿参与一切与金钱有关的摸图、上架谱面或委托制作谱面活动。** 对于参与[谱面审核流程](/wiki/Beatmap_ranking_procedure)的 osu! 官方团队（BN、NAT 或其他）成员的行为动机和合规性不能有任何疑问。

## 活跃度

BN 活跃度由他们的提名数量决定[^multi-key-mode]。活跃度也用于常规的 BN 评估流程（通常持续 90 天）。

- BN 必须在 90 天的周期内，至少提名 **6** 张谱面。
  - 如果 BN 在周期结束之前未达成活跃度要求，将会收到活跃度警告。
  - 如果 BN 提名谱面的数量低于 **4**，则会被开除。
- 多模式 BN 必须在 90 天的周期内，在每个所属的游戏模式内至少提名 **3** 张谱面。
  - 如果 BN 在周期结束之前未达成某个游戏模式的活跃度要求，将会收到活跃度警告。
  - 如果 BN 在某个游戏模式内提名谱面的数量低于 **2**，则这个游戏模式所属的 BN 身份会被移除。
- 如果收到了活跃度警告，则接下来的 30 天内的活跃度都需要高于要求。
- 如果 1 年内连续收到活跃度警告，则会被开除。

如果 BN 评估已经启动，但 BN 活跃度仍不及要求，他们将会收到 [NAT bot](https://osu.ppy.sh/users/6616586) 的提醒，并给予 **7** 天的宽限期。
...
- **当你认为谱面含有需要解决的问题时，可以在问题难度的摸图讨论页，提出谱面否决帖（可标为问题或者建议）。** 这会阻止谱面进入上架队列。如果需要在讨论中，让谱面保持在过审状态（而不是直接下架），可以使用标为建议的谱面否决贴。否决帖必须清晰标明谱面出现的需要否决的问题，以及终止上架程序的原因。提出否决的玩家必须全程参与由此产生的讨论，并寻求达成一致的方案。
- **不要因谱面含有不符合上架标准 (Unrankble) 的问题而提出否决。** 如果谱面含有此类问题，提名者通常不应提名。同时，谱师如果想上架谱面，则必须修改这类问题，这和支持否决谱面是同样的结果。此时执行重置提名流程即可。
- **请不要提名正处于否决状态下的谱面。** 只有以下两种情况才算消除了谱面的否决状态：提出否决的人，在讨论后得到了令自己满意的结果，或是谱面在做出适当修改后能够接受，主动消除了谱面的否决状态；或是谱师和提出否决的人要求陪审团调解，而陪审团[^new-jury]认为否决不必要或者无效。在重新提名之前，必须消除谱面的否决状态。

## 备注

[^multi-key-mode]: 如果提名的谱面含有多个键位模式，在计算活跃度时，会被算作 2 次提名。
[^new-jury]: “陪审团”指的是相关游戏模式的 BN 与 NAT 成员；若否决与游戏模式无关，则其对应所有 BN 与 NAT 成员。
