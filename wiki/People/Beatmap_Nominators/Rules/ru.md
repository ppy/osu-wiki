...
Если пришло время регулярной проверки, а номинатор ещё не успел номинировать достаточно карт, ему отводится **7** дней на то, чтобы это сделать. Напоминание придёт от ::бот-аккаунта команды NAT::{ user-id=6616586 }.
