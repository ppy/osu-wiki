...
::Echo::{ user-id=431 } a créé un prototype pour le premier test de modding en 2008, mais il n'a pas été utilisé pour les nouveaux candidats aux BN (anciennement connue sous le nom de [Beatmap Approval Team](/wiki/People/Beatmap_Appreciation_Team) (*BAT*)). Les utilisateurs devaient signaler les erreurs d'une beatmap volontairement défectueuse, [Arashi - Kitto Daijoubu](http://up.ppy.sh/files/Arashi%20-%20Kitto%20Daijoubu.rar).

Des énigmes de modding similaires ont été créées pour le plaisir jusqu'en 2015, lorsque le premier test des Beatmap Nominators a été créé pour un usage pratique. Le test comportait une section à choix multiples similaire au test des BN actuel, ainsi qu'un puzzle de modding pratique créé par ::Charles445::{ user-id=85000 }. La beatmap du test de modding comportait 10 difficultés avec de courts segments mappés. Un [niveau de difficulté](/wiki/Beatmap/Difficulty) a été attribué à chaque segment et les problèmes devaient être trouvés dans ce contexte.

Le deuxième test (créé par ::Irreversible::{ user-id=1287964 } et [Mao](https://osu.ppy.sh/users/2204515)), le troisième test (créé par Irreversible, Mao et ::Myxo::{ user-id=2202645 }) et le quatrième test (créé par Irreversible et Mao) ont été conçus comme des extensions complètes d'une beatmap. Ils visaient à donner l'impression de modder une véritable beatmap.
