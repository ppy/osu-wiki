...
- **不能提名含有明显违反上架规则 (RC) 问题的谱面。** [通用上架规则 (RC)](/wiki/Ranking_Criteria) 和附加的游戏模式细分上架规则适用于谱面内所有难度。
...
- **如果不确定谱面某一部分内容是否合理，请不要提名谱面。** 如遇到不熟悉的游戏模式、模棱两可的灰色地带、稍稍违反上架规则的问题、可能不达标的质量问题、非常复杂难以检查的歌曲时间轴时，应询求其他 BN 的帮助，而不是草率地做决定。
...
- **如果被提名过的谱面含有违反上架规则 (Unrankble) 的问题，则必须重置提名。** 不负责提名此谱面的 BN 也可以来帮助重置。
- **在决定重置提名前，你需确保检查的是最新版本的谱面。**

### 下架 (DQ)

- **任何情况下，只有正式 (Full) BN 才能下架谱面。** 见习 (prob) BN 也能像其他普通玩家那样参与下架相关的讨论和报告问题。
- **下架主要针对的是含有违反上架规则 (Unrankble) 问题的谱面，或是谱面拥有者请求下架的谱面。**
...
- **如果在过审 (Qualified) 谱面内检查出明显违反上架规则 (Unrankble) 的问题，则应立即下架整改。**
...
- **不要因谱面含有违反上架规则 (Unrankble) 的问题而提出否决。** 如果谱面含有此类问题，提名者通常不应提名。同时，谱师如果想上架谱面，则必须修改这类问题，这和支持否决谱面是同样的结果。此时执行重置提名流程即可。
