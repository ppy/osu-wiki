...
Êtes vous un [**Beatmap Nominator**](/wiki/People/The_Team/Beatmap_Nominators) *(BN)*, ou voulez-vous en devenir un ? Si la réponse est oui, vous êtes au bon endroit ! Cet article contient toutes les informations globales que vous aurez besoin en tant que nouveau Beatmap Nominator.

## Liens Importants

- Les [règles des Beatmaps Nominators](/wiki/People/The_Team/Beatmap_Nominators/Rules) : Explique à la fois comment entreprendre diverses actions, ainsi que comment ne pas faire les choses - il vous est aussi préférable d'éviter de les briser en toutes situations, donc soyez sûr de les avoir lu attentivement ! Vous pourrez toujours demander à un membre de la [Nomination Assessment Team](/wiki/People/The_Team/Nomination_Assessment_Team) *(NAT)* si vous n'êtes pas sûr à propos de quelque chose et ils se feront une joie de vous aider.
- Le [code de conduite pour le Mapping et le Modding](/wiki/Rules/Code_of_Conduct_for_Modding_and_Mapping) : En tant que BN, vous êtes censés être une figure exemplaire pour la communauté, quelqu'un que les autres admirent et respectent, donc une faute peut vous conduire à une réprimande ou vous faire expulser.
- Les [critères de Classement](/wiki/Ranking_Criteria) : Si vous êtes déjà un BN, alors vous devriez déjà être familier avec ceci. Essentiellement, il sert à expliquer quels aspects du mapping sont ou ne sont pas autorisés dans la Ranked section, ainsi qu'à inclure quelques directives générales que les gens doivent suivre dans la plupart des cas. 
- [Informations sur le Veto pour une Beatmap](/wiki/People/The_Team/Beatmap_Nominators/Beatmap_Veto) : Pour les beatmaps avec des problèmes subjectifs de qualité qui ne violent pas nécessairement les règles ou les directives de classement, mais qui, selon vous, rendent la beatmap inéligible à la Ranked section, le veto de beatmaps permet de retirer la qualification d'une beatmap.
...
## Les Capacités des Beatmap Nominator
...
**Ce bouton vous permet de nominer des maps.** Pour nominer une beatmap, le mapset doit avoir au moins 5 Hype et aucun Problem ou Suggestion stamps non-résolu. Assurez vous que vous êtes satisfait de toutes les difficultés, de même pour les autres modes de jeu, avant d'appuyer ici. Si la beatmap a fait l'objet d'un veto par le passé, vous devez faire attention à ce que le veto soit retiré avant de faire avancer un mapset. Quand une beatmap possède 2 nominations, elle sera qualifiée et entrera dans la file d'attente pour le classement.

### Disqualifier ou réinitialiser la nomination

![disqualify and reset buttons](img/reset_dq.png)

**Cela vous permet de réinitialiser la progression d'une beatmap dans le processus de classement.** Si une beatmap est actuellement nominée ou qualifiée, vous en tant que nominateur avez la responsabilité de réinitialiser la nomination d'une beatmap en postant un problème en utilisant un Problem stamp. Si une beatmap est qualifiée, elle ne peut être disqualifiée que par un Full BN. Ces options sont généralement utilisées quand il y a un problème majeur ou inclassable qui doit être résolu avant d'envisager le classement.

Étapes pour disqualifier une beatmap :

- Assurez vous que vous êtes sur la discussion de la beatmap pour le bon set.
...
Quand une médiation de veto est lancée, un jury choisi au hasard va être sélectionné. Si vous êtes l'un des membres choisi pour une médiation de veto, le NAT va vous envoyer un message avec des informations additionnelles sur comment utiliser le vote sur le [site BN](http://bn.mappersguild.com). Si vous ne pouvez pas participer, informez un membre du NAT afin qu'un autre BN puisse être choisi pour le jury à votre place.

### Assurance de Qualité

Les Full BN peuvent faire une Assurance de Qualité en vérifiant à tout moment les beatmaps qualifiées pour détecter des problèmes. Le comité de l'assurance qualité peut être vu [sur le site BN](https://bn.mappersguild.com/qualityassurance).
