...
SEV 是一套[审核评估团队](/wiki/People/The_Team/Nomination_Assessment_Team) (*NAT*) 使用的系统，用于评估[提名重置](/wiki/Beatmap_ranking_procedure#nomination-resets)与[谱面审核成员](/wiki/People/The_Team/Beatmap_Nominators) (*BN*) 的过错有多少关系，将提名重置透明化，连贯化。这个分数分为两个值，显示为明显性和严重性。它们的取值范围在 0 到 2 之间，以便于此系统易于理解，方便使用。

## 明显性和严重性

*注意：如果为了应用谱面未在过审之前提交的修改内容，而重置了谱面的提名，这个分数始终为 0/0。这套系统并不是用于阻止摸和修改并提升[过审 (Qualified)](/wiki/Beatmap/Category#qualified) 谱面的。*
...
| 1 | 有经验的人可以看出 | 需要一定的知识、经验、注意力来发现。通常工具或普通的玩家都无法察觉，例如时间轴 (Timing) 问题和歌曲元数据 (Metadata) 问题。 |
...
下面是一份详细的列表，列出了提名重置的原因以及各自的 SEV 分数。这些是 osu! 游戏模式中，从 2020年2月 到 2021年4月 之间的所有 SEV 分数统计数据！百分数表示这个问题有多常见。
...
- **1/0:** (14%)
  - 总体上缺失音效
  - 音效制作粗糙。例如每一拍都会出现不必要的拍手、军鼓、镲，或是类似声音
- **1/1:** (6%)
  - 主动点击物件被静音
