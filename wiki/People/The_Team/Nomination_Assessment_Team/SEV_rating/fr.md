...
| 1 | Peut être trouvé avec de l'expérience | Nécessite des connaissances/de l'expérience/de l'attention pour les repérer. Ne peut généralement pas être trouvé par les outils ou les utilisateurs habituels, par exemple pour des questions de timing ou de métadonnées. |
...
**La Sérétité** concerne le degré d'impact du problème sur la jouabilité.
...
Se trouve ci-dessous des exemples de notes SEV et la manière dont elles sont grossièrement interprétées par les évaluateurs :
...
| 1/1 | Il y a eu un problème qui aurait pu être résolu si les BNx avaient été plus attentifs. |
| 1/2 | Souvent, cela signifie que beaucoup de choses ont mal tourné, mais il faut de l'expérience pour les repérer facilement. |
| 2/0 | Un problème flagrant dans les paramètres de la carte, comme les métadonnées, a en quelque sorte été oublié. |
| 2/1 | Un problème flagrant dans la jouabilité de la carte, comme l'absence de hitsounding, a été manqué. |
| 2/2 | Un problème si grave qu'il est difficile de ne pas le repérer, comme de objets concurrents, a été manqué. |
...
Les réinitialisations dues à des problèmes de mapping ont rarement 2 d'évidence, car elles nécessitent une bonne connaissance du mapping/modding pour être identifiées facilement.
...
- **1/1:** (28%)
  - Erreurs de mapping courantes qui ont un impact négatif notable sur la map
    - Des pics de difficulté injustifiés (comme s'ils ne correspondaient pas à la chanson).
    - Rythmes denses/spacing élevés dans des sections calmes
    - Overmapper d'une manière qui est mal introduite/exécutée
    - Mapping d'un grand stream sur plusieurs layers et sons distincts
- **1/2:** (14%)
...
- **0/0:** (20%)
  - Ajustement du point d'aperçu/temps des kiai
  - Ajout d'une ligne rouge pour tenir compte du mod Nightcore
  - Utilisation du double/demi BPM
  - Légère erreur d'offset
    - Pour un timing simple, < 6 ms off
    - Pour un timing complexe, < 10 ms off
- **1/0:** (11%)
  - Mauvaise signature temporelle
- **1/1:** (49%)
  - Mauvais offset
    - Pour un timing simple, ~6-12 ms off
    - Pour un timing complexe, ~10+ ms off
...
Les réinitialisations liées aux fichiers beatmap n'ont presque jamais une gravité supérieure à 0, car elles n'affectent généralement pas la jouabilité. Une exception est l'utilisation de hitsounds dans le storyboard pour remplacer les hitsounds actifs.
...
- **0/0:** (11%)
  ...
  - Fin de sliders légèrement unsnapped que des outils ne peuvent pas détecter
- **1/0:** (21%)
  - Des snappings ratés qui affectent à peine le jeu
    - Fin de sliders légèrement unsnapped que des outils pourraient aider à trouver
...
- **1/2:** (8%)
  - Des snappings ratés qui affectent notablement la jouabilité
    - Des snappings ratés qui provoquent toujours des 100, parfois des 50, ou même des note locks
...
- **1/0:** (14%)
  ...
  - Mauvais hitsounding, par exemple l'utilisation de claps/snares/cymbales injustifiés sur chaque temps ou autre.
- **1/1:** (6%)
