...
NAT 멤버들은 빨간색 닉네임으로 표시되며 [Global Moderation Team](/wiki/People/The_Team/Global_Moderation_Team) (*GMT*)과 같이 웹의 전반적인 권한이 있습니다. 또한 팀 [비트맵 노미네이터](https://github.com/ppy/osu-wiki/blob/master/wiki/People/The_Team/Beatmap_Nominators)(BNs)의 [정식 멤버](https://github.com/ppy/osu-wiki/blob/master/wiki/People/The_Team/Beatmap_Nominators#full-beatmap-nominators)와 같이 비트맵 [지명](/wiki/Beatmap_ranking_procedure#nominations)과 [지명 초기화](/wiki/Beatmap_ranking_procedure#nomination-resets)의 권한을 갖고 있습니다. NAT 멤버들은 BN과 같지만 다른 업무 책임을 지기 때문에, 모든 [BN 규칙](/wiki/People/The_Team/Beatmap_Nominators/Rules)과 활동량을 제외한 [요구사항](/wiki/People/The_Team/Beatmap_Nominators/Expectations)을 따라야 합니다.
...
NAT는 전반적 매핑 업무를 담당하고 있으며 **평가**와 **고문** 중 하나의 자리를 맡아야 합니다. 두 역할에 관해서는 아래에서 설명합니다.
...
- **지명:** 1달에 최소 2개의 비트맵을 지명하는지 체크함으로서 비트맵 노미네이터 및 비트맵 노미네이터에 지원하는 사람이 어떤식으로 활동하는지 확인할 수 있습니다.
  - 모딩 활동과 다른 자잘한 활동은 전반적 활동량을 평가할 때 긍정적으로 작용할 수도 있습니다.

[BN applications](/wiki/People/The_Team/Beatmap_Nominators/Becoming_a_Beatmap_Nominator)을 통한 BN 지원자가 많은 많큼 평가위원들은 고문과 다르게 업무량이 많아 때때로 일을 분담하기 위해 일을 분담하기 위한 새 멤버를 뽑기도 합니다. 따라서 NAT에 가입하고자 하는 BN은 일차적으로 평가 능력을 기준으로 판단하고 대부분의 NAT이 평가위원의 자리를 맡게 됩니다.
...
- **개발:** [Mapset Verifier](https://github.com/Naxesss/MapsetVerifier), [Aiess](https://github.com/Naxesss/Aiess), [the NAT/BN website](https://bn.mappersguild.com/home)와 같이 랭크 과정을 개선하기 위한 프로그램 및 웹 사이트를 개발하고 관리합니다.
- **행동점검:** 유저의 신고를 처리하고 비트맵 노미네이터의 부적절한 행동 징계 및 비트맵 콘텐츠를 리뷰합니다.
...
활동하지 않는 것으로 판결된 구성원은 다른 NAT와 대조하여 조만간 다시 활동할 수 없는 경우 권한을 박탈합니다. 다른 매핑 관련 프로젝트에서 작업하는 평가위원은 작업중인 내용이 고문 하위 그룹에 더 적절할 경우 부서를 옮길 수 있습니다.

## NAT 승격

NAT가 되려면 먼저 정식 비트맵 노미네이터가 됐거나, 커뮤니티에서 활동하던 예전 NAT 멤버여야 합니다. 초기 NAT 후보는 대부분 매핑/모딩 커뮤니티를 지원하기 위한 노력을 바탕으로 선발하고, 일반적으로 얼마나 다양한 NAT과 관련된 직무에 기여할 수 있는지를 평가하여 승격시킵니다.

모든 신규 NAT는 평가위원으로 시작하기 때문에 NAT 후보들은 다른 구성원의 숙련도를 평가하는 데 탁월한 역량을 발휘하는 것이 중요합니다. Full BN은 [모의 평가](/wiki/People/The_Team/Nomination_Assessment_Team/Evaluations#mock-evaluations)에 참여하거나 임시로 평가팀에 합류하여 연습할 수 있는 기회가 주어질 수 있습니다. NAT 후보자의 평가가 철저하면서 NAT과 유사한 결론에 도달하는 경우(혹은 지원하는 다른 이유가 있는 경우) 승격의 가능성이 높습니다.

NAT는 오랜 시간에 걸쳐서 잠재적 NAT 후보를 찾아보고 BN 평가 방식과 유사하게 NAT 후보 승격을 결정하기 위해 회의를 소집할 수 있습니다. 또한 BN이 직접 NAT 승격을 고려하여 자기 역량에 대해 질문하는 것도 가능합니다. 그러나 현재 NAT 구성원의 활동량 및 기술 역량에 따라 새 NAT를 필요로 하지 않을 수 있습니다. 새 NAT 구성원은 현재 구성원 중 하나의 활동이 줄어들거나 더 많은 구성원이 필요할 만큼 작업량이 부담될 경우에만 승격될 수 있습니다.

### 고문이 되는 과정

앞서 서술한 자문 작업에 능숙함을 보여 현재 또는 향후 운영에 기여하는 데 필수 불가결하고 평가위원으로서의 필요성을 훨씬 능가한다면 고문이 될 수 있습니다. 해당 사항은 팀 내에서 이를 수행할 수 있는 지정된 구성원이 판단하며, 지금은 NAT 멤버들이 기술 고문에 관하여 질문할 수 있는 ::{ flag=US }:: [UberFazz](https://osu.ppy.sh/users/8646059)가 처리합니다.

새 고문을 추가하기 전에 이의가 제기될 경우를 대비해 나머지 NAT과 함께 추가 이의 제기가 온당한지 확인합니다. 문제가 없을 경우 새 고문은 특정 상황에 따라 업무를 받게 됩니다. 기여도가 부진하거나 이후 불필요하다고 판단되면 지정된 구성원이 다시 판단하여 평가위원으로 전환됩니다.

## 팀 멤버

*참고: 언급되지 않는 한 모든 NAT은 모두 아래 나열된 언어 외에 영어를 구사할 수 있습니다.

[Nomination Assessment Team 그룹 페이지](https://osu.ppy.sh/groups/7) 본 페이지에서 모든 팀 멤버를 열람할 수 있습니다. 아래에 배정된 NAT 멤버들의 그룹 역할 외에도 모든 구성원은 [비트맵의 오디오 및 시각적 컨텐츠](/wiki/Rules/Visual_Content_Considerations#getting-your-image-assessed)를 검토하는 데 참여합니다.
...
| ::{ flag=HK }:: [Faputa](https://osu.ppy.sh/users/845733) | 광둥어, 중국어 | 평가자 |
