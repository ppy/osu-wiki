...
토너먼트가 종료된 이후, 배지 상품을 요청하면 토너먼트 위원회에서 무작위 멤버 2명을 해당 토너먼트를 검토하도록 배정합니다. 이들 중에 해당 토너먼트와 관련하여 이해 관계가 충돌할 여지가 있는 경우, 해당 멤버는 이를 명시하며 다른 멤버가 대신 배정될 수 있도록 해야 합니다. 아래의 내용은 무조건 검토하게 됩니다:
