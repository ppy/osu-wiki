---
tags:
  - GMT
  - moderator
  - moderators
  - chat mod
outdated_translation: true
outdated_since: 2a73fa15eba9ab6f0c0fbe547d71933628aedc64
---

# Equipe de Moderação Global

A **Equipe de Moderação Global** (anteriormente conhecido como o Global Mod Team, Administrators, ou GreenBAT), referidos frequentemente como **GMT**, são membros da comunidade que são focados primariamente em moderação de jogadores e problemas pertinentes a equipe e a comunidade em geral.

**Todos os Moderadores Globais estão em juramento a manter o [Código de Conduta do Contribuidor](/wiki/Rules/Contributor_code_of_conduct) junto das [Regras da Comunidade](/wiki/Rules).**

Se você encontrar qualquer membro da Equipe de Moderação Global agindo inapropriadamente (ou agindo em violação ao Código de Conduta do Contribuidor), por favor entre em contato conosco em [abuse@ppy.sh](mailto:abuse@ppy.sh) imediatamente com o máximo de informação o possível. Para mais detalhes, veja [Denunciando Abuso](/wiki/Reporting_bad_behaviour/Abuse).

## Funções e responsibilidades

A Equipe de Moderação Global é responsável pelo bem-estar do chat e fórum, e toma conta de interações dentro da comunidade. Eles devem ser consultados com quaisquer perguntas pertinentes a moderação do jogo, que incluem (mas não são limitadas á) problemas com o chat (e.g. spam ou comentários impróprios).

## Se tornando um membro da Equipe de Moderação Global

Se tornar um membro da Equipe de Moderação Global requer uma notável quantidade de contribuição. Membros em potencial são geralmente notados pela sua vontade de ajudar os outros usuários, atividade dentro da comunidade, e bom comportamento. A habilidade de falar línguas além de Inglês é bem desejável. Candidatos são discutidos dentro da equipe para determinar a sua adição em potencial como um membro da equipe.

### Como ser considerado

Os fatores mais importantes para consideração são comportamento, atividade, entendimento das regras, e paciência:

1. Demonstrar um bom comportamento sendo educado e tratando os outros usuários com respeito. Manter sua conta livre de quaisquer silenciamentos sérios ou problemas é essencial.
2. Demonstrar atividade dentro do jogo em participação no chat e/ou nos fórums. Denuncie qualquer mau comportamento e problemas via o sistema de report. Responda perguntas e ajude os outros com problemas.
3. Tenha um bom conhecimento [das regras](/wiki/Rules) e como elas são aplicadas. É importante saber o que é aceitável no osu! e o que não é.
4. Seja paciente. O GMT não adiciona espontâneamente membros que vem ajudando por apenas alguns dias. Leva um bom tempo para que o GMT avalie alguém-sua personalidade, no que eles podem ajudar, sua atividade, etc.

Geralmente, a equipe irá observar as pessoas por várias semanas para ter certeza que elas são uma adição digna a equipe. Por favor seja paciente e não exagere. Sinta-se livre para perguntar qualquer um do GMT sobre quaisquer dúvidas sobre regras, denunciar usuários, e qualquer outros problemas.

### Algumas coisas a não se fazer

1. Não pergunte constantemente para o GMT sobre a sua posição como um candidato em potencial. O GMT não procura adicionar ninguém que está querendo contrubir com uma recompensa em mente. Um pensamento altruísta é a chave.
2. Não denuncie qualquer coisinha no chat. É uma habilidade importante saber o que merece ser denunciado e o que não deve.
3. Não responda a perguntas de outras pessoas sem saber se tal resposta está certa ou não, especialmente nos fórums. Sugestões são bem-vindas, mas dar informação errada pros outros não é útil para ninguém. Se você não sabe a solução de um problema, deixe que outra pessoa responda e aprenda deles.
4. Não exagere nas coisas. A equipe procura por contribuições consistentes, não um valor imenso. Isso é conhecido por desgastar as pessoas fazendo com que percam a paciência e comprometimento. Relaxe!

## Membros da equipe

A [página de grupo da Equipe de Moderação Global](https://osu.ppy.sh/groups/4) lista todos os membros da equipe.

*Nota: Todos os membros do GMT falam Inglês junto das línguas listadas abaixo, a não ser se mencionado ao contrário.*

| Nome | Línguas adicionais | Área de foco |
| :-- | :-- | :-- |
| ::{ flag=DE }:: ::- Felix::{ user=8503985 } | Alemão | Moderação de chat |
| ::{ flag=IT }:: ::- Marco -::{ user=1273955 } | Italiano | Moderação de chat, moderação de fórum |
| ::{ flag=IT }:: ::-kevincela-::{ user=266596 } | Italiano | Moderação da comunidade de mapping/modding |
| ::{ flag=US }:: ::-NoName-::{ user=10400730 } |  | Moderação de chat |
| ::{ flag=MY }:: ::\_Kobii::{ user=6209713 } | Chinês, malaio, cantonês, um pouco de japonês | Moderação da comunidade de mapping/modding |
| ::{ flag=CN }:: ::\_Stan::{ user=1653229 } | Chinês | Moderação da comunidade de mapping/modding, moderação de fórum |
| ::{ flag=CH }:: ::\[ryuu\]::{ user=5698467 } | Russo | Moderação de chat |
| ::{ flag=RS }:: ::0x84f::{ user=7944724 } | Sérvio | Liderança de equipe, moderação geral |
| ::{ flag=US }:: ::abraker::{ user=4635891 } |  | Moderação de fórum |
| ::{ flag=GB }:: ::aceticke::{ user=8838763 } |  | Moderação de chat, moderação da comunidade de mapping/modding, moderação geral |
| ::{ flag=NL }:: ::Albionthegreat::{ user=9853595 } | Holandês, um pouco de alemão | Moderação geral |
| ::{ flag=US }:: ::Atrue::{ user=1758523 } | Chinês | Moderação de chat, moderação de fórum, moderação da comunidade de mapping/modding |
| ::{ flag=CA }:: ::Aurele::{ user=654108 } | Francês | Moderação de chat, moderação de fórum, moderação da comunidade de mapping/modding |
| ::{ flag=CA }:: ::Azer::{ user=2155578 } |  | Administração de torneios |
| ::{ flag=MY }:: ::bibitaru::{ user=4482419 } | Chinês, malaio | Moderação de chat |
| ::{ flag=US }:: ::Blushing::{ user=5927823 } |  | Moderação de chat, moderação de fórum, suporte técnico |
| ::{ flag=US }:: ::ChillierPear::{ user=9501251 } | Sueco, espanhol | Moderação geral, administração de torneios |
| ::{ flag=GB }:: ::chromb::{ user=10238680 } |  | Moderação de chat |
| ::{ flag=KR }:: ::Civil oath::{ user=3216107 } | Coreano, japonês | Moderação de chat |
| ::{ flag=FR }:: ::Corne2Plum3::{ user=15646039 } | Francês | Moderação de fórum |
| ::{ flag=CA }:: ::D I O::{ user=3958619 } | Um pouco de espanhol | Administração de torneios |
| ::{ flag=BR }:: ::DizzyH::{ user=9896172 } | Português | Moderação de chat |
| ::{ flag=US }:: ::Death::{ user=3242450 } |  | Moderação de fórum, suporte técnico |
| ::{ flag=US }:: ::Dntm8kmeeatu::{ user=5428812 } |  | Suporte técnico |
| ::{ flag=AU }:: ::Ephemeral::{ user=102335 } |  | Suporte ao jogador, administração da wiki |
| ::{ flag=FI }:: ::Flutteh::{ user=5042507 } | Finlandês, um pouco de sueco | Moderação de chat |
| ::{ flag=CN }:: ::Fycho::{ user=1876867 } | Chinês | Moderação de chat, moderação da comunidade de mapping/modding |
| ::{ flag=PL }:: ::Galkan::{ user=169570 } | Polonês | Moderação de chat |
| ::{ flag=FR }:: ::Ganondorf::{ user=10660738 } | Francês | Moderação de chat |
| ::{ flag=US }:: ::Halfslashed::{ user=4598899 } |  | Moderação da comunidade de mapping/modding |
| ::{ flag=DE }:: ::Hopes::{ user=4907504 } | Alemão, turco | Moderação de chat |
| ::{ flag=ID }:: ::Ilham::{ user=3057154 } | Indonésio | Moderação da comunidade de mapping/modding |
| ::{ flag=FR }:: ::Imakuri::{ user=6100837 } | Francês, espanhol | Moderação de chat |
| ::{ flag=BR }:: ::Jaguar::{ user=3660954 } | Português | Moderação de chat |
| ::{ flag=HK }:: ::kanpakyin::{ user=394326 } | Cantonês, chinês, japonês | Moderação de chat |
| ::{ flag=RU }:: ::Kobold84::{ user=3227533 } | Russo | Liderança de equipe, moderação de chat, moderação de fórum |
| ::{ flag=RU }:: ::Kudou Chitose::{ user=9936528 } | Russo | Moderação de chat |
| ::{ flag=FI }:: ::Laurakko::{ user=7253731 } | Finlandês, um pouco de sueco | Moderação de chat |
| ::{ flag=BR }:: ::LeoFLT::{ user=3668779 } | Português, espanhol | Moderação de chat, administração de torneios |
| ::{ flag=VE }:: ::Leon Brigido::{ user=13570072 } | Espanhol | Moderação de chat |
| ::{ flag=DE }:: ::Loctav::{ user=71366 } | Alemão | Moderação da comunidade de mapping/modding |
| ::{ flag=GB }:: ::Log Off Now::{ user=4378277 } |  | Moderação de chat, moderação da comunidade de mapping/modding |
| ::{ flag=DE }:: ::Lyawi::{ user=5851253 } | Alemão | Moderação de chat, moderação de fórum |
| ::{ flag=GB }:: ::mangomizer::{ user=1893718 } | Cantonês, chinês | Administração de torneios |
| ::{ flag=BR }:: ::maot::{ user=3914271 } | Português | Moderação da comunidade de mapping/modding |
| ::{ flag=NO }:: ::MillhioreF::{ user=941094 } |  | Desenvolvimento do osu! |
| ::{ flag=VN }:: ::My Angel Chino::{ user=20547597 } | Vietnamita | Moderação de chat |
| ::{ flag=PH }:: ::Nathanael::{ user=2295078 } | Filipino | Moderação de chat, moderação de fórum, suporte técnico |
| ::{ flag=US }:: ::Nevo::{ user=7451883 } |  | Moderação da comunidade de mapping/modding |
| ::{ flag=ID }:: ::Niva::{ user=197805 } | Indonésio | Moderação de chat |
| ::{ flag=FR }:: ::Nozhomi::{ user=2716981 } | Francês | Moderação de chat |
| ::{ flag=DE }:: ::OnosakiHito::{ user=290128 } | Alemão, sérvio | Moderação de chat, moderação da comunidade de mapping/modding |
| ::{ flag=AU }:: ::osu!team::{ user=4341397 } |  | Presença oficial da equipe |
| ::{ flag=FR }:: ::Pachiru::{ user=2850983 } | Francês | Moderação de chat, moderação de fórum |
| ::{ flag=HK }:: ::Petal::{ user=7354729 } | Cantonês, chinês | Moderação geral |
| ::{ flag=KR }:: ::Petit::{ user=4637369 } | Coreano, japonês | Moderação de chat |
| ::{ flag=MY }:: ::QHideaki13::{ user=733998 } | Malaio | Moderação de chat |
| ::{ flag=PL }:: ::Redo\1::{ user=7122165 } | Polonês | Moderação da comunidade de skinning |
| ::{ flag=MX }:: ::Repflez::{ user=201392 } | Espanhol | Moderação de chat |
| ::{ flag=FI }:: ::Riverism::{ user=4980738 } | Finlandês, um pouco de sueco, um pouco de japonês | Moderação da comunidade de mapping/modding, moderação de fórum |
| ::{ flag=NL }:: ::Roan::{ user=8214639 } | Holandês, japonês | Moderação da comunidade de skinning |
| ::{ flag=DE }:: ::RockRoller::{ user=8388854 } | Alemão | Moderação da comunidade de skinning, moderação de fórum |
| ::{ flag=JP }:: ::S o h::{ user=2234772 } | Japonês | Moderação de chat |
| ::{ flag=SE }:: ::Saten::{ user=444506 } | Sueco, espanhol | Moderação de chat |
| ::{ flag=FR }:: ::Shiro::{ user=113005 } | Francês, espanhol | Moderação de chat |
| ::{ flag=ID }:: ::Shurelia::{ user=3807986 } | Indonésio | Moderação de chat |
| ::{ flag=ID }:: ::Sies::{ user=6491991 } | Indonésio, um pouco de vietnamita | Moderação de chat, moderação de fórum |
| ::{ flag=TW }:: ::spboxer3::{ user=197974 } | Chinês | Moderação de chat |
| ::{ flag=KR }:: ::Spectator::{ user=702598 } | Coreano | Moderação da comunidade de mapping/modding |
| ::{ flag=AT }:: ::Stefan::{ user=626907 } | Alemão, sérvio | Moderação de fórum |
| ::{ flag=FI }:: ::terho::{ user=6090105 } | Finlandês | Moderação de chat |
| ::{ flag=NZ }:: ::THAT\1otaku::{ user=11798717 } |  | Moderação de fórum, moderação de chat |
| ::{ flag=PE }:: ::Tiburon::{ user=18115022 } | Espanhol | Moderação de chat |
| ::{ flag=CH }:: ::TicClick::{ user=672931 } | Russo | Moderação de chat, administração da wiki |
| ::{ flag=PH }:: ::topecnz::{ user=2103927 } | Filipino | Moderação de chat |
| ::{ flag=TH }:: ::Trigonoculus::{ user=7627013 } | Tailandês | Moderação de chat |
| ::{ flag=ES }:: ::Trosk-::{ user=3469385 } | Espanhol | Suporte técnico |
| ::{ flag=PL }:: ::Venix::{ user=5999631 } | Polonês | Liderança de equipe, moderação de chat, moderação da comunidade de mapping/modding |
| ::{ flag=IT }:: ::VoidGod::{ user=4724656 } | Italiano | Moderação de chat |
| ::{ flag=ID }:: ::wowcake::{ user=16121851 } | Indonésio | Moderação de chat |
| ::{ flag=PL }:: ::Yason::{ user=2574392 } | Polonês | Moderação de chat |
| ::{ flag=HM }:: ::Zallius::{ user=55 } |  | Moderação de chat |
| ::{ flag=CL }:: ::ZiRoX::{ user=200768 } | Espanhol | Moderação da comunidade de mapping/modding |

### Agrupado por línguas moderadas

| Canal | Subfórum | Moderadores |
| :-- | :-- | :-- |
| `#arabic` |  |  |
| `#bulgarian` |  |  |
| `#cantonese` |  | ::{ flag=HK }:: ::kanpakyin::{ user=394326 }, ::{ flag=HK }:: ::Petal::{ user=7354729 } |
| `#chinese` | [中文](https://osu.ppy.sh/community/forums/25) | ::{ flag=CN }:: ::\_Stan::{ user=1653229 }, ::{ flag=US }:: ::Atrue::{ user=1758523 }, ::{ flag=MY }:: ::bibitaru::{ user=4482419 }, ::{ flag=CN }:: ::Fycho::{ user=1876867 }, ::{ flag=HK }:: ::kanpakyin::{ user=394326 }, ::{ flag=TW }:: ::spboxer3::{ user=197974 } |
| `#czechoslovak` |  |  |
| `#dutch` | [Nederlands](https://osu.ppy.sh/community/forums/69) | ::{ flag=NL }:: ::Albionthegreat::{ user=9853595 } |
| `#english` |  | *Todos os moderadores* |
| `#filipino` | [Tagalog](https://osu.ppy.sh/community/forums/76) | ::{ flag=PH }:: ::Nathanael::{ user=2295078 }, ::{ flag=PH }:: ::topecnz::{ user=2103927 } |
| `#finnish` | [Suomi](https://osu.ppy.sh/community/forums/24) | ::{ flag=FI }:: ::Flutteh::{ user=5042507 },  ::{ flag=FI }:: ::Laurakko::{ user=7253731 }, ::{ flag=FI }:: ::Riverism::{ user=4980738 },  ::{ flag=FI }:: ::terho::{ user=6090105 } |
| `#french` | [Français](https://osu.ppy.sh/community/forums/34) | ::{ flag=CA }:: ::Aurele::{ user=654108 }, ::{ flag=FR }:: ::Corne2Plum3::{ user=15646039 }, ::{ flag=FR }:: ::Ganondorf::{ user=10660738 }, ::{ flag=FR }:: ::Imakuri::{ user=6100837 }, ::{ flag=FR }:: ::Nozhomi::{ user=2716981 }, ::{ flag=FR }:: ::Pachiru::{ user=2850983 }, ::{ flag=FR }:: ::Shiro::{ user=113005 } |
| `#german` | [Deutsch](https://osu.ppy.sh/community/forums/37) | ::{ flag=DE }:: ::- Felix::{ user=8503985 }, ::{ flag=DE }:: ::Hopes::{ user=4907504 }, ::{ flag=DE }:: ::Lyawi::{ user=5851253 }, ::{ flag=DE }:: ::OnosakiHito::{ user=290128 }, ::{ flag=DE }:: ::RockRoller::{ user=8388854 }, ::{ flag=AT }:: ::Stefan::{ user=626907 } |
| `#greek` |  |  |
| `#hebrew` |  |  |
| `#hungarian` | [Magyar](https://osu.ppy.sh/community/forums/95) |  |
| `#indonesian` | [Indonesian](https://osu.ppy.sh/community/forums/73) | ::{ flag=ID }:: ::Niva::{ user=197805 }, ::{ flag=ID }:: ::Shurelia::{ user=3807986 }, ::{ flag=ID }:: ::Sies::{ user=6491991 }, ::{ flag=ID }:: ::wowcake::{ user=16121851 } |
| `#italian` | [Italiano](https://osu.ppy.sh/community/forums/36) | ::{ flag=IT }:: ::- Marco -::{ user=1273955 }, ::{ flag=IT }:: ::VoidGod::{ user=4724656 } |
| `#japanese` | [日本語](https://osu.ppy.sh/community/forums/32) | ::{ flag=KR }:: ::Civil oath::{ user=3216107 }, ::{ flag=HK }:: ::kanpakyin::{ user=394326 }, ::{ flag=KR }:: ::Petit::{ user=4637369 }, ::{ flag=JP }:: ::S o h::{ user=2234772 } |
| `#korean` | [한국어](https://osu.ppy.sh/community/forums/58) | ::{ flag=KR }:: ::Civil oath::{ user=3216107 }, ::{ flag=KR }:: ::Petit::{ user=4637369 } |
| `#malaysian` | [Malaysian](https://osu.ppy.sh/community/forums/94) | ::{ flag=MY }:: ::bibitaru::{ user=4482419 }, ::{ flag=MY }:: ::QHideaki13::{ user=733998 } |
| `#polish` | [Polski](https://osu.ppy.sh/community/forums/26) | ::{ flag=PL }:: ::Galkan::{ user=169570 }, ::{ flag=PL }:: ::Venix::{ user=5999631 }, ::{ flag=PL }:: ::Yason::{ user=2574392 } |
| `#portuguese` | [Português](https://osu.ppy.sh/community/forums/74) | ::{ flag=BR }:: ::DizzyH::{ user=9896172 }, ::{ flag=BR }:: ::Jaguar::{ user=3660954 }, ::{ flag=BR }:: ::LeoFLT::{ user=3668779 } |
| `#romanian` |  |  |
| `#russian` | [Русский](https://osu.ppy.sh/community/forums/35) | ::{ flag=CH }:: ::\[ryuu\]::{ user=5698467 }, ::{ flag=RU }:: ::Kobold84::{ user=3227533 }, ::{ flag=RU }:: ::Kudou Chitose::{ user=9936528 }, ::{ flag=CH }:: ::TicClick::{ user=672931 } |
| `#skandinavian` | [Skandinavien](https://osu.ppy.sh/community/forums/77) | ::{ flag=US }:: ::ChillierPear::{ user=9501251 }, ::{ flag=SE }:: ::Saten::{ user=444506 } |
| `#spanish` | [Español](https://osu.ppy.sh/community/forums/33) | ::{ flag=BR }:: ::LeoFLT::{ user=3668779 }, ::{ flag=VE }:: ::Leon Brigido::{ user=13570072 }, ::{ flag=MX }:: ::Repflez::{ user=201392 }, ::{ flag=PE }:: ::Tiburon::{ user=18115022 } |
| `#thai` | [ภาษาไทย](https://osu.ppy.sh/community/forums/54) | ::{ flag=TH }:: ::Trigonoculus::{ user=7627013 } |
| `#turkish` | [Türkçe](https://osu.ppy.sh/community/forums/93) | ::{ flag=DE }:: ::Hopes::{ user=4907504 } |
| `#ukrainian` |  |  |
| `#vietnamese` |  | ::{ flag=VN }:: ::My Angel Chino::{ user=20547597 }, ::{ flag=ID }:: ::Sies::{ user=6491991 } |
