...
Le *slider velocity* est un nombre qui décrit la distance parcourue par un [slider](/wiki/Hit_object/Slider) en un battement de mesure. Une vitesse de base peut être assignée par l'intermédiaire de l'onglet `Timing` dans l'[éditeur de beatmap](/wiki/Beatmap_Editor) de la fenêtre principale. Un multiplicateur de cette valeur peut ensuite être défini pour les différentes sections d'une beatmap en ajoutant des [inheriting timing points](/wiki/Beatmap_Editor/Timing) et en modifiant la valeur du multiplicateur du slider velocity.
