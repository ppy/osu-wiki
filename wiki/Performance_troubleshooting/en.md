...
- Are you using a hybrid graphics setup (such as NVIDIA Optimus)? If that is the case, try running the game on only one of them. For running the game only on the dedicated GPU, you can use a MUX switch if available, or you can connect the computer to an external display.
