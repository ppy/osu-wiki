...
- Are you using NVIDIA Optimus or any other hybrid graphics setup? If that is the case, try running the game on only one of them. Your integrated GPU might already be capable of running osu! at a sufficient frame rate. For running the game only on the dedicated GPU, you can use a MUX switch if available, or you can connect the computer to an external display.
