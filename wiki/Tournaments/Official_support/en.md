...
- The tournament must only use beatmaps of songs that comply with [content usage permissions](/wiki/Rules/Content_usage_permissions#artist-permissions). However, **"Ranked," "Approved," and "Loved" beatmaps are explicitly allowed** even if the track goes against these permissions. Any usage of tracks in other categories, such as Graveyard or Pending, which do not follow the content usage permissions, will revoke a tournament's eligibility for support. See the [ranking criteria](/wiki/Ranking_criteria#rules) for details regarding exceptions for artists or tracks which are listed as disallowed.
