...
Every staff and player must respect each other and follow the [osu! community rules](/wiki/Rules), no discussion about sensitive topics (example: religion, politics) and anything that could be offensive to others. This is a friendly 1v1 4k national tournament and not the place for such discussions. If a player breaks any of the osu! community rules, the player will be punished. Tournament staff have the right to punish players that show repeated and excessive toxic behavior toward any staff member, player, or spectator either in match multiplayer chatroom, the official discord server, or the official twitch channel chatroom.
...
2. Protecting & Banning:
   ...
   - A player **cannot** protect 2 maps of the same type (ex: player 1 protects hb1, now player 1 cannot protect any of the remaining hybrids), this rule applies to each person individually.
3. During the protect/ban/pick phases each player will be given a 2 minute time to choose their maps, if players end up taking too long, the ref will roll a number corresponding to the number of maps in the mappool (minus tb, example the mappool has 11 maps so the ref will !roll 10), to randomly choose a map for them. This is to ensure that a match doesn't taking too long as it does not benefit anyone.
4. Each player will receive one "timeout" of two minutes per match, to be used as extra time to pick a beatmap or to rest. The timeout may only be used by the player whose is picking the next map, the only exception being during tiebreaker scenarios where both players are given a two minute timeout regardless.
5. Players cannot pick or use a provocative warmup to ensure that the livestream is family friendly to watch. Players are not required to have a warm up map to be played, but if they do they must send their warmups to the referee at least 5 minutes before the match begins, but the earlier the better.
6. Duration of warm up maps must be under 4 minutes, but there is no star limit.
...
5. If a player is not cooperative with the referee/intentionally stalling the match, they will be warned by the referee. If the problem persist the host has the power to end the match and decide the winner on the spot.
