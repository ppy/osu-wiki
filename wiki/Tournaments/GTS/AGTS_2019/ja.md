...
**Advanced Global Taiko Showdown 2019** (***AGTS 2019***)は開催者![][flag_FR] [Kasumii-sama](https://osu.ppy.sh/users/6177263)と![][flag_FR] [TLQ\_Yoshii](https://osu.ppy.sh/users/7157133)による2vs2ダブルエリミネーション方式国別対抗戦である。Global Taiko Showdownシリーズ第三回である。
...
| ポジション | メンバー |
| :-- | :-- |
| マネージメント | ![][flag_FR] [Kasumii-sama](https://osu.ppy.sh/users/6177263), ![][flag_FR] [TLQ\_Yoshii](https://osu.ppy.sh/users/7157133) |
...
5. 試合は TeamVs と ScoreV2 のロビーで行います。 ロビーの名前は : "IGTS 2019: (赤チーム) vs (青チーム)"。
6. ウォームアップはチーム一曲ずつ（四分以下）行います。ウォームアップ中は全プレイヤーがロビーに入ってても構いません。
7. 各チームのキャプテンが１譜面ずつバンします。(Tiebreaker以外の譜面). バンされた譜面はこの試合中選択不可能です。
8. 各チームのキャプテンが `!roll`コマンドを使ってロールします。
9. **ロールで勝った方のチームがバンが二番目でピックが一番目となります。負けた方のチームはバンが一番目でピックが二番目となります。**
10. 譜面のピックは交代します。
11. キャプテンは制限なしでどのブラケットからでも譜面を選べます。
12. 選手の入れ替わりは制限なしです。
13. 各譜面開始前にプレイヤーの回線が切断し交代できるプレイヤーがいない場合試合は最大10分延期可能です。
14. 譜面のプレイ中にプレイヤーの回線が切断した場合、クリア失敗として扱われます。
15. 譜面の最初の30秒以内にデ回線が切断した場合、譜面をやり直すことが可能です。ただし各チーム最大一回までしか許されません。
16. クリア失敗したプレイヤーのスコアはチームのスコアに加算されません。
17. 譜面のスコアが同点で終わった場合、この点は無効になり譜面を再度プレイします。
...
4. 3 vs 3 のバトルロワイアルマッチが2つ行われます。譜面は12譜面(2 NM,HR,HD,DT,FM,EX)のMappoolからランダムで選択されます。(EX = 難しい譜面)
5. 以下のルールを使った4 vs 4 の試合も行われます：
   - Mappoolは 5 NM,HR,HD,DT,FM,EXと1 TB。(Modのルールは通常の試合と同じです)
   - 譜面バンはなし。
   - Modは!rollでテーブルに合わせて選ばれ、譜面はランダムで選ばれます。
   - 選手の入れ替わりは制限なしです。
