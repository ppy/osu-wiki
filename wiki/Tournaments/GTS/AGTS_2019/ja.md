...
6. ウォームアップはチーム一曲ずつ（四分以下）行います。ウォームアップ中は全プレイヤーがロビーに入っていても構いません。
7. 各チームのキャプテンが1ビートマップずつBANします。(Tiebreaker以外のビートマップ). BANされたビートマップはこの試合中選択不可能です。
8. 各チームのキャプテンが `!roll`コマンドを使ってロールします。
9. **ロールで勝った方のチームがBANが二番目でピックが一番目となります。負けた方のチームはBANが一番目でピックが二番目となります。**
10. ビートマップのピックは交代します。
