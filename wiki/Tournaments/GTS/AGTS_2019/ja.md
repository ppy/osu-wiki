...
6. 以下の場合以外はスタッフにより選ばれたチームキャプテンがチームのロスターを決めます：
   ...
   2. 各国の参加者が6人以上の場合、二つチームを作れるようロスターを分ける必要があります。できるだけ早くチームのメンバーを決定し、キャプテンを決め、試合スケジュールの調整などの準備をすることを推奨します。
...
4. チームは**4チームのグループ6個**に分けられます。日曜日8月11日に結果が発表されます。
...
1. 予選、グループステージ、準々決勝、準決勝、準決勝とグランドファイナルにはそれぞれ一つのマッププールが用意されます。
...
10. それぞれのステージのマッププールは一週間前にショーケースされます。
...
7. 各チームのキャプテンが1ビートマップずつバンします。(Tiebreaker以外のビートマップ). バンされたビートマップはこの試合中選択不可能です。
...
3. 投票数トップ2のプレイヤーがキャプテンになり、投票数1位のプレイヤーから残る10人のプレイヤーからチームのメンバーを選びます。チームメンバーの選択はA->B->B->A->A->B->B->A->A->Bの順番で選択されます
