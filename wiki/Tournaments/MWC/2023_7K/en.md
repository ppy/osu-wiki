...
8. The seeding method used for Qualifiers will be average rank, where each team's map rank will be multiplied by a predetermined weight and then averaged (added togheter, and subsequently divided by the amount of maps in the mappool) to construct that team's final score, which is then sorted from lowest to highest, lowest being the best placement.
