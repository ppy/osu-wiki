...
2. The provisional captain will then form their team by selecting players from the candidate pool for their country.
   ...
   - **Teams that do not not send their team list by January 4, 12:00 UTC will be disqualified from the competition.**
...
2. Each captain must use `!roll` once in the match's chatroom. After the rolls are set, the pick and ban phase will have the following steps:
   ...
   2. The roll loser of bans one beatmap, followed by the a ban from roll winner.
   3. The roll winner of the starts picking the first beatmap of the match.
3. After bans are decided, both teams will take turns in picking a beatmap from the mappool. After a specific amount of maps is picked (dependent on the tournament stage), captains will once again use `!roll`, after which the roll winner will ban one beatmap first, followed by the roll loser banning one beatmap. The beatmap to start this procedure varies by stage as follows:
   - Round of 32 and Round of 16: After the fourth map is played 
   - Quarterfinals and Semifinals: After the fifth map is played
   - Finals and Grand Finals: After the sixt map is played
...
2. Matches in Qualifiers will be held during specific time slots proposed by the tournament managers. Each team may choose a time slot in which they want to play.
   - **A team that does not pick a time slot by January 8, 23:59 UTC will be considered disqualified.**
