...
It's extremely common to have some [actions](Storyboard_Scripting_Commands "wikilink") in [Storyboarding](Storyboarding "wikilink") repeat themselves a fixed number of times. For instance, you may want to have an object to fade in and out constantly during a certain playtime. In this case, it would be best to use a **loop**.
...
In addition to the "implicit" player feedback via the separate [Pass/Fail layers](Storyboard_Scripting_General_Rules#Layers "wikilink"), you can use one of several **Trigger conditions** to cause a series of [events](Storyboard_Scripting_Commands "wikilink") to happen whenever that condition is fulfilled within a certain time period. The official specification calls these "trigger loops" due to their syntactic similarity to Loops (L), but they aren't loops at all, so here they are simply called "Triggers".
...
-   <triggerType> indicates the trigger condition and can be one of the following:
    -   HitSound\[SampleSet\]\[AdditionsSampleSet\]\[Addition\]\[CustomSampleSet\], where:
        ...
        -   All of these are optional, examples:
            -   HitSound (any [hitsound](Hitsounds "wikilink") is played)
