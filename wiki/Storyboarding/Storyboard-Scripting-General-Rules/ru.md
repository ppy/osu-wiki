Эта страница и [остальные части справки](Template:SBScripting/RU) описывают код скрипта, расположенный в секции \[Events\]. Если скрипт вписан в файл с расширением .osb, он будет работать во всех сложностях; если в файл .osu, то только для данной сложности.
...
-   Под "[объектом](RU:Storyboard_Objects)" понимается экземпляр спрайта или анимации в вашей сториборде. Также за объект можно принять звуковой файл (см. [соответствующую статью](RU:Storyboard_Audio)).
...
-   Координаты курсора отображаются в [редакторе](RU:Beatmap_Editor) на вкладке "Design".
...
-   Состояние во время игры (т.е. когда игроку нужно нажимать на ноты, чтобы предотвратить уменьшение здоровья):
    ...
    -   В [Catch the Beat](RU:Catch_the_Beat) текущее состояние всегда равно состоянию предыдущей части (например, первая игровая секция всегда имеет состояние успеха)
    -   В [тайко](RU:Taiko) текущее состояние — неудача, если игрок пропустил последнюю ноту, иначе — успех.
