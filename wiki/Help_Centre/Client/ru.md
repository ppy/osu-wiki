...
**Обычно это можно исправить, сбросив настройки на значения по умолчанию.**
...
Это не сработает, если вы поменяли клавишу, скрывающую таблицу рекордов. Чтобы увидеть её, откройте настройки, найдите кнопку `Назначение клавиш`, затем — раздел `In-Game`, и посмотрите, какая клавиша стоит напротив `Toggle Scoreboard`. Используйте её вместо `Tab` (например, не `Shift` + `Tab`, а `Shift` + `V`).
...
По умолчанию дым активируется, когда вы удерживаете клавишу `C`. Её можно переназначить в настройках: найдите в них кнопку `Назначение клавиш`, а затем — раздел `osu!`.
...
В зависимости от конкретных карт, это можно исправить, открыв перед началом игры таблицу мировых рекордов. Если статус сбросился у многих карт (или вообще у всех), попробуйте [удалить базу данных карт](#пропали-карты). У карт должны появиться правильные статусы вскоре после пересоздания базы данных.

#### Сервера osu! могли не работать {#сервера-не-работают}

**У серверов osu! редко бывают проблемы. Так ли это на самом деле, можно проверить на [странице состояния серверов](https://status.ppy.sh) и в твиттере [@osustatus](https://twitter.com/osustatus).**
...
Универсальный оффсет смещает всю музыку и эффекты в osu!, заставляя их звучать чуть раньше или чуть позже. Он зависит от конкретной конфигурации компьютера и его комплектующих.
