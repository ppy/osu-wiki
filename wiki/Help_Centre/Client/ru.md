...
#### Как обнаружить и отключить запуск в режиме совместимости в Windows {#совместимость-с-windows}
...
#### Как проверить, что в системе выбран браузер по умолчанию {#браузер-по-умолчанию}
...
Рекомендуется начать с перезагрузки компьютера. Как ни странно, это помогает против многих проблем, в том числе и с osu!.

Проверьте, что в настройках osu! в списке веток выбрана `стабильная` ветка.
...
**У серверов osu! редко бывают проблемы. Об инцидентах можно узнать на [странице состояния серверов](https://status.ppy.sh) и в твиттере [@osustatus](https://twitter.com/osustatus).**
...
К несчастью, все эти действия *чуть-чуть* увеличивают задержку ввода. Для большинства игроков это будет некритичным.
...
#### Как добавить osu! в исключения Kaspersky Internet Security {#файервол-касперского}
...
### osu! не может подключиться даже после добавления в исключения {#исключения-не-помогли}
...
#### Как провести трассировку с помощью tracert {#трассировка}
...
#### Вместе с osu! запущено очень много программ {#лаги-из-за-программ}
