...
| [Site web](/wiki/Help_Centre/Website) | Blocage des utilisateurs, contact avec le support, apparence du site |
...
**L'Observateur d'événements est un composant intégré de Windows qui peut être utilisé pour trouver des journaux de pannes lorsque osu! ne vous en fournit pas. Ces journaux d'incidents peuvent être utilisés pour aider à déterminer la cause de votre problème.**

Lorsque vous avez des problèmes de plantage d'osu!, si osu! ne vous donne pas un journal de plantage, le seul endroit où vous pouvez en trouver un est l'Observateur d'événements.

Si un membre de l'équipe d'assistance vous a demandé de trouver un journal de panne dans l'Observateur d'événements, voici comment procéder :

1. Après que osu! ait planté, appuyez sur `Win` + `R` pour ouvrir une fenêtre Exécuter.
...
8. Copiez le texte qui s'y trouve et collez-le dans votre ticket d'assistance ou votre message de forum.

### Frame Time Graph

**La Frame Time Graph est une fonctionnalité d'osu! qui peut nous aider à recueillir des informations plus utiles sur les problèmes de performance que vous pouvez rencontrer.**

Lorsque vous êtes confrontés à des problèmes de performance dans osu!, la Frame Time Graph est un bon outil pour nous aider à diagnostiquer le problème et vous aider à trouver une solution.

Si un membre de l'équipe d'assistance vous a demandé de prendre une vidéo ou une capture d'écran avec la Frame Time Graph, voici comment procéder :
...
4. Faites une capture d'écran en appuyant sur `Shift` + `F12`. Ceci téléchargera la capture d'écran sur les serveurs d'osu! et ouvrira la capture d'écran dans un onglet du navigateur web.
5. Copiez et collez le lien fourni dans votre ticket d'assistance ou votre message de forum.
