...
**玩家账号的受限状态是指暂时或永久被限制与社区的互动，包括聊天讨论、加入多人游戏、在排行榜上显示自己的成绩。**

如果你的账号受到限制，你可以和支持团队进行一次简短的对话来了解其中的原因。

如果我们已经确定了你的账号有违规游戏（比如作弊）或者其他不良行为的情况，我们将要求你等待从这个事件发生起的三个月，然后我们才会考虑你的解除限制请求，让你正常游戏。

请参考*[账号限制](/wiki/Help_Centre/Account_Restrictions)*一节来了解账号限制和申请恢复步骤的详细信息。
...
在新版网页中，在[谱面列表](https://osu.ppy.sh/beatmapsets)页面点击 “更多搜索选项”，然后将 “玩过” 一栏选为 “玩过” 即可激活这个筛选条件。你也可以[点击这里](https://osu.ppy.sh/beatmapsets?played=played)快速跳转到应用了该筛选条件的页面。
...
大多数 Wacom 数位板，可以在 `Wacom 数位板属性` 程序中的 `映射`标签页中修改映射。

![Wacom 数位板设定中的 “映射” 选项](img/Tabarea.png "用于修改屏幕映射的界面")
...
如果你想屏蔽被某个玩家圈到、某个玩家的私信和公共频道发送的消息，只需要在玩家的名字后面相应地加上 `@h`、`@p` 或 `@c` 即可。例如，`The_Gatekeeper@c` 将会屏蔽这个玩家在公共频道发送的所有消息。这些附加条件也可以叠加，`The_Gatekeeper@ph` 将会屏蔽该玩家圈到你的消息以及私信你的消息，但是该玩家在公频发送的消息依然可见。
...
<!-- 根据英文文章，这一片段需要重写以符合 osu!wiki 的标准。因为 "Foes" 列表在新版网页中已经去掉。
...
前往你的[账户设置](https://osu.ppy.sh/home/account/edit)，在隐私政策一节中勾选 `屏蔽来自陌生人的私信` 。来自好友的私信将继续发送提醒。
