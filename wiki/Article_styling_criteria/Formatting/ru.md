...
::: alert-notice
**Внимание**
osu! wiki показывает версию статьи на языке, выбранном в настройках сайта. Если перевода не существует, будет показана английская версия.
:::
...
::: alert-notice
**Внимание**
старайтесь добавлять этот тег переводам, которые нужно доработать. По возможности не переводите статьи, у которых уже есть этот тег.
:::
...
::: alert-notice
**Внимание**
по возможности не переводите статьи с этим тегом. Если тег есть у оригинала, он должен быть и у перевода.
:::
...
Эти теги добавляются автоматически после принятия пулл-реквеста с изменениями, их не надо расставлять вручную. Автоматическое добавление можно [выборочно отключить перед принятием пулл-реквеста](/wiki/osu!_wiki/Maintenance#устаревшие-переводы), если какой-то перевод на самом деле не устарел.
...
::: alert-notice
**Внимание**
этот тег ставят администраторы osu! wiki перед тем, как влить изменения.
:::
...
::: alert-notice
**Внимание**
если файлы Markdown используют `CRLF` для перевода строки, при загрузке через GitHub это поведение сохранится.
:::
...
```markdown
* ::Red\_Pixel::{ user=4170932 } is playing [kors k - Playing With Fire (Sota Fujimori Remix) (xlni) \[Pyrotechnics\]](https://osu.ppy.sh/beatmapsets/202252#osu/478605) (osu!)
```
...
```markdown
::: alert-note
**Примечание:** Подробнее о {каком-то явлении} {ссылка на статью}
:::
```
...
::: alert-note
**Примечание:** Эта статья — о различных игровых механиках. О поиске недочётов в карте см. [Моддинг](/wiki/Modding).
:::
```
...
```markdown
::: alert-note
**Примечание:** У этого термина существуют и другие значения, см. {название явления} (значения).
:::
```

## Примечание

Примечание содержит дополнительную информацию, которую необходимо обозначить отдельно. Оно должно начинаться с одного из следующих слов: `Внимание`, `Предупреждение`. Примечание также должно быть выделено курсивом, но, в отличие от [выноски](#выноски), оно является полным предложением, в конце которого стоит точка или восклицательный знак.
...
::: alert-notice
**Внимание**
эта кнопка работает без подтверждения.
:::

::: alert-warning
**Предупреждение**
[правила osu!](/wiki/Rules) запрещают иметь более одного аккаунта!
:::
```
...
::: alert-notice
**Внимание**
заголовки уровня 4 и 5 не будут показаны в содержании статьи, и на них нельзя сослаться напрямую.
:::
...
::: alert-notice
**Внимание**
если необходимо сослаться на букву алфавита, а не клавишу на клавиатуре, используйте кавычки.
:::
...
::: alert-notice
**Внимание**
форматы файлов (не путать с расширениями) пишутся заглавными буквами и не начинаются с точки.
:::
...
::: alert-notice
**Внимание**
внутренние ссылки ведут на страницы, которые расположены в домене `https://osu.ppy.sh`.
:::
...
```markdown
::: alert-note
**Примечание:** Подробнее о правилах тайминга [Критерии ранкинга § Тайминг](/wiki/Ranking_criteria#тайминг)
:::
```
...
::: alert-notice
**Внимание**
внешние ссылки ведут на страницы, не расположенные в домене `https://osu.ppy.sh`.
:::
...
```markdown
::: alert-note
**Примечание:** Подробнее о теории музыки [Теория музыки](https://ru.wikipedia.org/wiki/Теория_музыки)
:::
```
...
::: alert-notice
**Внимание**
расширение файла должно быть записано строчными буквами, иначе изображение не будет показано.
:::
