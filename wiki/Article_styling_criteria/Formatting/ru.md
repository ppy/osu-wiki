...
```markdown
[Camellia - OOPARTS](https://cametek.bandcamp.com/track/parts) выступает примером песни, созданной специально для osu!, поскольку она была записана для тайбрейкера osu! World Cup 2020.
```
...
Текст, заключённый в кавычки, называется подписью к изображению и используется для передачи дополнительной информации. Подпись показывается при наведении курсора на изображение, а также, если изображение самостоятельное, прямо под ним. В подписи нельзя использовать разметку Markdown.

### Отображение

Если параграф состоит только из изображения, оно будет размещено по центру, а под ним окажется текст подписи. Во всех остальных случаях изображение будет вставлено прямо в окружающий его текст, а подпись будет скрыта.

Имеет смысл помещать отдельно стоящие изображения в инфобокс<!-- TODO: link me! -->, чтобы придать статье более опрятный вид и выровнять иллюстрации по правому краю экрана:
...
*Примечание: полный список флагов можно найти в [issue \#328](https://github.com/ppy/osu-wiki/issues/328).*

Названия иконок флагов состоят из двухбуквенного кода страны по [ISO 3166-2](https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2), записанного заглавными буквами, и расширения `.gif`. При добавлении флага используйте следующий формат:
...
Применяйте цитирование только по прямому назначению, не используя его для оформления текста. Согласно правилам русского языка, перед именем автора цитаты не нужно ставить никаких дополнительных знаков препинания:
