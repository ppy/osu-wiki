...
- Внешние ссылки;
- Теги статей;
- Всё, что относится только к части сообщества, говорящей на языке перевода.
...
*Примечание: по возможности не переводите статьи с этим тегом. Кроме этого, старайтесь добавлять его переводам, которые нужно доработать.*

К статьям, которые нужно переоформить, дополнить или переписать, необходимо добавлять пометку `needs_cleanup`:

```yaml
needs_cleanup: true
```

Старайтесь оставить в теле статьи [комментарии](#комментарии) с пояснением, что именно требуется исправить, чтобы пометку можно было убрать. Кроме того, просьбу о доработке можно оформить в виде issue на GitHub.
...
В отличие от пометок в метаданных, ключевые слова относятся к содержанию статьи и описывают, по каким словам и фразам её можно найти через поиск по вики. Список таких слов должен быть написан **на языке статьи** и включать в себя ключевые слова оригинала.
...
Названия статей пишутся с заглавной буквы и описывают предмет или явление в единственном числе (см. [Википедия:Именование статей/Единственное число](https://ru.wikipedia.org/wiki/Википедия:Именование_статей/Единственное_число)). Кроме того, с заглавной буквы должно писаться только первое слово, а также имена собственные.
...
Каждая директория должна содержать в себе хотя бы один текстовый файл со статьёй. Когда в одной директории находится несколько поддиректорий с текстовыми файлами, нужна индексная статья. Она перечисляет вложенные статьи и, например, даёт к ним краткое описание.
...
Выноска — это маленькое примечание, которое ссылается на похожие или связанные статьи. Выноски должны быть выделены курсивом и располагаться в начале статьи или раздела, кроме того, их нельзя заканчивать точкой.
...
Разновидность выноски «[См. также](#см.-также)», которая содержит ссылку на подробную статью (или несколько), раскрывающую текущий раздел с другой стороны. Она обычно начинается со слов `Подробнее о`. Пример:
...
Кроме того, полужирным можно выделить название статьи в самом первом параграфе, где даётся его краткая расшифровка.

### Курсив

Чтобы применить курсив, используйте одинарные звёздочки (`*`). Курсивом выделяются названия игр и произведений (например, песен или изображений), кроме osu!, а также первое упоминание аббревиатуры или сокращения.

Помимо перечисленного, курсив служит для выделения логического ударения при письме.
