...
*สังเกต: บทความนี้ใช้ [RFC 2119](https://tools.ietf.org/html/rfc2119 "IETF Tools") เพื่ออธิบายระดับความต้องการ*

## ภาษา

นี้คือลิส สำหรับการแปลบทความ:

| ชื่อไฟล์ | ชื่อภาษา | ภาษาพื่นเมือง |
| :-- | :-- | :-- |
| `en.md` | อังกฤษ | English |
| `ar.md` | อาราบิก | اَلْعَرَبِيَّةُ |
...
การแปลขึ้นอยู่กับความเท่าเทียมกันของเนื้อหาที่เข้มงวดกับบทความภาษาอังกฤษ ในแง่ที่ว่าพวกเขาต้องมีข้อความเดียวกัน โดยไม่คำนึงถึงไวยากรณ์และไวยากรณ์ การเปลี่ยนแปลงความหมายของการแปลจะต้องมาพร้อมกับการเปลี่ยนแปลงที่เทียบเท่ากับบทความภาษาอังกฤษ
...
ต้องวางวัตถุด้านหน้าไว้ที่ด้านบนสุดของไฟล์ มันเขียนใน [YAML](https://en.wikipedia.org/wiki/YAML#Example "YAML Wikipedia article") และอธิบายข้อมูลเพิ่มเติมเกี่ยวกับบทความ ต้องล้อมรอบด้วยขีดกลางสามตัว (`---`) ในบรรทัดด้านบนและด้านล่าง และบรรทัดว่างจะต้องอยู่ตามหลังก่อนส่วนหัวของชื่อ

### บทความที่ต้องการความช่วยเหลือ

*หมายเหตุ: หลีกเลี่ยงการแปลบทความภาษาอังกฤษที่มีแท็กนี้ นอกจากนี้ ควรเพิ่มแท็กนี้เมื่อการแปลจำเป็นต้องท need_cleanup*

แท็ก `needs_cleanup` อาจถูกเพิ่มลงในบทความที่ต้องการความช่วยเหลือในการเขียนใหม่หรือการจัดรูปแบบ นอกจากนี้ยังเป็นที่ยอมรับในการเปิดปัญหาบน [Github](https://github.com/) เพื่อจุดประสงค์นี้ แท็กนี้จะต้องเขียนตามที่แสดงด้านล่าง:
...
*หมายเหตุ: ผู้ดูแล Wiki จะกำหนดและใช้เครื่องหมายนี้ก่อนที่จะรวมเข้าด้วยกัน*

บางครั้ง การแปลจะถูกเพิ่มลงในวิกิโดยไม่ได้รับการตรวจสอบจากเจ้าของภาษาคนอื่นๆ ในกรณีนี้ เครื่องหมาย `no_native_review` จะถูกเพิ่มเพื่อให้นักแปลในอนาคตทราบว่าอาจต้องตรวจสอบอีกครั้ง แท็กนี้จะต้องเขียนตามที่แสดงด้านล่าง:
...
*ดูเพิ่มเติมที่: [ชื่อโฟลเดอร์](#folder-names) และ [ชื่อ](#titles)*

ชื่อบทความควรเป็นเอกพจน์และใช้ตัวพิมพ์ประโยค ดู [บทความอนุสัญญาการตั้งชื่อวิกิพีเดีย](https://en.wikipedia.org/wiki/Wikipedia:Naming_conventions_(plurals) "Wikipedia") สำหรับรายละเอียดเพิ่มเติม
...
ชื่อโฟลเดอร์ต้องใช้อักขระเหล่านี้**เท่านั้น**:
...
[แก้ความกำกวม](/wiki/Disambiguation) บทความต้องอยู่ในโฟลเดอร์ `/wiki/Disambiguation` ต้องอัปเดตหน้าหลักเพื่อรวมบทความแก้ความกำกวม อ้างถึง [Disambiguation/Mod](/wiki/Disambiguation/Mod) เป็นตัวอย่าง
...
บันทึกย่อ ต้องเป็นตัวเอียงและวางทันทีหลังจากหัวข้อ หากมีการใช้ บันทึกย่อ หลายอัน ต้องอยู่ในย่อหน้าเดียวกันโดยคั่นด้วยการขึ้นบรรทัดใหม่

### หน้าหลัก

บันทึกย่อ *หน้าหลัก* นำผู้อ่านไปยังบทความหลักของหัวข้อ เมื่อใช้ บันทึกย่อ นี้ แสดงว่าส่วนที่อยู่บนนั้นเป็นบทสรุปของหน้าที่เชื่อมโยงนั้นเกี่ยวกับอะไร บันทึกย่อ นี้ควรมีลิงก์เดียวเท่านั้น ต้องจัดรูปแบบดังนี้:
...
*สำหรับ {description} ดู: {article} และ {article}*
```
### เพื่อไม่ให้สับสนกับ
...
ชื่องานหรือวิดีโอเกมควรเป็นตัวเอียง osu!—เกม—ได้รับการยกเว้นจากสิ่งนี้
...
*See also: [การตั้งชื่อบทความ](#การตั้งชื่อบทความ)*

*ข้อควรระวัง: ชื่อจะถูกแยกวิเคราะห์เป็นข้อความธรรมดา พวกเขาจะต้องไม่หลบหนี*
...
หัวเรื่องต้องอยู่ในบรรทัดแรก ยกเว้นกรณีที่มีการใช้ [front matter](#front-matter) หากเป็นกรณีนี้ หัวเรื่องจะต้องอยู่ข้างหลังและมีบรรทัดว่างก่อนหัวเรื่อง

### ส่วน

ส่วนหัวของส่วนต้องใช้ระดับ 2 ถึง 5 ส่วนหัวของหัวข้อที่ดำเนินการกับ [ส่วนหัวของชื่อ] (# ชื่อเรื่อง) ต้องเป็นส่วนหัวระดับ 2 หัวเรื่องอาจมีไอคอนรูปภาพขนาดเล็กต่างจากชื่อเรื่อง
...
## ไอเดียสำหรับการแข่งขันแบบผู้เล่นหลายคน {id=идеи-для-мультиплеера}<!-- alternative syntax for Unicode identifiers -->
```
...
ตัวเลขในรายการลำดับเลขจะต้องเพิ่มขึ้นเพื่อแสดงขั้นตอนของพวกเขา
...
3.โอสุ! จะเริ่มทำงานเมื่อการติดตั้งเสร็จสิ้น
4. ลงชื่อเข้าใช้
```
...
*หมายเหตุ: เมื่อใช้แทนตัวอักษร ไม่ใช่แป้นคีย์บอร์ด ให้ใช้เครื่องหมายคำพูดแทน*

เมื่อแสดงแป้นคีย์บอร์ด ให้ใช้อักษรตัวพิมพ์ใหญ่สำหรับอักขระตัวเดียวและตัวพิมพ์ชื่อเรื่องสำหรับตัวปรับแต่ง ใช้สัญลักษณ์บวก (`+`) (ไม่มีรหัส) เพื่อแสดงคีย์ผสม (ตัวอย่างที่แสดงด้านล่าง)
...
เมื่อแทนช่องว่างหรือสเปซบาร์ ให้ใช้ `` `Space` ``.

### ข้อความปุ่มและเมนู

เมื่อคัดลอกข้อความจากเมนูหรือปุ่ม ควรคัดลอกตัวพิมพ์เล็กตามที่ปรากฏ (ตัวอย่างที่แสดงด้านล่าง)
...
```markdown
โอสุ! ได้รับการติดตั้งในโฟลเดอร์ `AppData/Local` โดยค่าเริ่มต้น เว้นแต่จะระบุไว้เป็นอย่างอื่นระหว่างการติดตั้ง
```

### คำสำคัญและคำสั่ง

เมื่อคัดลอกคีย์เวิร์ดหรือคำสั่ง ควรคัดลอกตัวพิมพ์ใหญ่เล็กตามที่ปรากฏหรือวิธีที่ผู้อื่นพิมพ์ตามปกติ หากมี ให้เลือกอักษรตัวพิมพ์เล็ก (ตัวอย่างที่แสดงด้านล่าง)
...
เมื่อทำการคัดลอกชื่อไฟล์ ตัวพิมพ์ควรถูกคัดลอกตามที่ปรากฏ หากมี ให้เลือกอักษรตัวพิมพ์เล็ก (ตัวอย่างที่แสดงด้านล่าง)
...
```markdown
`#lobby` คือที่ที่คุณสามารถโฆษณาห้องหลายห้องของคุณได้
```

## ข้อความที่จัดรูปแบบไว้ล่วงหน้า (บล็อกโค้ด)

*ประกาศ: การเน้นไวยากรณ์สำหรับข้อความที่จัดรูปแบบไว้ล่วงหน้ายังไม่ได้ใช้งานบนเว็บไซต์เลย*

ข้อความที่จัดรูปแบบไว้ล่วงหน้า (หรือที่เรียกว่าบล็อคโค้ด) จะต้องล้อมรั้วโดยใช้เครื่องหมายหลุมฝังศพสามอัน พวกเขาควรตั้งค่าตัวระบุภาษาสำหรับการเน้นไวยากรณ์
...
ลิงก์ Wiki ต้องไม่ใช้การเปลี่ยนเส้นทางและต้องไม่มีเครื่องหมายทับ (`/`)
...
```markdown
*ดูเพิ่มเติมที่: [ตัวแก้ไข/การออกแบบ Beatmap](/wiki/Beatmap_Editor/การออกแบบ)*
```
...
ควรคัดลอก URL จากแถบที่อยู่ของเว็บเบราว์เซอร์ของคุณเหมือนกับเมื่อลิงก์ไปยัง osu! หน้าเว็บ. ต้องเก็บส่วน `https://osu.ppy.sh` ของ URL

####### โปรไฟล์ผู้ใช้

ชื่อผู้ใช้ทั้งหมดจะต้องเชื่อมโยงในครั้งแรก เหตุการณ์อื่นๆ เป็นทางเลือก แต่จะต้องสอดคล้องตลอดทั้งบทความสำหรับชื่อผู้ใช้ทั้งหมด หากระบุ ID ผู้ใช้ได้ยาก อาจถูกข้ามไป

เมื่อเชื่อมโยงไปยังโปรไฟล์ผู้ใช้ ต้องใช้หมายเลขประจำตัวผู้ใช้ ใช้เว็บไซต์ใหม่ (`https://osu.ppy.sh/users/{username})`) เพื่อรับ ID ของผู้ใช้

ข้อความลิงก์ของลิงก์ผู้ใช้ควรเป็นชื่อปัจจุบันของผู้ใช้
####### ความยาก

เมื่อใดก็ตามที่เชื่อมโยงไปยังปัญหาเดียว ให้ใช้รูปแบบนี้เป็นข้อความลิงก์:

```
{ศิลปิน} - {ชื่อ} ({ผู้สร้าง}) [{ชื่อระดับความยาก}]
```

ลิงค์ต้องเชื่อมโยงกับความยากลำบากนั้นจริง ๆ URL ความยากของ Beatmap ต้องจัดรูปแบบดังนี้:

```
https://osu.ppy.sh/beatmapsets/{BeatmapSetID}#{mode}/{BeatmapID}
```

ชื่อปัญหาอาจอยู่นอกข้อความลิงก์ แต่ต้องสอดคล้องกันตลอดทั้งบทความ
...
ต้องใช้โปรโตคอล "https" เว้นแต่ไซต์จะไม่รองรับ ลิงก์ภายนอกต้องเป็นลิงก์ที่สะอาดและตรงไปยังแหล่งที่เชื่อถือได้ ควรคัดลอก URL จากแถบที่อยู่ของเว็บเบราว์เซอร์เหมือนกับเมื่อลิงก์ไปยังหน้าภายนอกอื่นๆ

ไม่มีความแตกต่างทางสายตาระหว่างภายนอกและ osu! ลิงค์เว็บ ด้วยเหตุนี้ ชื่อเว็บไซต์จึงควรรวมอยู่ในข้อความชื่อ ดูตัวอย่างต่อไปนี้:

```markdown
*ดูข้อมูลเพิ่มเติมเกี่ยวกับทฤษฎีดนตรีได้ที่ [ทฤษฎีดนตรี](https://en.wikipedia.org/wiki/Music_theory "Wikipedia")*
```

## รูปภาพ

ลิงก์รูปภาพมีสองประเภท: แบบอินไลน์และการอ้างอิง ตัวอย่าง:
...
รูปภาพควรใช้รูปแบบ JPG ที่คุณภาพ 8 (80 หรือ 80% ขึ้นอยู่กับโปรแกรม) หากรูปภาพมีความโปร่งใสหรือมีข้อความที่ต้องอ่านได้ ให้ใช้รูปแบบ PNG แทน หากรูปภาพมีแอนิเมชั่น คุณสามารถใช้รูปแบบ GIF ได้ อย่างไรก็ตาม ควรใช้เท่าที่จำเป็น เนื่องจากอาจใช้เวลาในการโหลดนานกว่าหรือใหญ่กว่า [ขนาดไฟล์สูงสุด] (#file-size)
...
ใช้ขีดกลาง (`-`) เมื่อเว้นวรรคคำ เมื่อตั้งชื่อรูปภาพ ชื่อไฟล์ควรมีความหมายหรือสื่อความหมายแต่สั้น
...
1. รูปภาพเป็นของตัวเอง
2. รูปภาพไม่อยู่ในหัวเรื่อง
3. รูปภาพมีข้อความชื่อ

คำบรรยายจะถือว่าผ่านข้อความชื่อซึ่งจะต้องเป็นข้อความธรรมดา รูปภาพพร้อมคำอธิบายภาพจะเน้นที่ภาพบนเว็บไซต์ด้วย
...
ควรหลีกเลี่ยงการใส่คำอธิบายประกอบรูปภาพ เนื่องจากเป็นการยากสำหรับนักแปล (และบรรณาธิการอื่นๆ) ในการแก้ไข

#### กำลังแปลรูปภาพที่มีคำอธิบายประกอบ

เมื่อแปลรูปภาพที่มีคำอธิบายประกอบ เวอร์ชันรูปภาพที่แปลแล้วจะต้องอยู่ในไดเรกทอรีเดียวกันกับเวอร์ชันดั้งเดิม (เช่น เวอร์ชันภาษาอังกฤษ) ชื่อไฟล์ของรูปภาพเวอร์ชันแปลต้องขึ้นต้นด้วยชื่อเวอร์ชันดั้งเดิม ตามด้วยยัติภังค์ ตามด้วยชื่อสถานที่ (เป็นตัวพิมพ์ใหญ่) ดูตัวอย่างต่อไปนี้:
...
ภาพหน้าจอของการเล่นเกมทั้งหมดต้องทำในบิลด์ที่เสถียร เว้นแต่จะเป็นสำหรับฟีเจอร์เฉพาะที่ไม่พร้อมใช้งานในบิลด์ที่เสถียร คุณควรใช้คุณสมบัติภาพหน้าจอในเกม (`F12`)
...
- เลือกภาษา: `English`
- ต้องการข้อมูลเมตาในภาษาต้นฉบับ: `Enabled`
...
ไอคอนแฟล็กข้างลิงก์ผู้ใช้ต้องแยกจากข้อความลิงก์ ดูตัวอย่างต่อไปนี้:

`` มาร์คดาวน์
![][flag_AU] [เร็ว](https://osu.ppy.sh/users/2)
```
...
ไอคอนรูปธงจะใช้รหัสตัวอักษรสองตัว (เป็นตัวพิมพ์ใหญ่ทั้งหมด) และลงท้ายด้วย `.gif` เมื่อเพิ่มแฟล็กอินไลน์ ให้ใช้รูปแบบนี้:

`` มาร์คดาวน์
![](/wiki/shared/flag/xx.gif)
```

โดยที่ `xx` คือ [ISO 3166-2](https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2 "Wikipedia") รหัสประเทศสองตัวอักษรสำหรับธง
...
ต้องไม่ตกแต่งโต๊ะให้สวยงาม (ห้ามปูเซลล์ที่มีช่องว่างพิเศษเพื่อทำให้ความกว้างเท่ากัน) ต้องมีแถบแนวตั้ง (`|`) ที่ด้านซ้ายและด้านขวา และข้อความของแต่ละเซลล์จะต้องมีช่องว่างหนึ่งช่องทั้งสองด้าน เซลล์ว่างต้องใช้แถบแนวตั้ง (`|`) ตามด้วยช่องว่างสองช่อง ตามด้วยแถบแนวตั้งอีกอัน (`|`)
...
ตัวแบ่งเฉพาะเรื่อง (หรือที่เรียกว่ากฎแนวนอนหรือเส้น) ควรใช้เท่าที่จำเป็น การใช้งานบางส่วนอาจรวมถึง (แต่ไม่จำกัดเพียง):
