...
::: alert-note
**备注：** 关于写作标准，请参见[文章风格规范/写作](../Writing)
:::
...
前言（元数据）应置于文章顶部，使用 [YAML](https://zh.wikipedia.org/wiki/YAML#%E7%AF%84%E4%BE%8B) 语言编写，并提供文章的其他有关信息。必须在前言上下两行用三个连字符（`---`）包围，并且在前言之后，必须留一空行。
...
```markdown
::: alert-note
**备注：** 关于 {description}，请参见{article}
:::

::: alert-note
**备注：** 关于 {description}，请参见{article} 和 {article}
:::
```
...
```markdown
::: alert-note
**备注：** 对于更多义项，参见{disambiguation article}。
:::
```
...
提示应放在适当的章节开头中，并使用斜体。如有必要，可使用粗体标注，但应谨慎使用粗体。提示应是一条完整的句子。因此，与大多数[顶注](#顶注)不同，提示需要使用句号 (`.`) 或感叹号 (`!`) 结尾。通知内包含的所有内容必须用斜体表示。格式如下：
