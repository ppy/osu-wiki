...
*다른 의미를 가진 단어는* 햇노트는 *헷갈리지 마세요* 햇노트와 비슷하지만, [동음이의어 문서](#동음이의어-문서)에 직접 연결됩니다. 이 햇노트의 링크는 동음이의어 문서에만 연결되어야 합니다. 다음과 같은 형식으로 사용하세요:

```markdown
*다른 의미를 가진 단어는 {동음이의어 문서}에서 확인할 수 있습니다.*
```
