...
::: alert-notice
**알림**
웹사이트는 유저가 선택한 언어의 문서를 표시합니다. 표시하지 못할 경우에는 영어 문서로 표시됩니다.
:::
...
::: alert-notice
**알림**
이 태그가 있는 영어 문서는 번역하지 마세요. 이 외에도, 번역 자체를 정리해야 할 필요가 있다면 이 태그를 추가하세요.
:::
...
::: alert-notice
**알림**
이 태그가 있는 영어 문서는 번역하지 마세요. 만약 영어 문서가 이 태그를 가지고 있다면, 번역된 문서에도 반드시 이 태그를 추가해야 합니다.
:::
...
::: alert-notice
**알림**
위키 관리자가 병합하기 전 이 마크를 추가할지 결정합니다.
:::
...
::: alert-note
**참고:** 또한 [폴더 이름](#폴더-이름)와 [제목](#제목)도 참조하세요.
:::
...
::: alert-note
**참고:** 또한 [문서 명명 규칙](#문서-명명-규칙)도 참조하세요.
:::
...
::: alert-caution
**주의**
GitHub을 이용해 `CRLF`(캐리지 리턴 및 라인 피드)를 사용한 Markdown 파일을 업로드하는 경우 해당 파일이 `CRLF`를 사용하게 됩니다. 이를 막기 위해, 업로드하기 전 라인의 끝을 `LF`(라인 피드)로 설정해 주세요.
:::
...
```markdown
::: alert-note
**참고:** {내용}이라면 {문서}를 참조하세요.
:::

::: alert-note
**참고:** {내용}이라면 {문서} 및 {문서}를 참조하세요.
:::
```
...
```markdown
::: alert-note
**참고:** 다른 의미를 찾으려면 {동음이의어 문서}를 참조하세요.
:::
```
...
::: alert-notice
**알림**
웹사이트에서는 4, 5단계 헤더는 목차에 표시되지 않습니다.
:::
...
::: alert-notice
**알림**
키보드 키가 아닌 문자 자체를 나타낼 때에는 따옴표를 대신 사용하세요.
:::
...
::: alert-notice
**알림**
파일 형식(파일 확장자랑 헷갈리지 마세요)은 반드시 마침표(`.`)를 제외한 대문자로 표기되어야 합니다.
:::
...
::: alert-notice
**알림**
서식이 지정된 텍스트의 구문 강조는 웹사이트에서는 아직 구현되지 않았습니다.
:::
...
::: alert-notice
**알림**
내부 링크는 `https://osu.ppy.sh/` 도메인 내부에 있는 링크를 나타냅니다.
:::
...
```markdown
::: alert-note
**참고:** 또한 [Beatmap Editor/Design](/wiki/Client/Beatmap_editor/Design)도 참조하세요.
:::
```
...
::: alert-notice
**알림**
웹사이트에서는 4, 5단계 헤더에는 id 속성을 주지 않습니다. 즉, 이들은 직접적으로 링크할 수 없다는 것을 의미합니다.
:::
...
::: alert-notice
**알림**
외부 링크는 `https://osu.ppy.sh` 도메인 외부로 나가는 링크를 나타냅니다.
:::
...
::: alert-notice
**알림**
파일 확장자는 반드시 소문자로 적어야 합니다. 그 외의 경우 이미지를 불러올 수 없습니다!
:::
...
::: alert-notice
**알림**
위키 작성을 위해 현재 설정을 바꾸고 싶지 않다면 `osu!.<컴퓨터유저명>.cfg` 파일을 osu! 폴더 바깥으로 빼두었다가 다시 돌려놓으세요.
:::
