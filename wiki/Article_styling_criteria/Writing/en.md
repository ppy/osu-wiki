...
Contraction usage depends on the type of article that is being written. Formal articles should not use contractions.

## Perspective

`player`, `user`, `skinner`, `storyboarder`, and `creator` should be when referring to the reader or another person. `they`, `them`, `their`, and `theirs` may also be used when needed.
