...
All online offset values are stored locally in the [osu!.db](/wiki/osu!_File_Formats/Db_(file_format)) file for later use (as `short`s). This means online offset can be applied even if a player is playing offline as long as they have been connected to the internet prior to importing or playing the beatmap.
