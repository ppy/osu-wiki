---
tags:
  - o!mmwc
  - mmwc
---

# osu!mania Mapping World Cup 2026

![contest banner](img/banner.jpg)

The **osu!mania Mapping World Cup 2026** (***MMWC 2026***) is a country-based, multi-stage osu!mania mapping contest that aims to crown which nation is the best at creating osu!mania beatmaps.

## Schedule

| Event | Timestamp (UTC) |
| :-- | :-- |
| Registration phase | 2026-03-15 – 2026-04-01 (23:59) |
| Qualification Stage mapping phase | 2026-04-03 – 2026-04-23 (23:59) |
| Qualification Stage judging phase | 2026-04-25 – 2026-05-15 (23:59) |
| Group Stage mapping phase | 2026-05-23 – 2026-06-11 (23:59) |
| Group Stage judging phase | 2026-06-13 – 2026-06-26 (23:59) |
| Final Stage mapping phase | 2026-07-03 – 2026-07-23 (23:59) |
| Final Stage judging phase | 2026-07-25 – 2026-08-07 (23:59) |
| Final results | 2026-08-15 |

## Prizes

| Placing | Prize(s) |
| :-: | :-- |
| ![Gold crown](/wiki/shared/crown-gold.png "1st place") | 55% of total cash prize[^cash-prize], unique profile badge, [Contest points](/wiki/Contests/Contest_points) |
| ![Silver crown](/wiki/shared/crown-silver.png "2nd place") | 30% of total cash prize[^cash-prize], [Contest points](/wiki/Contests/Contest_points) |
| ![Bronze crown](/wiki/shared/crown-bronze.png "3rd place") | 15% of total cash prize[^cash-prize], [Contest points](/wiki/Contests/Contest_points) |

## Organisation

The osu!mania Mapping World Cup 2026 is run by various community members.

| Position | Member(s) |
| :-- | :-- |
| Hosts | ::{ flag=ID }:: ::Ilham::{ user=3057154 }, ::{ flag=ID }:: ::Maxus::{ user=4335785 } |
| Designer | ::{ flag=ID }:: ::Syadow-::{ user=3484485 } |
| Statistician | ::{ flag=ID }:: ::Reihynn::{ user=16630515 } |
| Video Editor | ::{ flag=ID }:: ::Mirsaaa::{ user=15328093 } |
| Commentators | ::{ flag=SG }:: ::Raveille::{ user=1388767 }, ::{ flag=PH }:: ::SurfChu85::{ user=4469895 } |
| Judges | ::{ flag=VN }:: ::\[TCD\] Dzar03::{ user=16712231 }, ::{ flag=DO }:: ::Antalf::{ user=8793773 }, ::{ flag=PH }:: ::chocomilku-::{ user=16687756 }, ::{ flag=AU }:: ::fvrex::{ user=11863699 }, ::{ flag=CN }:: ::gzdongsheng::{ user=8660315 }, ::{ flag=GB }:: ::Hydria::{ user=808176 }, ::{ flag=RU }:: ::Kurisu Makise::{ user=2790640 }, ::{ flag=CA }:: ::Ryax::{ user=8949769 }, ::{ flag=KR }:: ::secXcscX::{ user=13543418 }, ::{ flag=US }:: ::Unpredictable::{ user=7560872 } |

## Participants

|  | Country/Continent | Members | Note |
| :-: | :-: | :-- | :-- |
| ::{ flag=US }:: ::{ flag=CA }:: | **North America** | **::Zonifyy::{ user=10244543 }**, ::Asymetrical::{ user=15863480 }, ::Nezu-::{ user=21963245 }, ::snowsnowsonsnow::{ user=8160564 } | - |
| ::{ flag=AR }:: ::{ flag=PE }:: ::{ flag=BR }:: ::{ flag=CL }:: | **South America** | **::APRL03::{ user=11394892 }**, ::Gndsklgb::{ user=25617005 }, ::MilesNoobkk::{ user=22656756 }, ::XxFoxFirexX352::{ user=25479095 } | - |
| ::{ flag=TW }:: ::{ flag=HK }:: | **East Asia A** | **::Xinhong1003::{ user=19821902 }**, ::NineSey::{ user=25454675 }, ::Slowpoke104::{ user=23633474 } | - |
| ::{ flag=CN }:: ::{ flag=HK }:: | **East Asia B** | **::Hoshimegu Mio::{ user=17753835 }**, ::DraterTD::{ user=11671725 }, ::Koyori Chan::{ user=11238501 } | - |
| ::{ flag=MY }:: ::{ flag=SG }:: | **Southeast Asia** | **::Aris Tendou::{ user=25831078 }**, ::256::{ user=30419111 }, ::growcanine::{ user=10468512 } | - |
| ::{ flag=IL }:: ::{ flag=RO }:: ::{ flag=GR }:: | **Central Europe** | **::Skibidi Toilet::{ user=18782031 }**, ::Bei::{ user=14039549 }, ::flvxify::{ user=17698463 }, ::Mich\_::{ user=11784492 } | - |
| ::{ flag=NL }:: ::{ flag=DE }:: | **Western Europe A** | **::Monsterr::{ user=16131837 }**, ::Kitacchi::{ user=37911221 }, ::Roasted Chicken::{ user=9461160 } | - |
| ::{ flag=FR }:: ::{ flag=IT }:: ::{ flag=BE }:: ::{ flag=GB }:: | **Western Europe B** | **::\[HD\]Distinos::{ user=8505454 }**, ::Minion24::{ user=10021819 }, ::villoit::{ user=28159495 }, ::Xuliyauk::{ user=39310053 } | - |
| ::{ flag=AR }:: | **Argentina** | **::uL-::{ user=10652110 }**, ::DmX::{ user=13592523 }, ::LilSanik::{ user=20102743 }, ::Thrushpelt::{ user=22174188 } | - |
| ::{ flag=AU }:: | **Australia** | **::oct4::{ user=10303887 }**, ::anatharax::{ user=14390680 }, ::eplup::{ user=12208273 }, ::uoop::{ user=10901058 } | - |
| ::{ flag=BR }:: | **Brazil** | **::Davix150::{ user=26512810 }**, ::Kleiton::{ user=26645946 }, ::Villanovinho::{ user=29512763 } | - |
| ::{ flag=CA }:: | **Canada** | **::Livium::{ user=9828552 }**, ::Some1SomeWhere::{ user=28548745 }, ::Zembonics::{ user=16119061 } | - |
| ::{ flag=CN }:: | **China A** | **::VividCycle::{ user=14351534 }**, ::chy030_ft_pH5_6::{ user=9492856 }, ::G1NYA::{ user=31767575 }, ::PlatinumSplash::{ user=23427252 } | - |
| ::{ flag=CN }:: | **China B** | **::purakon::{ user=22412151 }**, ::nai_::{ user=37783133 }, ::nyamushi::{ user=17537004 }, ::oid45::{ user=27710732 } | - |
| ::{ flag=CN }:: | **China C** | **::Twlight_Shelter::{ user=36658406 }**, ::7912::{ user=14875348 }, ::Bulanzel::{ user=26259108 }, ::Columbina Neko::{ user=32331584 } | - |
| ::{ flag=CN }:: | **China D** | **::Leaper::{ user=36115565 }**, ::BenZn::{ user=32406156 }, ::litape::{ user=19240795 }, ::zzzbc::{ user=34699496 } | - |
| ::{ flag=CO }:: | **Colombia** | **::xNett::{ user=4998501 }**, ::AlexGB::{ user=24500138 }, ::eveille::{ user=35548476 }, ::MarvelousCosmos::{ user=14189527 } | - |
| ::{ flag=CZ }:: | **Czechia** | **::dying hamster::{ user=8623171 }**, ::Addaamm007::{ user=19126992 }, ::kojodat::{ user=28849296 }, ::zCombo::{ user=36426983 } | - |
| ::{ flag=FR }:: | **France** | **::Paturages::{ user=1375479 }**, ::Adri::{ user=4579132 }, ::poketoine::{ user=27254270 }, ::Ventilo le vrai::{ user=13661408 } | - |
| ::{ flag=HK }:: | **Hong Kong** | **::Ricizus::{ user=10395139 }**, ::-SoraIro-::{ user=11493286 }, ::-Terakomari::{ user=15399393 }, ::Popy::{ user=30829023 } | - |
| ::{ flag=ID }:: | **Indonesia A** | **::Ainer::{ user=13371424 }**, ::Antipole::{ user=17258072 }, ::FelixSpade::{ user=2651304 }, ::Revv-::{ user=12424909 } | - |
| ::{ flag=ID }:: | **Indonesia B** | **::My Angel Chen::{ user=24285111 }**, ::Catzu::{ user=11330898 }, ::kartofle::{ user=32074763 }, ::SatoriMaiden::{ user=15522414 } | - |
| ::{ flag=ID }:: | **Indonesia C** | **::BAMUPRI72::{ user=32848415 }**, ::Envily::{ user=32455325 }, ::Gamesnake7::{ user=28253504 }, ::lucknut::{ user=19724595 } | - |
| ::{ flag=ID }:: | **Indonesia D** | **::Oscyy::{ user=4818604 }**, ::ChocoCoki-::{ user=3565870 }, ::upan::{ user=5214397 } | - |
| ::{ flag=IT }:: | **Italy** | **::rikan::{ user=25263357 }**, ::Relae::{ user=14455536 }, ::ricetoasty::{ user=15339833 } | - |
| ::{ flag=JP }:: | **Japan** | **::f_ronte::{ user=33532882 }**, ::ICECAKE::{ user=16100800 }, ::omegaY\_::{ user=14727263 }, ::yoshyap::{ user=16608860 } | - |
| ::{ flag=MY }:: | **Malaysia** | **::\_Kobii::{ user=6209713 }**, ::\[-Leon-\]::{ user=13382147 }, ::Mochineko::{ user=3793196 } | - |
| ::{ flag=MX }:: | **Mexico** | **::Naiibe::{ user=21340543 }**, ::kilotek::{ user=23833099 }, ::Sakuragi Kaware::{ user=19638984 }, ::tochi::{ user=13875577 } | - |
| ::{ flag=NL }:: | **Netherlands** | **::Linlime::{ user=14438535 }**, ::DannyPX::{ user=11253722 }, ::Muse Dash::{ user=13695676 }, ::Saemitsu::{ user=14262789 } | - |
| ::{ flag=PK }:: | **Pakistan** | **::Frank\_WhyNot::{ user=27213915 }**, ::Axos::{ user=32097633 }, ::Labrynth Rat::{ user=33332515 }, ::nico-::{ user=15206217 } | - |
| ::{ flag=PE }:: | **Peru** | **::Miaurichesu::{ user=10055648 }**, ::dendro\_::{ user=17152485 }, ::Sakisagee::{ user=9938020 } | - |
| ::{ flag=PH }:: | **Philippines** | **::Silicosis 2::{ user=18560307 }**, ::Hytex::{ user=8536263 }, ::Lleethenoob::{ user=22462348 }, ::S k y y -::{ user=18611918 } | - |
| ::{ flag=PL }:: | **Poland A** | **::caracol::{ user=10764851 }**, ::fabsoneq::{ user=13708921 }, ::Paraxia::{ user=14001000 } | - |
| ::{ flag=PL }:: | **Poland B** | **::malza::{ user=19141228 }**, ::Archaic84::{ user=8611177 }, ::bagjettka::{ user=18338179 }, ::Eclipse-::{ user=8493070 } | - |
| ::{ flag=RU }:: | **Russian Federation** | **::Son\_66::{ user=27315846 }**, ::astat11::{ user=12338913 }, ::Lazurent::{ user=17272017 }, ::riftoyu::{ user=26702763 } | - |
| ::{ flag=KR }:: | **South Korea A** | **::Nicknem_::{ user=16615040 }**, ::Ppass::{ user=4782226 }, ::Stereotype::{ user=25311011 } | - |
| ::{ flag=KR }:: | **South Korea C** | **::komugii::{ user=16723762 }**, ::11Bit::{ user=14804526 }, ::LeiN-::{ user=2162939 } | - |
| ::{ flag=KR }:: | **South Korea D** | **::Stellium::{ user=10910337 }**, ::artsim::{ user=12127515 }, ::eospjm1::{ user=16290625 }, ::paradoxus_::{ user=11185275 } | - |
| ::{ flag=KR }:: | **South Korea E** | **::hemoo::{ user=4420703 }**, ::GDBlack::{ user=16976072 }, ::Raon2007::{ user=25850141 } | - |
| ::{ flag=KR }:: | **South Korea F** | **::ikasaros::{ user=37896900 }**, ::LucaMe::{ user=38266759 }, ::nooboonooboono::{ user=36630049 } | - |
| ::{ flag=TH }:: | **Thailand** | **::nanonbandusty::{ user=15543726 }**, ::haerinforever::{ user=12852613 }, ::MatchaLatte-::{ user=15944956 }, ::t9Wa2::{ user=17768395 } | - |
| ::{ flag=GB }:: | **United Kingdom B** | **::LeCrazy::{ user=28272898 }**, ::Sillyguyig::{ user=34979537 }, ::ultrasimz::{ user=34672644 } | - |
| ::{ flag=GB }:: | **United Kingdom D** | **::sqr::{ user=9943860 }**, ::-XxbluezaperxX-::{ user=12264218 }, ::spanner dude::{ user=12489832 }, ::Veridore::{ user=11527878 } | - |
| ::{ flag=US }:: | **United States A** | **::mohca::{ user=14499702 }**, ::\[-Omni-\]::{ user=24730930 }, ::ChaoticZX50::{ user=20454588 }, ::Znow::{ user=15513303 } | - |
| ::{ flag=US }:: | **United States B** | **::elexire::{ user=9206093 }**, ::Ballistic::{ user=5170506 }, ::lemonguy::{ user=4693052 }, ::Toaph Daddy::{ user=7616811 } | - |
| ::{ flag=US }:: | **United States C** | **::mashu::{ user=12028216 }**, ::Nepijin::{ user=11734610 }, ::PeachMarrow::{ user=15926730 }, ::rinstea::{ user=10206161 } | - |
| ::{ flag=US }:: | **United States D** | **::Astronian::{ user=22028323 }**, ::Alumence::{ user=30357961 }, ::kooo::{ user=15470738 }, ::Seitora::{ user=18473578 } | - |
| ::{ flag=US }:: | **United States E** | **::fishstream::{ user=11638901 }**, ::Kokistarr::{ user=25027073 }, ::m0rtz::{ user=17276152 }, ::Spook\_Mistborn::{ user=35709550 } | - |
| ::{ flag=VN }:: | **Vietnam A** | **::Lott::{ user=13821222 }**, ::Akasha-::{ user=2596306 }, ::JustAnYDudEykNw::{ user=29268726 }, ::Micleak::{ user=16140674 } | - |
| ::{ flag=VN }:: | **Vietnam B** | **::GamerChris::{ user=36042814 }**, ::\[GB\]Kita-::{ user=27175001 }, ::\[TCD\]SnowyCutie::{ user=26853034 }, ::soup3r::{ user=25088164 } | - |
| ::{ flag=VN }:: | **Vietnam C** | **::minh2506::{ user=27706969 }**, ::imaKea::{ user=25845757 }, ::phuongkhoa102::{ user=18054129 }, ::Shoka Shirokane::{ user=33461938 } | - |

## Links

- [Announcement news post](https://osu.ppy.sh/home/news/2026-03-15-osumania-mapping-world-cup-2026-registrations)
- [Forum post](https://osu.ppy.sh/community/forums/topics/2185948)
- [Contest listing](https://osu.ppy.sh/community/contests/283)
- [Discord server](https://discord.com/invite/a7kpKK2rNj)
- [Contest spreadsheet](https://docs.google.com/spreadsheets/d/1qP-zPogcytTcMB3SMIAQkWYiTcxOzfMXo4gauRmORbI/edit?usp=sharing)
- [**Registration form**](https://docs.google.com/forms/d/e/1FAIpQLScpB8MP8-D4bHGuT1aR6_iq-cVprS0zrOmiEEbVQpJL9AyDug/viewform?usp=sharing&ouid=103196331030427249584)

## Ruleset

### Format

- This contest only accepts osu!mania 4K submissions.
- This is a team-based contest.
- Each team must consist of 3 or 4 mappers from the same country as listed on their profile page.
- Signups may be done with a pre-formed team, or individually if you do not have a team. The organisers will assist in the formation of teams from the individual signup list based on their country or geographical location.
- There is no team limit per country on the first stage. However, only a limited number of teams from each country will be allowed to advance to later stages based on their results.
- There will be a total of 3 stages, each with its own specifications. Teams must create 1 map per stage according to the specifications.

| Stage | Specification |
| :-- | :-- |
| Qualification Stage | *Limited Drain Time* — Each team is required to create a map with a specific drain time. |
| Group Stage | *Limited Artist* — Each team is required to create a map using tracks by artists selected by the hosts. The available choices and selection order are influenced by a team's placement in the previous stage. |
| Final Stage | *Limited Song Choice – Diverse Mapping Style* — Each team must create a map using a song selected from two songs provided by the hosts. Song selection order is determined by a team's placement in the previous stage. Each mapper in the team should contribute a section with a distinct interpretation and mapping style. |

- Each stage will introduce difficulty requirements for submissions. These will get progressively harder as the rounds go on.
- The judging criteria for each stage will remain the same.

### Rules

- **The [osu! community rules](/wiki/Rules) are in place at all times during this contest.**
- **At least one team member must join the contest's [Discord server](https://discord.com/invite/a7kpKK2rNj)**
- **Map submissions must be created collaboratively by team members.** Any form of plagiarism involving outside parties is prohibited.
- **Each map must be mapped by at least 3 different mappers.** This is a team-based competition. Collaboration is necessary to determine the strengths of each participating country. We want to avoid situations amongst teams where some members get carried. Providing feedback does not count as a part of collaboration.
- **A mapper on the winning team must participate in at least 2 out of the 3 maps made to be eligible for the badge and points rewards.**
- **Submissions will not be accepted after the deadlines set for each stage.**
- **All participants are expected to uphold the highest standards of sportsmanship and integrity throughout the contest.** Demonstrating mutual respect toward other participants, observers, judges, and organisers is expected.
- **Sharing your work outside your team before judging concludes, or leaking confidential information, will lead to a disqualification from the contest.** At the same time, it is also necessary to avoid being seen by others in the editor as this could expose confidential song selection information.

### Judging criteria

- **Representation (30):** Points will be assigned based on how well the beatmap represents the song through its overall arrangement, patterns, and structure. As well as how well the map demonstrates the essence of collaboration mapping, which the beatmap could be diverse in execution but still packed as a one solid map.
- **Creativity (25):** Points will be assigned based on how innovative the beatmap's design is, including unique structural arrangements, logical intensity changes, and other technical aspects.
- **Playability (25):** Points will be assigned based on the extent to which the beatmap provides a cohesive playing, visual, and entertainment experience while maintaining a reasonable difficulty curve and fitting into the current stage's difficulty range.
- **Impression (20):** Points will be assigned based on the judges' overall thought regarding the submission, as well as factors not covered by the three aspects above.

Scores are reported as standardised scores.

## Notes

[^cash-prize]: The cash prize pool was generously crowdfunded by the osu!mania community.
