...
Пишутся в верхнем регистре: `CS`, `AR`.
...
- Название клавиши: `B`, `Ctrl` + `Shift` + `A` (клавиши-модификаторы пишутся с большой буквы)
- Надпись на кнопке в интерфейсе: `1. Открыть профиль`
- Название папки: `Экспорт`, `Загрузки`.
  - Такие слова можно склонять (скачанные карты появятся в `загрузках`)
- Название директории в пути: `/Songs/`, `/Skins/User/`
- Ключевое слово: `ComboBurstStyle`, `[Events]`
- Расширение файла: `.jpg`, `.osz` (с точкой перед названием)
...
Если вы решили добавить как полноразмерные изображения, так и превью, заведите две папки (имена файлов в них должны совпадать), например, `pr/` для превью и `fs` для полных размеров.
