...
**Resnap** refers to a pair of commands under the `Timing` tab in the [beatmap editor](/wiki/Beatmap_Editor). One of which applies to the entire [beatmap](/wiki/Beatmap) and the other to the current timing section. If the beatmap has only one timing section, both commands are the same.
