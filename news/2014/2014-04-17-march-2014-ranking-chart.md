---
layout: post
title: March 2014 Ranking Chart
date: 2014-04-17 00:00:00 +0000
tumblr_url: http://osunews.tumblr.com/post/83033256875/march-2014-ranking-chart
series: beatmap_spotlights
---

The Ranking Charts for March 2014 have just been released and [can be found here](https://osu.ppy.sh/p/chart?ch=MONTH1403)!

The charts have been created by letting the the [CAT](https://osu.ppy.sh/groups/21) and [BAT](https://osu.ppy.sh/groups/7) vote for the most noteworthy maps of each respective month. They will run until the 15th May, 2014.

Congratulations to winner of the January Ranking Charts 2014:

**osu! standard**

- **1st** ::Nitro::{ user=654098 } \- 1 month supporter
- **2nd** ::randomizer89::{ user=648613 }
- **3rd** ::SapphireGhost::{ user=388602 }

**Taiko**

- **1st** ::Midnaait::{ user=1506011 } \- 1 month supporter
- **2nd** ::Alicewave::{ user=1772471 }
- **3rd** ::yi3725::{ user=3458795 }

**Catch the Beat**

- **1st** ::\[\1Crystal\]::{ user=1646397 } \- 1 month supporter
- **2nd** ::Awabiz::{ user=654296 }
- **3rd** ::\_Lorenzo\_::{ user=3354524 }

**osu!mania**

- **1st** ::flzhej2::{ user=1809594 } \- 1 month supporter
- **2nd** ::Cryolien::{ user=1626983 }
- **3rd** ::iLLUi::{ user=654098 }

Moreover, congratulations to the winners of February Ranking Charts 2014:

**osu! standard**

- **1st** ::randomizer89::{ user=648613 } \- 1 month supporter
- **2nd** ::SapphireGhost::{ user=388602 }
- **3rd** ::Elinia::{ user=1406196 }

**Taiko**

- **1st** ::asuasu\1yura::{ user=2875968 } \- 1 month supporter
- **2nd** ::horikawa159::{ user=11573650 }
- **3rd** ::Midnaait::{ user=1506011 }

**Catch the Beat**

- **1st** ::CLSW::{ user=531253 } \- 1 month supporter
- **2nd** ::DakkyChan::{ user=2107017 }
- **3rd** ::dae6254::{ user=563262 }

**osu!mania**

- **1st** ::\- R e b a -::{ user=3087654 } \- 1 month supporter
- **2nd** ::Cryolien::{ user=1626983 }
- **3rd** ::iLLUi::{ user=37203884 }

Good luck with the new charts!

—Loctav
