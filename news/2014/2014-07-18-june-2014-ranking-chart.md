---
layout: post
title: June 2014 Ranking Chart
date: 2014-07-18 00:00:00 +0000
tumblr_url: http://osunews.tumblr.com/post/92131524533/june-2014-ranking-chart
series: beatmap_spotlights
---

The Ranking Charts for June 2014 have just been released and [can be found here](https://osu.ppy.sh/p/chart?ch=MONTH1406)! Along with that, we want to introduce you some changes to make the Monthly Ranking Charts more attractive for everyone!

First of all, we implemented a [long-term ladderboard](https://docs.google.com/spreadsheets/d/1rgaKVJj_MDo0PwGrGXbKRVzjE9fhOUb4XZXRsOjTaoQ/edit). Everytime you reach the Top 10 at the end of a monthly ranking chart, you will receive a score according to your position. At the end of each season, which consists of three months of charts, the season winner will be rewarded with an additional **3 months supporter tag** and the user title **Charts Champion** for 3 months. The winner of each individual ranking chart will still receive 1 month supporter tag. The Summer season will regroup the May, June and July ranking charts.

The charts have been created by letting the [CAT](https://osu.ppy.sh/groups/21) and [BAT](https://osu.ppy.sh/groups/7) vote for the most noteworthy maps of each respective month. They will end on August 15th, 2014. **[You can check out the vote results here!](https://docs.google.com/spreadsheets/d/1bQQsvecPxgHJtFTMGWjSj25FcCmhGu82M8jvvNoKOic/edit)**

Congratulations to the winners of the May 2014 Ranking Charts. They get rewarded with a **1 month supporter tag**:

- **osu! standard:** ::randomizer89::{ user=648613 }
- **Taiko:** ::Konpaku Sariel::{ user=533502 }
- **Catch the Beat:** ::Serena-::{ user=2193444 }
- **osu!mania:** ::Qnyxiie::{ user=2940660 }

Additionally, we reward the highest voted map by providing **1 month supporter tag** to the mapper:

- **osu! standard:** ::RLC::{ user=1047883 } for [Himeringo - Yotsuya-san ni Yoroshiku](https://osu.ppy.sh/beatmaps/378781?m=0) (20 votes)
- **Taiko:** ::TKS::{ user=940878 } for [kors k - Insane Techniques](https://osu.ppy.sh/beatmapsets/155288) (10 votes)
- **Catch the Beat:** ::CLSW::{ user=531253 } for [nano - Nevereverland](https://osu.ppy.sh/beatmapsets/149570) (11 votes)
- **osu!mania:** ::Rumia-::{ user=1787171 } for [Sharlo - Sleeping Butterfly](https://osu.ppy.sh/beatmapsets/160695) (9 votes)

Good luck with the new charts!

—Loctav
