---
layout: post
title: "Monthly Beatmapping Contest: June"
date: 2020-06-04 22:00:00 +0000
---

Jumping into June with more competitive beatmapping! Last month's finalists are ready for your votes and put your best foot forward for this month's contest!

![](/wiki/shared/news/banners/monthly-beatmapping-contest.png)

Curious about how the Monthly Beatmapping Contest (*MBC*) works? See what you've missed on the [Monthly Beatmapping Contest wiki article](/wiki/Contests/Monthly_Beatmapping_Contest), including instructions, previous winners, and all of the juicy rewards.

## Community Vote

With help from our team experienced beatmap screeners, we've selected the **top 5 finalists** from May's contest! Check them out and vote for your **favourite 2 maps**. Last month's contest required *all entries to use the same pre-defined rhythm*, so you should be focusing on how entries managed to create engaging beatmaps through object placements.

[**Vote for May's contest HERE!**](https://osu.ppy.sh/community/contests/95)

While we'd love it if you gave each entry a playthrough, you can also quickly eye your potential favorites in this video playlist:

<iframe width="100%" height="315" src="https://www.youtube.com/embed/videoseries?list=PLp7-THR1EUHGDyZCXarV-cOZmWosLWN03" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>

## Schedule

As per usual, the timeline for this month's contest is as follows:

- **Beatmapping (3 weeks, ends June 26th):** Contest is announced and beatmappers create their entries.
- **Screening (1 week):** Entries are evaluated by experienced beatmappers.
- **Voting (2 weeks):** Top entries from screening are published for the osu! community to vote on.
- **Rewards:** Winners are announced and prizes are redeemed, including osu!supporter, profile badges, and the *Elite Mapper* user title!

## Beatmapping

### <a id="limitation"></a>Limitation

The MBC tries to keep mappers on their toes by featuring different mapping limitations each month. One previous contest required straight sliders only and another required star rating to be lower than 4.00 stars, to name a few.

This month's limitation: **sliders cannot be 1/2 of a beat long**.

![limitation example](/wiki/shared/news/2020-06-04-monthly-beatmapping-contest-june/mbc6.png)

For extra clarification, 1/2 sliders with reverse arrows are not allowed, while a 1/4 slider with a repeat is allowed. This limitation is focused on the length of the original slider!

We'd also like to provide three complimentary therapy sessions to mappers who can't imagine life without 1/2 sliders. We're sorry but it's time to let go.

### <a id="songs"></a>Songs

Each MBC provides beatmappers a choice between 5 songs from one of osu!'s [Featured Artists](https://osu.ppy.sh/beatmaps/artists).

This month's artist is the drum 'n' bass legend [**Station Earth / Blue Marble**](https://osu.ppy.sh/beatmaps/artists/66), and the following songs are viable for contest submissions:

#### Blue Marble - Darkest Heights

Beatmap template download: [Full version](https://assets.ppy.sh/artists/66/osz/Blue%20Marble%20-%20Darkest%20Heights.osz) (3:10) / [Short version](https://assets.ppy.sh/contests/96/short/Blue%20Marble%20-%20Darkest%20Heights.osz) (1:42)

<audio controls>
    <source src="https://assets.ppy.sh/artists/66/previews/1847.mp3" type="audio/mpeg">
</audio>

#### Ayah Marar & Blue Marble - Invincible

Beatmap template download: [Full version](https://assets.ppy.sh/artists/66/osz/Ayah%20Marar%20&%20Blue%20Marble%20-%20Invincible.osz) (3:47) / [Short version](https://assets.ppy.sh/contests/96/short/Ayah%20Marar%20&%20Blue%20Marble%20-%20Invincible.osz) (1:57)

<audio controls>
    <source src="https://assets.ppy.sh/artists/66/previews/2202.mp3" type="audio/mpeg">
</audio>

#### Blue Marble - Love

Beatmap template download: [Full version](https://assets.ppy.sh/artists/66/osz/Blue%20Marble%20-%20Love.osz) (4:12) / [Short version](https://assets.ppy.sh/contests/96/short/Blue%20Marble%20-%20Love.osz) (2:22)

<audio controls>
    <source src="https://assets.ppy.sh/artists/66/previews/1850.mp3" type="audio/mpeg">
</audio>

#### Blue Marble - Parhelia

Beatmap template download: [Full version](https://assets.ppy.sh/artists/66/osz/Blue%20Marble%20-%20Parhelia.osz) (4:02) / [Short version](https://assets.ppy.sh/contests/96/short/Blue%20Marble%20-%20Parhelia.osz) (2:12)

<audio controls>
    <source src="https://assets.ppy.sh/artists/66/previews/1851.mp3" type="audio/mpeg">
</audio>

#### Roos Denayer - The Style of Evermore (Blue Marble Remix)

Beatmap template download: [Full version](https://assets.ppy.sh/artists/66/osz/Roos%20Denayer%20-%20The%20Style%20of%20Evermore%20(Blue%20Marble%20Remix).osz) (3:35) / [Short version](https://assets.ppy.sh/contests/96/short/Roos%20Denayer%20-%20The%20Style%20of%20Evermore%20(Blue%20Marble%20Remix).osz) (1:55)

<audio controls>
    <source src="https://assets.ppy.sh/artists/66/previews/1856.mp3" type="audio/mpeg">
</audio>

These songs were chosen specifically because they support tons of 1/2 sliders, so we wish you luck when composing rhythm!

## Rules

So how do you participate? Submissions must...

- **follow the month's [beatmapping limitation](#limitation): *sliders cannot be 1/2 of a beat long***
- **be in `.osz` format.** This allows your entry to use custom hitsound files and a background of your choice, however storyboarded visuals will be removed from all submissions.
- **be a single difficulty for one of the month's [selected songs](#songs).**
- **be in the osu! game mode.**
- **not be uploaded to the osu! website through the Beatmap Submission System (*BSS*).** All contest submissions are anonymous during the screening/voting phases to avoid bias. **DO NOT SHARE YOUR ENTRY!**
- **be solely created by you.** Collaborations are not allowed.
- **abide by the [ranking criteria](/wiki/Ranking_Criteria).** While we can be forgiving for mistakes, submissions that could not be ranked without major changes will be discarded.

Any entries breaking these rules will not be viable for the contest.

---

Head over to the [contest listing](https://osu.ppy.sh/community/contests/96) when you're ready to submit your entry. **3 weeks** remain!

And in case you haven't already, contribute to last month's contest by [voting for your 2 favourite entries here](https://osu.ppy.sh/community/contests/95)!

Have fun mapping!

—pishifat
