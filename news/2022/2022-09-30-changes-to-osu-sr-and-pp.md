...
A general balancing pass, mostly for performance points calculation, was applied in [this proposal](https://github.com/ppy/osu/pull/19001) by ::StanR::{ user=7217455 }. This aims to alleviate some concerns shared by both the team of developers and the community.
...
Following a refactor effort by ::Apo11o::{ user=9558549 } involving [two](https://github.com/ppy/osu/pull/18368) [changes](https://github.com/ppy/osu/pull/18458), this is now much less of a problem, allowing for more specific and comprehensive adjustments to be made much more easily, or outright possible to begin with.
...
This nerf has now been applied to star rating as well with [this change](https://github.com/ppy/osu/pull/16524) proposed by ::StanR::{ user=7217455 }, making the aim requirement for medals around 20% higher (on average) for all touch screen players.

### Flashlight changes

The incredible illuminator ::MBmasher::{ user=4498616 } continues his focused efforts on improving Flashlight difficulty calculation, yielding a series of changes to the Flashlight skill that were introduced in the last deployment.
...
Throughout the development process for the Flashlight changes, a number of grid-style maps such as ::DJPop::{ user=2363 }'s [beatMARIO - Night of Knights [SOLO]](https://osu.ppy.sh/beatmapsets/15920#osu/58063) became increasingly concerning as their values rose. By reducing the influence of repeated angles, grid-style maps such as this one will now settle at a more suitable value for the skill involved.
...
- [**The speed skill now measures distance correctly.**](https://github.com/ppy/osu/pull/15758) ::GoldenMine::{ user=8058206 } discovered that distance has been calculated improperly since the last set of changes, causing aim from sliders to be potentially overemphasised. This has now been corrected.
- [**The miss count estimate is now smooth.**](https://github.com/ppy/osu/pull/16331) Previously, the miss count estimate could jump from 1 miss to 2 misses within a combo, but this transition is done more smoothly now that the estimated miss count is calculated as a decimal value, thanks to ::StanR::{ user=7217455 }.
- [**1x100 scores no longer break miss count estimate.**](https://github.com/ppy/osu/pull/19804) ::StanR::{ user=7217455 } addressed an issue where a single 100 on a score would potentially count as more than one miss.
