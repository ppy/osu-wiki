---
layout: post
title: "Mapping Global Taiko Showdown 2025: Registrations Open!"
date: 2025-06-01 23:00:00 +0000
---

Grab a friend or two and join us for one last dance in GTS' endeavour in the osu!taiko mapping scene!

![](/wiki/shared/news/2025-06-01-mgts-2025-registrations/banner.png)

The Mapping Global Taiko Showdown (MGTS) returns once again to bring avid osu!taiko mappers the opportunity to flex their editor muscles and stand their ground on who's the best out there!

## What's new?

While the contest remains single-round and team-based, we've lowered the team size from 4–6 players to just 2–3 to make the competition more accessible, and in classic GTS fashion, we're providing **three custom-made songs** to map!

Do note that you'll need to map exactly 2 of the 3 songs — please read the [contest's rules](https://gtsosu.com/2025/mgts/home) for more details.

### Songs

This wouldn't be a GTS event without entirely original songs and beatmaps to accompany it, only this time of course *you* will be collaborating for the mapping part!

Check them all out below:

![](/wiki/shared/news/2025-06-01-mgts-2025-registrations/illusoryprogram97.png)

| Song | [Kyotsugyon](https://soundcloud.com/kyotsugyon) & [Sh0wtime](https://soundcloud.com/sh0wtimelol) - [ILLUSORYPROGRAM97](https://soundcloud.com/kyotsugyon/illusoryprogram97) |
| :-- | :-- |
| **Length** | 5:10 |
| **BPM** | 149–198 (198) |
| **Note** | Tiebreaker style |

<audio controls>
    <source src="https://assets.ppy.sh/media/news/mgts-2025-illusoryprogram97.mp3">
</audio>

![](/wiki/shared/news/2025-06-01-mgts-2025-registrations/axisbreak.jpg)

| Song | [X](https://soundcloud.com/foodbot)/[Y](https://soundcloud.com/mrcool909090)/[Z](https://soundcloud.com/silis) - [axis::BREAK](https://soundcloud.com/foodbot/axisbreak) |
| :-- | :-- |
| **Length** | 2:52 |
| **BPM** | 202 |
| **Note** | Consistency pick |

<audio controls>
    <source src="https://assets.ppy.sh/media/news/mgts-2025-axisbreak.mp3">
</audio>

![](/wiki/shared/news/2025-06-01-mgts-2025-registrations/hatching.png)

| Song | [Faxyne](https://soundcloud.com/faxyne) vs. [blobdash](https://soundcloud.com/blobdash) - [Hatching](https://blobdash.bandcamp.com/track/hatching) |
| :-- | :-- |
| **Length** | 2:38 |
| **BPM** | 200 |
| **Note** | Tech pick |

<audio controls>
    <source src="https://assets.ppy.sh/media/news/mgts-2025-hatching.mp3">
</audio>

## Rules

Everything about the contest's rules, procedures, and judging criteria can be found in both the [contest's website](https://gtsosu.com/2025/mgts/home) and its [osu! wiki page](/wiki/Contests/MGTS/2025) — please make sure you and your team go through them thoroughly in order to make sure you end up with a valid submission!

## How do I participate?

First of all, joining the [Discord server](https://discord.com/invite/3mGC3HB) is mandatory for participation, because all important announcements will be communicated there.

Your next step will be to form a team with 2 to 3 members, and have the captain [register the team in the GTS website](https://gtsosu.com/2025/mgts/home).

Once that's done, download one of the two beatmap templates below to start mapping:

- [Beatmap template 1: ILLUSORYPROGRAM97 + axis\:\:BREAK](https://assets.ppy.sh/media/news/MGTS%202025%20Template%201.osz)
- [Beatmap template 2: ILLUSORYPROGRAM97 + Hatching](https://assets.ppy.sh/media/news/MGTS%202025%20Template%202.osz)

Make sure to update the beatmap creator's name to **your team captain's name** in order to have a valid submission.

After you're done mapping, have **your team captain** head over to the [submission page](https://osu.ppy.sh/community/contests/248) to submit your entry. However, don't be too slow about it because you have **29 days** before the submission deadline!

If you're having any issues with the registration process, please don't hesitate to [send Hivie a message](https://osu.ppy.sh/community/chat?sendto=14102976)!

## Schedule

| Event | Timestamp (UTC) |
| --: | :-- |
| Registration and submission phase | 1 June – 29 June 23:59 |
| Judging phase | 1 July – 26 July |
| Results | 27 July |

The schedule may be subject to change depending on how the contest progresses — we'll be sure to announce any changes in the [Discord server](https://discord.com/invite/3mGC3HB).

Don't forget to tune in to the [Twitch channel](https://www.twitch.tv/gtsosu) for the results livestream!

## Prizes

The top 3 teams that reign supreme will be awarded with the following prizes:

- **[Contest points](/wiki/Contests/Contest_points) and badges:** These may be awarded after the contest has concluded (pending approval).
- **Ranked status:** The top 3 teams' entries will be fast-tracked to the Ranked section if the mappers choose to.

---

Got all of that? Good. Your next task is to lock in with your team and start cooking your best beatmap creation up to date.

—Hivie
