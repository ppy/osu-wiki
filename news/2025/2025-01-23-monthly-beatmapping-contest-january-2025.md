---
layout: post
title: "Monthly Beatmapping Contest: January 2025"
date: 2025-01-23 19:30:00 +0000
series: monthly_beatmapping_contest
---

Only one thing stands between you and proof that you're a good mapper: the Monthly Beatmapping Contest.

![](https://assets.ppy.sh/media/monthly-beatmapping-contest.png)

In case you're unaware, the [Monthly Beatmapping Contest](/wiki/Contests/Monthly_Beatmapping_Contest) (*MBC*) is a beatmapping contest that's held monthly. Big surprise, we know.

This month's iteration is for **osu!taiko** and **osu!catch** mappers:

- [osu!taiko contest listing](https://osu.ppy.sh/community/contests/229)
- [osu!catch contest listing](https://osu.ppy.sh/community/contests/230)

This article covers results from the last contest, plus criteria for this contest. Skip around with these links:

- Results for [November's MBC](https://osu.ppy.sh/home/news/2024-11-17-monthly-beatmapping-contest-november-2024)
  - [osu!taiko results](#osu!taiko-results)
  - [osu!catch results](#osu!catch-results)
- [**Criteria for this month's MBC**](#game-modes)

### osu!taiko results

[See the full results here.](https://mappersguild.com/contests/results?contest=673a52e36ddd4f95155aa52a)

#### Summary

| Placement | Users | Beatmap | Badge prize | Title prize | [Contest points](/wiki/Contests/Contest_points) |
| :-- | :-- | :-- | :-- | :-- | :-- |
| Winner | ::-Kazuha::{ user=29978316 } | [Raphiiel - RoquiRa : Between Life and Death](https://osu.ppy.sh/beatmapsets/2310932) | ![](https://assets.ppy.sh/profile-badges/mbc-2020-2.png) | *Elite Mapper* | +2 (7 total) |
| Winner | ::Doug::{ user=13806900 } | [Raphiiel - RoquiRa : Between Life and Death](https://osu.ppy.sh/beatmapsets/2310932) | ![](https://assets.ppy.sh/profile-badges/mbc-2020-1.png) | - | +2 (2 total) |
| Runner-up | ::uone::{ user=5321719 } | [BilliumMoto x Akiri - Cryochemistry](https://osu.ppy.sh/beatmapsets/2310911) | ![](https://assets.ppy.sh/profile-badges/mbc-2020-2.png) | - | +1 (9 total) |
| Runner-up | ::tasuke912::{ user=2774767 } | [BilliumMoto x Akiri - Cryochemistry](https://osu.ppy.sh/beatmapsets/2310911) | - | - | +1 (3 total) |

::-Kazuha::{ user=29978316 } and [Doug](https://osu.ppy.sh/users/13806900) produced a monster of a 10-minute marathon beatmap together, landing -Kazuha a spot in osu!'s *Elite Mappers*. Meanwhile, ::tasuke912::{ user=2774767 } and veteran *Elite Mapper* ::uone::{ user=5321719 } came in a close 2nd with their own 6-minute marathon.

#### Top 10

<div align="center" class="osu-md__paragraph">
    <iframe width="95%" style="aspect-ratio: 16 / 9;" src="https://www.youtube.com/embed/S41RaoHGP84" frameborder="0" allowfullscreen></iframe>
</div>

Special thanks to ::arcpotato::{ user=12842392 }, [BlackBN](https://osu.ppy.sh/users/6291741), ::SimplySynthy::{ user=15272997 }, and [Yasuho](https://osu.ppy.sh/users/8458835) for judging this month's maps!

### osu!catch results

[See the full results here.](https://mappersguild.com/contests/results?contest=673a534f6ddd4f95155aa675)

#### Summary

| Placement | User | Beatmap | Badge prize | Title prize | [Contest points](/wiki/Contests/Contest_points) |
| :-- | :-- | :-- | :-- | :-- | :-- |
| Winner | ::Jemzuu::{ user=7890134 } | [seatrus - Hekikuu no Yakusoku](https://osu.ppy.sh/beatmapsets/2310910) | ![](https://assets.ppy.sh/profile-badges/mbc-2020-2.png) | *Elite Mapper* | +2 (6 total) |
| Runner-up | ::Mniam::{ user=6050530 } | [Akiri - Leuenea](https://osu.ppy.sh/beatmapsets/2310966) | - | - | +1 (1 total) |

Yet another [seatrus](https://osu.ppy.sh/beatmaps/artists/185) submission pushed ::Jemzuu::{ user=7890134 } over the top and into osu!catch's *Elite Mappers*! Congratulations to him.

#### Top 10

<div align="center" class="osu-md__paragraph">
    <iframe width="95%" style="aspect-ratio: 16 / 9;" src="https://www.youtube.com/embed/vczvDoQ0Xkw" frameborder="0" allowfullscreen></iframe>
</div>

And thanks to this month's judges: ::fayew::{ user=12498861 }, [Unlucky_w](https://osu.ppy.sh/users/4820793), ::Chatie::{ user=6524765 }, and [-Rustyy](https://osu.ppy.sh/users/16355636)!

## Game modes

This month's contest is open to **osu!taiko** and **osu!catch** mappers.

- [osu!taiko contest listing](https://osu.ppy.sh/community/contests/229)
- [osu!catch contest listing](https://osu.ppy.sh/community/contests/230)

## Timeline

- **[Beatmapping](#beatmapping) (3 weeks):** Contest is announced and beatmappers create their entries.
- **[Screening](/wiki/Contests/Monthly_Beatmapping_Contest#screening) (1.5 weeks):** Entries are evaluated by experienced beatmappers. If there aren't many entries, this will be skipped in favour of a longer judging period.
- **[Judging](/wiki/Contests/Monthly_Beatmapping_Contest#judging) (1.5 weeks):** Top entries from screening are judged in detail by a different crew of experienced beatmappers.
- **[Rewards](#rewards) (~2 months from now):** Winners are announced at the start of another MBC.

## Beatmapping

### Limitation

*Limitations* are a staple of the Monthly Beatmapping Contest. These are essentially extra rules that make each month's contest (and its submitted maps) distinct!

If you're confused about how these work, ask for clarification in the comments, or send a message to [Hivie](https://osu.ppy.sh/community/chat?sendto=14102976) (for osu!taiko) or ::Morusya::{ user=13681464 } (for osu!catch).

#### osu!taiko

Finishers are already big, but this contest aims to give them even more of a focus.

This month's osu!taiko theme is **finishers**.

Finishers should be used liberally, and in the judging phase, maps will be evaluated on how well finishers are used in pretty much every aspect (along with the usual mapping quality–related criteria). This includes finisher creativity, finisher playability, finishers in regards to map quality, and... you get the idea.

#### osu!catch

Difficulty settings are the basis of any map. This month's contest aims to change how you use these values.

This month's osu!catch limitation: **all difficulty settings must be equal.**

Circle size, approach rate, overall difficulty, and HP drain are the relevant settings in this case. Each of these must be the same value!

### Songs

Any song listed on [osu!'s Featured Artist listing](https://osu.ppy.sh/beatmaps/artists) is eligible for this contest. Cutting songs is allowed too!

## Rewards

The Monthly Beatmapping Contest rewards users with [contest points](/wiki/Contests/Contest_points) based on their placements:

- **Winner:** 2 points
- **Runner-up:** 1 point

Contest points reward users at certain thresholds:

- **Profile badges** are attainable at 2, 4, and 6 points each. The badges are tiered, meaning each new badge will replace the previous one!
- ***Elite Mapper* [user title](/wiki/Community/User_title)** is attainable at 6 points.

Additionally, a [**Mappers' Guild keychain**](/wiki/shared/news/2023-08-29-changes-to-the-mappers-guild/keychain.jpg) is optionally rewarded to the winner of each month's contest. Its design is based on your beatmap!

## Rules

- **Submissions must follow the month's beatmapping [limitation](#limitation):**
  - **osu!taiko:** create a map based around finishers
  - **osu!catch:** all difficulty settings must be equal
- **Submissions must be a single difficulty from one of the month's [selected songs](#songs).**
- **Submissions must be in `.osz` format.**
- **Submissions must NOT be uploaded to the osu! website until after the contest's results are finalised.**
- **Submissions must abide by the [ranking criteria](/wiki/Ranking_criteria).**
- **Submissions must be solely made by you.** Collaborations are not allowed for this contest!

Note that all submissions will be anonymised before they're reviewed by screeners/judges to avoid bias.

---

You've reached the end. Open the editor and start mapping already. Once you're done, drop your map in either of these:

- [osu!taiko contest listing](https://osu.ppy.sh/community/contests/229)
- [osu!catch contest listing](https://osu.ppy.sh/community/contests/230)

You have until **February 12th** to submit!

—pishifat, Hivie, Morusya
