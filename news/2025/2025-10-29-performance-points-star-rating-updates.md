...
[Thanks to](https://github.com/ppy/osu/pull/31351) ::tsunyoku::{ user-id=11315329 }, some buffs relating to AR, Hidden and OD were moved from PP algorithms into SR code. Since PP values are derived from SR, it means the two are now **more correlated**, but note that PP values are *not* affected.
...
This is the result of a [set](https://github.com/ppy/osu/pull/33066) of [changes](https://github.com/ppy/osu/pull/33170) by ::Givy120::{ user-id=10560705 } and [additional contributions](https://github.com/ppy/osu/pull/31234) by ::KermitNuggies::{ user-id=33452559 }.
...
Small, rapid velocity changes (i.e. fast irregular jumps, cut streams, etc.) had a buff that only applied to wide-angled patterns. With a [change](https://github.com/ppy/osu/pull/33541) by ::StanR::{ user-id=7217455 }, that restriction is now removed, meaning more of those types of patterns are buffed.
...
A [change](https://github.com/ppy/osu/pull/31409) by ::StanR::{ user-id=7217455 } was made to remove the requirement for patterns to have the "same" rhythm in order to award angle bonuses.
...
[Some](https://github.com/ppy/osu/pull/34215) [changes](https://github.com/ppy/osu/pull/33237) by ::Givy120::{ user-id=10560705 } were made in order to rebalance performance calculations for the Hidden mod:
...
::StanR::{ user-id=7217455 } also contributed a [change](https://github.com/ppy/osu/pull/34367) that makes AR and HD bonuses additive instead of multiplicative. This very slightly nerfs maps on both the high and low ends of AR, but can have more of an effect in fringe edge cases.
...
- A [fix](https://github.com/ppy/osu/pull/32694) by ::tsunyoku::{ user-id=11315329 } to ensure deviation calculations cannot break speed PP
- A [fix](https://github.com/ppy/osu/pull/34369) by ::tsunyoku::{ user-id=11315329 } to ensure Traceable's visibility bonus was handled correctly after the recent Hidden changes
- A [fix](https://github.com/ppy/osu/pull/34544) by ::tsunyoku::{ user-id=11315329 } to ensure that slider breaks are not awarded in impossible combo scenarios
- A [fix](https://github.com/ppy/osu/pull/34065) by ::Givy120::{ user-id=10560705 } to ensure OD accounts for osu!(lazer)'s updated hit windows
- A [fix](https://github.com/ppy/osu/pull/33110) by ::Givy120::{ user-id=10560705 } to ensure slider break estimations cannot produce an impossibly high number of slider breaks
- A [fix](https://github.com/ppy/osu/pull/32790) by ::Givy120::{ user-id=10560705 } to ensure rhythm calculations use the correct time between objects
- A [fix](https://github.com/ppy/osu/pull/34877) by ::StanR::{ user-id=7217455 } to ensure rhythm difficulty is reduced for double-tappable patterns
- A [change](https://github.com/ppy/osu/pull/33640) by ::tsunyoku::{ user-id=11315329 } to rebalance final values to meet community expectations
- A [change](https://github.com/ppy/osu/pull/32634) by ::Finadoggie::{ user-id=14182048 } to ensure slider accuracy bonuses are given to the ScoreV2 mod
- A [change](https://github.com/ppy/osu/pull/32434) by ::StanR::{ user-id=7217455 } to weaken punishment for 100s on scores set with the Relax mod
- A [change](https://github.com/ppy/osu/pull/33004) by ::StanR::{ user-id=7217455 } to handle the Magnetised mod in performance calculations
- A [change](https://github.com/ppy/osu/pull/34081) by ::StanR::{ user-id=7217455 } to handle the Deflate mod in performance calculations
- A [refactor](https://github.com/ppy/osu/pull/32410) by ::StanR::{ user-id=7217455 } to aid with maintaining slider-related calculations
- A [refactor](https://github.com/ppy/osu/pull/33218) by ::Natelytle::{ user-id=17607667 } to improve understanding of deviation calculations
- A [refactor](https://github.com/ppy/osu/pull/33265) by ::tsunyoku::{ user-id=11315329 } to aid with difficulty calculation maintenance
- A [refactor](https://github.com/ppy/osu/pull/33423) by ::Givy120::{ user-id=10560705 } to optimise rhythm calculations
- A [refactor](https://github.com/ppy/osu/pull/33271) by ::Givy120::{ user-id=10560705 } to improve readability of the new Hidden bonus

## osu!taiko

### Steeper accuracy scaling & rhythm penalties

A [change](https://github.com/ppy/osu/pull/34188) by ::rloseise::{ user-id=6793778 } was made to replace existing accuracy calculations with a steeper formula. This means differences in accuracy now have a larger effect on PP.
...
Longer maps award more PP, but this effect was capped at 1,500 objects. ::rloseise::{ user-id=6793778 } has [removed this cap and entirely replaced the "length bonus" formula](https://github.com/ppy/osu/pull/33582) as part of this update.
...
Each miss used to give a fixed multiplier to the PP value, but with [this change](https://github.com/ppy/osu/pull/33409) by ::rloseise::{ user-id=6793778 }, it's now more difficulty-aware.
...
A [change](https://github.com/ppy/osu/pull/33403) by ::ltca::{ user-id=11475208 } and [BabySnakes](https://osu.ppy.sh/users/4669728) was made in order to fix an issue with "unsnapped" objects unfairly buffing rhythm difficulty.
...
A [change](https://github.com/ppy/osu/pull/33380) by ::rloseise::{ user-id=6793778 } was made in order to fix maps with high stamina difficulty causing a harsh jump in the bonus applied to long maps.
...
- A [fix](https://github.com/ppy/osu/pull/33635) by ::ltca::{ user-id=11475208 } to ensure the mono length bonus cannot cause an increase in SR when removing notes
- A [fix](https://github.com/ppy/osu/pull/34829) by ::tsunyoku::{ user-id=11315329 } to ensure 0-strain maps cannot crash difficulty calculations
- A [change](https://github.com/ppy/osu/pull/34089) by ::rloseise::{ user-id=6793778 } to reduce the PP bonus for HD on scores set in osu!(lazer) due to its wider aspect ratio making fast reading easier
- A [change](https://github.com/ppy/osu/pull/33415) by ::rloseise::{ user-id=6793778 } to remove static nerfs for the EZ mod
- A [change](https://github.com/ppy/osu/pull/33500) by ::rloseise::{ user-id=6793778 } to change how difficulty and accuracy PP are summed
- A [change](https://github.com/ppy/osu/pull/34962) by ::rloseise::{ user-id=6793778 } to rebalance final values to meet community expectations 
- A [set](https://github.com/ppy/osu/pull/33233) of [changes](https://github.com/ppy/osu/pull/34327) by ::ltca::{ user-id=11475208 } and [rloseise](https://osu.ppy.sh/users/6793778) to add a "consistency factor" used to power the length bonus and miss penalty changes

## osu!catch

### Reduced combo scaling

A [change](https://github.com/ppy/osu/pull/33417) by ::KitsunIvy::{ user-id=7055384 } was created to reduce how harsh final PP values are scaled by a score's max combo.

![](/wiki/shared/news/2025-10-29-performance-points-star-rating-updates/catch-combo-scaling.png)

### Minor changes

- A [fix](https://github.com/ppy/osu/pull/32412) by ::KitsunIvy::{ user-id=7055384 } to ensure consistent detection of "buzz slider" patterns
- A [refactor](https://github.com/ppy/osu/pull/32655) by ::KitsunIvy::{ user-id=7055384 } to aid with development

## osu!mania

### Minor changes

- A [refactor](https://github.com/ppy/osu/pull/33411) by ::Natelytle::{ user-id=17607667 } to bring osu!mania difficulty calculation structure up to modern standards
