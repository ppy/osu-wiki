...
With over 550 favourites, Snow Drive by ::jieusieu::{ user-id=759439 } might be the most popular map in the vote this week, and for good reason.
...
Returning for its chance at redemption after not passing during the old voting system is ::handsome::{ user-id=2123087 }'s unravel.
...
A full set of a brutal aim map that disguises itself as a stream map. ::Krimek::{ user-id=2345078 }'s Genesis at Oasis is an incredibly high tempo map that builds its intensity towards the end of the map all at a blistering 212 BPM. See if you can handle this behemoth.
...
::Cassu2::{ user-id=2587961 }'s Foolish Hero bundles lapix's Hi-Tech style with some interesting high-tech slider patterns themselves. Overall, this proves to be an incredibly enjoyable and popular addition to the Loved vote this week.
...
::ProfessionalBox::{ user-id=3250792 }, [Asahina Momoko](https://osu.ppy.sh/users/3650145), and ::Nathan::{ user-id=4785223 } join forces in a fantastic collab to a fantastic song, with a few lower difficulties by ::toybot::{ user-id=2848604 } and [NeilPerry](https://osu.ppy.sh/users/841391) to fill out the set.
...
With almost 300 favourites and a guest difficulty by Guy, ::ShiraKai::{ user-id=487668 }'s Responsibility Response is a classic mapset which has seen tons of multiplayer play since 2015.
...
::Shiro::{ user-id=113005 }'s latest masterpiece, GvP gained quick popularity as an equally difficult jump and stream map. Shiro makes great use of various patterns throughout a very repetitive song including a guitar slider and an outro treble clef.
...
Returning for another chance at Loved after the initial rounds of voting, Grenade by ::Nanashima Asami::{ user-id=7370134 } is a very well constructed tech map that has been featured in multiple tournaments.
...
Getting another crack at the Loved category, ::Zapy::{ user-id=251395 }'s famous Game Brain mapset makes its return for individual map voting.
...
To end the list for the osu! mode, ::Necroluttah::{ user-id=2917471 }'s Preserved Valkyria needs little introduction as the set is already an incredibly popular one amongst top players.
...
Dnagib Kaerb, or Break Bigband as we'll be calling it, is an extremely infamous map. The TWC Finals Tie-Breaker, mapped by the also infamous ::Firce777::{ user-id=274072 }, is one of, if not the, hardest maps ever used in a world cup.
...
A very old map by today's standards, Knife is a map that has seen community-wide adoration. ::LZD::{ user-id=224335 }'s style shines in one of his oldest maps.
...
::yea::{ user-id=6874332 }'s Figue Folle is a really interesting take. Employing lots of spinners and sliders throughout, it separates it's take on the song from other maps by just simply being unique.
...
Ah, a funny little quirk about this map. The description for this actually says "This WILL be my first ranked mapset, or else I am not BrambleClaw!". Well, ::BrambleClaw::{ user-id=3487731 } actually changed his name to Bramble, and this will possibly be his first Loved mapset.
...
Souzou Forest is one of ::17VA::{ user-id=1895984 }'s most popular charts, it is a fun LN chart which is decently fast but also fairly simplistic stair based patterning, which makes it both easy to hit and hard to get good accuracy on.
...
A great collaboration between veteran charter ::Fullerene-::{ user-id=2531335 } and veteran speedster ::Staiain::{ user-id=86188 }, which we're sure you'll enjoy.
...
Our seventh chart on the list is a classic. This chart, created by ::Cirno::{ user-id=373999 } in 2013, is a chart that's heavily inspired by IIDX with some very interesting concepts executed.
...
The next submission is Quo Vadis, a mapset by ::JamesHappy::{ user-id=3901876 }, with guest difficulties from ::arcwinolivirus::{ user-id=2039089 }, [Evening](https://osu.ppy.sh/users/2193881) and XleonQ.
...
Another fan favourite, ApolloN is a mapset by (fairly) prolific charters ::bbu2::{ user-id=4307930 } and [LeiN-](https://osu.ppy.sh/users/2162939). LeiN-, being one of the best 4K charters in 2015, places another feather in his cap with solid 4K difficulties.
...
Once again, ::PiraTom::{ user-id=1847698 } and [inteliser](https://osu.ppy.sh/users/1824775) pull off an enjoyable LN chart which still stands as a popular challenge today.
