---
layout: post
title: "New Featured Artist: endofsystem"
date: 2026-06-27 21:30:00 +0000
series: featured_artists
---

Music illusionist **endofsystem** joins our Featured Artists!

![](/wiki/shared/news/2026-06-27-new-featured-artist-endofsystem/banner.png)

A master of unorthodox aural presentation, **endofsystem** (known here on osu! as ::*nicoshi*::{ user=18641641 }) specialises in ambient and experimental tracks featuring industrial tones and limited bit depth. Drawing similarities between avant-garde songs isn't always practical, but **endofsystem** loves listening to distorted tunes from familiar names like [*Frums*](https://osu.ppy.sh/beatmaps/artists/68), [*SOOOO*](https://osu.ppy.sh/beatmaps/artists/30), and [*meganeko*](https://osu.ppy.sh/beatmaps/artists/75), traces of which may bleed into **endofsystem**'s sound.

[UNTONE Music](https://osu.ppy.sh/beatmaps/artists/479) already hosts a collection of **endofsystem**'s music, and that list is expanding here today!

A total of **17** soundscapes are on [**endofsystem**'s Featured Artist listing](https://osu.ppy.sh/beatmaps/artists/552), ready for mapping by osu!'s most unconventional mappers.

Taste some of what **endofsystem** has to offer below.

### endofsystem - _0XB4D60DE

An experiment by both musician and mapper, [this variable-velocity 4K osu!mania map](https://osu.ppy.sh/beatmapsets/2283879) by ::maris_a::{ user=28581372 } embodies the progressive nature of everything **endofsystem** stands for. Witness the map for yourself (in both readable and not-so-readable forms):

<div align="center" class="osu-md__paragraph">
    <video width="95%" controls>
        <source src="https://assets.ppy.sh/media/news/endofsystem_showcase.mp4" type="video/mp4" preload="none">
    </video>
</div>

### endofsystem - 5 Beats

The Ranked section needs some experimentation too. ::Anwul_::{ user=17673611 } designed [this osu!mania map](https://osu.ppy.sh/beatmapsets/2463154) around a motif of *5*. We'll leave you to interpret the details.

<audio controls>
    <source src="https://assets.ppy.sh/artists/552/Songs/endofsystem%20-%205%20Beats.mp3">
</audio>

### endofsystem - ENDLESS//TIMEOLOGY -Aki's "Le gardien du temps" Rewind-

<audio controls>
    <source src="https://assets.ppy.sh/artists/552/Songs/endofsystem%20-%20ENDLESSTIMEOLOGY%20-Aki%27s%20Le%20gardien%20du%20temps%20Rewind-.mp3">
</audio>

### endofsystem - dissonified-world

<audio controls>
    <source src="https://assets.ppy.sh/artists/552/Songs/endofsystem_-_dissonified-world.mp3">
</audio>

### endofsystem - discard-everything-world

<audio controls>
    <source src="https://assets.ppy.sh/artists/552/Songs/endofsystem_-_discard-everything-world.mp3">
</audio>

---

If anomalous complexity is what you're after, dig through [**endofsystem**'s Featured Artist listing](https://osu.ppy.sh/beatmaps/artists/552) and find the hole made just for you.

Rather than announcing a new Featured Artist, we'll be highlighting some tracks from our current artist roster next weekend! Stay tuned.

—pishifat
