...
### ✅ Reprocess performance values of negligible scores (completed 2026-07-06)
...
### ✅ Reprocess performance values of all scores (completed 2026-07-11)
...
### ✅ Reprocess total PP values for all users (completed 2026-07-11)

This will bring everything back in line with expectations.

### ✅ Re-enabling of global rank history updates (completed 2026-07-11)
...
### ✅ Reindexing (completed 2026-07-12)
...
Thanks to a [brand new skill](https://github.com/ppy/osu/pull/33196) created by ::kwotaq::{ user=8195972 } (with some early contributions from ::moonpoint::{ user=9558549 }), these bonuses are now replaced by a new skill called Reading.
...
Thanks to a [change](https://github.com/ppy/osu/pull/33351) by ::Finadoggie::{ user=14182048 }, chunks are now tied to objects and are allowed to vary in size. This means that offset and BPM changes are no longer able to influence difficulty on their own.
...
A [change](https://github.com/ppy/osu/pull/35555) by ::Nisico::{ user=23384384 } improves this by changing which parts of the slider are used to form the angle.
...
Thanks to a [change](https://github.com/ppy/osu/pull/36417) by ::Natelytle::{ user=17607667 }, we can now change this mechanism without adverse effects to strain.

As a result, a [set](https://github.com/ppy/osu/pull/36792) of [changes](https://github.com/ppy/osu/pull/36813) by ::StanR::{ user=7217455 } were made in order to edge this mechanism closer towards rewarding BPM and distance increases equally:
...
Thanks to a [change](https://github.com/ppy/osu/pull/36902) by ::StanR::{ user=7217455 }, this distance bonus has been replaced with a new flow aim difficulty evaluator in the Aim skill.
...
Tapping deviation is a statistic similar to unstable rate created by ::Frostium::{ user=8202998 }, combining a score's hit statistics, OD and amount of difficult "speed notes" (high-strain notes relevant to speed PP) in a beatmap. This metric has already been in use in order to nerf scores that require tapping improperly (e.g. "rake tapping").

Thanks to a [change](https://github.com/ppy/osu/pull/36475) by ::StanR::{ user=7217455 }, we are now using this deviation metric as a replacement for accuracy scaling within Speed.
...
Well, thanks to a [change](https://github.com/ppy/osu/pull/36559) by ::kwotaq::{ user=8195972 }, acute-angled jumps now receive a nerf depending on their repetitiveness.
...
A [change](https://github.com/ppy/osu/pull/37040) by ::StanR::{ user=7217455 } was made in order to harshen the miss penalty's initial impact:
...
- A [fix](https://github.com/ppy/osu/pull/29993) by ::Givy120::{ user=10560705 } to make the comparison between slider and slider-less aim difficulty more accurate
- A [fix](https://github.com/ppy/osu/pull/37264) by ::Givy120::{ user=10560705 } to include 50s in osu!(stable) slider break estimations
- A [fix](https://github.com/ppy/osu/pull/37636) by ::Givy120::{ user=10560705 } to ensure that patterns too spaced to be doubletapped are not penalised by doubletap nerfs
- A [fix](https://github.com/ppy/osu/pull/37263) by ::Givy120::{ user=10560705 } to improve stability of osu!(stable) slider break estimations
- A [fix](https://github.com/ppy/osu/pull/36599) by ::molneya::{ user=8945180 } to nerf total difficulty for Flashlight when Reading difficulty is extremely high
- A [fix](https://github.com/ppy/osu/pull/36216) by ::StanR::{ user=7217455 } to fix aim slider bonuses being awarded to the incorrect note
- A [set](https://github.com/ppy/osu/pull/36569) of [fixes](https://github.com/ppy/osu/pull/36593) by ::StanR::{ user=7217455 } to improve slider-to-circle Rhythm difficulty
- A [fix](https://github.com/ppy/osu/pull/37609) by ::StanR::{ user=7217455 } to ensure rhythm complexity is decreased for long islands
- A [fix](https://github.com/ppy/osu/pull/38082) by ::StanR::{ user=7217455 } to ensure single-note timing point changes do not inflate difficulty
- A [set](https://github.com/ppy/osu/pull/36773) of [fixes](https://github.com/ppy/osu/pull/36806) by ::StanR::{ user=7217455 } to fix slider velocity calculations using inflated velocity
- A [fix](https://github.com/ppy/osu/pull/37624) by ::StanR::{ user=7217455 } to ensure the slider end penalty scales with difficulty correctly
- A [fix](https://github.com/ppy/osu/pull/36209) by ::tsunyoku::{ user=11315329 } to fix score-based miss estimations using incorrect multipliers for scores with mods
- A [set of fixes](https://github.com/ppy/osu/pull/36999) by ::tsunyoku::{ user=11315329 } to improve flow aim evaluation for overlapping notes and sliders
- A [change](https://github.com/ppy/osu/pull/35962) by ::Givy120::{ user=10560705 } to skip score-based miss estimations for ScoreV2
- A [change](https://github.com/ppy/osu/pull/33019) by ::Givy120::{ user=10560705 } to consider Hidden's "only hide approach circles" setting in Flashlight calculations
- A [change](https://github.com/ppy/osu/pull/35724) by ::Givy120::{ user=10560705 } to adjust full-combo threshold depending on how difficult sliders are
- A [change](https://github.com/ppy/osu/pull/36466) by ::Givy120::{ user=10560705 } to ensure alternating angles do not explode Reading difficulty
- A [change](https://github.com/ppy/osu/pull/36554) by ::Givy120::{ user=10560705 } to harshen Speed deviation scaling at lower difficulties
- A [change](https://github.com/ppy/osu/pull/36623) by ::Givy120::{ user=10560705 } to nerf Reading's perfect-stack bonus for Hidden
- A [change](https://github.com/ppy/osu/pull/36900) by ::molneya::{ user=8945180 } to remove accuracy bonus for Flashlight
- A [change](https://github.com/ppy/osu/pull/36555) by ::piiid::{ user=37091194 } to rebalance Aim bonuses for Traceable
- A [change](https://github.com/ppy/osu/pull/36148) by ::StanR::{ user=7217455 } to scale doubletap detections more harshly, punishing some edge case beatmaps
- A [change](https://github.com/ppy/osu/pull/36513) by ::StanR::{ user=7217455 } to remove a now unnecessary high CS slider nerf
- A [change](https://github.com/ppy/osu/pull/36063) by ::StanR::{ user=7217455 } to simplify final star rating calculations
- A [change](https://github.com/ppy/osu/pull/36669) by ::StanR::{ user=7217455 } to uncap Accuracy PP's object count bonus
- A [change](https://github.com/ppy/osu/pull/37099) by ::StanR::{ user=7217455 } to buff longer jump sections
- A [refactor](https://github.com/ppy/osu/pull/35817) by ::Givy120::{ user=10560705 } to remove some unnecessary difficulty code
- A [refactor](https://github.com/ppy/osu/pull/36465) by ::Givy120::{ user=10560705 } to simplify Reading's Hidden difficulty calculation
- A [refactor](https://github.com/ppy/osu/pull/37119) by ::kwotaq::{ user=8195972 } to improve Aim evaluation code quality
- A [set](https://github.com/ppy/osu/pull/36918) of [refactors](https://github.com/ppy/osu/pull/36944) by ::Rian8337::{ user=5383997 } to improve readability of Reading calculations
- A [refactor](https://github.com/ppy/osu/pull/37460) by ::Natelytle::{ user=17607667 } to improve difficulty code quality
- A [set](https://discord.com/channels/1166126757141827775/1166126758261686294/1506273339835158558) of [refactors](https://github.com/ppy/osu/pull/37623) by ::StanR::{ user=7217455 } to move more difficulty adjustments to apply per-note
- A [refactor](https://github.com/ppy/osu/pull/37654) by ::StanR::{ user=7217455 } to improve osu!(stable) slider break estimation code quality
- A [refactor](https://github.com/ppy/osu/pull/36112) by ::tsunyoku::{ user=11315329 } to improve performance in some areas of difficulty calculation

## osu!taiko

### Nerf long gaps during frequent rhythm changes

Patterns of frequent rhythm changes followed by long gaps would previously award unexpectedly high rhythm difficulty. This [change](https://github.com/ppy/osu/pull/37200) by ::rloseise::{ user=6793778 } targets and penalises this specific case.
...
- A [change](https://github.com/ppy/osu/pull/36628) by ::StanR::{ user=7217455 } to return 0 difficulty for swells and drum rolls
- A [change](https://github.com/ppy/osu/pull/36671) by ::StanR::{ user=7217455 } to use the full hit-window for rhythm calculations
...
A [change](https://github.com/ppy/osu/pull/37287) by ::Kitsunivy::{ user=7055384 } was made in order to ensure linear patterns which don't require additional inputs are punished accordingly.
