---
layout: post
title: "New Featured Artist: uselet"
date: 2024-02-17 19:00:00 +0000
series: featured_artists
---

A new batch of breakcore has landed on our doorstep thanks to our latest Featured Artist: **uselet**!

![](https://assets.ppy.sh/artists/402/header.jpg)

Many musicians enjoy seeing their music in osu!, but ::**uselet**::{ user-id=5256300 } has taken that a step further by ::mapping a bunch of their own tunes::{ user-id=5256300 }. While they're primarily an osu! player, their songs have seen the most traction in osu!taiko so far, namely via [circus](https://osu.ppy.sh/beatmapsets/1329329) and [cxvbiopxvcbn](https://osu.ppy.sh/beatmapsets/1742153).

**11** tunes — including all of those above — are up for grabs on [**uselet**'s Featured Artist listing](https://osu.ppy.sh/beatmaps/artists/402). (That's your cue to click the link.)

In case you're not in the *mapping mood* quite yet, this video of **uselet**'s *circus* should set the tone appropriately. Check that out, plus a few more previews:

<div align="center">
    <video width="95%" controls>
        <source src="https://assets.ppy.sh/artists/402/release_showcase.mp4" type="video/mp4" preload="none">
    </video>
</div>

### uselet - circus

Experience [the map from the video above](https://osu.ppy.sh/beatmapsets/1329329) hosted by ::The Law::{ user-id=11357694 }!

<audio controls>
    <source src="https://assets.ppy.sh/artists/402/Songs/uselet%20-%20circus.mp3">
</audio>

### uselet - cxvbiopxvcbn

Or bash some drums to [this map](https://osu.ppy.sh/beatmapsets/1742153) by ::op45667::{ user-id=21973491 }!

<audio controls>
    <source src="https://assets.ppy.sh/artists/402/Songs/uselet%20-%20cxvbiopxvcbn.mp3">
</audio>

### uselet - n'aie crainte, a present tout est fini

Essayez de jouer [cette 4K beatmap](https://osu.ppy.sh/beatmapsets/1667457) par ::Flade::{ user-id=11061804 }!

<audio controls>
    <source src="https://assets.ppy.sh/artists/402/Songs/uselet%20-%20n_aie%20crainte%2C%20a%20present%20tout%20est%20fini.mp3">
</audio>

---

Fun fact: any of the 11 beatmap templates you download from [**uselet**'s Featured Artist listing](https://osu.ppy.sh/beatmaps/artists/402) are already pre-timed. Less fun fact: I timed 11 songs yesterday.

That'll do it for this week. Stay tuned for another artist release in about 168 hours.

—pishifat
