---
layout: post
title: "Monthly Beatmapping Contest: December 2024"
date: 2024-12-06 23:55:00 +0000
series: monthly_beatmapping_contest
---

It's time to create your last map of the year.

![](https://assets.ppy.sh/media/monthly-beatmapping-contest.png)

Believe it or not, the [Monthly Beatmapping Contest](/wiki/Contests/Monthly_Beatmapping_Contest) (*MBC*) is a beatmapping contest that runs every month. This month's iteration is for **osu!** and **osu!mania** mappers:

- [osu! contest listing](https://osu.ppy.sh/community/contests/227)
- [osu!mania contest listing](https://osu.ppy.sh/community/contests/228)

This article covers results from the last contest, plus criteria for this contest. Skip around with these links:

- Results for [October's MBC](https://osu.ppy.sh/home/news/2024-10-02-monthly-beatmapping-contest-october-2024)
  - [osu! results](#osu!-results)
  - [osu!mania results](#osu!mania-results)
- [**Criteria for this month's MBC**](#game-modes)

## Previous contest results

### osu! results

[See the full results here.](https://mappersguild.com/contests/results?contest=66fdaf33b175a7a921c7ab24)

#### Summary

| Placement | Users | Beatmap | Badge prize | [Contest points](/wiki/Contests/Contest_points) |
| :-- | :-- | :-- | :-- | :-- |
| Winner | ::KomachiBaka::{ user=6155320 } & [Frakturehawkens](https://osu.ppy.sh/users/7458583) | [Marmalade butcher - NaRKiSSuS](https://osu.ppy.sh/beatmapsets/2288181) | ![](https://assets.ppy.sh/profile-badges/mbc-2020-1.png) | +2 each |
| Runner-up | ::iPhone 10::{ user=12473857 } & [-Doodle](https://osu.ppy.sh/users/12337329) | [Annabel - pedalnote](https://osu.ppy.sh/beatmapsets/2289404) | - | +1 each |

#### Top 10

<div align="center" class="osu-md__paragraph">
    <iframe width="95%" style="aspect-ratio: 16 / 9;" src="https://www.youtube.com/embed/VcMLL6hLI2k" frameborder="0" allowfullscreen></iframe>
</div>

::Daycore::{ user=5596337 }, [Beomsan](https://osu.ppy.sh/users/3626063), ::seros::{ user=10562853 }, [Ghouru](https://osu.ppy.sh/users/6273855), ::Net0::{ user=5099768 }, and [mintIceCream_](https://osu.ppy.sh/users/13221589) judged each and every map in this month's contest. After you send your appreciation to everyone who participated, thank these folks too.

### osu!mania results

[See the full results here.](https://mappersguild.com/contests/results?contest=66fdb1cab175a7a921c7accb)

#### Summary

| Placement | Users | Beatmap | Badge prize | [Contest points](/wiki/Contests/Contest_points) |
| :-- | :-- | :-- | :-- | :-- |
| Winner | ::-mint-::{ user=8976576 } & [elexire](https://osu.ppy.sh/users/9206093) | [KOAN Sound & Asa - Fuego](https://osu.ppy.sh/beatmapsets/2287548) | ![](https://assets.ppy.sh/profile-badges/mbc-2020-1.png) | +2 each |
| Runner-up | ::Saemitsu::{ user=14262789 } & [TheFunk](https://osu.ppy.sh/users/13981991) | [Culprate - Junglist Acid (osu! Edit)](https://osu.ppy.sh/beatmapsets/2287873) | - | +1 each |

#### Top 10

<div align="center" class="osu-md__paragraph">
    <iframe width="95%" style="aspect-ratio: 16 / 9;" src="https://www.youtube.com/embed/_4bLXt-ZSZo" frameborder="0" allowfullscreen></iframe>
</div>

Special thanks to ::taba2::{ user=7850508 }, [[GB]SuddenDeath](https://osu.ppy.sh/users/6699923), ::Antalf::{ user=8793773 }, [gzdongsheng](https://osu.ppy.sh/users/8660315), ::Hydria::{ user=808176 }, and [Monoseul](https://osu.ppy.sh/users/16010604) for judging this month's maps!

## Game modes

This month's contest is open to **osu!** and **osu!mania** mappers.

- [osu! contest listing](https://osu.ppy.sh/community/contests/227)
- [osu!mania contest listing](https://osu.ppy.sh/community/contests/228)

## Timeline

- **[Beatmapping](#beatmapping) (3 weeks):** Contest is announced and beatmappers create their entries.
- **[Screening](/wiki/Contests/Monthly_Beatmapping_Contest#screening) (1.5 weeks):** Entries are evaluated by experienced beatmappers. If there aren't many entries, this will be skipped in favour of a longer judging period.
- **[Judging](/wiki/Contests/Monthly_Beatmapping_Contest#judging) (1.5 weeks):** Top entries from screening are judged in detail by a different crew of experienced beatmappers.
- **[Rewards](#rewards) (~2 months from now):** Winners are announced at the start of another MBC.

## Beatmapping

### Limitation

While the general idea behind most mapping contests is "create the best map", the Monthly Beatmapping Contest adds a new layer called "limitations" to keep things from being stale. These are essentially extra rules specific to the month's contest.

If you're confused about how these work, ask for clarification in the comments, or send a message to [pishifat](https://osu.ppy.sh/community/chat?sendto=3178418) (for osu!) or [RandomeLoL](https://osu.ppy.sh/community/chat?sendto=7080063) (for osu!mania).

#### osu!

Slider design has evolved considerably over the years, in part thanks to a specific rule for Insane and Expert difficulties in the [Ranking criteria](/wiki/Ranking_criteria):

> The slider path is allowed to be ambiguous if the cursor can stay inside the slider's follow circle without any movement away from the slider head.

Many mappers have extrapolated on this rule, claiming that a slider is okay for ranking so long as full score can be earned without moving the cursor.

This month's contest aims to utilise this confusing rule to the fullest: **every slider's tail position must touch its head position.**

![](/wiki/shared/news/2024-12-06-monthly-beatmapping-contest-december-2024/slider-examples.jpg)

If this screenshot doesn't answer all of your questions, head to the comments section.

#### osu!mania

osu!(stable)'s editor comes with this handy `Beat Snap Divisor` tool in the top right: 

![](/wiki/shared/news/2024-12-06-monthly-beatmapping-contest-december-2024/beat-snap-divisor.jpg)

For this month's osu!mania contest:

- **You must use every beat snap divisor available in the osu!(stable) editor**
- **You must *not* use other beat snap divisors, such as those available in osu!(lazer) or through `.osu` file editing.**

Part of this contest's gimmick is seeing how effectively mappers can use variable beat snap divisors. Choose a song that allows for experimentation!

### Songs

Any song listed on [osu!'s Featured Artist listing](https://osu.ppy.sh/beatmaps/artists) is eligible for this contest. Cutting songs is allowed too!

## Rewards

The Monthly Beatmapping Contest rewards users with [contest points](/wiki/Contests/Contest_points) based on their placements:

- **Winner:** 2 points
- **Runner-up:** 1 point
- **Screening crew's #1 pick:** 1 point

These numbers may be higher depending on how many submissions the contest receives. Check [the wiki article](/wiki/Contests/Contest_points) for details.

Contest points reward users at certain thresholds:

- **Profile badges** are attainable at 2, 4, and 6 points each. The badges are tiered, meaning each new badge will replace the previous one!
- ***Elite Mapper* [user title](/wiki/Community/User_title)** is attainable at 6 points.

Additionally, a [**Mappers' Guild keychain**](/wiki/shared/news/2023-08-29-changes-to-the-mappers-guild/keychain.jpg) is optionally rewarded to the winner of each month's contest. Its design is based on your beatmap!

## Rules

- **Submissions must follow the month's beatmapping [limitation](#limitation):**
  - **osu!:** every slider's tail position must touch its head position.
  - **osu!mania:**
    - You must use every beat snap divisor available in the osu!(stable) editor.
    - You must *not* use other beat snap divisors, such as those available in osu!(lazer) or through `.osu` file editing.
- **Submissions must be a single difficulty from one of the month's [selected songs](#songs).**
- **Submissions must be in `.osz` format.**
- **Submissions must NOT be uploaded to the osu! website until after the contest's results are finalised.**
- **Submissions must abide by the [ranking criteria](/wiki/Ranking_criteria).** For osu!'s limitation, follow this in the most ambiguous way possible.
- **Submissions must be solely made by you.** Collaborations are not allowed for this contest!

Note that all submissions will be anonymised before they're reviewed by screeners/judges to avoid bias.

---

Stop reading these words and start reading the universal language on osu!: mapping. When your map is ready, plop it into one of these:

- [osu! contest listing](https://osu.ppy.sh/community/contests/227)
- [osu!mania contest listing](https://osu.ppy.sh/community/contests/228)

You have until **January 4th** to submit!

—pishifat, RandomeLoL
