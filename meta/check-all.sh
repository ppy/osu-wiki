...
run_test 'YAML style' uv run --no-sync osu-wiki-tools check-yaml
run_test link uv run --no-sync osu-wiki-tools check-links --all

if test -n "$(git log master..)"; then
  run_test 'outdated article' uv run --no-sync osu-wiki-tools check-outdated-articles --no-recommend-autofix
fi
