...
WORKDIR /root
COPY package.json package-lock.json scripts/requirements.txt /root/
RUN npm install
RUN pip3 install -r requirements.txt

# Prevent git from refusing to work in a repository with "dubious ownership".
# The repository, mounted with --volume /root, is owned by a host user, and the container user is root.
